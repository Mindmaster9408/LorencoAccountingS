...
(function () {
    ...
    var BASE = '/api/commander/document-requests';
    ...
    function loadSummary() {
        return CommanderAPI.fetch(BASE + '/summary')
    ...
    function loadClients() {
        return CommanderAPI.fetch('/api/commander/clients?is_active=true')
            .then(function (d) {
                _clients = d.clients || [];
    ...
    function loadMembers() {
        return CommanderAPI.fetch('/api/commander/team?active=true')
            .then(function (d) {
                _members = d.members || [];
    ...
    function loadRequests() {
        ...
        return CommanderAPI.fetch(BASE + '?limit=200')
            .then(function (d) {
    ...
    function applyDrFilters() {
        ...
        var results = _allRequests.filter(function (r) {
            ...
            if (search) {
                var name = (r.commander_clients && r.commander_clients.name) ? r.commander_clients.name.toLowerCase() : '';
    ...
    function renderRequests(reqs, today) {
        ...
        body.innerHTML = reqs.map(function (r) {
            ...
            var clientName  = r.commander_clients ? r.commander_clients.name : '—';
    ...
    function submitDrCreate() {
        ...
        CommanderAPI.fetch(BASE, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
            .then(function (d) {
    ...
    function drMarkReceived(id, fromView) {
        CommanderAPI.fetch(BASE + '/' + id + '/received', { method: 'PUT' })
            .then(function (d) {
                if (d.error) throw new Error(d.error);
    ...
    function drMarkReminder(id, fromView) {
        CommanderAPI.fetch(BASE + '/' + id + '/reminder-sent', { method: 'PUT' })
            .then(function (d) {
                if (d.error) throw new Error(d.error);
                if (fromView) closeDrViewModal();
                Promise.all([loadSummary(), loadRequests()]);
    ...
    function drWaive(id, fromView) {
        ...
        CommanderAPI.fetch(BASE + '/' + id + '/waive', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) })
            .then(function (d) {
                if (d.error) throw new Error(d.error);
                if (fromView) closeDrViewModal();
                Promise.all([loadSummary(), loadRequests()]);
    ...
    function drDelete(id, fromView) {
        ...
        CommanderAPI.fetch(BASE + '/' + id, { method: 'DELETE' })
            .then(function (d) {
                if (d.error) throw new Error(d.error);
                if (fromView) closeDrViewModal();
                Promise.all([loadSummary(), loadRequests()]);
    ...
    function loadChecklists() {
        ...
        CommanderAPI.fetch(BASE + '/checklists')
            .then(function (d) {
    ...
    function submitCreateChecklist() {
        ...
        CommanderAPI.fetch(BASE + '/checklists', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
            .then(function (d) {
                if (d.error) throw new Error(d.error);
                _clSubmitting = false;
    ...
    function submitDrApply() {
        ...
        CommanderAPI.fetch(BASE + '/checklists/' + _applyingId + '/apply', {
