...
(function () {
  ...
  function init() {
    var token = localStorage.getItem('commander_token') || localStorage.getItem('token');
  ...
  function loadTeamMembers() {
    CommanderAPI.json('/api/commander/team?active=true')
  ...
  function loadSummary() {
    CommanderAPI.json('/api/commander/client-health/summary')
      .then(function (d) {
  ...
  function loadList() {
    ...
    CommanderAPI.json('/api/commander/client-health?limit=500')
      .then(function (d) {
        hide('listLoading');
    ...
    var rows = filtered.map(function (c) {
      ...
      return '<tr>' +
        ...
        '<td class="td-actions">' +
          ...
          ' <a href="/commander/client-detail.html?id=' + c.client_id + '" class="btn btn-xs btn-ghost" style="text-decoration:none">Profile</a>' +
        '</td>' +
      '</tr>';
    }).join('');
  ...
  function recalcAll() {
    ...
    CommanderAPI.json('/api/commander/client-health/recalculate', {
      method:  'POST',
  ...
  function openHdModal(clientId, clientName) {
    ...
    CommanderAPI.json('/api/commander/client-health/' + clientId)
      .then(function (d) {
        renderHdModal(d);
      })
      .catch(function (err) {
  ...
  function renderHdModal(d) {
    ...
    var RISK_MODULE = {
      overdue_deadlines:      { label: 'Deadlines', path: '/commander/deadlines.html' },
      many_overdue_deadlines: { label: 'Deadlines', path: '/commander/deadlines.html' },
      review_backlog:         { label: 'Tasks',     path: '/commander/tasks.html' },
      unassigned_tasks:       { label: 'Tasks',     path: '/commander/tasks.html' },
      overdue_periods:        { label: 'Period Queue', path: '/commander/engagement-periods.html' },
      old_wip:                { label: 'Billing',   path: '/commander/billing.html' },
      high_writeoff:          { label: 'Billing',   path: '/commander/billing.html' },
      engagement_no_owner:    { label: 'Client', path: '/commander/client-detail.html?id=' + d.client_id },
      missing_recurrence:     { label: 'Client', path: '/commander/client-detail.html?id=' + d.client_id },
      no_client_owner:        { label: 'Client', path: '/commander/client-detail.html?id=' + d.client_id },
    };
  ...
  function loadClientActions(clientId) {
    ...
    CommanderAPI.json('/api/commander/client-health/' + clientId + '/actions')
      .then(function (d) {
  ...
  function renderActionsList(actions) {
    ...
    el.innerHTML = actions.map(function (a) {
      ...
      if (a.linked_task_id) meta.push('<a href="/commander/tasks.html" class="btn-link-plain" style="color:var(--accent)">Linked task #' + a.linked_task_id + '</a>');
  ...
  function submitActionForm() {
    ...
    var endpoint = '/api/commander/client-health/' + _activeClientId +
  ...
  function completeAction(actionId) {
    CommanderAPI.json('/api/commander/client-health/actions/' + actionId + '/complete', {
      method:  'PUT',
      headers: { 'Content-Type': 'application/json' },
      body:    JSON.stringify({}),
    })
  ...
  function dismissAction(actionId) {
    ...
    CommanderAPI.json('/api/commander/client-health/actions/' + actionId + '/dismiss', {
      method:  'PUT',
      headers: { 'Content-Type': 'application/json' },
      body:    JSON.stringify({}),
    })
