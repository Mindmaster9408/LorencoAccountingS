...
(function () {
    ...
    // if /api/commander/users fails, the picker used to silently fall back
    ...
    async function loadLoginUsers() {
        try {
            var res = await CommanderAPI.fetch('/api/commander/users');
            ...
            CommanderAPI.showToast('❌ Could not load login accounts for the "Link to Login Account" picker. Reopen Add/Edit Member to retry before saving a link.', true);
        }
    }
    ...
    async function loadTeam() {
        ...
        var url = '/api/commander/team?active=' + encodeURIComponent(active);
        try {
        ...
        var rows = page.map(function(m) {
            ...
            return '<tr>' +
                ...
                '<td><div class="td-actions">' +
                    ...
                    '<a href="/commander/capacity.html" class="btn btn-ghost btn-sm" style="text-decoration:none" title="Manage capacity for this member">Capacity</a>' +
                '</div></td>' +
            '</tr>';
        }).join('');
    ...
    async function saveMember(e) {
        ...
        try {
            var url    = editingId ? '/api/commander/team/' + editingId : '/api/commander/team';
            var method = editingId ? 'PUT' : 'POST';
            var res = await CommanderAPI.fetch(url, { method: method, body: JSON.stringify(body) });
            var d = await res.json();
            if (!res.ok) throw new Error(d.error || 'Save failed');
            closeModal();
            CommanderAPI.showToast(editingId ? 'Team member updated!' : 'Team member added!');
            loadTeam();
        } catch(err) {
    ...
    async function deactivateFromModal() {
        ...
        try {
            var res = await CommanderAPI.fetch('/api/commander/team/' + editingId, { method: 'DELETE' });
            if (!res.ok) throw new Error('Deactivate failed');
            closeModal();
            CommanderAPI.showToast('Team member deactivated.');
            loadTeam();
        } catch(e) {
    ...
    async function reactivateFromModal() {
        ...
        var m = allMembers.find(function(x) { return x.id === editingId; });
        try {
            var res = await CommanderAPI.fetch('/api/commander/team/' + editingId + '/reactivate', { method: 'PUT', body: '{}' });
            if (!res.ok) throw new Error('Reactivate failed');
            closeModal();
            CommanderAPI.showToast('Team member reactivated!');
            loadTeam();
        } catch(e) {
            CommanderAPI.showToast('❌ ' + e.message, true);
        }
    ...
    async function syncFromUsers() {
        ...
        try {
            var res = await CommanderAPI.fetch('/api/commander/team/sync-from-users', { method: 'POST', body: '{}' });
            var d = await res.json();
            if (!res.ok) throw new Error(d.error || 'Sync failed');
            if (d.imported === 0) {
                CommanderAPI.showToast(d.message || 'All users already in team — nothing to import.');
            } else {
                CommanderAPI.showToast(d.imported + ' user' + (d.imported === 1 ? '' : 's') + ' imported as Staff. Adjust roles as needed.');
                await loadTeam();
            }
        } catch(err) {
            CommanderAPI.showToast('❌ ' + err.message, true);
