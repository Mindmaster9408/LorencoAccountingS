...
(function () {
    ...
    async function init() {
        var token = localStorage.getItem('token') || localStorage.getItem('commander_token');
        if (!token) { window.location.href = '/'; return; }
        try {
            var res = await CommanderAPI.fetch('/api/auth/me');
        ...
        if (!clientId) {
            window.location.href = '/commander/clients.html';
            return;
        }
    ...
    async function loadTeam() {
        try {
            var res = await CommanderAPI.fetch('/api/commander/team?active=true');
            if (!res.ok) return;
    ...
    async function loadClient() {
        try {
            var res = await CommanderAPI.fetch('/api/commander/clients/' + clientId);
            ...
            if (commLink) commLink.href = '/commander/communications.html?client_id=' + clientId;
            var docLink = document.getElementById('docReqViewAllLink');
            if (docLink) docLink.href = '/commander/document-requests.html?client_id=' + clientId;
            var cpLink = document.getElementById('cpViewAllLink');
            if (cpLink) cpLink.href = '/commander/compliance-packs.html?client_id=' + clientId;
            var tpLink = document.getElementById('tpViewAllLink');
            if (tpLink) tpLink.href = '/commander/taxpayer-profiles.html?client_id=' + clientId;
            var ptLink = document.getElementById('ptViewAllLink');
            if (ptLink) ptLink.href = '/commander/provisional-tax.html?client_id=' + clientId;
            var itLink = document.getElementById('itViewAllLink');
            if (itLink) itLink.href = '/commander/individual-tax.html?client_id=' + clientId;
            var ctLink = document.getElementById('ctViewAllLink');
            if (ctLink) ctLink.href = '/commander/company-tax.html?client_id=' + clientId;
            var secLink = document.getElementById('secViewAllLink');
            if (secLink) secLink.href = '/commander/secretarial.html?client_id=' + clientId;
            loadContacts();
            loadEngagements();
            loadClientHealth();
    ...
    async function saveClientDetail(e) {
        ...
        try {
            ...
            CommanderAPI.showToast('Client saved!');
            document.getElementById('pageTitle').textContent = body.name || 'Client Profile';
        } catch(err) {
            CommanderAPI.showToast('❌ ' + err.message, true);
        } finally {
            btn.disabled = false;
        }
        return false;
    ...
    async function archiveClient() {
        ...
        try {
            var res = await CommanderAPI.fetch('/api/commander/clients/' + clientId, { method: 'DELETE' });
            if (!res.ok) throw new Error('Archive failed');
            CommanderAPI.showToast('Client archived.');
            setTimeout(function() { window.location.href = '/commander/clients.html'; }, 800);
        } catch(e) {
            CommanderAPI.showToast('❌ ' + e.message, true);
        }
    }
    ...
    async function loadContacts() {
        ...
        try {
            var res = await CommanderAPI.fetch('/api/commander/clients/' + clientId + '/contacts');
    ...
    async function saveContact(e) {
        ...
        try {
            var url    = editingContactId
                ? '/api/commander/clients/' + clientId + '/contacts/' + editingContactId
                : '/api/commander/clients/' + clientId + '/contacts';
            var method = editingContactId ? 'PUT' : 'POST';
            var res = await CommanderAPI.fetch(url, { method: method, body: JSON.stringify(body) });
            var d = await res.json();
            if (!res.ok) throw new Error(d.error || 'Save failed');
            closeContactModal();
            CommanderAPI.showToast(editingContactId ? 'Contact updated!' : 'Contact added!');
            loadContacts();
        } catch(err) {
            CommanderAPI.showToast('❌ ' + err.message, true);
        } finally {
    ...
    async function deactivateContact() {
        ...
        try {
            var res = await CommanderAPI.fetch(
                '/api/commander/clients/' + clientId + '/contacts/' + editingContactId,
                { method: 'DELETE' }
            );
            if (!res.ok) throw new Error('Remove failed');
            closeContactModal();
            CommanderAPI.showToast('Contact removed.');
            loadContacts();
        } catch(e) {
            CommanderAPI.showToast('❌ ' + e.message, true);
        }
    ...
    async function loadComplianceSuggestions() {
        ...
        try {
            var r = await CommanderAPI.fetch('/api/commander/compliance/suggestions/client/' + clientId);
    ...
    async function openSuggestionDeadlineModal(idx) {
        ...
        if (!_sdTeamLoaded) {
            try {
                var tr = await CommanderAPI.fetch('/api/commander/team?active=true');
                if (tr.ok) {
    ...
    async function saveSuggestionDeadline(e) {
        ...
        try {
            var r = await CommanderAPI.fetch('/api/commander/deadlines', {
                method: 'POST',
            ...
            CommanderAPI.showToast('Deadline created');
            closeSuggestionModal();
        } catch(err) {
            CommanderAPI.showToast('Error: ' + err.message, true);
        } finally {
    ...
    async function loadEngagements() {
        ...
        try {
            var res = await CommanderAPI.fetch('/api/commander/clients/' + clientId + '/engagements');
            if (!res.ok) throw new Error('Load failed');
            var d = await res.json();
        ...
        var cards = engagements.map(function(e) {
            ...
            return '<div class="engagement-card">' +
                ...
                '<div class="engagement-card-actions">' +
                    ...
                    '<a href="/commander/engagement-periods.html?engagement_id=' + e.id + '&client_id=' + e.client_id + '" class="btn btn-ghost btn-sm">Queue</a>' +
    ...
    async function _openEngagementModal(id) {
        ...
        if (id) {
            try {
                var res = await CommanderAPI.fetch('/api/commander/engagements/' + id);
    ...
    async function saveEngagement(e) {
        ...
        try {
            ...
            if (_editingEngagementId) {
                url    = '/api/commander/engagements/' + _editingEngagementId;
                method = 'PUT';
            } else {
                url    = '/api/commander/clients/' + clientId + '/engagements';
                method = 'POST';
            }
            var res = await CommanderAPI.fetch(url, { method: method, body: JSON.stringify(body) });
            var d   = await res.json();
            ...
            CommanderAPI.showToast(_editingEngagementId ? 'Engagement updated!' : 'Engagement added!');
            loadEngagements();
        } catch(err) {
            CommanderAPI.showToast('❌ ' + err.message, true);
        } finally {
    ...
    async function engagementAction(id, action) {
        ...
        try {
            var res;
            if (action === 'cancel') {
                res = await CommanderAPI.fetch('/api/commander/engagements/' + id, { method: 'DELETE' });
            } else {
                res = await CommanderAPI.fetch('/api/commander/engagements/' + id + '/' + action, { method: 'PUT', body: '{}' });
            }
            var d = await res.json();
            if (!res.ok) throw new Error(d.error || action + ' failed');
            CommanderAPI.showToast('Engagement ' + action + 'd.');
            loadEngagements();
        } catch(err) {
            CommanderAPI.showToast('❌ ' + err.message, true);
        }
    ...
    async function openEngHistoryModal(id) {
        ...
        try {
            var res = await CommanderAPI.fetch('/api/commander/engagements/' + id + '/history');
            if (!res.ok) throw new Error('Load failed');
            var d = await res.json();
    ...
    async function openGenerateModal(engId) {
        ...
        try {
            var res = await CommanderAPI.fetch('/api/commander/engagements/' + engId + '/generation-preview');
    ...
    async function submitGenerateWorkflow(e) {
        ...
        try {
            var res = await CommanderAPI.fetch(
                '/api/commander/engagements/' + _generateEngagementId + '/generate-workflow',
            ...
            CommanderAPI.showToast('✅ Workflow generated — ' + d.task_count + ' tasks created!');
            loadEngagements();
    ...
    async function previewPeriods() {
        ...
        if (!fromDate || !toDate) {
            CommanderAPI.showToast('❌ Please enter both From Date and To Date', true);
            return;
        }
        ...
        try {
            var res = await CommanderAPI.fetch(
                '/api/commander/engagements/' + _periodQueueEngId + '/periods/generate-preview',
    ...
    async function createPeriodQueue() {
        ...
        try {
            var res = await CommanderAPI.fetch(
                '/api/commander/engagements/' + _periodQueueEngId + '/periods/generate',
                { method: 'POST', body: JSON.stringify({ from_date: fromDate, to_date: toDate, max_periods: maxP }) }
            );
            var d = await res.json();
            ...
            CommanderAPI.showToast('✅ ' + d.created + ' period(s) queued!');
        } catch(err) {
            CommanderAPI.showToast('❌ ' + err.message, true);
    ...
    async function loadClientHealth() {
        ...
        if (!wrap) return;
        try {
            var res = await CommanderAPI.fetch('/api/commander/client-health/' + clientId);
    ...
    async function recalcClientHealth() {
        ...
        try {
            var res = await CommanderAPI.fetch('/api/commander/client-health/recalculate', {
                method: 'POST',
    ...
    async function loadClientCommunications() {
        ...
        try {
            var res = await CommanderAPI.fetch('/api/commander/communications?client_id=' + clientId + '&limit=10');
            if (!res.ok) throw new Error('Load failed');
            var d = await res.json();
    ...
    async function submitAddComm() {
        ...
        try {
            var res = await CommanderAPI.fetch('/api/commander/communications', {
                method:  'POST',
                headers: { 'Content-Type': 'application/json' },
                body:    JSON.stringify(payload),
            });
    ...
    async function markCommResponded(commId) {
        try {
            var res = await CommanderAPI.fetch('/api/commander/communications/' + commId + '/mark-responded', { method: 'PUT' });
    ...
    async function loadClientSecretarial() {
        ...
        if (!wrap) return;
        try {
            var res = await CommanderAPI.fetch('/api/commander/secretarial/' + clientId);
            ...
            var changesHtml = '';
            try {
                var chRes = await CommanderAPI.fetch('/api/commander/secretarial-workflows?client_id=' + clientId + '&limit=3');
                if (chRes.ok) {
            ...
            var governanceHtml = '';
            try {
                var govBase = '/api/commander/secretarial-governance';
                var govResults = await Promise.all([
                    CommanderAPI.fetch(govBase + '/resolutions?client_id=' + clientId + '&limit=2').then(function (r) { return r.ok ? r.json() : { resolutions: [] }; }),
                    CommanderAPI.fetch(govBase + '/meetings?client_id=' + clientId + '&limit=2').then(function (r) { return r.ok ? r.json() : { meetings: [] }; }),
                ]);
                var govItems = [];
            ...
            var boHtml = '';
            try {
                var boRes = await CommanderAPI.fetch('/api/commander/beneficial-ownership/client/' + clientId);
                if (boRes.ok) {
            ...
            var lifecycleHtml = '';
            try {
                var lcRes = await CommanderAPI.fetch('/api/commander/entity-lifecycle/client/' + clientId);
                if (lcRes.ok) {
            ...
            if (loading) loading.classList.add('hidden');
            wrap.classList.remove('hidden');
            wrap.innerHTML = '<p style="color:var(--muted);font-size:0.83rem;">No secretarial profile yet — <a href="/commander/secretarial.html?client_id=' + clientId + '">open Secretarial</a> to create one.</p>';
        }
    }
    ...
    async function loadClientDocumentRequests() {
        ...
        try {
            var res = await CommanderAPI.fetch('/api/commander/document-requests?client_id=' + clientId + '&limit=10');
            if (!res.ok) throw new Error('Load failed');
            var d = await res.json();
    ...
    async function submitAddDoc() {
        ...
        try {
            var res = await CommanderAPI.fetch('/api/commander/document-requests', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(payload),
            });
    ...
    async function cdDocMarkReceived(reqId) {
        try {
            var res = await CommanderAPI.fetch('/api/commander/document-requests/' + reqId + '/received', { method: 'PUT' });
    ...
    var _CP_BASE         = '/api/commander/compliance-packs';
    ...
    async function loadClientCompliancePacks() {
        ...
        try {
            var res = await CommanderAPI.fetch(_CP_BASE + '?client_id=' + clientId + '&limit=8');
            if (!res.ok) throw new Error('Load failed');
            var d    = await res.json();
    ...
    async function submitCdCreatePack() {
        ...
        try {
            var res = await CommanderAPI.fetch(_CP_BASE, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(payload),
            });
            ...
            CommanderAPI.showToast('Compliance pack created!');
            loadClientCompliancePacks();
        } catch(e) {
            _cdPackSubmitting = false;
            document.getElementById('cdCreatePackSubmitBtn').disabled = false;
    ...
    var _TP_BASE = '/api/commander/taxpayer-profiles';
    ...
    async function loadClientTaxpayerProfiles() {
        ...
        try {
            var res = await CommanderAPI.fetch(_TP_BASE + '?client_id=' + clientId + '&limit=8');
            if (!res.ok) throw new Error('Load failed');
            var d        = await res.json();
    ...
    async function submitCdCreateProfile() {
        ...
        try {
            var res = await CommanderAPI.fetch(_TP_BASE, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(payload)
            });
            ...
            CommanderAPI.showToast('Taxpayer profile created!');
            loadClientTaxpayerProfiles();
        } catch(e) {
            _cdProfileSubmitting = false;
            document.getElementById('cdCreateProfileSubmitBtn').disabled = false;
    ...
    var _PT_BASE = '/api/commander/provisional-tax';
    ...
    async function loadClientProvisionalTaxPlans() {
        ...
        try {
            var res = await CommanderAPI.fetch(_PT_BASE + '?client_id=' + clientId + '&limit=6');
            if (!res.ok) throw new Error('Load failed');
            var d     = await res.json();
    ...
    async function _cdPtLoadProfiles() {
        ...
        try {
            var res = await CommanderAPI.fetch('/api/commander/taxpayer-profiles?client_id=' + clientId + '&limit=20');
            if (!res.ok) throw new Error('Load failed');
            var d = await res.json();
    ...
    async function submitCdCreatePlan() {
        ...
        try {
            var res = await CommanderAPI.fetch(_PT_BASE, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(payload),
            });
            ...
            CommanderAPI.showToast('Provisional tax plan created!');
            loadClientProvisionalTaxPlans();
        } catch(e) {
            _cdPlanSubmitting = false;
            document.getElementById('cdCreatePlanSubmitBtn').disabled = false;
    ...
    var _IT_BASE = '/api/commander/individual-tax';
    ...
    async function loadClientIndividualTaxReturns() {
        ...
        try {
            var res = await CommanderAPI.fetch(_IT_BASE + '?client_id=' + _currentClientId + '&limit=6');
    ...
    async function _cdItLoadProfiles() {
        ...
        _cdItProfiles = [];
        try {
            var res = await CommanderAPI.fetch('/api/commander/taxpayer-profiles?client_id=' + _currentClientId + '&limit=100');
            if (!res.ok) return;
            var data = await res.json();
    ...
    async function submitCdCreateItReturn() {
        ...
        try {
            var res = await CommanderAPI.fetch(_IT_BASE, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
            ...
            CommanderAPI.showToast('Individual tax return created!');
            loadClientIndividualTaxReturns();
        } catch (e) {
            _cdItSubmitting = false;
            document.getElementById('cdCreateItReturnSubmitBtn').disabled = false;
    ...
    var _CT_BASE       = '/api/commander/company-tax';
    ...
    async function loadClientCompanyTaxReturns() {
        ...
        try {
            var res  = await CommanderAPI.fetch(_CT_BASE + '?client_id=' + _currentClientId + '&limit=6');
            if (!res.ok) throw new Error('Failed');
            var data = await res.json();
    ...
    async function submitCdCreateCtReturn() {
        ...
        try {
            var res = await CommanderAPI.fetch(_CT_BASE, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    client_id:           _currentClientId,
            ...
            CommanderAPI.showToast('Company tax return created!');
            loadClientCompanyTaxReturns();
        } catch(e) {
            _cdCtSubmitting = false;
            document.getElementById('cdCreateCtReturnSubmitBtn').disabled = false;
