...
async function loadClients() {
  try {
    var res = await CommanderAPI.fetch('/api/commander/clients?is_active=true&limit=500');
    if (!res.ok) return;
...
async function loadWorkflowsForClient() {
  ...
  if (!clientId) return;
  try {
    var res = await CommanderAPI.fetch('/api/commander/workflows/runs?client_id=' + clientId + '&limit=100');
    if (!res.ok) return;
    var d = await res.json();
...
async function loadTasksForClient() {
  ...
  try {
    ...
    var res = await CommanderAPI.fetch('/api/commander/tasks?client_id=' + clientId + '&status_not_in=completed,cancelled&limit=100');
    if (!res.ok) return;
    var d = await res.json();
...
async function loadWip() {
  ...
  try {
    var res = await CommanderAPI.fetch('/api/commander/time-entries/wip?' + params.join('&'));
    if (!res.ok) return;
    var d = await res.json();
    var b  = d.by_status || {};
...
async function logTime() {
  ...
  if (!hours || parseFloat(hours) <= 0) { CommanderAPI.showToast('Hours must be a positive number', true); return; }
  if (!date)                             { CommanderAPI.showToast('Date is required', true); return; }
  ...
  try {
    var res = await CommanderAPI.fetch('/api/commander/time-entries', {
      method: 'POST',
      body: JSON.stringify({
    ...
    CommanderAPI.showToast(hours + 'h logged');
    // Reset form
...
async function loadEntries() {
  ...
  try {
    var res = await CommanderAPI.fetch('/api/commander/time-entries?' + params.join('&'));
...
function renderEntries(entries, totalHours) {
  ...
  var rows = entries.map(function(e) {
    var clientName = e.commander_clients && e.commander_clients.name ? esc(e.commander_clients.name) : '–';
    var taskTitle  = e.commander_tasks   && e.commander_tasks.title  ? esc(e.commander_tasks.title)  : '–';
...
async function saveEdit(e) {
  ...
  if (!hours || !date) { CommanderAPI.showToast('Hours and date are required', true); return false; }
  ...
  try {
    var res = await CommanderAPI.fetch('/api/commander/time-entries/' + _editingId, {
      method: 'PUT',
      body: JSON.stringify({
    ...
    CommanderAPI.showToast('Entry updated');
    await Promise.all([loadEntries(), loadWip()]);
  } catch (err) {
    CommanderAPI.showToast('Error: ' + err.message, true);
  } finally {
...
async function deleteCurrentEntry() {
  ...
  try {
    var res = await CommanderAPI.fetch('/api/commander/time-entries/' + _editingId, { method: 'DELETE' });
    var d   = await res.json();
    if (!res.ok) throw new Error(d.error || 'Delete failed');
    closeModal('editModal');
    CommanderAPI.showToast('Entry deleted');
    await Promise.all([loadEntries(), loadWip()]);
  } catch (err) {
    CommanderAPI.showToast('Error: ' + err.message, true);
  }
...
async function submitForReview(id) {
  try {
    var res = await CommanderAPI.fetch('/api/commander/time-entries/' + id + '/submit-review', {
...
async function approveEntry(id) {
  try {
    var res = await CommanderAPI.fetch('/api/commander/time-entries/' + id + '/approve', {
      method: 'PUT', body: JSON.stringify({})
    });
    var d = await res.json();
    if (!res.ok) throw new Error(d.error || 'Approve failed');
    CommanderAPI.showToast('Time entry approved');
    await Promise.all([loadEntries(), loadWip()]);
  } catch (e) {
    CommanderAPI.showToast('Error: ' + e.message, true);
  }
...
async function submitReject() {
  ...
  try {
    var res = await CommanderAPI.fetch('/api/commander/time-entries/' + _rejectingId + '/reject', {
      method: 'PUT',
    ...
    CommanderAPI.showToast('Entry rejected');
    await Promise.all([loadEntries(), loadWip()]);
  } catch (e) {
    CommanderAPI.showToast('Error: ' + e.message, true);
  }
