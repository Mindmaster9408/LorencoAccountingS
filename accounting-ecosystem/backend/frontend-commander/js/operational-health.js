...
(function () {
    ...
    var BASE = '/api/commander/operational-health';
    var _tab = 'checklist';
    var _latestRun = null;
    ...
    function _loadLatest() {
        window.CommanderAPI.fetch(BASE + '/runs?limit=1')
            .then(function (r) { return r.json(); })
            .then(function (d) {
                var latest = (d.runs || [])[0];
                if (!latest || latest.run_status !== 'completed') { _renderScoreHero(null); return; }
                return window.CommanderAPI.fetch(BASE + '/runs/' + latest.id).then(function (r2) { return r2.json(); }).then(function (d2) {
                    _latestRun = d2.run;
    ...
    function ohRunHealthCheck() {
        ...
        window.CommanderAPI.fetch(BASE + '/run', { method: 'POST' })
            .then(function (r) { return r.json(); })
            .then(function (d) {
    ...
    function ohOpenRunDetail(id) {
        Promise.all([
            ...
            window.CommanderAPI.fetch(BASE + '/runs/' + id + '/events').then(function (r) { return r.json(); }),
