...
// Manages commander_tax_checklist_templates + controlled apply.
// No localStorage/KV for business data.
(function () {
    'use strict';

    var esc  = CommanderAPI.escHtml;
    var BASE = '/api/commander/tax-checklists';
    ...
    async function tcLoad() {
        ...
        try {
            var res  = await CommanderAPI.fetch(BASE + '/templates' + (qs.length ? '?' + qs.join('&') : ''));
    ...
    async function tcSeedDefaults() {
        ...
        _submitting = true;
        try {
            var res  = await CommanderAPI.fetch(BASE + '/seed-defaults', { method: 'POST' });
            var data = await res.json();
            if (!res.ok) throw new Error(data.error || ('HTTP ' + res.status));
            CommanderAPI.showToast('Seeded ' + data.templates_created + ' template(s). ' + data.templates_skipped + ' skipped (already existed).');
            tcLoad();
        } catch (err) {
            CommanderAPI.showToast('Seed failed: ' + err.message, true);
        } finally {
            _submitting = false;
        }
    }
    ...
    async function tcDeactivate(btn) {
        ...
        try {
            var res  = await CommanderAPI.fetch(BASE + '/templates/' + id, { method: 'DELETE' });
            var data = await res.json();
            if (!res.ok) throw new Error(data.error || ('HTTP ' + res.status));
            CommanderAPI.showToast('Template deactivated.');
    ...
    async function tcSubmitTemplate() {
        ...
        try {
            var res  = id
                ? await CommanderAPI.fetch(BASE + '/templates/' + id, { method: 'PUT',  headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) })
                : await CommanderAPI.fetch(BASE + '/templates',       { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
            var data = await res.json();
            if (!res.ok) throw new Error(data.error || ('HTTP ' + res.status));
            _closeModal('tcTemplateModal');
            CommanderAPI.showToast(id ? 'Template updated.' : 'Template created.');
            tcLoad();
        } catch (err) {
    ...
    async function tcOpenItems(btn) {
        ...
        try {
            var res  = await CommanderAPI.fetch(BASE + '/templates/' + id + '/items');
            var data = await res.json();
    ...
    async function tcSubmitItem() {
        ...
        try {
            var res  = await CommanderAPI.fetch(BASE + '/templates/' + templateId + '/items', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
            var data = await res.json();
            if (!res.ok) throw new Error(data.error || ('HTTP ' + res.status));
    ...
    async function tcDeleteItem(btn) {
        ...
        try {
            var res  = await CommanderAPI.fetch(BASE + '/templates/' + templateId + '/items/' + itemId, { method: 'DELETE' });
            var data = await res.json();
            if (!res.ok) throw new Error(data.error || ('HTTP ' + res.status));
    ...
    async function _loadApplyData() {
        try {
            var [clRes, pkRes, irRes, crRes] = await Promise.all([
                CommanderAPI.fetch('/api/commander/clients?is_active=true&limit=500'),
                CommanderAPI.fetch('/api/commander/compliance-packs?limit=200'),
                CommanderAPI.fetch('/api/commander/individual-tax?limit=200'),
                CommanderAPI.fetch('/api/commander/company-tax?limit=200'),
            ]);
    ...
    async function tcSubmitApply() {
        ...
        try {
            var res  = await CommanderAPI.fetch(BASE + '/templates/' + templateId + '/apply', {
                method:  'POST',
