...
(function () {
    ...
    var _BASE      = '/api/commander/individual-tax';
    var _CLIENTS_B = '/api/commander/clients';
    var _PROF_B    = '/api/commander/taxpayer-profiles';
    ...
    async function init() {
        ...
        if (typeof CommanderAPI !== 'undefined' && CommanderAPI.requireAuth) {
            var ok = await CommanderAPI.requireAuth();
            if (!ok) return;
        }
    ...
    async function loadClients() {
        try {
            var res = await CommanderAPI.fetch(_CLIENTS_B + '?limit=500');
            if (!res.ok) return;
    ...
    async function loadSummary() {
        try {
            var res = await CommanderAPI.fetch(_BASE + '/summary');
            if (!res.ok) return;
            var data = await res.json();
            var s = data.summary || {};
            var by = s.by_status || {};
    ...
    async function loadReturns() {
        ...
        try {
            var res = await CommanderAPI.fetch(_BASE + '?' + params.toString());
            ...
            hide('itTableLoading');
            CommanderAPI.showToast(e.message || 'Error loading returns', 'error');
        }
    }
    ...
    async function itClientChanged() {
        ...
        if (!clientId) return;
        try {
            var res = await CommanderAPI.fetch(_PROF_B + '?client_id=' + clientId + '&limit=100');
            if (!res.ok) return;
            var data = await res.json();
    ...
    async function submitCreateReturn() {
        ...
        try {
            var res = await CommanderAPI.fetch(_BASE, {
                method: 'POST',
            ...
            _createSubmitting = false;
            closeCreateModal();
            CommanderAPI.showToast('Tax return created!');
            loadSummary();
            loadReturns();
        } catch (e) {
            _createSubmitting = false;
    ...
    async function openDetailModal(returnId) {
        ...
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + returnId);
            if (!res.ok) throw new Error('Failed to load return');
            var data = await res.json();
            _activeReturn = data.tax_return;
            _renderOverview();
        } catch (e) {
            CommanderAPI.showToast(e.message || 'Error loading return', 'error');
            closeDetailModal();
        }
    }
    ...
    async function itSaveOverview() {
        ...
        errEl.classList.add('hidden');
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + _activeReturn.id, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
    ...
    async function itSaveStatus() {
        ...
        if (!status) { CommanderAPI.showToast('Select a status first', 'error'); return; }
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + _activeReturn.id, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
            ...
            _activeReturn = data.tax_return;
            _renderOverview();
            CommanderAPI.showToast('Status updated!');
            loadReturns();
            loadSummary();
        } catch (e) {
            CommanderAPI.showToast(e.message || 'Failed to update status', 'error');
        }
    }
    ...
    async function itRecalcReadiness() {
        if (!_activeReturn) return;
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + _activeReturn.id + '/recalculate-readiness', { method: 'POST' });
            if (!res.ok) { var d = await res.json(); throw new Error(d.error || 'Failed'); }
            var data = await res.json();
            _activeReturn.readiness_score  = data.readiness.score;
            _activeReturn.readiness_status = data.readiness.readiness_status;
            _renderOverview();
            CommanderAPI.showToast('Readiness recalculated!');
            loadReturns();
            loadSummary();
        } catch (e) {
            CommanderAPI.showToast(e.message || 'Failed to recalculate readiness', 'error');
        }
    }
    ...
    async function itGenerateItems() {
        if (!_activeReturn) return;
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + _activeReturn.id + '/generate-default-items', { method: 'POST' });
            if (res.status === 409) {
                // Try force append missing items
                var res2 = await CommanderAPI.fetch(_BASE + '/' + _activeReturn.id + '/generate-default-items?force=true', { method: 'POST' });
                if (!res2.ok) {
                    var d2 = await res2.json();
                    throw new Error(d2.error || 'Failed');
                }
                var data2 = await res2.json();
                CommanderAPI.showToast(data2.inserted > 0 ? 'Added ' + data2.inserted + ' missing item(s).' : 'All default items already exist.');
            } else if (!res.ok) {
                var d = await res.json();
                throw new Error(d.error || 'Failed');
            } else {
                var data = await res.json();
                CommanderAPI.showToast('Generated ' + data.inserted + ' checklist item(s)!');
            }
            // If currently on items tab, refresh it
            if (_activeTab === 'items') loadItems();
            itSwitchTab('items', document.getElementById('itTabBtnItems'));
        } catch (e) {
            CommanderAPI.showToast(e.message || 'Failed to generate checklist', 'error');
        }
    }
    ...
    async function loadItems() {
        ...
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + _activeReturn.id + '/items');
            ...
            hide('itItemsLoading');
            CommanderAPI.showToast(e.message || 'Error loading items', 'error');
        }
    }
    ...
    async function itUpdateItemStatus(sel) {
        ...
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + _activeReturn.id + '/items/' + itemId, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
            ...
            CommanderAPI.showToast(e.message || 'Failed to update item', 'error');
        }
    }
    ...
    async function itDeleteItem(itemId) {
        if (!_activeReturn) return;
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + _activeReturn.id + '/items/' + itemId, { method: 'DELETE' });
            if (!res.ok) { var d = await res.json(); throw new Error(d.error || 'Failed'); }
            loadItems();
        } catch (e) {
            CommanderAPI.showToast(e.message || 'Failed to delete item', 'error');
        }
    }
    ...
    async function submitAddItem() {
        ...
        if (!type)  { CommanderAPI.showToast('Select an item type', 'error');   return; }
        if (!label) { CommanderAPI.showToast('Label is required', 'error'); return; }
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + _activeReturn.id + '/items', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
            ...
            CommanderAPI.showToast(e.message || 'Failed to add item', 'error');
        }
    }
    ...
    async function loadIncome() {
        ...
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + _activeReturn.id + '/income');
            ...
            hide('itIncomeLoading');
            CommanderAPI.showToast(e.message || 'Error loading income', 'error');
        }
    }
    ...
    async function submitAddIncome() {
        ...
        if (!type) { CommanderAPI.showToast('Select income type', 'error'); return; }
        ...
        try {
            ...
            CommanderAPI.showToast(e.message || 'Failed to add income entry', 'error');
        }
    }
    ...
    async function submitEditIncome() {
        ...
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + _activeReturn.id + '/income/' + incomeId, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
            ...
            CommanderAPI.showToast(e.message || 'Failed to save income entry', 'error');
        }
    }
    ...
    async function deleteIncome(incomeId) {
        if (!_activeReturn) return;
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + _activeReturn.id + '/income/' + incomeId, { method: 'DELETE' });
            if (!res.ok) { var d = await res.json(); throw new Error(d.error || 'Failed'); }
            loadIncome();
        } catch (e) {
            CommanderAPI.showToast(e.message || 'Failed to delete income entry', 'error');
        }
    }
    ...
    async function loadDeductions() {
        ...
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + _activeReturn.id + '/deductions');
            ...
            hide('itDedLoading');
            CommanderAPI.showToast(e.message || 'Error loading deductions', 'error');
        }
    }
    ...
    async function submitAddDeduction() {
        ...
        if (!type) { CommanderAPI.showToast('Select deduction type', 'error'); return; }
        ...
        try {
            ...
            CommanderAPI.showToast(e.message || 'Failed to add deduction', 'error');
        }
    }
    ...
    async function submitEditDeduction() {
        ...
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + _activeReturn.id + '/deductions/' + dedId, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
            ...
            CommanderAPI.showToast(e.message || 'Failed to save deduction', 'error');
        }
    }
    ...
    async function deleteDeduction(dedId) {
        if (!_activeReturn) return;
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + _activeReturn.id + '/deductions/' + dedId, { method: 'DELETE' });
            if (!res.ok) { var d = await res.json(); throw new Error(d.error || 'Failed'); }
            loadDeductions();
        } catch (e) {
            CommanderAPI.showToast(e.message || 'Failed to delete deduction', 'error');
        }
    }
    ...
    async function loadEvents() {
        ...
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + _activeReturn.id + '/events');
            ...
            hide('itHistoryLoading');
            CommanderAPI.showToast(e.message || 'Error loading events', 'error');
        }
    }
    ...
    async function loadCalculations() {
        ...
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + _activeReturn.id + '/calculations');
            ...
            hide('itCalcLoading');
            CommanderAPI.showToast(e.message || 'Error loading calculations', 'error');
        }
    }
    ...
    async function itRunDraftCalc() {
        ...
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + _activeReturn.id + '/calculations/run-draft', {
                method: 'POST',
            });
            ...
            CommanderAPI.showToast('Draft calculation v' + data.calculation.calculation_version + ' created!');
            loadCalculations();
            ...
            if (btn) btn.disabled = false;
            hide('itCalcLoading');
            CommanderAPI.showToast(e.message || 'Failed to run calculation', 'error');
        }
    }
    ...
    async function openCalcDetailModal(calcId) {
        ...
        try {
            var res = await CommanderAPI.fetch(_BASE + '/calculations/' + calcId);
            if (!res.ok) throw new Error('Failed to load calculation');
            var data = await res.json();
            _activeCalc = data.calculation;
            _renderCalcDetail();
        } catch (e) {
            CommanderAPI.showToast(e.message || 'Error loading calculation', 'error');
    ...
    async function itSaveCalcNotes() {
        ...
        try {
            var res = await CommanderAPI.fetch(_BASE + '/calculations/' + _activeCalc.id, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
            ...
            CommanderAPI.showToast(e.message || 'Failed to save', 'error');
        }
    }
    ...
    async function itSubmitCalcForReview() {
        if (!_activeCalc) return;
        try {
            var res = await CommanderAPI.fetch(_BASE + '/calculations/' + _activeCalc.id + '/submit-review', { method: 'POST' });
            if (!res.ok) { var d = await res.json(); throw new Error(d.error || 'Failed'); }
            var data = await res.json();
            _activeCalc = data.calculation;
            _renderCalcDetail();
            CommanderAPI.showToast('Submitted for review!');
            loadCalculations();
        } catch (e) {
            CommanderAPI.showToast(e.message || 'Failed to submit for review', 'error');
        }
    }
    ...
    async function itApproveCalc() {
        if (!_activeCalc) return;
        try {
            var res = await CommanderAPI.fetch(_BASE + '/calculations/' + _activeCalc.id + '/approve', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({}),
            });
            ...
            _activeCalc = data.calculation;
            _renderCalcDetail();
            CommanderAPI.showToast('Calculation approved!');
            loadCalculations();
        } catch (e) {
            CommanderAPI.showToast(e.message || 'Failed to approve', 'error');
        }
    }
    ...
    async function itRejectCalc() {
        ...
        try {
            var res = await CommanderAPI.fetch(_BASE + '/calculations/' + _activeCalc.id + '/reject', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
            ...
            _renderCalcDetail();
            CommanderAPI.showToast('Calculation rejected.');
            loadCalculations();
        } catch (e) {
            CommanderAPI.showToast(e.message || 'Failed to reject', 'error');
        }
    }
    ...
    async function loadReviewPacks() {
        ...
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + _activeReturn.id + '/review-packs');
            ...
            hide('itRpLoading');
            CommanderAPI.showToast(e.message || 'Error loading review packs', 'error');
        }
    }
    ...
    async function itGenerateReviewPack() {
        if (!_activeReturn) return;
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + _activeReturn.id + '/review-packs/generate', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({}),
            });
            if (!res.ok) { var d = await res.json(); throw new Error(d.error || 'Failed to generate'); }
            CommanderAPI.showToast('Review pack generated.');
            loadReviewPacks();
        } catch (e) {
            CommanderAPI.showToast(e.message || 'Failed to generate review pack', 'error');
        }
    }
    ...
    async function viewRpReport(packId, format) {
        try {
            var res = await CommanderAPI.fetch(_BASE + '/review-packs/' + packId + '/report-' + format);
            ...
            CommanderAPI.showToast(e.message || 'Failed to open report', 'error');
        }
    }
    ...
    async function itSubmitPackForReview(packId) {
        try {
            var res = await CommanderAPI.fetch(_BASE + '/review-packs/' + packId + '/submit-review', {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({}),
            });
            if (!res.ok) { var d = await res.json(); throw new Error(d.error || 'Failed'); }
            CommanderAPI.showToast('Pack submitted for review.');
            loadReviewPacks();
        } catch (e) {
            CommanderAPI.showToast(e.message || 'Failed to submit for review', 'error');
        }
    ...
    async function itApproveReviewPack(packId) {
        try {
            var res = await CommanderAPI.fetch(_BASE + '/review-packs/' + packId + '/approve', {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({}),
            });
            if (!res.ok) { var d = await res.json(); throw new Error(d.error || 'Failed'); }
            CommanderAPI.showToast('Review pack approved.');
            loadReviewPacks();
        } catch (e) {
            CommanderAPI.showToast(e.message || 'Failed to approve', 'error');
        }
    ...
    async function itConfirmRejectPack() {
        ...
        if (!reason) { CommanderAPI.showToast('Rejection reason is required', 'error'); return; }
        try {
            var res = await CommanderAPI.fetch(_BASE + '/review-packs/' + _activePackRejectId + '/reject', {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
            ...
            if (!res.ok) { var d = await res.json(); throw new Error(d.error || 'Failed'); }
            closeRpRejectModal();
            CommanderAPI.showToast('Review pack rejected.');
            loadReviewPacks();
        } catch (e) {
            CommanderAPI.showToast(e.message || 'Failed to reject', 'error');
        }
    ...
    async function loadRpEvents(packId) {
        ...
        try {
            var res = await CommanderAPI.fetch(_BASE + '/review-packs/' + packId + '/events');
            if (!res.ok) throw new Error('Failed to load events');
            var data = await res.json();
            var events = data.events || [];
