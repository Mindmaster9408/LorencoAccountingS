...
(function () {
    ...
    var BASE = '/api/commander/pricing-review';
    ...
    function prPrepare() {
        ...
        window.CommanderAPI.fetch(BASE + '/prepare?' + qs)
            .then(function (r) { return r.json(); })
            .then(function (d) {
                if (d.error) { return; }
                _prepared = d;
    ...
    function prAction(action) {
        window.CommanderAPI.fetch(BASE + '/' + _currentReviewId + '/' + action, { method: 'PUT' })
            .then(function (r) { return r.json(); })
            .then(function (d) {
                if (d.error) { _showToast(d.error); return; }
    ...
    function prSubmitItem() {
        ...
        window.CommanderAPI.fetch(BASE + '/' + _currentReviewId + '/items', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
            .then(function (r) { return r.json(); })
