(function () {
    ...
    var BASE      = '/api/commander/tax-submissions';
    var PAY_BASE  = '/api/commander/tax-payments';
    ...
    function _loadSummary() {
        CommanderAPI.fetch(BASE + '/summary')
    ...
    function _renderFooter(d) {
        ...
        html += '<a href="/commander/tax-disputes.html?submission_id=' + encodeURIComponent(d.id) + '" ' +
        ...
        html += '<a href="/commander/tax-completion.html?submission_id=' + encodeURIComponent(d.id) + '" ' +
        ...
        html += '<a href="/commander/knowledge-base.html?linked_type=tax_submission&linked_id=' + encodeURIComponent(d.id) + '" ' +
    ...
    function _loadEvidenceTab(body) {
        ...
        CommanderAPI.fetch(BASE + '/' + _currentId + '/evidence')
            .then(function (r) { return r.json(); })
    ...
    function _loadPaymentsTab(body) {
        ...
        CommanderAPI.fetch(PAY_BASE + '?submission_id=' + _currentId)
            .then(function (r) { return r.json(); })
    ...
    function _renderPaymentsTab(items, body) {
        ...
        html += '<a href="/commander/tax-payments.html?submission_id=' + _currentId + '" style="display:inline-block;margin-top:6px;padding:7px 16px;background:#667eea;color:#fff;border-radius:7px;font-size:.82rem;font-weight:700;text-decoration:none;">Open Payment Register</a>';
        html += ' <a href="/commander/sars-recon.html?submission_id=' + _currentId + '" style="display:inline-block;margin-top:6px;padding:7px 16px;background:#2d3748;color:#e2e8f0;border-radius:7px;font-size:.82rem;font-weight:700;text-decoration:none;">SARS Recon ↗</a>';
        html += '</div>';
        body.innerHTML = html;
    }
    ...
    function _loadEventsTab(body) {
        ...
        CommanderAPI.fetch(BASE + '/' + _currentId + '/events')
            .then(function (r) { return r.json(); })
    ...
    function tslVerifyEvidence(evidenceId) {
        if (_submitting) return;
        _submitting = true;
        CommanderAPI.fetch(BASE + '/' + _currentId + '/evidence/' + evidenceId + '/verify', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: '{}' })
            .then(function (r) { return r.json().then(function (d) { return { ok: r.ok, data: d }; }); })
            .then(function (r) {
                _submitting = false;
    ...
    function tslDeleteEvidence(evidenceId) {
        ...
        _submitting = true;
        CommanderAPI.fetch(BASE + '/' + _currentId + '/evidence/' + evidenceId, { method: 'DELETE' })
            .then(function (r) { return r.json().then(function (d) { return { ok: r.ok, data: d }; }); })
            .then(function (r) {
                _submitting = false;
    ...
    function tslCancelSubmission() {
        ...
        _submitting = true;
        CommanderAPI.fetch(BASE + '/' + _currentId, {
            method:  'DELETE',
            headers: { 'Content-Type': 'application/json' },
    ...
    function _tslSimpleAction(endpoint, confirmMsg, successMsg) {
        ...
        _submitting = true;
        CommanderAPI.fetch(BASE + '/' + _currentId + '/' + endpoint, {
            method:  'PUT',
            headers: { 'Content-Type': 'application/json' },
            body:    JSON.stringify({}),
        })
