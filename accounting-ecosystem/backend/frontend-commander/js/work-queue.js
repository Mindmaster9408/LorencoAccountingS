...
(function () {
    ...
    var BASE = '/api/commander/work-queue';
    var NOTIF_BASE = '/api/commander/notifications';
    var _view = 'my_work';
    ...
    function _loadPreferences() {
        return window.CommanderAPI.fetch(BASE + '/preferences')
    ...
    function wqToggleSection(key) {
        ...
        window.CommanderAPI.fetch(BASE + '/preferences', {
    ...
    function _loadSummary() {
        window.CommanderAPI.fetch(BASE + '/summary' + _withViewAs(''))
            .then(function (r) { return r.json(); })
            .then(function (d) {
                if (d.unlinked) {
    ...
    function _loadQueue() {
        ...
        window.CommanderAPI.fetch(BASE + '/' + (VIEW_ENDPOINTS[_view] || 'my-work') + _withViewAs(qs))
            .then(function (r) { return r.json(); })
            .then(function (d) {
                var items = d.items || [];
    ...
    function _loadWaitingOnMe() {
        window.CommanderAPI.fetch(BASE + '/waiting-on-me' + _withViewAs(''))
            .then(function (r) { return r.json(); })
            .then(function (d) {
                var items = d.items || [];
    ...
    function _loadWaitingOnOthers() {
        window.CommanderAPI.fetch(BASE + '/waiting-on-others' + _withViewAs(''))
            .then(function (r) { return r.json(); })
            .then(function (d) {
                var items = d.items || [];
    ...
    function _renderItem(item) {
        ...
        var delegateBtn = delegateModule
            ? '<button class="wi-open" style="background:#805ad5;margin-left:6px;" onclick="event.stopPropagation();window.location.href=\'/commander/delegation.html?delegate=1&source_module=' + delegateModule + '&source_id=' + item.source_id + '&role=' + encodeURIComponent(item.role) + '\'">Delegate</button>'
            : '';
    ...
    function _loadNotifications() {
        window.CommanderAPI.fetch(BASE + '/notifications?limit=10')
            .then(function (r) { return r.json(); })
            .then(function (d) {
    ...
    function wqOpenNotification(id) {
        window.CommanderAPI.fetch(NOTIF_BASE + '/' + id + '/read', { method: 'PUT' })
            .then(function () {
                _logEvent('item_opened', 'notifications', 'commander_notification', id);
                window.location.href = '/commander/notifications.html';
            })
            .catch(function () { window.location.href = '/commander/notifications.html'; });
    }

    // ── Completed ─────────────────────────────────────────────────────────────

    function _loadCompleted() {
        window.CommanderAPI.fetch(BASE + '/completed' + _withViewAs(''))
            .then(function (r) { return r.json(); })
            .then(function (d) {
                var items = d.items || [];
