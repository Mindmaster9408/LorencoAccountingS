...
async function loadTeamMembers() {
  try {
    var res = await CommanderAPI.fetch('/api/commander/team');
    if (!res.ok) return;
    var d    = await res.json();
...
async function loadTemplates() {
  try {
    var res = await CommanderAPI.fetch('/api/commander/workflows/templates');
...
function openNewTemplate() {
  window.location.href = '/commander/workflow-template.html';
}

function editTemplate(id) {
  window.location.href = '/commander/workflow-template.html?id=' + id;
}
...
async function submitGenerate(e) {
  ...
  if (createDeadline && !dueDate && !templateHasOffset) {
    CommanderAPI.showToast('Please enter a due date for the compliance deadline.', true);
  ...
  try {
    var res = await CommanderAPI.fetch('/api/commander/workflows/generate', {
      method: 'POST',
