...
(function () {
    ...
    var _BASE        = '/api/commander/tax-configs';
    ...
    async function loadConfigs() {
        ...
        try {
            var res = await CommanderAPI.fetch(url);
            ...
            hide('tcListLoading');
            CommanderAPI.showToast(e.message || 'Error loading configs', 'error');
        }
    }
    ...
    async function quickActivate(id) {
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + id + '/activate', { method: 'PUT' });
            if (!res.ok) { var d = await res.json(); throw new Error(d.error || 'Failed'); }
            CommanderAPI.showToast('Config activated!');
            loadConfigs();
        } catch (e) { CommanderAPI.showToast(e.message || 'Failed to activate', 'error'); }
    }

    async function quickArchive(id) {
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + id + '/archive', { method: 'PUT' });
            if (!res.ok) { var d = await res.json(); throw new Error(d.error || 'Failed'); }
            CommanderAPI.showToast('Config archived.');
            loadConfigs();
        } catch (e) { CommanderAPI.showToast(e.message || 'Failed to archive', 'error'); }
    }
    ...
    async function openDetailModal(configId) {
        ...
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + configId);
            ...
            CommanderAPI.showToast(e.message || 'Error loading config', 'error');
            closeTcDetailModal();
        }
    }
    ...
    async function tcSaveOverview() {
        ...
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + _activeConfig.id, {
                method: 'PUT',
    ...
    async function tcSaveRebates() {
        ...
        try {
            ...
            var data = await res.json();
            _activeConfig = data.config;
            CommanderAPI.showToast('Rebates & credits saved!');
        } catch (e) {
            errEl.textContent = e.message || 'Failed to save';
            errEl.classList.remove('hidden');
        }
    ...
    async function tcSaveLimits() {
        ...
        try {
            ...
            var data = await res.json();
            _activeConfig = data.config;
            CommanderAPI.showToast('Deduction limits saved!');
        } catch (e) {
            errEl.textContent = e.message || 'Failed to save';
            errEl.classList.remove('hidden');
        }
    ...
    async function tcActivateConfig() {
        if (!_activeConfig) return;
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + _activeConfig.id + '/activate', { method: 'PUT' });
            if (!res.ok) { var d = await res.json(); throw new Error(d.error || 'Failed'); }
            var data = await res.json();
            _activeConfig = data.config;
            _renderDetailOverview();
            CommanderAPI.showToast('Config activated! Draft calculations will now use this config for tax year ' + _activeConfig.tax_year);
            loadConfigs();
        } catch (e) { CommanderAPI.showToast(e.message || 'Failed to activate', 'error'); }
    }
    ...
    async function tcArchiveConfig() {
        if (!_activeConfig) return;
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + _activeConfig.id + '/archive', { method: 'PUT' });
            if (!res.ok) { var d = await res.json(); throw new Error(d.error || 'Failed'); }
            var data = await res.json();
            _activeConfig = data.config;
            _renderDetailOverview();
    ...
    async function tcLockConfig() {
        ...
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + _activeConfig.id + '/lock', { method: 'PUT' });
            if (!res.ok) { var d = await res.json(); throw new Error(d.error || 'Failed'); }
            var data = await res.json();
            _activeConfig = data.config;
            _renderDetailOverview();
            _renderBrackets();
            CommanderAPI.showToast('Config locked.');
        } catch (e) { CommanderAPI.showToast(e.message || 'Failed to lock', 'error'); }
    }
    ...
    async function tcSubmitAddBracket() {
        ...
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + _activeConfig.id + '/brackets', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(payload),
            });
            ...
            _renderBrackets();
            closeAddBracketForm();
            CommanderAPI.showToast('Bracket added!');
        } catch (e) {
    ...
    async function tcSubmitEditBracket() {
        ...
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + _activeConfig.id + '/brackets/' + _editBracketId, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(payload),
            });
            ...
            CommanderAPI.showToast('Bracket updated!');
        } catch (e) {
    ...
    async function tcDeleteBracket() {
        ...
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + _activeConfig.id + '/brackets/' + _editBracketId, { method: 'DELETE' });
            if (!res.ok) { var d = await res.json(); throw new Error(d.error || 'Failed'); }
            _activeBrackets = _activeBrackets.filter(function (x) { return x.id !== _editBracketId; });
            _renderBrackets();
            closeEditBracketModal();
            CommanderAPI.showToast('Bracket deleted.');
        } catch (e) {
    ...
    async function loadConfigHistory() {
        ...
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + _activeConfig.id + '/events');
            ...
            hide('tcHistoryLoading');
            CommanderAPI.showToast(e.message || 'Error loading history', 'error');
        }
    }
    ...
    async function tcSubmitCreate() {
        ...
        try {
            ...
            var data = await res.json();
            closeTcCreateModal();
            CommanderAPI.showToast('Config created! Opening detail editor…');
            loadConfigs();
    ...
    async function tcRunSeed() {
        ...
        try {
            var res = await CommanderAPI.fetch(_BASE + '/seed-from-js', { method: 'POST' });
            ...
            if (seeded.some(function (s) { return s.action === 'created'; })) {
                CommanderAPI.showToast('Seeded! Verify values before activating.');
                loadConfigs();
            }
        } catch (e) {
            CommanderAPI.showToast(e.message || 'Seed failed', 'error');
            btn.disabled = false;
        }
    }
