...
async function loadTemplate(id) {
  try {
    var res = await CommanderAPI.fetch('/api/commander/workflows/templates/' + id);
    ...
    if (sopLink) {
      sopLink.href = '/commander/practice-sop.html?linked_type=workflow_template&linked_id=' + encodeURIComponent(id);
      sopLink.style.display = '';
    }
    await loadSteps(id);
  } catch (e) {
    CommanderAPI.showToast('Failed to load template: ' + e.message, true);
  }
}
...
async function saveTemplate() {
  ...
  if (!name) {
    CommanderAPI.showToast('Template name is required', true);
  ...
  try {
    ...
    var url    = '/api/commander/workflows/templates' + (isNew ? '' : '/' + _currentTemplate.id);
    var method = isNew ? 'POST' : 'PUT';
    var res = await CommanderAPI.fetch(url, { method: method, body: JSON.stringify(body) });
    var d   = await res.json();
    if (!res.ok) throw new Error(d.error || 'Save failed');
    CommanderAPI.showToast('Template saved');
    if (isNew) {
      // Redirect to edit mode with the new ID so steps section appears
      window.location.href = '/commander/workflow-template.html?id=' + d.template.id;
    } else {
...
async function loadSteps(templateId) {
  ...
  if (!tid) return;
  try {
    var res = await CommanderAPI.fetch('/api/commander/workflows/templates/' + tid + '/steps');
    var d = await res.json();
  ...
  el.innerHTML = steps.map(function(s) {
    ...
    return '<div class="step-card" data-step-id="' + s.id + '">' +
      ...
      '<div class="step-card-actions">' +
        ...
        '<a class="btn-ghost-sm" href="/commander/practice-sop.html?linked_type=workflow_step&linked_id=' + s.id + '" title="View SOPs attached to this step" aria-label="View instructions">📋 Instructions</a>' +
...
async function openEditStep(id) {
  try {
    var res = await CommanderAPI.fetch('/api/commander/workflows/templates/' + _currentTemplate.id + '/steps');
    var d = await res.json();
    var s = (d.steps || []).find(function(x) { return x.id === id; });
    if (!s) { CommanderAPI.showToast('Step not found', true); return; }
    _editingStepId = id;
    ...
    CommanderAPI.showToast('Failed to load step: ' + e.message, true);
  }
}
...
async function submitStepForm(e) {
  ...
  if (!title) { CommanderAPI.showToast('Step title is required', true); return; }
  ...
  try {
    var url, method;
    if (_editingStepId) {
      url    = '/api/commander/workflows/templates/' + _currentTemplate.id + '/steps/' + _editingStepId;
      method = 'PUT';
    } else {
      url    = '/api/commander/workflows/templates/' + _currentTemplate.id + '/steps';
      method = 'POST';
    }
    var res = await CommanderAPI.fetch(url, { method: method, body: JSON.stringify(body) });
    var d   = await res.json();
    if (!res.ok) throw new Error(d.error || 'Save failed');
    CommanderAPI.showToast(_editingStepId ? 'Step updated' : 'Step added');
    closeStepModal();
    await loadSteps();
  } catch (err) {
    CommanderAPI.showToast('Error: ' + err.message, true);
...
async function confirmDeleteStep(id) {
  ...
  try {
    var res = await CommanderAPI.fetch(
      '/api/commander/workflows/templates/' + _currentTemplate.id + '/steps/' + id,
      { method: 'DELETE' }
    );
    var d = await res.json();
    if (!res.ok) throw new Error(d.error || 'Delete failed');
    CommanderAPI.showToast('Step deleted');
    await loadSteps();
  } catch (err) {
    CommanderAPI.showToast('Error: ' + err.message, true);
  }
...
async function sendReorder() {
  ...
  try {
    var res = await CommanderAPI.fetch(
      '/api/commander/workflows/templates/' + _currentTemplate.id + '/steps/reorder',
      { method: 'PUT', body: JSON.stringify({ stepIds: stepIds }) }
    );
    var d = await res.json();
    if (!res.ok) throw new Error(d.error || 'Reorder failed');
    CommanderAPI.showToast('Order saved');
  } catch (err) {
    CommanderAPI.showToast('Reorder failed — ' + err.message, true);
