...
(function () {
    ...
    var _BASE = '/api/commander/provisional-tax';
    ...
    async function openDetailModal(planId) {
        ...
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + planId);
            if (!res.ok) throw new Error('Load failed');
            var data = await res.json();
    ...
    async function ptSaveOverviewEdits() {
        ...
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + _activePlan.id, { method: 'PUT', body: JSON.stringify(body) });
            if (!res.ok) { var e = await res.json(); throw new Error(e.error || 'Failed'); }
            var data = await res.json();
            _activePlan = data.plan;
    ...
    async function ptUpdateStatus() {
        ...
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + _activePlan.id, { method: 'PUT', body: JSON.stringify({ status: newStatus }) });
            if (!res.ok) { var e = await res.json(); throw new Error(e.error || 'Failed'); }
            var data = await res.json();
            _activePlan = data.plan;
            renderDetailHeader(_activePlan);
    ...
    async function ptMarkReviewed() {
        ...
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + _activePlan.id + '/review', { method: 'POST' });
            if (!res.ok) { var e = await res.json(); throw new Error(e.error || 'Failed'); }
            var data = await res.json();
            _activePlan = data.plan;
            renderDetailHeader(_activePlan);
    ...
    async function ptCreatePeriods() {
        ...
        clearError('ptOverviewError');
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + _activePlan.id + '/create-periods', { method: 'POST' });
            if (res.status === 409) {
    ...
    async function ptSavePeriod(periodId) {
        ...
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + _activePlan.id + '/periods/' + periodId, { method: 'PUT', body: JSON.stringify(body) });
            if (!res.ok) { var e = await res.json(); throw new Error(e.error || 'Failed'); }
    ...
    async function ptUpdatePeriodStatus(periodId) {
        ...
        if (!status) return;
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + _activePlan.id + '/periods/' + periodId + '/status', {
                method: 'PUT',
    ...
    async function loadEvents() {
        ...
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + _activePlan.id + '/events');
            if (!res.ok) throw new Error('Load failed');
            var data   = await res.json();
