/* ============================================================
   ...
   ============================================================ */
(function () {
    var _BASE     = '/api/commander/company-tax';
    ...
    async function loadClients() {
        try {
            var res = await CommanderAPI.fetch('/api/commander/clients?is_active=true');
            if (!res.ok) return;
            var d = await res.json();
            _clients = d.clients || [];
    ...
    async function loadReturns() {
        ...
        try {
            ...
            var res = await CommanderAPI.fetch(_BASE + (qs ? '?' + qs : ''));
    ...
    async function ctLoadProfilesForClient() {
        ...
        try {
            var res = await CommanderAPI.fetch('/api/commander/taxpayer-profiles?client_id=' + clientId + '&limit=100');
    ...
    async function loadCtOverview() {
        ...
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + _currentReturnId);
    ...
    async function ctSaveOverview() {
        ...
        try {
            ...
            _currentReturn = d.company_tax_return;
            renderOverview(_currentReturn);
            CommanderAPI.showToast('Return saved.');
            loadReturns();
        } catch(e) {
    ...
    async function ctCancelReturn() {
        ...
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + _currentReturnId, { method: 'DELETE' });
            if (!res.ok) { var d = await res.json(); throw new Error(d.error); }
            CommanderAPI.showToast('Return cancelled.');
            closeDetailModal();
            loadReturns();
        } catch(e) {
            CommanderAPI.showToast('Failed to cancel: ' + (e.message || ''), true);
        }
    }
    ...
    async function ctRecalculateReadiness() {
        if (!_currentReturnId) return;
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + _currentReturnId + '/recalculate-readiness', { method: 'POST' });
            ...
            CommanderAPI.showToast('Failed to recalculate.', true);
        }
    }
    ...
    async function ctSaveAfs() {
        ...
        try {
            ...
            var d = await res.json();
            _currentReturn = d.company_tax_return;
            CommanderAPI.showToast('AFS inputs saved.');
        } catch(e) {
    ...
    async function loadCtAdjustments() {
        ...
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + _currentReturnId + '/adjustments');
            if (!res.ok) throw new Error('Failed');
            var d = await res.json();
    ...
    async function submitAdj() {
        ...
        try {
            ...
            var res    = await CommanderAPI.fetch(url, { method, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
            if (!res.ok) { var ed = await res.json(); throw new Error(ed.error || 'Failed'); }
            _adjSubmitting = false;
            closeAdjModal();
            CommanderAPI.showToast(_adjEditingId ? 'Adjustment updated.' : 'Adjustment added.');
            loadCtAdjustments();
        } catch(e) {
            _adjSubmitting = false;
            document.getElementById('ctAdjSubmitBtn').disabled = false;
    ...
    async function deleteAdj(adjId) {
        ...
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + _currentReturnId + '/adjustments/' + adjId, { method: 'DELETE' });
            if (!res.ok) throw new Error('Failed');
            CommanderAPI.showToast('Adjustment deleted.');
            loadCtAdjustments();
        } catch(e) {
            CommanderAPI.showToast('Failed to delete adjustment.', true);
        }
    }
    ...
    async function loadCtReadiness() {
        ...
        try {
            var [itemsRes, returnRes] = await Promise.all([
                CommanderAPI.fetch(_BASE + '/' + _currentReturnId + '/items'),
                CommanderAPI.fetch(_BASE + '/' + _currentReturnId),
            ]);
    ...
    async function ctUpdateItemStatus(itemId, status) {
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + _currentReturnId + '/items/' + itemId, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
            ...
            CommanderAPI.showToast('Failed to update item status.', true);
        }
    }
    ...
    async function ctDeleteReadinessItem(itemId) {
        ...
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + _currentReturnId + '/items/' + itemId, { method: 'DELETE' });
            if (!res.ok) throw new Error('Failed');
            CommanderAPI.showToast('Item removed.');
            loadCtReadiness();
            ctRecalculateReadiness();
        } catch(e) {
            CommanderAPI.showToast('Failed to remove item.', true);
        }
    }
    ...
    async function ctGenerateDefaultItems() {
        if (!_currentReturnId) return;
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + _currentReturnId + '/generate-default-items', { method: 'POST' });
            if (!res.ok) { var d = await res.json(); throw new Error(d.error || 'Failed'); }
            var d = await res.json();
            CommanderAPI.showToast('Generated ' + d.items_created + ' items (' + d.items_skipped + ' already existed).');
            loadCtReadiness();
            loadCtOverview();
        } catch(e) {
            CommanderAPI.showToast('Failed to generate items: ' + (e.message || ''), true);
        }
    }
    ...
    async function loadCtEvents() {
        ...
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + _currentReturnId + '/events');
            if (!res.ok) throw new Error('Failed');
            var d = await res.json();
    ...
    var _CALC_BASE        = '/api/commander/company-tax';
    ...
    async function loadCtCalcs() {
        ...
        try {
            var res = await CommanderAPI.fetch(_CALC_BASE + '/' + _currentReturnId + '/calculations');
    ...
    async function ctRunDraft() {
        ...
        try {
            ...
            CommanderAPI.showToast('Draft calculation created.');
            ...
            _calcSubmitting = false;
            if (btn) btn.disabled = false;
            CommanderAPI.showToast('Error: ' + (e.message || 'Failed to run calculation.'));
        }
    }
    ...
    async function ctOpenCalcDetail(calcId) {
        ...
        try {
            var res = await CommanderAPI.fetch(_CALC_BASE + '/calculations/' + calcId);
    ...
    async function ctSubmitForReview(calcId) {
        ...
        if (errEl) errEl.classList.add('hidden');
        try {
            var res = await CommanderAPI.fetch(_CALC_BASE + '/calculations/' + calcId + '/submit-review', { method: 'POST' });
            var d   = await res.json();
    ...
    async function ctApproveCalc(calcId) {
        ...
        if (errEl) errEl.classList.add('hidden');
        try {
            var res = await CommanderAPI.fetch(_CALC_BASE + '/calculations/' + calcId + '/approve', { method: 'POST' });
            var d   = await res.json();
            if (!res.ok) throw new Error(d.error || 'Failed');
    ...
    var _RP_BASE      = '/api/commander/company-tax';
    ...
    async function loadCtReviewPacks() {
        ...
        try {
            var res = await CommanderAPI.fetch(_RP_BASE + '/' + _currentReturnId + '/review-packs');
    ...
    async function ctGenerateReviewPack() {
        ...
        try {
            ...
            CommanderAPI.showToast('Error: ' + (e.message || 'Failed to generate review pack.'));
        } finally {
            _rpSubmitting = false;
    ...
    async function ctOpenRpDetail(packId) {
        ...
        try {
            var res = await CommanderAPI.fetch(_RP_BASE + '/review-packs/' + packId);
            var d = await res.json();
    ...
    async function ctViewRpReport(packId) {
        try {
            var res = await CommanderAPI.fetch(_RP_BASE + '/review-packs/' + packId + '/report-html');
            ...
            CommanderAPI.showToast('Error: ' + (e.message || 'Could not open report.'));
        }
    }

    async function ctDownloadRpPdf(packId) {
        try {
            var res = await CommanderAPI.fetch(_RP_BASE + '/review-packs/' + packId + '/report-pdf');
            if (res.status === 501) {
                CommanderAPI.showToast('PDF not available — use "View Report" for the HTML version.');
                return;
            }
            ...
            CommanderAPI.showToast('Error: ' + (e.message || 'Could not download PDF.'));
        }
    }

    async function ctSubmitRpReview(packId) {
        try {
            var res = await CommanderAPI.fetch(_RP_BASE + '/review-packs/' + packId + '/submit-review', { method: 'PUT' });
            var d = await res.json();
            if (!res.ok) throw new Error(d.error || 'Failed');
            CommanderAPI.showToast('Submitted for review.');
            await loadCtReviewPacks();
            ctOpenRpDetail(packId);
        } catch (e) { CommanderAPI.showToast('Error: ' + (e.message || 'Failed.')); }
    }
