...
(function () {
    ...
    // ── Constants ─────────────────────────────────────────────────────────────
    var BASE = '/api/commander/tax-completion';
    ...
    function _loadList() {
        ...
        CommanderAPI.fetch(BASE + '/?' + params.toString())
            .then(function (d) {
                _total = d.total || 0;
    ...
    function _renderEventsTab(packId) {
        ...
        CommanderAPI.fetch(BASE + '/' + packId + '/events')
            .then(function (d) {
                var events = d.events || [];
                if (!events.length) {
    ...
    function _renderFooter(d) {
        ...
        if (d.submission_id && s !== 'cancelled') {
            html += ' <a href="/commander/tax-disputes.html?submission_id=' + encodeURIComponent(d.submission_id) + '" ' +
        ...
        html += ' <a href="/commander/knowledge-base.html?linked_type=tax_completion_pack&linked_id=' + encodeURIComponent(d.id) + '" ' +
        ...
        html += ' <a href="/commander/practice-sop.html?linked_type=completion_pack&linked_id=' + encodeURIComponent(d.id) + '" ' +
        ...
        html += ' <a href="/commander/quality-management.html?linked_type=completion_pack&linked_id=' + encodeURIComponent(d.id) + '" ' +
            'style="display:inline-flex;align-items:center;padding:7px 14px;background:#1e2d4d;color:#90cdf4;border-radius:8px;font-size:.82rem;font-weight:700;text-decoration:none;" ' +
            'title="View quality reviews for this completion pack">QMS Review ↗</a>';
        // Codebox 49 — Risk Register integration
        html += ' <a href="/commander/risk-register.html?source_type=completion_pack&source_id=' + encodeURIComponent(d.id) + '" ' +
    ...
    function tcToggleItem(itemId) {
        ...
        CommanderAPI.fetch(BASE + '/' + _currentId + '/items/' + itemId, {
            method:  'PUT',
            headers: { 'Content-Type': 'application/json' },
            body:    JSON.stringify({ completed: !item.completed }),
        })
        .then(function () { return CommanderAPI.fetch(BASE + '/' + _currentId); })
        .then(function (d) {
            _submitting  = false;
            _currentPack = d;
            _renderDetail(d);
    ...
    function tcDeleteItem(itemId) {
        ...
        CommanderAPI.fetch(BASE + '/' + _currentId + '/items/' + itemId, { method: 'DELETE' })
            .then(function () { return CommanderAPI.fetch(BASE + '/' + _currentId); })
            .then(function (d) {
                _currentPack = d;
                _renderDetail(d);
    ...
    function tcSaveItem() {
        ...
        CommanderAPI.fetch(BASE + '/' + _currentId + '/items', {
            method:  'POST',
            headers: { 'Content-Type': 'application/json' },
    ...
    function tcGenerateDefaults() {
        ...
        CommanderAPI.fetch(BASE + '/' + _currentId + '/generate-default-items', { method: 'POST' })
            .then(function () { return CommanderAPI.fetch(BASE + '/' + _currentId); })
            .then(function (d) {
                _currentPack = d;
                _renderDetail(d);
    ...
    function _refreshDetail() {
        return CommanderAPI.fetch(BASE + '/' + _currentId)
            .then(function (d) {
                _currentPack = d;
                _renderDetail(d);
                tcLoad();
    ...
    function tcSubmitReview() {
        ...
        if (_submitting) return;
        _submitting = true;
        CommanderAPI.fetch(BASE + '/' + _currentId + '/submit-review', {
    ...
    function tcApprove() {
        ...
        if (_submitting) return;
        _submitting = true;
        CommanderAPI.fetch(BASE + '/' + _currentId + '/approve', {
            method:  'PUT',
            headers: { 'Content-Type': 'application/json' },
    ...
    function tcComplete() {
        ...
        CommanderAPI.fetch(BASE + '/' + _currentId + '/complete', {
            method:  'PUT',
            headers: { 'Content-Type': 'application/json' },
    ...
    function tcSaveOverride() {
        ...
        CommanderAPI.fetch(BASE + '/' + _currentId + '/partner-override', {
            method:  'PUT',
            headers: { 'Content-Type': 'application/json' },
