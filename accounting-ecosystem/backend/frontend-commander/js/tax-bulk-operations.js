...
(function () {
    ...
    async function _loadTeamMembers() {
        try {
            var data = await CommanderAPI.fetch('/api/commander/team?active=true');
    ...
    async function _loadTemplates() {
        try {
            var data = await CommanderAPI.fetch('/api/commander/tax-checklists/templates?limit=100');
    ...
    async function _load() {
        ...
        var data = await CommanderAPI.fetch('/api/commander/tax-bulk-operations?' + params.toString());
    ...
    function tboNextStep() {
        if (_currentStep === 1 && !_selectedOpType) {
            CommanderAPI.showToast('Select an operation type first', true); return;
        }
    ...
    async function tboPreview() {
        if (!_selectedOpType) { CommanderAPI.showToast('Select an operation type first', true); return; }
        ...
        var data     = await CommanderAPI.fetch('/api/commander/tax-bulk-operations/preview', {
            method:  'POST',
    ...
    async function tboSaveAndContinue() {
        if (!_previewData) { CommanderAPI.showToast('Preview first', true); return; }
        if (!_previewData.clients || !_previewData.clients.length) {
            CommanderAPI.showToast('No clients matched — adjust filters', true); return;
        }

        var payload          = _buildPayload();
        payload.preview_snapshot = _previewData;

        var data = await CommanderAPI.fetch('/api/commander/tax-bulk-operations', {
            method:  'POST',
            headers: { 'Content-Type': 'application/json' },
            body:    JSON.stringify(payload),
        });
        ...
        if (!data || !data.operation) {
            CommanderAPI.showToast('Failed to save operation', true); return;
        }
    ...
    async function tboExecute() {
        if (!_savedOpId) { CommanderAPI.showToast('No saved operation', true); return; }

        var btn = document.getElementById('tboExecuteBtn');
        if (btn) { btn.disabled = true; btn.textContent = 'Running…'; }

        var data = await CommanderAPI.fetch('/api/commander/tax-bulk-operations/' + _savedOpId + '/execute', {
            method: 'POST',
        });
    ...
    async function tboExecuteExisting(opId) {
        ...
        var data = await CommanderAPI.fetch('/api/commander/tax-bulk-operations/' + opId + '/execute', {
            method: 'POST',
        });

        if (!data) return;
        var s = data.result_summary || {};
        CommanderAPI.showToast('Done — ' + (s.created || 0) + ' created, ' + (s.failed || 0) + ' failed');
        _load();
    }
    ...
    async function tboViewItems(opId) {
        var data  = await CommanderAPI.fetch('/api/commander/tax-bulk-operations/' + opId + '/items?limit=200');
    ...
    async function tboCancel(opId) {
        ...
        var data = await CommanderAPI.fetch('/api/commander/tax-bulk-operations/' + opId + '/cancel', {
            method: 'PUT',
        });
        if (!data) return;
        CommanderAPI.showToast('Operation cancelled');
        _load();
    }
