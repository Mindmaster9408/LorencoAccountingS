...
(function () {
    ...
    var BASE = '/api/commander/client-success';
    var CONTACTS_BASE = '/api/commander/clients';   // pre-existing endpoint — see docs/new-app/61_client_success.md
    var _tab = 'clients';
    ...
    function csLoadClients() {
        ...
        window.CommanderAPI.fetch(BASE + '/' + (qs.length ? '?' + qs.join('&') : ''))
            .then(function (r) { return r.json(); })
            .then(function (d) {
                var rows = d.clients || [];
    ...
    function csOpenClientDetail(clientId) {
        ...
        Promise.all([
            window.CommanderAPI.fetch(BASE + '/' + clientId).then(function (r) { return r.json(); }),
            window.CommanderAPI.fetch(BASE + '/' + clientId + '/health').then(function (r) { return r.json(); }),
            window.CommanderAPI.fetch(CONTACTS_BASE + '/' + clientId + '/contacts').then(function (r) { return r.json(); }).catch(function () { return { contacts: [] }; }),
            ...
            window.CommanderAPI.fetch('/api/commander/engagement-management/client/' + clientId + '/profile').then(function (r) { return r.json(); }).catch(function () { return null; }),
            // Codebox 73 — current-month profitability reused from
            // Profitability (no duplicate margin/realization logic here).
            window.CommanderAPI.fetch('/api/commander/profitability/client/' + clientId + '?period_start=' + periodStart + '&period_end=' + periodEnd).then(function (r) { return r.json(); }).catch(function () { return null; }),
            window.CommanderAPI.fetch('/api/commander/profitability/reviews?client_id=' + clientId).then(function (r) { return r.json(); }).catch(function () { return { reviews: [] }; }),
            // Codebox 74 — pricing reviews reused from Pricing Review (no
            // duplicate workflow logic here).
            window.CommanderAPI.fetch('/api/commander/pricing-review/?client_id=' + clientId).then(function (r) { return r.json(); }).catch(function () { return { reviews: [] }; }),
    ...
    function _loadResponsibleScorecard(teamMemberId) {
        var el = document.getElementById('responsibleScorecardBody');
        if (!el) return;
        window.CommanderAPI.fetch('/api/commander/partner-scorecards/snapshots?team_member_id=' + teamMemberId + '&limit=1')
            .then(function (r) { return r.json(); })
            .then(function (d) {
    ...
    function _renderClientDetail(detail, health, contacts, engagementProfile, profitability, profitabilityReviews, pricingReviews) {
        ...
        // this modal opens.
        html += '<div class="detail-section-title">Responsible Team Member Performance <a href="/commander/partner-scorecards.html" class="btn-action btn-secondary" style="text-decoration:none;">Open Partner Scorecards →</a></div>';
        ...
        html += '<div class="detail-section-title">Engagements <a href="/commander/engagement-management.html?client_id=' + client.id + '" class="btn-action btn-secondary" style="text-decoration:none;">Manage Engagements →</a></div>';
        ...
        html += '<div class="detail-section-title">Profitability <a href="/commander/profitability.html" class="btn-action btn-secondary" style="text-decoration:none;">Open Profitability →</a></div>';
        ...
        html += '<div class="detail-section-title">Pricing Reviews <a href="/commander/pricing-review.html?client_id=' + client.id + '" class="btn-action btn-secondary" style="text-decoration:none;">Open Pricing Reviews →</a></div>';
    ...
    function csRecalculate() {
        ...
        window.CommanderAPI.fetch(BASE + '/' + _currentClientId + '/recalculate', { method: 'POST' })
            .then(function (r) { return r.json(); })
    ...
    function csSubmitOverride() {
        ...
        window.CommanderAPI.fetch(BASE + '/' + _currentClientId + '/override', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
            .then(function (r) { return r.json(); })
    ...
    function csClearOverride() {
        window.CommanderAPI.fetch(BASE + '/' + _currentClientId + '/override', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ clear: true }) })
            .then(function (r) { return r.json(); })
    ...
    function csSubmitRelEdit() {
        ...
        window.CommanderAPI.fetch(BASE + '/' + _currentClientId, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
            .then(function (r) { return r.json(); })
    ...
    function csSubmitActivity() {
        ...
        window.CommanderAPI.fetch(BASE + '/' + _currentClientId + '/activities', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
            .then(function (r) { return r.json(); })
    ...
    function csSubmitMeeting() {
        ...
        window.CommanderAPI.fetch(BASE + '/' + _currentClientId + '/meetings', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
            .then(function (r) { return r.json(); })
    ...
    function csSubmitOpportunity() {
        ...
        if (!payload.title) { _showToast('Title is required.'); return; }
        window.CommanderAPI.fetch(BASE + '/' + _currentClientId + '/opportunities', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
            .then(function (r) { return r.json(); })
    ...
    function csLoadOpportunities() {
        ...
        window.CommanderAPI.fetch(BASE + '/opportunities/all' + (status ? '?status=' + encodeURIComponent(status) : ''))
            .then(function (r) { return r.json(); })
            .then(function (d) {
    ...
    // ── Contacts (proxies the pre-existing /api/commander/clients/:id/contacts endpoint) ──
    ...
    function csSubmitContact() {
        ...
        window.CommanderAPI.fetch(CONTACTS_BASE + '/' + _currentClientId + '/contacts', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
            .then(function (r) { return r.json(); })
    ...
    function csLoadHistory() {
        window.CommanderAPI.fetch(BASE + '/events/log')
            .then(function (r) { return r.json(); })
            .then(function (d) {
                var rows = d.events || [];
