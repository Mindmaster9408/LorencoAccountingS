(function () {
    ...
    function esc(s) { return CommanderAPI.escHtml(String(s || '')); }
    ...
    async function loadSummary() {
        try {
            var res = await CommanderAPI.fetch('/api/commander/dashboard/summary');
    ...
    // Fallback to the legacy /api/commander/dashboard endpoint
    async function loadSummaryFallback() {
        try {
            var res = await CommanderAPI.fetch('/api/commander/dashboard');
            if (!res.ok) return;
            var d = await res.json();
    ...
    async function loadRisk() {
        ...
        try {
            var res = await CommanderAPI.fetch('/api/commander/dashboard/risk');
    ...
    async function loadWorkload() {
        ...
        try {
            var res = await CommanderAPI.fetch('/api/commander/dashboard/workload');
            if (!res.ok) throw new Error('status ' + res.status);
            var d   = await res.json();
            var rows = d.workload || [];
    ...
    async function loadActivity() {
        ...
        try {
            var res = await CommanderAPI.fetch('/api/commander/dashboard/activity');
            if (!res.ok) throw new Error('status ' + res.status);
            var d      = await res.json();
            var events = d.events || [];
