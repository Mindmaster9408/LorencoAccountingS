...
(function () {
    ...
    var BASE = '/api/commander/partner-scorecards';
    ...
    function _renderTabBar() {
        var tabs = [['commander', 'Practice'], ['partners', 'Partners'], ['managers', 'Managers'], ['teams', 'Teams'], ['history', 'History'], ['reviews', 'Reviews']];
    ...
    function _loadPickers() {
        window.CommanderAPI.fetch(TEAM_BASE)
            ...
            .catch(function () {});
        window.CommanderAPI.fetch(BASE + '/team-keys')
            .then(function (r) { return r.json(); })
            .then(function (d) {
    ...
    function _loadLinkedObjectives(teamMemberId, kind) {
        window.CommanderAPI.fetch('/api/commander/strategic-planning/objectives?owner_team_member_id=' + teamMemberId)
            .then(function (r) { return r.json(); })
            .then(function (d) {
                ...
                el.innerHTML = objectives.length
                    ? '<strong>Linked Strategic Objectives:</strong> ' + objectives.map(function (o) { return _html(o.objective_title) + ' (' + _html(o.objective_status) + ')'; }).join(', ') +
                      ' <a href="/commander/strategic-planning.html" style="color:#a3bffa;">Open Strategic Planning →</a>'
                    : '';
            })
    ...
    function pscSaveSnapshot(kind) {
        ...
        var payload = {
            scorecard_type: kind === 'commander' ? 'commander' : kind === 'partners' ? 'partner' : kind === 'managers' ? 'manager' : 'team',
        ...
        window.CommanderAPI.fetch(BASE + '/snapshots', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
            .then(function (r) { return r.json(); })
    ...
    function pscLoadHistory() {
        ...
        window.CommanderAPI.fetch(BASE + '/snapshots' + (qs.length ? '?' + qs.join('&') : ''))
            .then(function (r) { return r.json(); })
    ...
    function pscOpenSnapshot(id) {
        ...
        window.CommanderAPI.fetch(BASE + '/snapshots/' + id)
            .then(function (r) { return r.json(); })
            .then(function (d) {
                if (d.error) { _showToast(d.error); return; }
                var s = d.scorecard;
    ...
    function pscSubmitCreateReview() {
        ...
        window.CommanderAPI.fetch(BASE + '/reviews', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
            .then(function (r) { return r.json(); })
    ...
    function pscLoadReviews() {
        ...
        window.CommanderAPI.fetch(BASE + '/reviews' + (status ? '?review_status=' + status : ''))
            .then(function (r) { return r.json(); })
    ...
    function pscReviewAction(action) {
        window.CommanderAPI.fetch(BASE + '/reviews/' + _currentReviewId + '/' + action, { method: 'PUT' })
            .then(function (r) { return r.json(); })
    ...
    function pscSubmitNotes() {
        ...
        window.CommanderAPI.fetch(BASE + '/reviews/' + _currentReviewId + '/' + _pendingNotesAction, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ notes: notes || null }) })
            .then(function (r) { return r.json(); })
