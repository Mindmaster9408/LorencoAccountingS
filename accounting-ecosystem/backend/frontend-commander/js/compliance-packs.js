...
(function () {
    ...
    var BASE   = '/api/commander/compliance-packs';
    ...
    async function loadClients() {
        try {
            var res = await CommanderAPI.fetch('/api/commander/clients?active=true&limit=500');
            if (!res.ok) return;
    ...
    async function loadSummary() {
        try {
            var res = await CommanderAPI.fetch(BASE + '/summary');
            if (!res.ok) return;
            var d   = await res.json();
            var s   = d.summary || {};
    ...
    async function loadPacks() {
        ...
        try {
            var res = await CommanderAPI.fetch(BASE + '?' + params.toString());
    ...
    function renderPackList(packs) {
        ...
        tbody.innerHTML = packs.map(function(p) {
            ...
            var clientName  = p.commander_clients ? esc(p.commander_clients.name) : '<span class="col-muted">—</span>';
    ...
    async function submitCreatePack() {
        ...
        try {
            var res = await CommanderAPI.fetch(BASE, {
                method: 'POST',
    ...
    async function openDetailModal(packId) {
        ...
        try {
            var [packRes, itemsRes] = await Promise.all([
                CommanderAPI.fetch(BASE + '/' + packId),
                CommanderAPI.fetch(BASE + '/' + packId + '/items'),
            ]);
    ...
    function renderDetailHeader(pack, rdy) {
        ...
        var clientName  = pack.commander_clients ? pack.commander_clients.name : '';
        ...
        if (subEl) {
            ...
            wrap.innerHTML = '<a href="/commander/practice-sop.html?linked_type=compliance_pack&linked_id=' + encodeURIComponent(pack.id) + '" ' +
    ...
    async function cpUpdateStatus() {
        ...
        try {
            var res = await CommanderAPI.fetch(BASE + '/' + _currentPackId, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
            ...
            CommanderAPI.showToast('❌ ' + (e.message || 'Update failed.'), true);
        }
    }
    ...
    async function cpRecalculate() {
        ...
        try {
            var res = await CommanderAPI.fetch(BASE + '/' + _currentPackId + '/recalculate-readiness', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: '{}',
            });
            ...
            CommanderAPI.showToast('❌ ' + (e.message || 'Recalculation failed.'), true);
        } finally {
            btn.disabled = false;
    ...
    async function cpGenerateDefaults() {
        ...
        try {
            var res = await CommanderAPI.fetch(BASE + '/' + _currentPackId + '/generate-default-items', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: '{}',
            });
            ...
            if (res.status === 409) {
                ...
                var r2 = await CommanderAPI.fetch(BASE + '/' + _currentPackId + '/generate-default-items?force=true', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: '{}',
                });
            ...
            CommanderAPI.showToast(d.created + ' default item(s) generated!');
            openDetailModal(_currentPackId);
        } catch(e) {
            CommanderAPI.showToast('❌ ' + (e.message || 'Generation failed.'), true);
        } finally {
            btn.disabled = false;
    ...
    async function cpGenerateFromDocs() {
        ...
        try {
            var res = await CommanderAPI.fetch(BASE + '/' + _currentPackId + '/generate-from-documents', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: '{}',
            });
            if (!res.ok) { var e = await res.json(); throw new Error(e.error || 'Failed'); }
            var d = await res.json();
            CommanderAPI.showToast(d.created > 0 ? d.created + ' document request(s) linked!' : (d.message || 'No new links added.'));
            if (d.created > 0) openDetailModal(_currentPackId);
        } catch(e) {
            CommanderAPI.showToast('❌ ' + (e.message || 'Link failed.'), true);
        } finally {
            btn.disabled = false;
    ...
    async function cpCancelPack() {
        ...
        try {
            var res = await CommanderAPI.fetch(BASE + '/' + _currentPackId, { method: 'DELETE' });
            if (!res.ok) { var e = await res.json(); throw new Error(e.error || 'Failed'); }
            CommanderAPI.showToast('Pack cancelled.');
            closeCpDetailModal();
            await Promise.all([loadSummary(), loadPacks()]);
        } catch(e) {
            CommanderAPI.showToast('❌ ' + (e.message || 'Failed.'), true);
        }
    }
    ...
    async function cpItemUpdateStatus(itemId, newStatus) {
        if (!_currentPackId) return;
        try {
            var res = await CommanderAPI.fetch(BASE + '/' + _currentPackId + '/items/' + itemId, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ status: newStatus }),
            });
            ...
            var itemsRes = await CommanderAPI.fetch(BASE + '/' + _currentPackId + '/items');
            if (itemsRes.ok) {
    ...
    async function submitCpAddItem() {
        ...
        try {
            var res = await CommanderAPI.fetch(BASE + '/' + _currentPackId + '/items', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(payload),
            });
            ...
            closeCpAddItemModal();
            CommanderAPI.showToast('Item added!');
            // Reload items
            var itemsRes = await CommanderAPI.fetch(BASE + '/' + _currentPackId + '/items');
            if (itemsRes.ok) {
                var id = await itemsRes.json();
