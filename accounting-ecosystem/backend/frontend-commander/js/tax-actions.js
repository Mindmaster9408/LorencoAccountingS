...
// Reads and manages commander_tax_work_actions via /api/commander/tax-actions/*
// No localStorage for business data.
(function () {
    'use strict';

    var esc      = CommanderAPI.escHtml;
    var BASE     = '/api/commander/tax-actions';
    ...
    async function _load() {
        ...
        var qs = _buildQS();
        try {
            var res  = await CommanderAPI.fetch(BASE + '?' + qs);
            var data = await res.json();
    ...
    async function taComplete(btn) {
        ...
        btn.disabled = true;
        try {
            var res  = await CommanderAPI.fetch(BASE + '/' + id + '/complete', { method: 'PUT' });
            var data = await res.json();
            if (!res.ok) throw new Error(data.error || ('HTTP ' + res.status));
            CommanderAPI.showToast('Action completed.');
            _load();
        } catch (err) {
    ...
    async function taDismiss(btn) {
        ...
        btn.disabled = true;
        try {
            var res  = await CommanderAPI.fetch(BASE + '/' + id + '/dismiss', { method: 'PUT' });
            var data = await res.json();
            if (!res.ok) throw new Error(data.error || ('HTTP ' + res.status));
            CommanderAPI.showToast('Action dismissed.');
            _load();
        } catch (err) {
            CommanderAPI.showToast('Error: ' + err.message, true);
            btn.disabled = false;
