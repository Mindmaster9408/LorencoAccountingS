...
(function () {
    ...
    var BASE = '/api/commander/secretarial-integrity';
    var _tab = 'runs';
    ...
    function siRunAudit() {
        ...
        window.CommanderAPI.fetch(BASE + '/run', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ run_type: 'manual' }) })
            .then(function (r) { return r.json(); })
            .then(function (d) {
    ...
    function _loadResolvedFindings() {
        window.CommanderAPI.fetch(BASE + '/findings?status=resolved')
            .then(function (r) { return r.json(); })
            .then(function (d1) {
                window.CommanderAPI.fetch(BASE + '/findings?status=accepted_risk')
    ...
    function siOpenFinding(id) {
        window.CommanderAPI.fetch(BASE + '/findings/' + id)
            .then(function (r) { return r.json(); })
            .then(function (d) {
                if (d.error) { _showToast(d.error); return; }
                var f = d.finding;
    ...
    function siAcknowledge(id) {
        window.CommanderAPI.fetch(BASE + '/findings/' + id + '/acknowledge', { method: 'PUT' })
            .then(function (r) { return r.json(); })
    ...
    function siResolve(id) {
        window.CommanderAPI.fetch(BASE + '/findings/' + id + '/resolve', { method: 'PUT' })
            .then(function (r) { return r.json(); })
    ...
    function siReopen(id) {
        window.CommanderAPI.fetch(BASE + '/findings/' + id + '/reopen', { method: 'PUT' })
            .then(function (r) { return r.json(); })
    ...
    function siSubmitAcceptRisk() {
        ...
        window.CommanderAPI.fetch(BASE + '/findings/' + id + '/accept-risk', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ notes: reason }) })
            .then(function (r) { return r.json(); })
