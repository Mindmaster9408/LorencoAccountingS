...
async function loadClients() {
  try {
    var res = await CommanderAPI.fetch('/api/commander/clients?limit=500');
...
async function loadUsers() {
  ...
  try {
    var res = await CommanderAPI.fetch('/api/commander/team?limit=200');
    var d   = await res.json();
...
async function loadTasks(resetPage) {
  ...
  try {
    var res = await CommanderAPI.fetch('/api/commander/tasks?' + params.toString());
...
function renderTasks(tasks, total) {
  ...
  tasks.forEach(function(t) {
    var clientName = (t.commander_clients && t.commander_clients.name) ? esc(t.commander_clients.name) : '';
    ...
    html += '<div class="task-card" data-task-id="' + t.id + '">' +
      '<div class="task-card-header">' +
        ...
        '<div style="display:flex;gap:6px;align-items:center;">' +
          '<a class="btn-ghost-sm" href="/commander/practice-sop.html?linked_type=task&linked_id=' + t.id + '" title="View standard procedure for this task" aria-label="Standard procedure">📋 Procedure</a>' +
          '<a class="btn-ghost-sm" href="/commander/quality-management.html?linked_type=task&linked_id=' + t.id + '" title="View quality reviews for this task" aria-label="QMS review">QMS Review ↗</a>' +
...
async function checkTaskScope(taskId, clientId, taskType) {
  ...
  try {
    var res = await CommanderAPI.fetch('/api/commander/work-authorization/check', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ client_id: clientId, work_type: workType, source_module: 'tasks', source_type: 'task', source_id: taskId }),
    });
    var d = await res.json();
    if (d.error) { CommanderAPI.showToast('Error: ' + d.error, true); return; }
    CommanderAPI.showToast(d.recommended_action || 'Scope checked.', d.authorization.scope_result !== 'in_scope');
  } catch (e) {
    CommanderAPI.showToast('Failed to check scope: ' + e.message, true);
  }
}
...
async function quickStatus(taskId, newStatus) {
  try {
    var res = await CommanderAPI.fetch('/api/commander/tasks/' + taskId, {
      method: 'PUT',
    ...
    CommanderAPI.showToast('Error: ' + e.message, true);
  }
}
...
async function openTaskModal(taskId) {
  ...
  if (isEdit) {
    try {
      ...
      CommanderAPI.showToast('Failed to load task: ' + e.message, true);
      return;
    }
  }
...
async function saveTask(e) {
  ...
  if (!title) {
    CommanderAPI.showToast('Task title is required', true);
    return false;
  }
  ...
  try {
    ...
    var url    = '/api/commander/tasks' + (isNew ? '' : '/' + _currentTaskId);
    var method = isNew ? 'POST' : 'PUT';
    var res    = await CommanderAPI.fetch(url, { method: method, body: JSON.stringify(body) });
    var d      = await res.json();
    if (!res.ok) throw new Error(d.error || 'Save failed');
    CommanderAPI.showToast(isNew ? 'Task created' : 'Task updated');
...
async function deleteCurrentTask() {
  ...
  try {
    var res = await CommanderAPI.fetch('/api/commander/tasks/' + _currentTaskId, { method: 'DELETE' });
    var d   = await res.json();
    if (!res.ok) throw new Error(d.error || 'Delete failed');
    CommanderAPI.showToast('Task deleted');
    closeModal('taskModal');
    await loadTasks();
  } catch (err) {
    CommanderAPI.showToast('Error: ' + err.message, true);
...
async function quickReviewAction(action, taskId) {
  try {
    var res = await CommanderAPI.fetch('/api/commander/tasks/' + taskId + '/' + action, {
      method: 'PUT',
      body:   JSON.stringify({})
    });
    var d = await res.json();
    if (!res.ok) throw new Error(d.error || 'Action failed');
    CommanderAPI.showToast(reviewActionLabel(action) + ' complete');
    await loadTasks();
  } catch (e) {
    CommanderAPI.showToast('Error: ' + e.message, true);
  }
...
async function submitReviewAction(action) {
  ...
  if (action === 'approve-review') {
    ...
    if (!reason) {
      CommanderAPI.showToast('Rejection reason is required', true);
  ...
  try {
    var res = await CommanderAPI.fetch('/api/commander/tasks/' + _reviewTaskId + '/' + action, {
      method: 'PUT',
    ...
    CommanderAPI.showToast(successMsgs[action] || 'Done');
    closeModal(modalId);
    await loadTasks();
  } catch (e) {
    CommanderAPI.showToast('Error: ' + e.message, true);
...
async function openReviewHistory(taskId, taskTitle) {
  ...
  try {
    var res = await CommanderAPI.fetch('/api/commander/tasks/' + taskId + '/review-events');
    if (!res.ok) throw new Error('HTTP ' + res.status);
    var d = await res.json();
