...
// Use CommanderAPI.json() (not CommanderAPI.fetch()) everywhere in this file —
...
async function loadClients() {
  try {
    var res = await apiJson('/api/commander/clients?is_active=true');
...
async function loadBillingStats() {
  try {
    var wipRes = await apiJson('/api/commander/billing/wip');
    ...
    var packsRes = await apiJson('/api/commander/billing/packs?limit=200');
...
async function loadWip() {
  ...
  try {
    ...
    var res = await apiJson('/api/commander/billing/wip' + qs);
...
function renderWip(byClient, entries) {
  ...
  var entriesByClient = {};
  entries.forEach(e => {
    var clientId = (e.commander_clients && e.commander_clients.id) ? e.commander_clients.id : 'none';
  ...
  byClient.forEach(function(c) {
    ...
    cEntries.forEach(function(e) {
      ...
      html += '<td class="wip-checkbox-col"><input type="checkbox" data-entry-id="' + e.id +
              '" data-client-id="' + (e.commander_clients && e.commander_clients.id ? e.commander_clients.id : '') +
...
function toggleEntrySelection(event) {
  ...
  if (cb.checked) {
    ...
    if (_selectedIds.size > 0) {
      ...
      var firstClientId = firstEntry && firstEntry.commander_clients
        ? String(firstEntry.commander_clients.id) : '';
...
function updateSelectedCount() {
  ...
  _wipEntries.forEach(function(e) {
    var cid = e.commander_clients && e.commander_clients.id ? e.commander_clients.id : 'none';
...
async function createPack() {
  ...
  try {
    var res = await apiJson('/api/commander/billing/packs', {
      method: 'POST',
      body: JSON.stringify({
...
async function loadPacks() {
  ...
  try {
    ...
    var res = await apiJson('/api/commander/billing/packs?' + qs.join('&'));
...
function renderPacks(packs) {
  ...
  packs.forEach(function(p) {
    var clientName = p.commander_clients ? p.commander_clients.name : '–';
    var period = '';
...
async function openPackDetail(packId) {
  ...
  try {
    var res  = await apiJson('/api/commander/billing/packs/' + packId);
    var pack = res.pack;
    var lines = res.lines || [];
...
function renderPackDetail(pack, lines) {
  ...
  lines.forEach(function(l) {
    var te        = l.commander_time_entries || {};
...
async function savePackNotes() {
  ...
  try {
    await apiJson('/api/commander/billing/packs/' + _currentPackId, {
      method: 'PUT',
      body: JSON.stringify({
...
async function recalculatePack() {
  if (!_currentPackId) return;
  try {
    await apiJson('/api/commander/billing/packs/' + _currentPackId + '/recalculate', {
      method: 'PUT', body: '{}'
    });
...
async function approvePack() {
  ...
  try {
    await apiJson('/api/commander/billing/packs/' + _currentPackId + '/approve', {
      method: 'PUT', body: '{}'
    });
...
async function lockPack() {
  ...
  try {
    await apiJson('/api/commander/billing/packs/' + _currentPackId + '/lock', {
      method: 'PUT', body: '{}'
    });
...
async function refreshPackDetail() {
  if (!_currentPackId) return;
  var res   = await apiJson('/api/commander/billing/packs/' + _currentPackId);
...
async function submitWriteoff() {
  ...
  try {
    await apiJson(
      '/api/commander/billing/packs/' + _currentPackId + '/lines/' + _writeoffLineId + '/writeoff',
...
async function excludeLine(lineId) {
  ...
  try {
    await apiJson(
      '/api/commander/billing/packs/' + _currentPackId + '/lines/' + lineId + '/exclude',
      { method: 'PUT', body: '{}' }
    );
...
async function openHistoryModal() {
  ...
  try {
    var res = await apiJson('/api/commander/billing/packs/' + _currentPackId + '/history');
...
async function viewReport() {
  ...
  try {
    ...
    var resp = await fetch('/api/commander/billing/packs/' + _currentPackId + '/report-html', {
...
async function downloadPdf() {
  ...
  try {
    var token = localStorage.getItem('token') || localStorage.getItem('commander_token');
    var resp = await fetch('/api/commander/billing/packs/' + _currentPackId + '/report-pdf', {
      headers: { 'Authorization': 'Bearer ' + token }
    });
    if (!resp.ok) {
      var err = await resp.json().catch(function() { return { error: resp.statusText }; });
