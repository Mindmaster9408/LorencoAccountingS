...
// All data from /api/commander/tax-dashboard/* and /api/commander/tax-actions/*
// No localStorage/KV for business data.
(function () {
    'use strict';

    var esc      = CommanderAPI.escHtml;
    var BASE     = '/api/commander/tax-dashboard';
    var ACT_BASE = '/api/commander/tax-actions';
    ...
    async function _loadRisk() {
        ...
        try {
            var res = await CommanderAPI.fetch(BASE + '/risk');
            var d   = await res.json();
                    ...
                    return '<div class="td-risk-item">' +
                        ...
                        '<div class="td-risk-body">' +
                            ...
                            '<div class="td-risk-btns">' +
                                ...
                                _openLink('/commander/deadlines.html') +
                            '</div>' +
                        '</div></div>';
                }).join('')
            );
            ...
            _setEl('tdRiskBlocked', blocked.length === 0
                ...
                : blocked.slice(0, 8).map(function (r) {
                    ...
                    var page    = r.source_type === 'individual' ? '/commander/individual-tax.html' : '/commander/company-tax.html';
                    return '<div class="td-risk-item">' +
                    ...
                    return '<div class="td-risk-item">' +
                        ...
                        '<div class="td-risk-body">' +
                            ...
                            '<div class="td-risk-btns">' +
                                ...
                                _openLink('/commander/provisional-tax.html') +
                            '</div>' +
                        '</div></div>';
                }).join('')
            );
            ...
            _setEl('tdRiskCalcWarn', calcWarnList.length === 0
                ...
                : calcWarnList.slice(0, 8).map(function (c) {
                    ...
                    var page      = c.source_type === 'individual' ? '/commander/individual-tax.html' : '/commander/company-tax.html';
                    return '<div class="td-risk-item">' +
    ...
    async function _loadWorkload() {
        ...
        try {
            var res = await CommanderAPI.fetch(BASE + '/workload');
            var d   = await res.json();
            var wl  = d.workload || [];
    ...
    async function _loadReviewQueue() {
        ...
        try {
            var res  = await CommanderAPI.fetch(ACT_BASE + '/review-queue' + (qs.length ? '?' + qs.join('&') : ''));
            var data = await res.json();
    ...
    function _rqPageHref(sourceType) {
        var m = {
            individual_return: '/commander/individual-tax.html',
            company_return:    '/commander/company-tax.html',
            provisional_plan:  '/commander/provisional-tax.html',
            individual_calculation: '/commander/individual-tax.html',
            company_calculation:    '/commander/company-tax.html',
            individual_review_pack: '/commander/individual-tax.html',
            company_review_pack:    '/commander/company-tax.html',
        };
    ...
    async function tdSubmitAction() {
        ...
        try {
            var res  = await CommanderAPI.fetch(ACT_BASE + '/from-dashboard-risk', {
                method:  'POST',
            ...
            var el = document.getElementById('tdActionModal');
            if (el) el.style.display = 'none';
            CommanderAPI.showToast('Action created.');
        } catch (err) {
    ...
    async function _loadReturns() {
        ...
        try {
            var res  = await CommanderAPI.fetch(BASE + '/returns?' + qs);
            var data = await res.json();
    ...
    function _pageHref(sourceType) {
        var m = { individual: '/commander/individual-tax.html', company: '/commander/company-tax.html', provisional: '/commander/provisional-tax.html' };
        return m[sourceType] || '/practice';
    }
    ...
    async function _loadActivity() {
        ...
        try {
            var res  = await CommanderAPI.fetch(BASE + '/activity');
            var data = await res.json();
