...
(function () {
    ...
    var BASE = '/api/commander/engagement-management';
    ...
    function _loadClientProfile() {
        window.CommanderAPI.fetch(BASE + '/client/' + _currentClientId + '/profile')
            .then(function (r) { return r.json(); })
            .then(function (d) {
    ...
    function emSubmitCreate() {
        ...
        window.CommanderAPI.fetch(BASE + '/', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
            .then(function (r) { return r.json(); })
            .then(function (d) {
                if (d.error) { _showToast(d.error); return; }
    ...
    function _loadEngagementPricingReviews(engagementId) {
        ...
        if (!el) return;
        window.CommanderAPI.fetch('/api/commander/pricing-review/?engagement_id=' + engagementId)
            .then(function (r) { return r.json(); })
            .then(function (d) {
                var rows = d.reviews || [];
                el.innerHTML = (rows.length ? rows.map(function (r) {
                    ...
                    '<div style="margin-top:8px;"><a href="/commander/pricing-review.html" class="btn-action btn-secondary" style="text-decoration:none;">Open Pricing Reviews →</a></div>';
            })
    ...
    function _loadEngagementProfitability(engagementId) {
        ...
        window.CommanderAPI.fetch('/api/commander/profitability/engagement/' + engagementId + '?period_start=' + periodStart + '&period_end=' + periodEnd)
            .then(function (r) { return r.json(); })
            .then(function (d) {
                ...
                el.innerHTML = '<div class="mini-card-meta" style="margin-bottom:8px;">Current month (' + periodStart + ' – ' + periodEnd + ')</div>' +
                    ...
                    '<div style="margin-top:8px;"><a href="/commander/profitability.html" class="btn-action btn-secondary" style="text-decoration:none;">Open Profitability →</a></div>';
            })
            .catch(function () { el.innerHTML = '<div class="empty-state">Failed to load.</div>'; });
    }
    ...
    function _loadLinkedAuthorizations(engagementId, clientId) {
        window.CommanderAPI.fetch('/api/commander/work-authorization?client_id=' + clientId)
            .then(function (r) { return r.json(); })
            .then(function (d) {
                ...
                }).join('') + '<div style="margin-top:8px;"><a href="/commander/work-authorization.html" class="btn-action btn-secondary" style="text-decoration:none;">Open Work Authorization →</a></div>'
    ...
    function emAction(action) {
        window.CommanderAPI.fetch(BASE + '/' + _currentEngagementId + '/' + action, { method: 'PUT' })
            .then(function (r) { return r.json(); })
            .then(function (d) {
                if (d.error) {
    ...
    function emSubmitReason() {
        ...
        window.CommanderAPI.fetch(url, { method: method, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ reason: reason }) })
            .then(function (r) { return r.json(); })
    ...
    function emSubmitRisk() {
        ...
        if (!reason) { _showToast('A reason is required.'); return; }
        window.CommanderAPI.fetch(BASE + '/' + _currentEngagementId + '/accept-risk', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ reason: reason }) })
            .then(function (r) { return r.json(); })
    ...
    function emSubmitLetter() {
        ...
        window.CommanderAPI.fetch(BASE + '/' + _currentEngagementId + '/letters', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
            .then(function (r) { return r.json(); })
    ...
    function emLetterAction(letterId, action) {
        window.CommanderAPI.fetch(BASE + '/letters/' + letterId + '/' + action, { method: 'PUT' })
            .then(function (r) { return r.json(); })
    ...
    function emSubmitReasonDispatch() {
        if (_pendingReasonAction && _pendingReasonAction.kind === 'waive-letter') {
            var reason = document.getElementById('rfReason').value;
            if (!reason) { _showToast('A reason is required.'); return; }
            window.CommanderAPI.fetch(BASE + '/letters/' + _pendingWaiveLetterId + '/waive', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ reason: reason }) })
                .then(function (r) { return r.json(); })
    ...
    function _loadDetailEvents(id) {
        window.CommanderAPI.fetch(BASE + '/' + id + '/events')
            .then(function (r) { return r.json(); })
            .then(function (d) {
                var rows = d.events || [];
