...
async function loadClients() {
  try {
    var r = await CommanderAPI.fetch('/api/commander/clients?is_active=true');
    if (!r.ok) return;
    var d = await r.json();
...
async function loadTeamMembers() {
  try {
    var r = await CommanderAPI.fetch('/api/commander/team?active=true');
    if (!r.ok) return;
    var d = await r.json();
...
async function loadDeadlines() {
  ...
  try {
    var r = await CommanderAPI.fetch('/api/commander/deadlines?' + params.toString());
...
async function updateSummary() {
  ...
  try {
    var r = await CommanderAPI.fetch('/api/commander/deadlines?limit=1000');
    if (!r.ok) return;
    var d = await r.json();
    var all = d.deadlines || [];
...
function renderList() {
  ...
  var rows = _allDeadlines.map(function(dl) {
    ...
    var clientName = dl.commander_clients ? dl.commander_clients.name : '—';
    var responsible = dl.commander_team_members ? dl.commander_team_members.display_name : '—';
...
async function loadRules() {
  ...
  try {
    var r = await CommanderAPI.fetch('/api/commander/compliance/rules');
    if (!r.ok) throw new Error();
    var d = await r.json();
    var rules = d.rules || [];
    if (!rules.length) {
...
async function openDeadlineModal(id) {
  ...
  if (id) {
    try {
      var r = await CommanderAPI.fetch('/api/commander/deadlines/' + id);
      if (!r.ok) throw new Error();
      var d = await r.json();
      var dl = d.deadline;
      ...
      CommanderAPI.showToast('Failed to load deadline', true);
      return;
    }
  }
...
async function saveDeadline(e) {
  ...
  try {
    ...
    var url    = '/api/commander/deadlines' + (_editDeadlineId ? '/' + _editDeadlineId : '');
    var r = await CommanderAPI.fetch(url, { method: method, body: JSON.stringify(payload) });
    var d = await r.json();
    if (!r.ok) throw new Error(d.error || 'Save failed');
    CommanderAPI.showToast(_editDeadlineId ? 'Deadline updated' : 'Deadline created');
...
async function cancelDeadline() {
  ...
  try {
    var r = await CommanderAPI.fetch('/api/commander/deadlines/' + _editDeadlineId, { method: 'DELETE' });
    if (!r.ok) { var d = await r.json(); throw new Error(d.error || 'Failed'); }
    CommanderAPI.showToast('Deadline cancelled');
    closeModal('deadlineModal');
    await refresh();
  } catch(err) {
    CommanderAPI.showToast('Error: ' + err.message, true);
...
async function openRuleModal(id) {
  ...
  if (id) {
    try {
      ...
      CommanderAPI.showToast('Failed to load rule', true); return;
    }
  }
...
async function saveRule(e) {
  ...
  try {
    ...
    var url    = '/api/commander/compliance/rules' + (_editRuleId ? '/' + _editRuleId : '');
    var r = await CommanderAPI.fetch(url, { method: method, body: JSON.stringify(payload) });
    var d = await r.json();
    if (!r.ok) throw new Error(d.error || 'Save failed');
    CommanderAPI.showToast(_editRuleId ? 'Rule updated' : 'Rule created');
...
async function deactivateRule() {
  ...
  try {
    var r = await CommanderAPI.fetch('/api/commander/compliance/rules/' + _editRuleId, { method: 'DELETE' });
    if (!r.ok) { var d = await r.json(); throw new Error(d.error || 'Failed'); }
    CommanderAPI.showToast('Rule deactivated');
    closeModal('ruleModal');
    loadRules();
  } catch(err) {
    CommanderAPI.showToast('Error: ' + err.message, true);
...
async function saveStatus(e) {
  ...
  try {
    var r = await CommanderAPI.fetch('/api/commander/deadlines/' + _statusTargetId + '/status', {
      method: 'PUT',
      body: JSON.stringify({ status: status, submission_reference: ref, event_note: note })
    });
    var d = await r.json();
    if (!r.ok) throw new Error(d.error || 'Failed');
    CommanderAPI.showToast('Status updated to: ' + status);
