...
(function () {
    ...
    var _BASE = '/api/commander/taxpayer-profiles';
    ...
    async function init() {
        ...
        try {
            var authRes = await CommanderAPI.fetch('/api/auth/me');
    ...
    /* ── Toast / Error helpers ─────────────────────────────── */
    function showToast(msg) { CommanderAPI.showToast(msg); }
    ...
    async function submitCreateProfile() {
        ...
        try {
            var res = await CommanderAPI.fetch(_BASE, { method: 'POST', body: JSON.stringify(body) });
    ...
    async function openDetailModal(profileId) {
        ...
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + profileId);
            if (!res.ok) throw new Error('Load failed');
            var data = await res.json();
    ...
    async function tpUpdateTaxStatus() {
        ...
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + _activeProfile.id, {
                method: 'PUT',
    ...
    async function tpRecalculate() {
        if (!_activeProfile) return;
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + _activeProfile.id + '/recalculate-readiness', { method: 'POST' });
    ...
    async function tpGenerateDefaults() {
        if (!_activeProfile) return;
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + _activeProfile.id + '/generate-default-items', { method: 'POST' });
            if (res.status === 409) {
                if (!confirm('This profile already has readiness items. Regenerate and add any missing defaults?')) return;
                var res2 = await CommanderAPI.fetch(_BASE + '/' + _activeProfile.id + '/generate-default-items?force=true', { method: 'POST' });
    ...
    async function loadIncomeSources(profileId) {
        ...
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + profileId + '/income-sources');
            if (!res.ok) throw new Error('Load failed');
            var data = await res.json();
    ...
    async function submitAddSource() {
        ...
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + _activeProfile.id + '/income-sources', {
                method: 'POST',
    ...
    async function removeIncomeSource(sourceId) {
        ...
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + _activeProfile.id + '/income-sources/' + sourceId, { method: 'DELETE' });
    ...
    async function loadDeductions(profileId) {
        ...
        show('tpDDedsLoading');
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + profileId + '/deductions');
            if (!res.ok) throw new Error('Load failed');
            var data = await res.json();
            hide('tpDDedsLoading');
    ...
    async function submitAddDeduction() {
        ...
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + _activeProfile.id + '/deductions', {
                method: 'POST',
    ...
    async function removeDeduction(dedId) {
        ...
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + _activeProfile.id + '/deductions/' + dedId, { method: 'DELETE' });
            if (!res.ok) throw new Error('Remove failed');
    ...
    async function loadReadinessItems(profileId) {
        ...
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + profileId + '/readiness');
            if (!res.ok) throw new Error('Load failed');
            var data = await res.json();
    ...
    async function _tpUpdateItemStatus(itemId, status) {
        if (!_activeProfile) return;
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + _activeProfile.id + '/readiness-items/' + itemId, {
                method: 'PUT',
    ...
    async function submitAddReadinessItem() {
        ...
        try {
            var res = await CommanderAPI.fetch(_BASE + '/' + _activeProfile.id + '/readiness-items', {
                method: 'POST',
