...
(function () {
    ...
    var BASE     = '/api/commander/tax-disputes';
    var _currentId   = null;
    ...
    function _renderOverviewTab(c, body) {
        ...
        if (c.submission_id) {
            ...
            html += '<a href="/commander/tax-submissions.html?submission_id=' + c.submission_id + '" style="font-size:.8rem;color:#667eea;text-decoration:none;">View Tax Submission #' + c.submission_id + ' ↗</a>';
            html += '</div>';
        }
        html += '</div>';
        body.innerHTML = html;
    ...
    function tdDeleteEvidence(evidenceId) {
        ...
        CommanderAPI.fetch(BASE + '/' + _currentId + '/evidence/' + evidenceId, { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: '{}' })
            .then(function (r) { return r.json().then(function (d) { return { ok: r.ok, data: d }; }); })
            .then(function (r) {
    ...
    function _renderFooter(c) {
        ...
        if (c.submission_id) {
            html += '<a href="/commander/tax-disputes.html?submission_id=' + encodeURIComponent(c.submission_id) + '" ' +
        ...
        html += '<a href="/commander/knowledge-base.html?linked_type=tax_dispute&linked_id=' + encodeURIComponent(c.id) + '" ' +
        ...
        html += '<a href="/commander/risk-register.html?source_type=tax_dispute&source_id=' + encodeURIComponent(c.id) + '" ' +
    ...
    function tdSubmitAction() {
        ...
        _submitting = true;
        CommanderAPI.fetch(url, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: body })
            .then(function (r) { return r.json().then(function (d) { return { ok: r.ok, status: r.status, data: d }; }); })
            .then(function (r) {
                _submitting = false;
