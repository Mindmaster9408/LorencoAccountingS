...
(function () {
    ...
    var BASE = '/api/commander/communications';
    ...
    document.addEventListener('DOMContentLoaded', function () {
        var token = typeof getToken === 'function' ? getToken() : localStorage.getItem('commander_token');
    ...
    function loadClientsAndMembers() {
        return Promise.all([
            CommanderAPI.fetch('/api/commander/clients?limit=500').catch(function () { return { clients: [] }; }),
            CommanderAPI.fetch('/api/commander/team?limit=200').catch(function () { return { members: [] }; }),
    ...
    function openCreateModal(presetClientId) {
        ...
        document.getElementById('cVisibility').value    = 'commander';
    ...
    function submitCreate() {
        ...
        var payload = {
            ...
            visibility:              valEl('cVisibility') || 'commander',
    ...
    function markResponded(id) {
        CommanderAPI.fetch(BASE + '/' + id + '/mark-responded', { method: 'PUT' })
    ...
    function cancelCommResponse(id) {
        ...
        CommanderAPI.fetch(BASE + '/' + id + '/cancel-response', { method: 'PUT' })
            .then(function () { Promise.all([loadSummary(), loadList()]); })
    ...
    function createReminderFromView() {
        ...
        closeViewModal();
        CommanderAPI.fetch(BASE + '/' + id + '/create-reminder', {
            method:  'POST',
            headers: { 'Content-Type': 'application/json' },
            body:    JSON.stringify({}),
        })
    ...
    function createTaskFromView() {
        ...
        var id = _viewingComm.id;
        closeViewModal();
        CommanderAPI.fetch(BASE + '/' + id + '/create-task', {
            method:  'POST',
            headers: { 'Content-Type': 'application/json' },
            body:    JSON.stringify({}),
        })
