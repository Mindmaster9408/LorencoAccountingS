...
(function () {
    ...
    var BASE = '/api/commander/skills-matrix';
    ...
    function skSeedDefaults() {
        window.CommanderAPI.fetch(BASE + '/seed-defaults', { method: 'POST' })
            .then(function (r) { return r.json(); })
            .then(function (d) {
    ...
    function skLoadCategories() {
        window.CommanderAPI.fetch(BASE + '/categories')
            .then(function (r) { return r.json(); })
            .then(function (d) {
    ...
    function skSubmitCategory() {
        ...
        window.CommanderAPI.fetch(url, { method: method, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
            .then(function (r) { return r.json(); })
            .then(function (d) {
                if (d.error) { _showToast(d.error); return; }
    ...
    function skArchiveCategory() {
        ...
        window.CommanderAPI.fetch(BASE + '/categories/' + _editingCategoryId, { method: 'DELETE' })
            .then(function (r) { return r.json(); })
    ...
    function skLoadSkills() {
        ...
        window.CommanderAPI.fetch(BASE + '/skills' + (catId ? '?category_id=' + catId : ''))
            .then(function (r) { return r.json(); })
            .then(function (d) {
                _skills = d.skills || [];
                document.getElementById('skillBody').innerHTML = _skills.length ? _skills.map(function (s) {
                    return '<tr class="clickable" onclick="skOpenSkill(' + s.id + ')"><td>' + _html(s.display_name) + '</td><td>' + _html(s.commander_skill_categories ? s.commander_skill_categories.display_name : '—') + '</td><td><button class="btn-action btn-secondary" onclick="event.stopPropagation();skOpenSkill(' + s.id + ')">Edit</button></td></tr>';
    ...
    function skArchiveSkill() {
        ...
        window.CommanderAPI.fetch(BASE + '/skills/' + _editingSkillId, { method: 'DELETE' })
            .then(function (r) { return r.json(); })
    ...
    function skLoadCompetency() {
        ...
        window.CommanderAPI.fetch(BASE + '/team-skills?team_member_id=' + memberId)
            .then(function (r) { return r.json(); })
            .then(function (d) {
                var haveSkillIds = {};
                var rows = (d.team_skills || []).map(function (ts) {
                    haveSkillIds[ts.skill_id] = true;
                    return _competencyRow(ts.skill_id, ts.commander_skills ? ts.commander_skills.display_name : '#' + ts.skill_id, ts);
                });
    ...
    function _competencyRow(skillId, skillName, ts) {
        ...
        var catName = ts && ts.commander_skills ? '' : '';
    ...
    function skSubmitTeamSkill() {
        ...
        window.CommanderAPI.fetch(BASE + '/team-skills', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
            .then(function (r) { return r.json(); })
            .then(function (d) {
                if (d.error) { _showToast(d.error); return; }
    ...
    function _loadCertTypes() {
        window.CommanderAPI.fetch(BASE + '/certifications')
            .then(function (r) { return r.json(); })
            .then(function (d) {
    ...
    function skArchiveCertType(id) {
        ...
        window.CommanderAPI.fetch(BASE + '/certifications/' + id, { method: 'DELETE' })
            .then(function (r) { return r.json(); })
    ...
    function skLoadTeamCerts() {
        window.CommanderAPI.fetch(BASE + '/team-certifications')
            .then(function (r) { return r.json(); })
            .then(function (d) {
                var rows = d.team_certifications || [];
                document.getElementById('teamCertBody').innerHTML = rows.length ? rows.map(function (r) {
                    return '<tr><td>' + _html(r.team_member_name) + '</td><td>' + _html(r.commander_certifications ? r.commander_certifications.certification_name : '—') + '</td>' +
    ...
    function skArchiveTeamCert(id) {
        ...
        window.CommanderAPI.fetch(BASE + '/team-certifications/' + id, { method: 'DELETE' })
            .then(function (r) { return r.json(); })
    ...
    function skLoadTraining() {
        window.CommanderAPI.fetch(BASE + '/team-skills')
            .then(function (r) { return r.json(); })
            .then(function (d) {
                ...
                document.getElementById('trainingBody').innerHTML = rows.length ? rows.map(function (ts) {
                    return '<tr><td>' + _html(ts.team_member_name) + '</td><td>' + _html(ts.commander_skills ? ts.commander_skills.display_name : '—') + '</td>' +
