...
(function () {
    ...
    var BASE = '/api/commander/notifications';
    var _notifications = [];
    var _selected = {};
    ...
    function ntLoadList() {
        ...
        window.CommanderAPI.fetch(BASE + _qs())
            .then(function (r) { return r.json(); })
            .then(function (d) {
    ...
    function ntBulk(action) {
        ...
        window.CommanderAPI.fetch(BASE + '/bulk-' + action, {
    ...
    function ntOpenDetail(id) {
        window.CommanderAPI.fetch(BASE + '/' + id)
            ...
            .then(function (d) {
                ...
                _renderDetail();
                return window.CommanderAPI.fetch(BASE + '/' + id + '/events');
            })
            .then(function (r) { return r.json(); })
    ...
    var SOURCE_DEEP_LINKS = {
        'tasks': '/commander/tasks.html?open=',
        'deadlines': '/commander/deadlines.html?open=',
        'reminders': '/commander/reminders.html?open=',
        'risk-register': '/commander/risk-register.html?open=',
        'qms': '/commander/quality-management.html?open=',
        'compliance-packs': '/commander/compliance-packs.html?open=',
        'document-requests': '/commander/document-requests.html?open=',
        'communications': '/commander/communications.html?open=',
        'tax-individual': '/commander/individual-tax.html?open=',
        'tax-company': '/commander/company-tax.html?open=',
    };
    ...
    function ntAction(action) {
        ...
        window.CommanderAPI.fetch(BASE + '/' + _currentId + '/' + action, { method: 'PUT' })
            .then(function (r) { return r.json(); })
            .then(function (d) {
    ...
    function ntCancel() {
        ...
        window.CommanderAPI.fetch(BASE + '/' + _currentId, { method: 'DELETE' })
            .then(function (r) { return r.json(); })
            .then(function (d) {
                if (d.error) { _showToast(d.error); return; }
    ...
    function ntSubmitSnooze() {
        ...
        window.CommanderAPI.fetch(BASE + '/' + _currentId + '/snooze', {
