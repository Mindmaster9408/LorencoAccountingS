...
(function () {
    ...
    var BASE = '/api/commander/planning-board';
    ...
    function _loadSummary() {
        window.CommanderAPI.fetch(BASE + '/summary' + _weekQs())
            ...
            .then(function (d) {
                ...
                var cards = [
                    ...
                    { count: d.notifications_unread_count, label: 'Unread Notifications', cls: '', href: '/commander/notifications.html' },
    ...
    function _loadWeek() {
        window.CommanderAPI.fetch(BASE + '/week' + _weekQs())
            .then(function (r) { if (_deniedCheck(r.status)) throw new Error('denied'); return r.json(); })
            .then(function (d) {
                _weekData = d;
                _renderWeekTabBar();
    ...
    function _delegationModule(item) {
        if (item.source_module === 'qms') return item.source_type === 'commander_quality_finding' ? 'qms-finding' : 'qms-review';
    ...
    function _renderItem(item) {
        ...
        var delegateBtn = delegateModule
            ? '<button class="mc-link" style="flex:none;padding:4px 10px;margin-left:8px;align-self:center;background:#805ad5;" onclick="event.stopPropagation();window.location.href=\'/commander/delegation.html?delegate=1&source_module=' + delegateModule + '&source_id=' + item.source_id + '&role=' + encodeURIComponent(item.role) + '\'">Delegate</button>'
            : '';
    ...
    function _loadTeam() {
        window.CommanderAPI.fetch(BASE + '/team' + _weekQs())
            .then(function (r) { if (_deniedCheck(r.status)) throw new Error('denied'); return r.json(); })
            .then(function (d) {
                _teamData = d.team || [];
    ...
    function _renderTeam() {
        ...
        grid.innerHTML = rows.map(function (m) {
            ...
            return '<div class="member-card cap-' + _html(m.capacity_status) + '">' +
                ...
                (m.strategic_initiatives_due_count || m.strategic_initiatives_overdue_count ? '<a class="mc-link" href="/commander/strategic-planning.html">Strategic Planning</a>' : '') +
                (m.active_onboardings_count ? '<a class="mc-link" href="/commander/client-onboarding.html">Onboarding</a>' : '') +
                '</div></div>';
        }).join('');
    }
    ...
    function _loadDeadlines() {
        window.CommanderAPI.fetch(BASE + '/deadlines')
            .then(function (r) { if (_deniedCheck(r.status)) throw new Error('denied'); return r.json(); })
            .then(function (d) {
    ...
    function _loadNotes() {
        window.CommanderAPI.fetch(BASE + '/planning-notes' + _weekQs())
            .then(function (r) { if (_deniedCheck(r.status)) throw new Error('denied'); return r.json(); })
            .then(function (d) {
                _notesData = d.notes || [];
    ...
    function pbArchiveNote(id) {
        ...
        window.CommanderAPI.fetch(BASE + '/planning-notes/' + id, { method: 'DELETE' })
            .then(function (r) { return r.json(); })
            .then(function (d) {
                if (d.error) { _showToast(d.error); return; }
