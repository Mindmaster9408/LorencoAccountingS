...
(function () {
    ...
    var BASE = '/api/commander/secretarial';
    var CLIENTS_BASE = '/api/commander/clients';
    var _tab = 'profile';
    ...
    function _loadClientPicker() {
        window.CommanderAPI.fetch(CLIENTS_BASE)
            .then(function (r) { return r.json(); })
            .then(function (d) {
    ...
    function secOnClientChange() {
        ...
        if (link) link.href = '/commander/secretarial-workflows.html?client_id=' + _currentClientId;
        var govLink = document.getElementById('secGovernanceLink');
        if (govLink) govLink.href = '/commander/secretarial-governance.html?client_id=' + _currentClientId;
        var boLink = document.getElementById('secBoLink');
        if (boLink) boLink.href = '/commander/beneficial-ownership.html?client_id=' + _currentClientId;
        var calLink = document.getElementById('secCalendarLink');
        if (calLink) calLink.href = '/commander/secretarial-calendar.html?client_id=' + _currentClientId;
        var lifecycleLink = document.getElementById('secLifecycleLink');
        if (lifecycleLink) lifecycleLink.href = '/commander/entity-lifecycle.html?client_id=' + _currentClientId;
        var integrityLink = document.getElementById('secIntegrityLink');
        if (integrityLink) integrityLink.href = '/commander/secretarial-integrity.html?client_id=' + _currentClientId;
        var onboardingLink = document.getElementById('secOnboardingLink');
        if (onboardingLink) onboardingLink.href = '/commander/client-onboarding.html?client_id=' + _currentClientId;
        secLoadClientData();
        _loadRecentChanges();
    ...
    function secLoadClientData() {
        ...
        window.CommanderAPI.fetch(BASE + '/' + _currentClientId)
            .then(function (r) { return r.json(); })
            .then(function (d) {
    ...
    function _loadRecentChanges() {
        ...
        if (!el) return;
        window.CommanderAPI.fetch('/api/commander/secretarial-workflows?client_id=' + _currentClientId + '&limit=5')
            .then(function (r) { return r.json(); })
            .then(function (d) {
                var rows = d.cases || [];
    ...
    function _loadRecentGovernance() {
        ...
        if (!el) return;
        var GOV_BASE = '/api/commander/secretarial-governance';
        Promise.all([
            window.CommanderAPI.fetch(GOV_BASE + '/resolutions?client_id=' + _currentClientId + '&limit=3').then(function (r) { return r.json(); }).catch(function () { return { resolutions: [] }; }),
            window.CommanderAPI.fetch(GOV_BASE + '/meetings?client_id=' + _currentClientId + '&limit=3').then(function (r) { return r.json(); }).catch(function () { return { meetings: [] }; }),
            window.CommanderAPI.fetch(GOV_BASE + '/decisions?client_id=' + _currentClientId + '&limit=3').then(function (r) { return r.json(); }).catch(function () { return { decisions: [] }; }),
    ...
    function _loadBoSummary() {
        ...
        if (!el) return;
        window.CommanderAPI.fetch('/api/commander/beneficial-ownership/client/' + _currentClientId)
            .then(function (r) { return r.json(); })
            .then(function (d) {
    ...
    function _loadStatutoryPanel() {
        ...
        if (!el) return;
        window.CommanderAPI.fetch('/api/commander/secretarial-calendar/calendar?client_id=' + _currentClientId)
            .then(function (r) { return r.json(); })
            .then(function (d) {
                var counts = d.counts || {};
    ...
    function _loadLifecyclePanel() {
        ...
        if (!el) return;
        window.CommanderAPI.fetch('/api/commander/entity-lifecycle/client/' + _currentClientId)
            .then(function (r) { return r.json(); })
            .then(function (d) {
                if (d.error) { el.innerHTML = '<div class="empty-state">' + _html(d.error) + '</div>'; return; }
    ...
    function _loadIntegrityPanel() {
        ...
        if (!el) return;
        window.CommanderAPI.fetch('/api/commander/secretarial-integrity/findings?client_id=' + _currentClientId + '&status=open')
            .then(function (r) { return r.json(); })
            .then(function (d) {
    ...
    function _loadOnboardingPanel() {
        ...
        if (!el) return;
        window.CommanderAPI.fetch('/api/commander/client-onboarding/profiles/' + _currentClientId)
            .then(function (r) { return r.json(); })
            .then(function (d) {
                if (d.error) { el.innerHTML = '<div class="empty-state">' + _html(d.error) + '</div>'; return; }
                if (!d.profile) { el.innerHTML = '<div class="empty-state">Not yet onboarded — <a href="/commander/client-onboarding.html?client_id=' + _currentClientId + '">start onboarding</a>.</div>'; return; }
    ...
    function secSaveProfile() {
        ...
        window.CommanderAPI.fetch(BASE + '/' + _currentClientId + '/profile', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
            .then(function (r) { return r.json(); })
    ...
    function secSubmitDirector() {
        ...
        window.CommanderAPI.fetch(BASE + '/' + _currentClientId + '/directors', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
            .then(function (r) { return r.json(); })
    ...
    function secResignDirector(id) {
        window.CommanderAPI.fetch(BASE + '/directors/' + id, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status: 'resigned' }) })
            .then(function (r) { return r.json(); })
    ...
    function secSubmitShareholder() {
        ...
        window.CommanderAPI.fetch(BASE + '/' + _currentClientId + '/shareholders', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
            .then(function (r) { return r.json(); })
    ...
    function secTransferShareholder(id) {
        window.CommanderAPI.fetch(BASE + '/shareholders/' + id, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status: 'transferred' }) })
            .then(function (r) { return r.json(); })
    ...
    function secSubmitReturn() {
        ...
        window.CommanderAPI.fetch(BASE + '/' + _currentClientId + '/annual-returns', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
            .then(function (r) { return r.json(); })
    ...
    function secSubmitReturnAction(id) {
        window.CommanderAPI.fetch(BASE + '/annual-returns/' + id, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status: 'submitted' }) })
            .then(function (r) { return r.json(); })
    ...
    function secSubmitNote() {
        ...
        window.CommanderAPI.fetch(BASE + '/' + _currentClientId + '/timeline/note', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ notes: notes }) })
            .then(function (r) { return r.json(); })
