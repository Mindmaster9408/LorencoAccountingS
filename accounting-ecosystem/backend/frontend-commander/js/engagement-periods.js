...
(function () {
    ...
    async function loadFilterOptions() {
        try {
            var [clientsRes, engRes] = await Promise.all([
                CommanderAPI.fetch('/api/commander/clients?is_active=all&limit=500'),
                CommanderAPI.fetch('/api/commander/clients/0/engagements').catch(function() { return null; })
            ]);
    ...
    async function loadPeriods() {
        ...
        try {
            var res = await CommanderAPI.fetch('/api/commander/engagement-periods?' + params.toString());
            var d   = await res.json();
    ...
    function renderPeriodsTable(periods) {
        ...
        tbody.innerHTML = periods.map(function(p) {
            var clientName  = (p.commander_clients && p.commander_clients.name)
                ? esc(p.commander_clients.name) : '<span class="col-muted">—</span>';
            var engName     = (p.commander_client_engagements && p.commander_client_engagements.engagement_name)
                ? esc(p.commander_client_engagements.engagement_name) : '<span class="col-muted">—</span>';
            var statusBadge = '<span class="badge status-' + p.status + '">' + esc(p.status) + '</span>';
            var workflowLink = p.workflow_run_id
                ? '<a href="/commander/workflows.html" class="badge badge-info">#' + p.workflow_run_id + '</a>'
    ...
    async function openGenFromPeriod(periodId) {
        ...
        try {
            var res = await CommanderAPI.fetch('/api/commander/engagement-periods/' + periodId);
    ...
    async function submitGenFromPeriod(e) {
        ...
        try {
            var res = await CommanderAPI.fetch(
                '/api/commander/engagement-periods/' + _activePeriodId + '/generate-workflow',
            ...
            CommanderAPI.showToast('✅ Workflow generated — ' + d.task_count + ' tasks!');
    ...
    async function submitSkipPeriod() {
        ...
        if (!reason) {
            CommanderAPI.showToast('❌ Reason is required to skip a period', true);
            return;
        }
        _submitting = true;
        ...
        try {
            var res = await CommanderAPI.fetch(
                '/api/commander/engagement-periods/' + _activePeriodId + '/skip',
                { method: 'PUT', body: JSON.stringify({ reason }) }
            );
            var d = await res.json();
            if (!res.ok) throw new Error(d.error || 'Skip failed');
            closeSkipModal();
            CommanderAPI.showToast('Period skipped.');
            loadPeriods();
        } catch(err) {
            CommanderAPI.showToast('❌ ' + err.message, true);
            document.getElementById('skipSubmitBtn').disabled = false;
    ...
    async function cancelPeriod(periodId) {
        ...
        try {
            var res = await CommanderAPI.fetch(
                '/api/commander/engagement-periods/' + periodId + '/cancel',
                { method: 'PUT', body: '{}' }
            );
            var d = await res.json();
            if (!res.ok) throw new Error(d.error || 'Cancel failed');
            CommanderAPI.showToast('Period cancelled.');
            loadPeriods();
        } catch(err) {
            CommanderAPI.showToast('❌ ' + err.message, true);
        }
