(function () {
    ...
    var BASE = '/api/commander/sars-recon';
    ...
    function _loadList() {
        ...
        CommanderAPI.fetch(BASE + '/lines' + _qs())
            .then(function (r) { return r.json(); })
    ...
    function srLoadUnmatched() {
        ...
        CommanderAPI.fetch(BASE + '/payment-events/unmatched')
            .then(function (r) { return r.json(); })
    ...
    function srOpenDetail(id) {
        ...
        CommanderAPI.fetch(BASE + '/lines/' + id)
            .then(function (r) { return r.json(); })
            .then(function (d) {
    ...
    function _loadEventsTab(body) {
        ...
        CommanderAPI.fetch(BASE + '/lines/' + _currentId + '/events')
            .then(function (r) { return r.json(); })
    ...
    function _renderFooter(d) {
        ...
        html += ' <a href="/commander/tax-disputes.html?source_type=sars_statement_line&source_id=' + encodeURIComponent(d.id) + '" ' +
    ...
    function srCreateDisputeFromLine() {
        ...
        var d = _currentLine;
        CommanderAPI.fetch('/api/commander/tax-disputes/create-from-sars-line', {
            method:  'POST',
            headers: { 'Content-Type': 'application/json' },
            body:    JSON.stringify({
            ...
            if (!r.ok) {
                if (r.status === 409) {
                    if (window.confirm('Active dispute case already exists (ID #' + r.data.existing_case_id + '). Open it?')) {
                        window.location.href = '/commander/tax-disputes.html?source_type=sars_statement_line&source_id=' + encodeURIComponent(d.id);
                    }
                } else {
    ...
    function srSubmitMatch() {
        ...
        _submitting = true;
        CommanderAPI.fetch(BASE + '/lines/' + _currentId + '/match-payment-event', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
    ...
    function srCancelLine() {
        ...
        _submitting = true;
        CommanderAPI.fetch(BASE + '/lines/' + _currentId, { method: 'DELETE' })
            .then(function (r) { return r.json().then(function (data) { return { ok: r.ok, data: data }; }); })
            .then(function (r) {
                _submitting = false;
