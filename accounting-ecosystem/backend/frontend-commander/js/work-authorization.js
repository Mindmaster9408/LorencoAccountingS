...
(function () {
    ...
    var BASE = '/api/commander/work-authorization';
    ...
    function waSubmitCheck() {
        ...
        window.CommanderAPI.fetch(BASE + '/check', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
            .then(function (r) { return r.json(); })
            .then(function (d) {
    ...
    function waOpenDetail(id) {
        _currentAuthId = id;
        Promise.all([
            window.CommanderAPI.fetch(BASE + '/' + id).then(function (r) { return r.json(); }),
            window.CommanderAPI.fetch(BASE + '/' + id + '/events').then(function (r) { return r.json(); }),
    ...
    function waApproveOverride() {
        window.CommanderAPI.fetch(BASE + '/' + _currentAuthId + '/approve-override', { method: 'PUT' })
            .then(function (r) { return r.json(); })
            .then(function (d) {
                if (d.error) { _showToast(d.error); return; }
    ...
    function waSubmitReason() {
        ...
        window.CommanderAPI.fetch(url, { method: method, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) })
            .then(function (r) { return r.json(); })
            .then(function (d) {
                if (d.error) { _showToast(d.error); return; }
