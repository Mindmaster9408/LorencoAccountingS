/* =============================================================
   ...
   Mounted at /api/commander/provisional-tax
   ============================================================= */
'use strict';
...
async function verifyPlanOwnership(cid, planId) {
    const { data } = await supabase
        .from('commander_provisional_tax_plans')
        .select('*')
        .eq('id', planId)
        .eq('company_id', cid)
        .single();
...
async function verifyPeriodOwnership(cid, planId, periodId) {
    const { data } = await supabase
        .from('commander_provisional_tax_periods')
        .select('*')
        .eq('id', periodId)
        .eq('plan_id', planId)
        .eq('company_id', cid)
...
async function logEvent(cid, planId, periodId, eventType, extras = {}) {
    await supabase.from('commander_provisional_tax_events').insert({
        company_id:    cid,
        plan_id:       planId,
...
router.get('/', async (req, res) => {
    ...
    try {
        let q = supabase
            ...
            .select(`
                *,
                clients:commander_clients!client_id(name)
            `)
            .eq('company_id', cid)
...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        await auditFromReq(req, 'provisional_tax_plan_created', 'commander_provisional_tax_plans', plan.id, { metadata: { tax_year: plan.tax_year } });
...
router.put('/:id', async (req, res) => {
    ...
    try {
        ...
        await auditFromReq(req, 'provisional_tax_plan_updated', 'commander_provisional_tax_plans', planId);
...
router.delete('/:id', async (req, res) => {
    ...
    try {
        ...
        await auditFromReq(req, 'provisional_tax_plan_cancelled', 'commander_provisional_tax_plans', planId);
...
router.post('/:id/create-periods', async (req, res) => {
    ...
    try {
        ...
        await auditFromReq(req, 'provisional_tax_periods_created', 'commander_provisional_tax_plans', planId, { metadata: { count: periods.length } });
...
router.put('/:id/periods/:periodId/status', async (req, res) => {
    ...
    try {
        ...
        await auditFromReq(req, 'provisional_tax_period_status_changed', 'commander_provisional_tax_periods', periodId, { metadata: { plan_id: planId, new_status: status } });
...
router.put('/:id/periods/:periodId', async (req, res) => {
    ...
    try {
        ...
        await auditFromReq(req, 'provisional_tax_period_updated', 'commander_provisional_tax_periods', periodId, { metadata: { plan_id: planId } });
...
router.post('/:id/review', async (req, res) => {
    ...
    try {
        ...
        await auditFromReq(req, 'provisional_tax_reviewed', 'commander_provisional_tax_plans', planId);
...
router.get('/:id/events', async (req, res) => {
    ...
    try {
        const { data: events, error } = await supabase
            .from('commander_provisional_tax_events')
            .select('*')
            .eq('plan_id', planId)
            .eq('company_id', cid)
            .order('created_at', { ascending: false })
