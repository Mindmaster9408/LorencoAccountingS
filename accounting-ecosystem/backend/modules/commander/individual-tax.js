/* =============================================================
   ...
   Mounted at /api/commander/individual-tax
   ============================================================= */
'use strict';
...
async function logEvent(cid, returnId, eventType, extras = {}) {
    await supabase.from('commander_individual_tax_events').insert({
        company_id:    cid,
...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        await auditFromReq(req, 'individual_tax_return_created', 'commander_individual_tax_returns', ret.id, { metadata: { tax_year: ret.tax_year } });
...
router.post('/:id/generate-default-items', async (req, res) => {
    ...
    try {
        ...
        await auditFromReq(req, 'individual_tax_items_generated', 'commander_individual_tax_returns', returnId, { metadata: { count: items.length } });
...
router.get('/:id/events', async (req, res) => {
    ...
    const { data, error } = await supabase
        .from('commander_individual_tax_events')
        .select('*')
        .eq('tax_return_id', returnId)
        .eq('company_id', cid)
        .order('created_at', { ascending: false })
...
router.put('/:id', async (req, res) => {
    ...
    try {
        ...
        await auditFromReq(req, 'individual_tax_return_updated', 'commander_individual_tax_returns', returnId);
...
router.delete('/:id', async (req, res) => {
    ...
    try {
        ...
        await auditFromReq(req, 'individual_tax_return_cancelled', 'commander_individual_tax_returns', returnId);
