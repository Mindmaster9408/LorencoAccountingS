...
async function _loadAllRules(cid) {
    ...
    const { data, error } = await supabase.from('commander_alert_rules').select('*').eq('company_id', cid);
    if (error) {
...
async function _verifyRule(id, cid) {
    const { data } = await supabase.from('commander_alert_rules').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
    return data || null;
}

async function _writeEvent(cid, ruleId, ruleKey, eventType, oldValue, newValue, req, notes, meta) {
    await supabase.from('commander_alert_rule_events').insert({
        company_id: cid,
        rule_id: ruleId || null,
...
router.get('/summary', async (req, res) => {
    try {
        ...
        const { data, error } = await supabase.from('commander_alert_rules').select('category, severity, enabled, system_rule').eq('company_id', cid);
        ...
        console.error('GET /api/commander/alert-rules/summary', err);
...
router.get('/groups', async (req, res) => {
    try {
        ...
        const [groupsRes, rulesRes] = await Promise.all([
            supabase.from('commander_alert_rule_groups').select('*').eq('company_id', cid).order('sort_order'),
            supabase.from('commander_alert_rules').select('id, group_id, category, enabled').eq('company_id', cid),
        ]);
        ...
        console.error('GET /api/commander/alert-rules/groups', err);
...
router.post('/groups/:groupId/reset', async (req, res) => {
    try {
        ...
        const { data: group } = await supabase.from('commander_alert_rule_groups').select('*').eq('id', groupId).eq('company_id', cid).maybeSingle();
        if (!group) return res.status(404).json({ error: 'Rule group not found.' });

        const { data: rules } = await supabase.from('commander_alert_rules').select('*').eq('company_id', cid).eq('group_id', groupId);
        const results = [];
        for (const rule of (rules || [])) {
            const before = { ...rule };
            const { data: updated, error } = await supabase.from('commander_alert_rules').update({
        ...
        console.error('POST /api/commander/alert-rules/groups/:groupId/reset', err);
...
router.post('/seed-defaults', async (req, res) => {
    try {
        ...
        const { data: existingGroups } = await supabase.from('commander_alert_rule_groups').select('*').eq('company_id', cid);
        ...
        if (groupsToInsert.length) {
            const { data, error } = await supabase.from('commander_alert_rule_groups').insert(groupsToInsert).select();
            if (error) throw error;
            insertedGroups = data || [];
        }

        const { data: allGroups } = await supabase.from('commander_alert_rule_groups').select('*').eq('company_id', cid);
        const groupIdByKey = {};
        (allGroups || []).forEach(g => { groupIdByKey[g.group_key] = g.id; });

        const { data: existingRules } = await supabase.from('commander_alert_rules').select('rule_key').eq('company_id', cid);
        ...
        if (rulesToInsert.length) {
            const { data, error } = await supabase.from('commander_alert_rules').insert(rulesToInsert).select();
            if (error) throw error;
            insertedRules = data || [];
        }
        ...
        console.error('POST /api/commander/alert-rules/seed-defaults', err);
...
router.get('/export', async (req, res) => {
    try {
        const cid = req.companyId;
        const { data, error } = await supabase.from('commander_alert_rules').select('*').eq('company_id', cid).order('category').order('sort_order');
        if (error) throw error;
        ...
        console.error('GET /api/commander/alert-rules/export', err);
...
router.post('/import', async (req, res) => {
    try {
        ...
        const { data: existingRules } = await supabase.from('commander_alert_rules').select('*').eq('company_id', cid);
        const byKey = {};
        ...
        for (const { existing, payload } of updates) {
            ...
            const { data: updated, error } = await supabase.from('commander_alert_rules').update(patch).eq('id', existing.id).eq('company_id', cid).select().single();
            if (error) throw error;
        ...
        for (const payload of creates) {
            ...
            const { data: created, error } = await supabase.from('commander_alert_rules').insert(insertRow).select().single();
            if (error) throw error;
        ...
        console.error('POST /api/commander/alert-rules/import', err);
...
router.post('/validate', async (req, res) => {
    try {
        ...
        let existingKeys = [];
        if (isCreate) {
            const { data } = await supabase.from('commander_alert_rules').select('rule_key').eq('company_id', cid);
            existingKeys = (data || []).map(r => r.rule_key);
        }
        const errors = _validatePayload(body, { isCreate, existingKeys });
        res.json({ valid: errors.length === 0, errors });
    } catch (err) {
        console.error('POST /api/commander/alert-rules/validate', err);
...
router.get('/events', async (req, res) => {
    try {
        const cid = req.companyId;
        let q = supabase.from('commander_alert_rule_events').select('*').eq('company_id', cid).order('created_at', { ascending: false }).limit(200);
        if (req.query.rule_id) q = q.eq('rule_id', Number(req.query.rule_id));
        const { data, error } = await q;
        if (error) throw error;
        res.json({ events: data || [] });
    } catch (err) {
        console.error('GET /api/commander/alert-rules/events', err);
...
router.get('/', async (req, res) => {
    try {
        const cid = req.companyId;
        let q = supabase.from('commander_alert_rules').select('*').eq('company_id', cid).order('category').order('sort_order');
        ...
        console.error('GET /api/commander/alert-rules', err);
...
router.get('/:id', async (req, res) => {
    try {
        ...
        const { data: events } = await supabase.from('commander_alert_rule_events').select('*').eq('company_id', req.companyId).eq('rule_id', rule.id).order('created_at', { ascending: false }).limit(20);
        res.json({ rule, recent_events: events || [] });
    } catch (err) {
        console.error('GET /api/commander/alert-rules/:id', err);
...
router.post('/', async (req, res) => {
    try {
        ...
        let groupId = null;
        const { data: g } = await supabase.from('commander_alert_rule_groups').select('id').eq('company_id', cid).eq('group_key', body.category).maybeSingle();
        groupId = g ? g.id : null;
        ...
        const { data, error } = await supabase.from('commander_alert_rules').insert(insertRow).select().single();
        if (error) {
        ...
        console.error('POST /api/commander/alert-rules', err);
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const { data: updated, error } = await supabase.from('commander_alert_rules').update(patch).eq('id', rule.id).eq('company_id', cid).select().single();
        if (error) throw error;
        ...
        console.error('PUT /api/commander/alert-rules/:id', err);
...
router.post('/:id/reset', async (req, res) => {
    try {
        ...
        console.error('POST /api/commander/alert-rules/:id/reset', err);
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        const { error } = await supabase.from('commander_alert_rules').delete().eq('id', rule.id).eq('company_id', cid);
        if (error) throw error;
        ...
        console.error('DELETE /api/commander/alert-rules/:id', err);
...
router.get('/:id/events', async (req, res) => {
    try {
        ...
        const rule = await _verifyRule(req.params.id, cid);
        if (!rule) return res.status(404).json({ error: 'Rule not found.' });
        const { data, error } = await supabase.from('commander_alert_rule_events').select('*').eq('company_id', cid).eq('rule_id', rule.id).order('created_at', { ascending: false });
        if (error) throw error;
        res.json({ events: data || [] });
    } catch (err) {
        console.error('GET /api/commander/alert-rules/:id/events', err);
        res.status(500).json({ error: 'Failed to load rule history.' });
    }
});
