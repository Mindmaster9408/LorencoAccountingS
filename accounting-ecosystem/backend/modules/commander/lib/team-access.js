...
// every commander_* route module had its own copy-pasted
// MANAGER_ROLES / _myTeamMember / _isManager / _requireManager block that
// looked a user up in commander_team_members by user_id ONLY. When a roster
...
// ever checked req.user.isSuperAdmin. All commander_* modules now delegate
...
// Only used when a super admin has no commander_team_members roster row at
...
// Resolves the commander_team_members row for the calling user.
...
async function getMyTeamMember(supabase, companyId, user) {
    ...
    const { data: byUserId } = await supabase.from('commander_team_members')
    ...
    if (user.email) {
        const { data: candidates } = await supabase.from('commander_team_members')
            .select(TEAM_MEMBER_FIELDS)
            .eq('company_id', companyId).eq('is_active', true).is('user_id', null)
            .ilike('email', user.email);
        if (candidates && candidates.length === 1) {
            const { data: healed } = await supabase.from('commander_team_members')
