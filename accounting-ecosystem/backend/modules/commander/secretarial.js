...
// management (commander_clients, tax modules, and document-requests.js stay
...
async function _writeEvent(cid, clientId, eventType, entityType, entityId, actorUserId, notes, meta) {
    await supabase.from('commander_secretarial_events').insert({
...
async function _getOrInitProfile(cid, clientId) {
    const { data: existing } = await supabase.from('commander_secretarial_profiles')
...
// identity/tax fields that already live on commander_clients and
// commander_taxpayer_profiles rather than re-storing them. No scoring, no
...
async function getCorporateProfile(cid, clientId) {
    ...
    const [profileRes, taxpayerRes, directorsRes, shareholdersRes, returnsRes, timelineRes] = await Promise.all([
        supabase.from('commander_secretarial_profiles').select('*').eq('company_id', cid).eq('client_id', clientId).maybeSingle(),
        supabase.from('commander_taxpayer_profiles').select('income_tax_reference, financial_year_end').eq('company_id', cid).eq('client_id', clientId).maybeSingle(),
        supabase.from('commander_company_directors').select('*').eq('company_id', cid).eq('client_id', clientId).order('status').order('director_name'),
        supabase.from('commander_company_shareholders').select('*').eq('company_id', cid).eq('client_id', clientId).order('status').order('shareholder_name'),
        supabase.from('commander_annual_returns').select('*').eq('company_id', cid).eq('client_id', clientId).order('return_year', { ascending: false }),
        supabase.from('commander_secretarial_events').select('*').eq('company_id', cid).eq('client_id', clientId).order('created_at', { ascending: false }).limit(30),
    ]);
...
async function getGovernanceSummary(cid, clientId) {
    ...
    const [returnsRes, directorsRes] = await Promise.all([
        supabase.from('commander_annual_returns').select('id, status, due_date').eq('company_id', cid).eq('client_id', clientId).in('status', ['pending', 'overdue']),
        supabase.from('commander_company_directors').select('id').eq('company_id', cid).eq('client_id', clientId).eq('status', 'active'),
    ]);
...
router.get('/summary', async (req, res) => {
    ...
    try {
        const [profiles, directors, returns] = await Promise.all([
            supabase.from('commander_secretarial_profiles').select('company_status').eq('company_id', cid),
            supabase.from('commander_company_directors').select('status').eq('company_id', cid).eq('status', 'active'),
            supabase.from('commander_annual_returns').select('status, due_date').eq('company_id', cid),
        ]);
...
router.put('/directors/:id', async (req, res) => {
    ...
    const existing = await supabase.from('commander_company_directors').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
...
router.post('/:clientId/shareholders', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_company_shareholders').insert({
...
router.put('/shareholders/:id', async (req, res) => {
    ...
    const existing = await supabase.from('commander_company_shareholders').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
...
router.get('/:clientId/annual-returns', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_annual_returns')
...
router.post('/:clientId/annual-returns', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_annual_returns').insert({
...
router.put('/annual-returns/:id', async (req, res) => {
    ...
    const existing = await supabase.from('commander_annual_returns').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
...
router.get('/:clientId/timeline', async (req, res) => {
    ...
    try {
        ...
        const { data, error } = await supabase.from('commander_secretarial_events')
...
router.get('/events/log', async (req, res) => {
    ...
    try {
        let q = supabase.from('commander_secretarial_events').select('*').eq('company_id', cid).order('created_at', { ascending: false }).limit(Math.min(500, parseInt(limit) || 100));
...
// (commander_secretarial_events) that this module's own routes write to,
