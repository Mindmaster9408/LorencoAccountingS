...
// management. NOT client master data (commander_clients stays authoritative
...
// commander_clients.health_score/health_status (client-health.js, Codebox
...
async function _writeEvent(cid, eventType, entityType, entityId, clientId, actorUserId, notes, meta) {
    await supabase.from('commander_client_success_events').insert({
...
async function calculateClientHealth(cid, clientId) {
    ...
    await supabase.from('commander_client_success').update({
...
// Ensures a commander_client_success row exists (created lazily on first
...
async function _getOrInitSuccessRow(cid, clientId) {
    const { data: existing } = await supabase.from('commander_client_success')
        .select('*').eq('company_id', cid).eq('client_id', clientId).maybeSingle();
    if (existing) return existing;

    const { data: created, error } = await supabase.from('commander_client_success')
...
router.get('/summary', async (req, res) => {
    ...
    try {
        const [success, opportunities] = await Promise.all([
            supabase.from('commander_client_success').select('relationship_status, next_review_date, next_planned_contact_date, last_meaningful_contact_date').eq('company_id', cid),
            supabase.from('commander_client_opportunities').select('status, estimated_value').eq('company_id', cid),
        ]);
...
// team member (commander_clients) or the relationship owner (client-success).
router.get('/', async (req, res) => {
    ...
    try {
        ...
        const [clients, success, members] = await Promise.all([
            clientQ,
            supabase.from('commander_client_success').select('*').eq('company_id', cid),
...
router.get('/:clientId', async (req, res) => {
    ...
    try {
        const [success, activities, meetings, opportunities, governance] = await Promise.all([
            supabase.from('commander_client_success').select('*').eq('company_id', cid).eq('client_id', clientId).maybeSingle(),
            supabase.from('commander_client_success_activities').select('*').eq('company_id', cid).eq('client_id', clientId).order('scheduled_date', { ascending: false, nullsFirst: false }),
            supabase.from('commander_client_meetings').select('*').eq('company_id', cid).eq('client_id', clientId).order('meeting_date', { ascending: false }),
            supabase.from('commander_client_opportunities').select('*').eq('company_id', cid).eq('client_id', clientId).order('created_at', { ascending: false }),
...
router.put('/:clientId', async (req, res) => {
    ...
    try {
        ...
        const { data, error } = await supabase.from('commander_client_success')
...
router.get('/:clientId/activities', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_client_success_activities')
...
router.post('/:clientId/activities', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_client_success_activities').insert({
...
router.put('/activities/:id', async (req, res) => {
    ...
    const existing = await supabase.from('commander_client_success_activities').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
...
router.get('/:clientId/meetings', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_client_meetings')
...
router.post('/:clientId/meetings', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_client_meetings').insert({
        ...
        await _getOrInitSuccessRow(cid, clientId);
        const successRow = await supabase.from('commander_client_success').select('last_meaningful_contact_date').eq('company_id', cid).eq('client_id', clientId).single();
        ...
        await supabase.from('commander_client_success').update(successUpdate).eq('company_id', cid).eq('client_id', clientId);
...
router.put('/meetings/:id', async (req, res) => {
    ...
    const existing = await supabase.from('commander_client_meetings').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
...
router.get('/opportunities/all', async (req, res) => {
    ...
    try {
        let q = supabase.from('commander_client_opportunities').select('*').eq('company_id', cid).order('created_at', { ascending: false });
        if (status) q = q.eq('status', status);
        const [opportunities, clients] = await Promise.all([
            q,
            supabase.from('commander_clients').select('id, name').eq('company_id', cid),
        ]);
...
router.get('/:clientId/opportunities', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_client_opportunities')
...
router.post('/:clientId/opportunities', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_client_opportunities').insert({
...
router.put('/opportunities/:id', async (req, res) => {
    ...
    const existing = await supabase.from('commander_client_opportunities').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
...
router.get('/events/log', async (req, res) => {
    ...
    try {
        let q = supabase.from('commander_client_success_events').select('*').eq('company_id', cid).order('created_at', { ascending: false }).limit(Math.min(500, parseInt(limit) || 100));
