...
const SECTION_KEYS = [
    'executive_summary', 'commander_health', 'strategy', 'kpis', 'partner_scorecards',
...
async function _verifyReport(id, cid) {
    const { data } = await supabase.from('commander_executive_reports').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
    return data || null;
}

async function _verifyDecision(id, cid) {
    const { data } = await supabase.from('commander_executive_decisions').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
    return data || null;
}

async function _verifyAction(id, cid) {
    const { data } = await supabase.from('commander_executive_action_register').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
    return data || null;
}

async function _writeEvent(cid, { reportId, decisionId, actionId }, eventType, oldStatus, newStatus, userId, notes, meta) {
    await supabase.from('commander_executive_events').insert({
        company_id: cid,
...
async function _notificationCounts(cid) {
    ...
    const [{ count: unread }, { count: overdue }, { count: critical }] = await Promise.all([
        supabase.from('commander_notifications').select('id', { count: 'exact', head: true }).eq('company_id', cid).in('notification_status', ['new', 'read']),
        supabase.from('commander_notifications').select('id', { count: 'exact', head: true }).eq('company_id', cid).in('notification_status', ['new', 'read']).lt('due_date', today),
        supabase.from('commander_notifications').select('id', { count: 'exact', head: true }).eq('company_id', cid).eq('severity', 'critical').in('notification_status', ['new', 'read']),
    ]);
...
async function _learningCounts(cid) {
    const { data } = await supabase.from('commander_learning_plans').select('status, overall_progress').eq('company_id', cid);
    const rows = data || [];
...
async function buildExecutiveReport(cid, { periodStart, periodEnd }) {
    ...
    try {
        practiceScorecard = await partnerScorecards.buildScorecard(cid, { scorecardType: 'commander', periodStart, periodEnd });
    } catch (e) {
    ...
    // Sections — each entry becomes one commander_executive_report_sections
    // row (upserted by section_key). data is exactly what's reused above;
    // nothing here recomputes a KPI/score.
    const sectionDefs = [
        { section_key: 'commander_health', section_title: 'Practice Health', section_order: 1, data: { score: practiceScore, summary } },
...
async function _upsertSections(cid, reportId, sectionDefs, userId) {
    const { data: existing } = await supabase.from('commander_executive_report_sections').select('section_key, section_status').eq('report_id', reportId).eq('company_id', cid);
    const existingByKey = {};
    ...
    for (const def of sectionDefs) {
        ...
        await supabase.from('commander_executive_report_sections').upsert({
...
router.get('/summary', async (req, res) => {
    ...
    try {
        const [{ data: latestRows }, { count: awaitingApproval }, { count: outstandingActions }, { count: openDecisions }] = await Promise.all([
            supabase.from('commander_executive_reports').select('id, report_title, report_type, report_status, period_start, period_end, generated_at, created_at').eq('company_id', cid).not('report_status', 'in', '("cancelled")').order('created_at', { ascending: false }).limit(1),
            supabase.from('commander_executive_reports').select('id', { count: 'exact', head: true }).eq('company_id', cid).eq('report_status', 'under_review'),
            supabase.from('commander_executive_action_register').select('id', { count: 'exact', head: true }).eq('company_id', cid).in('status', ['open', 'in_progress', 'waiting']),
            supabase.from('commander_executive_decisions').select('id', { count: 'exact', head: true }).eq('company_id', cid).in('decision_status', ['proposed', 'deferred']),
        ]);
        ...
        console.error('GET /api/commander/executive-reporting/summary', err);
...
router.get('/', async (req, res) => {
    ...
    try {
        ...
        let q = supabase.from('commander_executive_reports')
        ...
        console.error('GET /api/commander/executive-reporting', err);
...
router.post('/', async (req, res) => {
    const cid = req.companyId;
    const { report_title, report_type, period_start, period_end, period_key, executive_summary, commander_health_summary, key_wins, key_concerns, key_decisions, recommendations } = req.body || {};
    ...
    try {
        ...
        const { data: report, error } = await supabase.from('commander_executive_reports').insert({
            ...
            executive_summary: executive_summary || null, commander_health_summary: commander_health_summary || null,
        ...
        await auditFromReq(req, 'executive_report_created', 'commander_executive_report', report.id, {});

        return res.status(201).json({ report });
    } catch (err) {
        console.error('POST /api/commander/executive-reporting', err);
...
router.get('/:id(\\d+)', async (req, res) => {
    ...
    try {
        ...
        console.error('GET /api/commander/executive-reporting/:id', err);
...
router.put('/:id', async (req, res) => {
    const cid = req.companyId;
    const EDITABLE = ['report_title', 'executive_summary', 'commander_health_summary', 'key_wins', 'key_concerns', 'key_decisions', 'recommendations'];
    ...
    try {
        ...
        const { data: updated, error } = await supabase.from('commander_executive_reports').update({ ...patch, updated_by: req.user?.userId }).eq('id', report.id).eq('company_id', cid).select().single();
        if (error) throw error;

        await _writeEvent(cid, { reportId: report.id }, 'report_updated', null, null, req.user?.userId, null, { fields: Object.keys(patch) });
        await auditFromReq(req, 'executive_report_updated', 'commander_executive_report', report.id, { fields: Object.keys(patch) });

        return res.json({ report: updated });
    } catch (err) {
        console.error('PUT /api/commander/executive-reporting/:id', err);
...
router.delete('/:id', async (req, res) => {
    ...
    try {
        ...
        const { data: updated, error } = await supabase.from('commander_executive_reports').update({ report_status: 'cancelled', cancellation_reason: String(reason).trim(), updated_by: req.user?.userId }).eq('id', report.id).eq('company_id', cid).select().single();
        if (error) throw error;
        ...
        await auditFromReq(req, 'executive_report_cancelled', 'commander_executive_report', report.id, {});

        return res.json({ report: updated });
    } catch (err) {
        console.error('DELETE /api/commander/executive-reporting/:id', err);
...
router.post('/:id/generate', async (req, res) => {
    ...
    try {
        ...
        const { data: updated, error } = await supabase.from('commander_executive_reports').update({
        ...
        await auditFromReq(req, 'executive_report_generated', 'commander_executive_report', report.id, { confidence });

        return res.json({ report: updated, warnings, missing_information: missingInformation, confidence });
    } catch (err) {
        console.error('POST /api/commander/executive-reporting/:id/generate', err);
...
router.post('/generate', async (req, res) => {
    ...
    try {
        ...
        console.error('POST /api/commander/executive-reporting/generate', err);
        return res.status(500).json({ error: 'Failed to generate executive report.' });
    }
});
...
router.put('/:id/submit-review', async (req, res) => {
    ...
    try {
        ...
        const { data: updated, error } = await supabase.from('commander_executive_reports').update({ report_status: 'under_review', updated_by: req.user?.userId }).eq('id', report.id).eq('company_id', cid).select().single();
        if (error) throw error;

        await _writeEvent(cid, { reportId: report.id }, 'report_reviewed', report.report_status, 'under_review', req.user?.userId, notes || null, {});
        await auditFromReq(req, 'executive_report_submitted_review', 'commander_executive_report', report.id, {});

        return res.json({ report: updated });
    } catch (err) {
        console.error('PUT /api/commander/executive-reporting/:id/submit-review', err);
...
router.put('/:id/approve', async (req, res) => {
    ...
    try {
        ...
        const now = new Date().toISOString();
        const { data: updated, error } = await supabase.from('commander_executive_reports').update({ report_status: 'approved', approved_by: req.user?.userId || null, approved_at: now, updated_by: req.user?.userId }).eq('id', report.id).eq('company_id', cid).select().single();
        if (error) throw error;

        await _writeEvent(cid, { reportId: report.id }, 'report_approved', 'under_review', 'approved', req.user?.userId, notes || null, {});
        await auditFromReq(req, 'executive_report_approved', 'commander_executive_report', report.id, {});

        return res.json({ report: updated });
    } catch (err) {
        console.error('PUT /api/commander/executive-reporting/:id/approve', err);
...
router.put('/:id/publish', async (req, res) => {
    ...
    try {
        ...
        const now = new Date().toISOString();
        const { data: updated, error } = await supabase.from('commander_executive_reports').update({ report_status: 'published', published_by: req.user?.userId || null, published_at: now, updated_by: req.user?.userId }).eq('id', report.id).eq('company_id', cid).select().single();
        if (error) throw error;

        await _writeEvent(cid, { reportId: report.id }, 'report_published', 'approved', 'published', req.user?.userId, notes || null, {});
        await auditFromReq(req, 'executive_report_published', 'commander_executive_report', report.id, {});

        return res.json({ report: updated });
    } catch (err) {
        console.error('PUT /api/commander/executive-reporting/:id/publish', err);
...
router.put('/:id/archive', async (req, res) => {
    ...
    try {
        ...
        const { data: updated, error } = await supabase.from('commander_executive_reports').update({ report_status: 'archived', updated_by: req.user?.userId }).eq('id', report.id).eq('company_id', cid).select().single();
        if (error) throw error;

        await _writeEvent(cid, { reportId: report.id }, 'report_archived', report.report_status, 'archived', req.user?.userId, notes || null, {});
        await auditFromReq(req, 'executive_report_archived', 'commander_executive_report', report.id, {});

        return res.json({ report: updated });
    } catch (err) {
        console.error('PUT /api/commander/executive-reporting/:id/archive', err);
...
router.get('/:id/report-data', async (req, res) => {
    ...
    try {
        const report = await _verifyReport(req.params.id, cid);
        if (!report) return res.status(404).json({ error: 'Executive report not found.' });
        const { data: sections } = await supabase.from('commander_executive_report_sections').select('*').eq('report_id', report.id).eq('company_id', cid).order('section_order');
        return res.json({ report, sections: sections || [], disclaimer: DISCLAIMER });
    } catch (err) {
        console.error('GET /api/commander/executive-reporting/:id/report-data', err);
...
router.get('/:id/report-html', async (req, res) => {
    ...
    try {
        const report = await _verifyReport(req.params.id, cid);
        if (!report) return res.status(404).json({ error: 'Executive report not found.' });
        const { data: sections } = await supabase.from('commander_executive_report_sections').select('*').eq('report_id', report.id).eq('company_id', cid).eq('section_status', 'included').order('section_order');
        const { data: decisions } = await supabase.from('commander_executive_decisions').select('*').eq('report_id', report.id).eq('company_id', cid).order('created_at');
        const { data: actions } = await supabase.from('commander_executive_action_register').select('*').eq('report_id', report.id).eq('company_id', cid).order('created_at');
        ...
        console.error('GET /api/commander/executive-reporting/:id/report-html', err);
...
router.get('/:id/report-pdf', async (req, res) => {
    ...
    try {
        ...
        console.error('GET /api/commander/executive-reporting/:id/report-pdf', err);
...
router.get('/:id/events', async (req, res) => {
    ...
    try {
        const report = await _verifyReport(req.params.id, cid);
        if (!report) return res.status(404).json({ error: 'Executive report not found.' });
        const { data, error } = await supabase.from('commander_executive_events').select('*').eq('report_id', report.id).eq('company_id', cid).order('created_at', { ascending: false });
        if (error) throw error;
        return res.json({ events: data || [] });
    } catch (err) {
        console.error('GET /api/commander/executive-reporting/:id/events', err);
...
router.get('/:id/sections', async (req, res) => {
    ...
    try {
        const report = await _verifyReport(req.params.id, cid);
        if (!report) return res.status(404).json({ error: 'Executive report not found.' });
        const { data, error } = await supabase.from('commander_executive_report_sections').select('*').eq('report_id', report.id).eq('company_id', cid).order('section_order');
        if (error) throw error;
        return res.json({ sections: data || [] });
    } catch (err) {
        console.error('GET /api/commander/executive-reporting/:id/sections', err);
...
router.post('/:id/sections', async (req, res) => {
    ...
    try {
        ...
        const { data: existing } = await supabase.from('commander_executive_report_sections').select('id').eq('report_id', report.id).eq('company_id', cid).eq('section_key', section_key).maybeSingle();
        if (existing) return res.status(409).json({ error: `A "${section_key}" section already exists on this report.` });

        const { data, error } = await supabase.from('commander_executive_report_sections').insert({
        ...
        console.error('POST /api/commander/executive-reporting/:id/sections', err);
...
router.put('/:id/sections/:sectionId', async (req, res) => {
    ...
    try {
        ...
        const { data: section } = await supabase.from('commander_executive_report_sections').select('*').eq('id', req.params.sectionId).eq('report_id', req.params.id).eq('company_id', cid).maybeSingle();
        if (!section) return res.status(404).json({ error: 'Section not found.' });

        const { data: updated, error } = await supabase.from('commander_executive_report_sections').update({ ...patch, updated_by: req.user?.userId }).eq('id', section.id).eq('company_id', cid).select().single();
        if (error) throw error;
        return res.json({ section: updated });
    } catch (err) {
        console.error('PUT /api/commander/executive-reporting/:id/sections/:sectionId', err);
...
router.delete('/:id/sections/:sectionId', async (req, res) => {
    ...
    try {
        ...
        const { error } = await supabase.from('commander_executive_report_sections').delete().eq('id', section.id).eq('company_id', cid);
        if (error) throw error;
        return res.json({ success: true });
    } catch (err) {
        console.error('DELETE /api/commander/executive-reporting/:id/sections/:sectionId', err);
        return res.status(500).json({ error: 'Failed to remove section.' });
    }
});

// ── Routes: Decisions ─────────────────────────────────────────────────────────────
// Executive decision register only — never creates commander_tasks or
// workflow items.
...
router.get('/decisions', async (req, res) => {
    ...
    try {
        ...
        let q = supabase.from('commander_executive_decisions').select('*', { count: 'exact' }).eq('company_id', cid);
        ...
        console.error('GET /api/commander/executive-reporting/decisions', err);
...
router.post('/:id/decisions', async (req, res) => {
    ...
    try {
        ...
        const { data, error } = await supabase.from('commander_executive_decisions').insert({
        ...
        await auditFromReq(req, 'executive_decision_created', 'commander_executive_decision', data.id, {});

        return res.status(201).json({ decision: data });
    } catch (err) {
        console.error('POST /api/commander/executive-reporting/:id/decisions', err);
...
router.put('/decisions/:id', async (req, res) => {
    ...
    try {
        ...
        const { data: updated, error } = await supabase.from('commander_executive_decisions').update({ ...patch, updated_by: req.user?.userId }).eq('id', decision.id).eq('company_id', cid).select().single();
        if (error) throw error;
        ...
        await auditFromReq(req, 'executive_decision_updated', 'commander_executive_decision', decision.id, { fields: Object.keys(patch) });

        return res.json({ decision: updated });
    } catch (err) {
        console.error('PUT /api/commander/executive-reporting/decisions/:id', err);
...
router.put('/decisions/:id/complete', async (req, res) => {
    ...
    try {
        ...
        const now = new Date().toISOString();
        const { data: updated, error } = await supabase.from('commander_executive_decisions').update({ decision_status: 'implemented', completed_at: now, updated_by: req.user?.userId }).eq('id', decision.id).eq('company_id', cid).select().single();
        if (error) throw error;
        ...
        await auditFromReq(req, 'executive_decision_completed', 'commander_executive_decision', decision.id, {});

        return res.json({ decision: updated });
    } catch (err) {
        console.error('PUT /api/commander/executive-reporting/decisions/:id/complete', err);
...
router.delete('/decisions/:id', async (req, res) => {
    ...
    try {
        ...
        const { data: updated, error } = await supabase.from('commander_executive_decisions').update({ decision_status: 'cancelled', cancellation_reason: String(reason).trim(), updated_by: req.user?.userId }).eq('id', decision.id).eq('company_id', cid).select().single();
        if (error) throw error;
        ...
        await auditFromReq(req, 'executive_decision_cancelled', 'commander_executive_decision', decision.id, {});

        return res.json({ decision: updated });
    } catch (err) {
        console.error('DELETE /api/commander/executive-reporting/decisions/:id', err);
...
router.get('/actions', async (req, res) => {
    ...
    try {
        ...
        let q = supabase.from('commander_executive_action_register').select('*', { count: 'exact' }).eq('company_id', cid);
        ...
        console.error('GET /api/commander/executive-reporting/actions', err);
...
router.post('/:id/actions', async (req, res) => {
    ...
    try {
        ...
        const { data, error } = await supabase.from('commander_executive_action_register').insert({
        ...
        await auditFromReq(req, 'executive_action_created', 'commander_executive_action', data.id, {});

        return res.status(201).json({ action: data });
    } catch (err) {
        console.error('POST /api/commander/executive-reporting/:id/actions', err);
...
router.put('/actions/:id', async (req, res) => {
    ...
    try {
        ...
        const { data: updated, error } = await supabase.from('commander_executive_action_register').update({ ...patch, updated_by: req.user?.userId }).eq('id', action.id).eq('company_id', cid).select().single();
        if (error) throw error;
        ...
        await auditFromReq(req, 'executive_action_updated', 'commander_executive_action', action.id, { fields: Object.keys(patch) });

        return res.json({ action: updated });
    } catch (err) {
        console.error('PUT /api/commander/executive-reporting/actions/:id', err);
...
router.put('/actions/:id/complete', async (req, res) => {
    ...
    try {
        ...
        const now = new Date().toISOString();
        const { data: updated, error } = await supabase.from('commander_executive_action_register').update({ status: 'completed', completed_at: now, updated_by: req.user?.userId }).eq('id', action.id).eq('company_id', cid).select().single();
        if (error) throw error;
        ...
        await auditFromReq(req, 'executive_action_completed', 'commander_executive_action', action.id, {});

        return res.json({ action: updated });
    } catch (err) {
        console.error('PUT /api/commander/executive-reporting/actions/:id/complete', err);
...
router.delete('/actions/:id', async (req, res) => {
    ...
    try {
        ...
        const { data: updated, error } = await supabase.from('commander_executive_action_register').update({ status: 'cancelled', cancellation_reason: String(reason).trim(), updated_by: req.user?.userId }).eq('id', action.id).eq('company_id', cid).select().single();
        if (error) throw error;
        ...
        await auditFromReq(req, 'executive_action_cancelled', 'commander_executive_action', action.id, {});

        return res.json({ action: updated });
    } catch (err) {
        console.error('DELETE /api/commander/executive-reporting/actions/:id', err);
...
router.get('/events', async (req, res) => {
    ...
    try {
        let q = supabase.from('commander_executive_events').select('*', { count: 'exact' }).eq('company_id', cid);
        if (report_id) q = q.eq('report_id', parseInt(report_id, 10));
        ...
        console.error('GET /api/commander/executive-reporting/events', err);
        return res.status(500).json({ error: 'Failed to load events.' });
    }
});
...
<p>${_esc(report.commander_health_summary) || '<em>No practice health summary provided.</em>'}</p>
...
function _buildPdfReport(doc, report, sections, decisions, actions) {
    ...
    doc.fontSize(9).text(report.commander_health_summary || 'No practice health summary provided.');
