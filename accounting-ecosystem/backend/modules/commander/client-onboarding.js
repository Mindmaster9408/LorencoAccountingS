...
// commander_clients row and tracks progress toward "fully operational."
//
...
const ONBOARDING_STATUSES = ['draft', 'information_collection', 'document_collection', 'secretarial_setup', 'tax_setup', 'commander_setup', 'review', 'completed', 'cancelled'];
const PRIORITIES = ['low', 'normal', 'high', 'urgent'];
const RISK_LEVELS = ['low', 'medium', 'high', 'critical'];
const STEP_STATUSES = ['pending', 'in_progress', 'completed', 'skipped', 'blocked'];
const CHECKLIST_ITEM_TYPES = ['information', 'document', 'registration', 'secretarial', 'tax', 'commander', 'review', 'custom'];

// entity_type values match commander_secretarial_profiles.company_type
// exactly (both pty_ltd/cc/npc/trust/partnership/sole_proprietor/other) — no
// translation needed. commander_entity_lifecycle_profiles.entity_category
...
const ONBOARDING_CHECKLIST_DEFAULTS = {
    pty_ltd: [
        ...
        _item('Engagement letter signed', 'commander'),
...
async function _verifyClient(cid, clientId) {
    const { data } = await supabase.from('commander_clients').select('id, name').eq('id', clientId).eq('company_id', cid).eq('is_active', true).maybeSingle();
    return data || null;
}

async function _writeEvent(cid, clientId, sourceType, sourceId, eventType, oldStatus, newStatus, actorUserId, notes, meta) {
    await supabase.from('commander_onboarding_events').insert({
...
async function _getProfile(cid, clientId) {
    const { data } = await supabase.from('commander_onboarding_profiles').select('*').eq('company_id', cid).eq('client_id', clientId).maybeSingle();
    return data || null;
}
...
async function _recomputeCompletion(cid, profileId) {
    const [stepsRes, checklistRes] = await Promise.all([
        supabase.from('commander_onboarding_steps').select('status').eq('company_id', cid).eq('profile_id', profileId),
        supabase.from('commander_onboarding_checklists').select('required, completed').eq('company_id', cid).eq('profile_id', profileId),
    ]);
    ...
    await supabase.from('commander_onboarding_profiles').update({ completion_percentage: pct, updated_at: new Date().toISOString() }).eq('id', profileId).eq('company_id', cid);
    return pct;
}
...
async function buildOnboardingWorkspace(cid, clientId, entityType, actorUserId) {
    ...
    if (!profile) {
        const { data: created, error } = await supabase.from('commander_onboarding_profiles')
    ...
    const { count: stepCount } = await supabase.from('commander_onboarding_steps').select('id', { count: 'exact', head: true }).eq('company_id', cid).eq('profile_id', profile.id);
    let steps = [];
    if (!stepCount) {
        const rows = STEP_DEFAULTS.map((name, i) => ({ company_id: cid, client_id: clientId, profile_id: profile.id, step_name: name, sort_order: i }));
        const { data, error } = await supabase.from('commander_onboarding_steps').insert(rows).select();
        if (error) throw new Error(error.message);
        steps = data || [];
        await Promise.all(steps.map(s => _writeEvent(cid, clientId, 'step', s.id, 'step_created', null, s.status, actorUserId, s.step_name, {})));
    } else {
        const { data } = await supabase.from('commander_onboarding_steps').select('*').eq('company_id', cid).eq('profile_id', profile.id);
        steps = data || [];
    }
    ...
    const { count: checklistCount } = await supabase.from('commander_onboarding_checklists').select('id', { count: 'exact', head: true }).eq('company_id', cid).eq('profile_id', profile.id);
    if (!checklistCount) {
...
async function _completeStep(cid, clientId, step, actorUserId, notes) {
    const { data, error } = await supabase.from('commander_onboarding_steps')
...
async function _generateChecklist(cid, clientId, profile, actorUserId, force) {
    ...
    const { data: existing } = await supabase.from('commander_onboarding_checklists').select('item_name').eq('company_id', cid).eq('profile_id', profile.id);
    ...
    const { data, error } = await supabase.from('commander_onboarding_checklists').insert(toInsert).select();
    if (error) throw new Error(error.message);
...
async function _initSecretarial(cid, clientId, entityType, actorUserId) {
    const { data: existing } = await supabase.from('commander_secretarial_profiles').select('id, company_type').eq('company_id', cid).eq('client_id', clientId).maybeSingle();
    if (existing) return { module: 'secretarial', status: 'existing', detail: { id: existing.id } };
    const { data: created, error } = await supabase.from('commander_secretarial_profiles')
...
async function _initEntityLifecycle(cid, clientId, entityType) {
    const { data: existing } = await supabase.from('commander_entity_lifecycle_profiles').select('id, current_lifecycle_status').eq('company_id', cid).eq('client_id', clientId).maybeSingle();
    ...
    const { data: created, error } = await supabase.from('commander_entity_lifecycle_profiles')
...
async function _initClientSuccess(cid, clientId) {
    const { data: existing } = await supabase.from('commander_client_success').select('id').eq('company_id', cid).eq('client_id', clientId).maybeSingle();
...
async function _initBeneficialOwnership(cid, clientId, actorUserId) {
    const { count: before } = await supabase.from('commander_bo_readiness_items').select('id', { count: 'exact', head: true }).eq('company_id', cid).eq('client_id', clientId);
...
async function _initEvidenceTemplates(cid, actorUserId) {
    const { count: before } = await supabase.from('commander_secretarial_evidence_templates').select('id', { count: 'exact', head: true }).eq('company_id', cid);
    await secretarialEvidence.ensureDefaultTemplates(cid, actorUserId);
    const { count: after } = await supabase.from('commander_secretarial_evidence_templates').select('id', { count: 'exact', head: true }).eq('company_id', cid);
...
async function _detectModules(cid, clientId) {
    const [obligations, evidenceChecklists, integrityFindings, riskEntries, knowledgeLinks, taxpayerProfiles] = await Promise.all([
        supabase.from('commander_statutory_obligations').select('id', { count: 'exact', head: true }).eq('company_id', cid).eq('client_id', clientId),
        supabase.from('commander_secretarial_evidence_checklists').select('id', { count: 'exact', head: true }).eq('company_id', cid).eq('client_id', clientId),
        supabase.from('commander_secretarial_integrity_findings').select('id', { count: 'exact', head: true }).eq('company_id', cid).eq('client_id', clientId).eq('status', 'open'),
        supabase.from('commander_risks').select('id', { count: 'exact', head: true }).eq('company_id', cid).eq('linked_client_id', clientId),
        supabase.from('commander_knowledge_links').select('id', { count: 'exact', head: true }).eq('company_id', cid).eq('linked_type', 'client').eq('linked_id', clientId),
        supabase.from('commander_taxpayer_profiles').select('id', { count: 'exact', head: true }).eq('company_id', cid).eq('client_id', clientId),
    ]);
    return {
...
router.get('/summary', async (req, res) => {
    ...
    try {
        const { data: profiles } = await supabase.from('commander_onboarding_profiles').select('onboarding_status, expected_go_live_date, completion_percentage, created_at').eq('company_id', cid);
        const rows = profiles || [];
...
router.get('/profiles', async (req, res) => {
    ...
    try {
        let q = supabase.from('commander_onboarding_profiles').select('*').eq('company_id', cid).order('created_at', { ascending: false }).limit(Math.min(300, parseInt(limit) || 100));
        ...
        let nameById = {};
        if (clientIds.length) {
            const { data: clients } = await supabase.from('commander_clients').select('id, name').in('id', clientIds).eq('company_id', cid);
...
router.get('/profiles/:clientId', async (req, res) => {
    ...
    try {
        ...
        const [stepsRes, checklistRes, detected] = await Promise.all([
            supabase.from('commander_onboarding_steps').select('*').eq('company_id', cid).eq('profile_id', profile.id).order('sort_order'),
            supabase.from('commander_onboarding_checklists').select('*').eq('company_id', cid).eq('profile_id', profile.id).order('sort_order'),
...
router.put('/profiles/:clientId', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_onboarding_profiles').update(update).eq('id', profile.id).eq('company_id', cid).select().single();
        if (error) return res.status(500).json({ error: error.message });
...
router.get('/profiles/:clientId/steps', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_onboarding_steps').select('*').eq('company_id', cid).eq('profile_id', profile.id).order('sort_order');
        if (error) return res.status(500).json({ error: error.message });
        res.json({ steps: data || [] });
    } catch (err) {
...
router.put('/steps/:id', async (req, res) => {
    ...
    const existing = await supabase.from('commander_onboarding_steps').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
    ...
    try {
        const { data, error } = await supabase.from('commander_onboarding_steps').update(update).eq('id', id).eq('company_id', cid).select().single();
        if (error) return res.status(500).json({ error: error.message });
...
router.put('/checklist/:id', async (req, res) => {
    ...
    const existing = await supabase.from('commander_onboarding_checklists').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
    ...
    try {
        const { data, error } = await supabase.from('commander_onboarding_checklists').update(update).eq('id', id).eq('company_id', cid).select().single();
        if (error) return res.status(500).json({ error: error.message });
        await _recomputeCompletion(cid, existing.data.profile_id);
        res.json({ item: data });
    } catch (err) {
...
router.put('/profiles/:clientId/status', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_onboarding_profiles')
...
router.put('/profiles/:clientId/approve', async (req, res) => {
    ...
    try {
        ...
        const { data: reviewStep } = await supabase.from('commander_onboarding_steps').select('*').eq('company_id', cid).eq('profile_id', profile.id).eq('step_name', 'Review completed').maybeSingle();
...
router.put('/profiles/:clientId/complete', async (req, res) => {
    ...
    try {
        ...
        const { data: goLiveStep } = await supabase.from('commander_onboarding_steps').select('*').eq('company_id', cid).eq('profile_id', profile.id).eq('step_name', 'Go-live approved').maybeSingle();
...
router.get('/events', async (req, res) => {
    ...
    try {
        let q = supabase.from('commander_onboarding_events').select('*').eq('company_id', cid).order('created_at', { ascending: false }).limit(Math.min(500, parseInt(limit) || 100));
