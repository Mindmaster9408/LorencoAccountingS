...
// commander_clients keeps its own copy of VAT/registration/tax-reference fields
// for legacy/unlinked clients, but when a client is linked to a real
// operational company (commander_clients.eco_client_id -> eco_clients.id ->
...
// (commander_clients.coida_registration_number <-> companies.coid_reference_number).
...
// Legacy type values (original commander_deadlines.type column — backward compat)
...
router.get('/status', (req, res) => {
  res.json({ module: 'commander', status: 'active', version: '1.0.0' });
});
...
// Commander (Firmflow-derived, being adapted for international companies) is
// gated at two levels, mirroring Firmflow's own gate exactly:
// 1. companies.modules_enabled must include 'commander'
// 2. companies.account_holder_type must be 'accounting_practice'
// Super admins bypass both checks (needed for testing and support) — this is
// currently the ONLY way Commander is reachable at all, since no company has
// 'commander' in modules_enabled yet (2026-09-12: intentionally inactive for
// every real client while this is adapted from the Firmflow copy).
async function requireCommanderModule(req, res, next) {
  ...
  try {
    ...
    if (!Array.isArray(co.modules_enabled) || !co.modules_enabled.includes('commander')) {
      return res.status(403).json({
        error: 'Commander is not enabled for your company.',
        code: 'COMMANDER_MODULE_NOT_ENABLED'
      });
    }
    if (co.account_holder_type !== 'accounting_practice') {
      return res.status(403).json({
        error: 'Commander is only available to accounting practices.',
        code: 'NOT_ACCOUNTING_PRACTICE'
      });
    }
    next();
  } catch (err) {
    console.error('[commander] requireCommanderModule error:', err.message);
    return res.status(500).json({ error: 'Failed to verify Commander module access.' });
  }
}
router.use(requireCommanderModule);

// ─── KV Store (UI preferences only — not business data) ──────────────────────
router.get('/kv/:key', async (req, res) => {
  const kvKey = `commander_${req.companyId}_${req.params.key}`;
...
function sanitizeProfileBody(body) {
  const allowed = [
    'tax_practitioner_number', 'vat_registration_number', 'commander_type',
    'commander_email', 'commander_phone', 'commander_website',
...
router.post('/profile', async (req, res) => {
  ...
  await auditFromReq(req, 'CREATE', 'commander_profile', data.id, { module: 'commander' });
...
router.put('/profile', async (req, res) => {
  ...
  await auditFromReq(req, 'UPDATE', 'commander_profile', data.id, { module: 'commander' });
...
router.get('/dashboard', async (req, res) => {
  ...
  try {
    ...
    const [clients, openTasks, overdue, upcoming, timeThisMonth] = await Promise.all([
      supabase.from('commander_clients').select('id', { count: 'exact', head: true }).eq('company_id', cid).eq('is_active', true),
      supabase.from('commander_tasks').select('id', { count: 'exact', head: true }).eq('company_id', cid).in('status', ['open', 'in_progress']),
      supabase.from('commander_tasks').select('id', { count: 'exact', head: true }).eq('company_id', cid).in('status', ['open', 'in_progress']).lt('due_date', today),
      supabase.from('commander_tasks').select('id', { count: 'exact', head: true }).eq('company_id', cid).in('status', ['open', 'in_progress']).gte('due_date', today).lte('due_date', nextMonth),
      supabase.from('commander_time_entries').select('hours').eq('company_id', cid).gte('date', today.slice(0, 7) + '-01'),
    ]);
...
// picker is a <select> populated from /api/commander/users, but nothing
...
// (its /api/commander/users load can fail silently), leaving user_id NULL
...
router.get('/team/me', async (req, res) => {
  try {
    ...
    console.error('GET /api/commander/team/me', err);
...
router.post('/team', async (req, res) => {
  ...
  await auditFromReq(req, 'CREATE', 'commander_team_member', data.id, { module: 'commander' });
...
router.put('/team/:id', async (req, res) => {
  ...
  const { data: existing } = await supabase
    .from('commander_team_members').select('id').eq('id', req.params.id).eq('company_id', req.companyId).single();
  ...
  await auditFromReq(req, 'UPDATE', 'commander_team_member', data.id, { module: 'commander' });
  res.json({ member: data });
});
...
router.delete('/team/:id', async (req, res) => {
  ...
  const { error } = await supabase
    ...
    .eq('company_id', req.companyId);
  if (error) return res.status(500).json({ error: error.message });
  await auditFromReq(req, 'DEACTIVATE', 'commander_team_member', parseInt(req.params.id), { module: 'commander' });
  res.json({ success: true });
});
...
router.put('/team/:id/reactivate', async (req, res) => {
  ...
  const { error } = await supabase
    ...
    .eq('company_id', req.companyId);
  if (error) return res.status(500).json({ error: error.message });
  await auditFromReq(req, 'REACTIVATE', 'commander_team_member', parseInt(req.params.id), { module: 'commander' });
  res.json({ success: true });
});
...
router.put('/team/:id/capacity', async (req, res) => {
  ...
  await auditFromReq(req, 'UPDATE', 'commander_team_member', parseInt(req.params.id), {
    module: 'commander',
...
// commander_team_members rows for any not already linked. Default role: staff.
...
router.post('/team/sync-from-users', async (req, res) => {
  try {
    ...
    await auditFromReq(req, 'CREATE', 'commander_team_member', null, {
      module: 'commander',
    ...
    console.error('[commander] sync-from-users error:', err.message);
...
// commander_clients rows for any not already linked by eco_client_id.
// Existing commander_clients are never touched — only new rows are added.
router.post('/clients/sync-from-eco', async (req, res) => {
  try {
    ...
    // Map 'business' → 'company' for commander_clients (most common SA entity type).
    ...
    await auditFromReq(req, 'CREATE', 'commander_client', null, {
      module: 'commander',
    ...
    console.error('[commander] sync-from-eco error:', err.message);
...
router.get('/clients/:id', async (req, res) => {
  ...
  if (data.eco_client_id) {
    ...
    if (ec) {
      ...
      // registration/tax-reference values over commander_clients' own copy
      // (see COMPLIANCE_FIELD_MAP above) — falls back to commander_clients'
...
router.post('/clients', async (req, res) => {
  ...
  // eco_client must exist BEFORE commander_client is inserted.
  // If eco_client cannot be created/found, we abort — no orphaned commander_client.
  ...
  if (linkEcoId) {
    ...
    // Check: is there already a commander_client for this eco_client?
    ...
    if (existingPcForLink && existingPcForLink.length > 0) {
      return res.status(409).json({
        ...
        existing_commander_client_id: existingPcForLink[0].id,
  ...
  if (!ecoClientId && idForLookup) {
    ...
    if (existingEco && existingEco.length > 0) {
      // Eco_client already exists — check if there is already a commander_client linked to it
      ...
      if (existingPc && existingPc.length > 0) {
        return res.status(409).json({
          ...
          code: 'DUPLICATE_PRACTICE_CLIENT',
          existing_commander_client_id: existingPc[0].id,
  ...
  if (!ecoClientId) {
    ...
    // eco_clients.apps controls PCC billing; Practice visibility comes from commander_clients link.
    ...
    if (ecErr || !ec) {
      console.error('[commander] eco_client creation failed:', ecErr?.message);
  ...
  // ── Phase 2: insert commander_client with eco_client_id already set ─────────
  ...
  await auditFromReq(req, 'CREATE', 'commander_client', data.id, { module: 'commander' });
...
router.put('/clients/:id', async (req, res) => {
  ...
  if (existing.eco_client_id) {
    try {
      ...
      // company (legacy/practice-only clients — commander_clients' own copy,
      ...
      console.error('[commander] eco_client identity sync failed:', ecoSyncErr.message);
    }
  }

  await auditFromReq(req, 'UPDATE', 'commander_client', data.id, { module: 'commander' });
...
router.delete('/clients/:id', async (req, res) => {
  ...
  const { error } = await supabase
    ...
    .eq('company_id', req.companyId);
  if (error) return res.status(500).json({ error: error.message });
  await auditFromReq(req, 'ARCHIVE', 'commander_client', parseInt(req.params.id), { module: 'commander' });
  res.json({ success: true });
});
...
router.get('/clients/:id/contacts', async (req, res) => {
  ...
  const { data, error } = await supabase
    .from('commander_client_contacts')
    .select('*')
...
router.post('/clients/:id/contacts', async (req, res) => {
  ...
  const { data, error } = await supabase.from('commander_client_contacts').insert(body).select().single();
  if (error) return res.status(500).json({ error: error.message });
  await auditFromReq(req, 'CREATE', 'commander_client_contact', data.id, { module: 'commander' });
...
router.put('/clients/:clientId/contacts/:contactId', async (req, res) => {
  ...
  const { data, error } = await supabase
    ...
    .select().single();
  if (error) return res.status(500).json({ error: error.message });
  await auditFromReq(req, 'UPDATE', 'commander_client_contact', data.id, { module: 'commander' });
...
router.delete('/clients/:clientId/contacts/:contactId', async (req, res) => {
  ...
  const { error } = await supabase
    ...
    .eq('company_id', req.companyId);
  if (error) return res.status(500).json({ error: error.message });
  await auditFromReq(req, 'DEACTIVATE', 'commander_client_contact', parseInt(req.params.contactId), { module: 'commander' });
  res.json({ success: true });
});
...
async function logReviewEvent(companyId, taskId, eventType, opts = {}) {
  try {
    await supabase.from('commander_task_review_events').insert({
...
router.get('/tasks', async (req, res) => {
  ...
  let q = supabase
    ...
    .select(`*,
      commander_clients:client_id(name),
...
router.post('/tasks', async (req, res) => {
  ...
  const { data, error } = await supabase
    ...
    .select().single();
  if (error) return res.status(500).json({ error: error.message });
  await auditFromReq(req, 'CREATE', 'commander_task', data.id, { module: 'commander' });
...
router.put('/tasks/:id', async (req, res) => {
  ...
  await auditFromReq(req, 'UPDATE', 'commander_task', data.id, { module: 'commander' });
  res.json({ task: data });
});
...
router.delete('/tasks/:id', async (req, res) => {
  const { error } = await supabase
    ...
    .eq('company_id', req.companyId);
  if (error) return res.status(500).json({ error: error.message });
  // Note: commander_task_review_events.task_id has ON DELETE SET NULL — review history is preserved
  res.json({ success: true });
});
...
router.get('/tasks/:id/review-events', async (req, res) => {
  ...
  const { data, error } = await supabase
    .from('commander_task_review_events')
    .select('*')
    .eq('company_id', req.companyId)
...
router.put('/tasks/:id/submit-review', async (req, res) => {
  ...
  await auditFromReq(req, 'UPDATE', 'commander_task', data.id, { module: 'commander', action: 'submit_review' });
  res.json({ task: data });
});
...
router.put('/tasks/:id/start-review', async (req, res) => {
  ...
  await auditFromReq(req, 'UPDATE', 'commander_task', data.id, { module: 'commander', action: 'start_review' });
  res.json({ task: data });
});
...
router.put('/tasks/:id/approve-review', async (req, res) => {
  ...
  await auditFromReq(req, 'UPDATE', 'commander_task', data.id, { module: 'commander', action: 'approve_review' });
  res.json({ task: data });
});
...
router.put('/tasks/:id/reject-review', async (req, res) => {
  ...
  await auditFromReq(req, 'UPDATE', 'commander_task', data.id, { module: 'commander', action: 'reject_review' });
  res.json({ task: data });
});
...
router.put('/tasks/:id/approve-final', async (req, res) => {
  ...
  await auditFromReq(req, 'UPDATE', 'commander_task', data.id, { module: 'commander', action: 'approve_final' });
  res.json({ task: data });
});
...
router.put('/tasks/:id/reject-final', async (req, res) => {
  ...
  await auditFromReq(req, 'UPDATE', 'commander_task', data.id, { module: 'commander', action: 'reject_final' });
  res.json({ task: data });
});
...
router.put('/tasks/:id/qa-lock', async (req, res) => {
  ...
  await auditFromReq(req, 'UPDATE', 'commander_task', data.id, { module: 'commander', action: 'qa_lock' });
  res.json({ task: data });
});
...
router.put('/tasks/:id/qa-unlock', async (req, res) => {
  ...
  await auditFromReq(req, 'UPDATE', 'commander_task', data.id, { module: 'commander', action: 'qa_unlock' });
  res.json({ task: data });
});
...
router.get('/time-entries', async (req, res) => {
  ...
  let q = supabase
    .from('commander_time_entries')
    .select(`*, commander_clients:client_id(name), commander_tasks:task_id(title)`, { count: 'exact' })
    .eq('company_id', req.companyId)
...
router.post('/time-entries', async (req, res) => {
  ...
  await auditFromReq(req, 'CREATE', 'commander_time_entry', data.id, {
    module: 'commander',
...
router.put('/time-entries/:id', async (req, res) => {
  ...
  await auditFromReq(req, 'UPDATE', 'commander_time_entry', data.id, { module: 'commander' });
...
router.put('/time-entries/:id/submit-review', async (req, res) => {
  ...
  await auditFromReq(req, 'time_submitted_review', 'commander_time_entry', data.id, { module: 'commander' });
  res.json({ time_entry: data });
});
...
router.put('/time-entries/:id/approve', async (req, res) => {
  ...
  await auditFromReq(req, 'time_approved', 'commander_time_entry', data.id, { module: 'commander' });
  res.json({ time_entry: data });
});
...
router.put('/time-entries/:id/reject', async (req, res) => {
  ...
  await auditFromReq(req, 'time_rejected', 'commander_time_entry', data.id, { module: 'commander' });
  res.json({ time_entry: data });
});
...
async function logDeadlineEvent(companyId, deadlineId, eventType, opts = {}) {
  await supabase.from('commander_deadline_events').insert({
    company_id: companyId,
...
router.get('/deadlines', async (req, res) => {
  ...
  let q = supabase
    .from('commander_deadlines')
    .select('*, commander_clients:client_id(name), commander_team_members!responsible_team_member_id(display_name)')
    .eq('company_id', req.companyId)
  ...
  if (search) {
    ...
    results = results.filter(d =>
      ...
      (d.commander_clients?.name && d.commander_clients.name.toLowerCase().includes(s))
    );
  }
...
router.post('/deadlines', async (req, res) => {
  ...
  if (body.client_id) {
    const cc = await supabase.from('commander_clients').select('id').eq('id', body.client_id).eq('company_id', req.companyId).single();
  ...
  if (body.responsible_team_member_id) {
    const tm = await supabase.from('commander_team_members').select('id').eq('id', body.responsible_team_member_id).eq('company_id', req.companyId).single();
  ...
  await auditFromReq(req, 'CREATE', 'commander_deadline', data.id, { module: 'commander' });
...
router.put('/deadlines/:id', async (req, res) => {
  ...
  await auditFromReq(req, 'UPDATE', 'commander_deadline', data.id, { module: 'commander' });
...
router.put('/deadlines/:id/status', async (req, res) => {
  ...
  await auditFromReq(req, 'UPDATE', 'commander_deadline', data.id, { module: 'commander', action_detail: `status: ${oldStatus} → ${status}` });
  await logDeadlineEvent(req.companyId, data.id, 'status_changed', {
    oldStatus,
    newStatus: status,
...
router.delete('/deadlines/:id', async (req, res) => {
  ...
  await auditFromReq(req, 'CANCEL', 'commander_deadline', parseInt(req.params.id), { module: 'commander' });
...
router.get('/deadline-settings', async (req, res) => {
  const cid = req.companyId;
  const { data, error } = await supabase
    .from('commander_deadline_settings')
...
router.put('/deadline-settings', async (req, res) => {
  ...
  if (error) return res.status(500).json({ error: error.message });
  await auditFromReq(req, 'UPDATE', 'commander_deadline_settings', null, { module: 'commander', count: rows.length });
...
router.post('/deadlines/generate', async (req, res) => {
  ...
  await auditFromReq(req, 'GENERATE', 'commander_deadlines', null, { module: 'commander', period, created, skipped: existingKeys.size });
...
router.get('/compliance/calendar', async (req, res) => {
  ...
  let q = supabase
    .from('commander_deadlines')
    .select('id, title, due_date, status, priority, compliance_area, deadline_type, type, client_id, responsible_team_member_id, commander_clients:client_id(name), commander_team_members!responsible_team_member_id(display_name)')
    .eq('company_id', req.companyId)
    .eq('is_active', true)
    .order('due_date', { ascending: true });
  ...
  const events = (data || []).map(d => ({
    id: d.id,
    title: d.title,
    client_name: d.commander_clients?.name || null,
    due_date: d.due_date,
    status: d.status,
    priority: d.priority,
    compliance_area: d.compliance_area,
    deadline_type: d.deadline_type || d.type,
    responsible_name: d.commander_team_members?.display_name || null,
...
router.get('/compliance/rules', async (req, res) => {
  ...
  let q = supabase
    .from('commander_compliance_rules')
    .select('*')
    .eq('company_id', req.companyId)
...
router.post('/compliance/rules', async (req, res) => {
  ...
  await auditFromReq(req, 'CREATE', 'commander_compliance_rule', data.id, { module: 'commander' });
...
router.put('/compliance/rules/:id', async (req, res) => {
  ...
  await auditFromReq(req, 'UPDATE', 'commander_compliance_rule', data.id, { module: 'commander' });
  res.json({ rule: data });
});
...
router.delete('/compliance/rules/:id', async (req, res) => {
  ...
  await auditFromReq(req, 'DEACTIVATE', 'commander_compliance_rule', parseInt(req.params.id), { module: 'commander' });
  res.json({ success: true });
});
...
router.get('/deadlines/:id/events', async (req, res) => {
  ...
  const { data, error } = await supabase
    .from('commander_deadline_events')
    .select('*')
...
// NOT another Deadlines module — links to commander_deadlines, never duplicates.
