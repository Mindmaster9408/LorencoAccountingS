...
//
// THE CONTROL POINT: Codebox 62's registers (commander_company_directors,
// commander_company_shareholders, commander_secretarial_profiles,
// commander_annual_returns) are never edited through THIS module except via
...
async function _requireManager(req, res) {
    return teamAccess.requireManager(req, res, supabase, 'Only owners, partners, admins, and practice managers can manage statutory change cases.');
}

async function _verifyClient(cid, clientId) {
    const { data } = await supabase.from('commander_clients').select('id, name')
        .eq('id', clientId).eq('company_id', cid).eq('is_active', true).maybeSingle();
    return data || null;
}

async function _getCase(cid, id) {
    const { data } = await supabase.from('commander_secretarial_change_cases').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
    return data || null;
}

async function _writeCaseEvent(cid, caseId, eventType, oldStatus, newStatus, actorUserId, notes, meta) {
    await supabase.from('commander_secretarial_change_events').insert({
...
async function _implDirectorAppointment(cid, clientId, payload) {
    ...
    const { data, error } = await supabase.from('commander_company_directors').insert({
...
async function _implDirectorResignation(cid, clientId, payload, effectiveDate) {
    ...
    const { data: existing } = await supabase.from('commander_company_directors').select('*').eq('id', director_id).eq('company_id', cid).eq('client_id', clientId).maybeSingle();
    if (!existing) return { error: 'payload.director_id does not match an existing director for this client.' };
    const { data, error } = await supabase.from('commander_company_directors')
...
async function _implShareTransfer(cid, clientId, payload, effectiveDate) {
    ...
    const { data: existing } = await supabase.from('commander_company_shareholders').select('*').eq('id', shareholder_id).eq('company_id', cid).eq('client_id', clientId).maybeSingle();
    if (!existing) return { error: 'payload.shareholder_id does not match an existing shareholder for this client.' };
    const { data, error } = await supabase.from('commander_company_shareholders')
...
function _implProfileField(field, label) {
    return async function (cid, clientId, payload) {
        ...
        const { data, error } = await supabase.from('commander_secretarial_profiles')
...
async function _implAnnualReturn(cid, clientId, payload) {
    ...
    const { data: existing } = await supabase.from('commander_annual_returns').select('*').eq('company_id', cid).eq('client_id', clientId).eq('return_year', parseInt(return_year)).maybeSingle();
    const body = {
    ...
    let data, error;
    if (existing) {
        ({ data, error } = await supabase.from('commander_annual_returns').update(body).eq('id', existing.id).select().single());
    } else {
        ({ data, error } = await supabase.from('commander_annual_returns').insert(body).select().single());
    }
    if (error) return { error: error.message };
...
router.get('/summary', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_secretarial_change_cases').select('case_status, change_type, effective_date').eq('company_id', cid);
...
router.get('/', async (req, res) => {
    ...
    try {
        let q = supabase.from('commander_secretarial_change_cases').select('*', { count: 'exact' }).eq('company_id', cid).order('created_at', { ascending: false });
...
router.get('/:id', async (req, res) => {
    ...
    try {
        ...
        const [client, checklist] = await Promise.all([
            supabase.from('commander_clients').select('id, name').eq('id', kase.client_id).eq('company_id', cid).maybeSingle(),
            supabase.from('commander_secretarial_change_checklist_items').select('*').eq('company_id', cid).eq('change_case_id', id).order('sort_order'),
        ]);
...
router.post('/', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_secretarial_change_cases').insert({
...
router.put('/:id', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_secretarial_change_cases').update(update).eq('id', id).eq('company_id', cid).select().single();
        if (error) return res.status(500).json({ error: error.message });
...
router.delete('/:id', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_secretarial_change_cases')
...
router.post('/:id/generate-checklist', async (req, res) => {
    ...
    try {
        const { count } = await supabase.from('commander_secretarial_change_checklist_items').select('id', { count: 'exact', head: true }).eq('company_id', cid).eq('change_case_id', id);
        ...
        if (count > 0 && req.query.force === 'true') {
            await supabase.from('commander_secretarial_change_checklist_items').delete().eq('company_id', cid).eq('change_case_id', id);
        }
        ...
        const { data, error } = await supabase.from('commander_secretarial_change_checklist_items').insert(rows).select();
        if (error) return res.status(500).json({ error: error.message });
...
router.get('/:id/checklist', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_secretarial_change_checklist_items').select('*').eq('company_id', cid).eq('change_case_id', id).order('sort_order');
        if (error) return res.status(500).json({ error: error.message });
...
router.put('/:id/checklist/:itemId', async (req, res) => {
    ...
    const existing = await supabase.from('commander_secretarial_change_checklist_items').select('*').eq('id', itemId).eq('change_case_id', id).eq('company_id', cid).maybeSingle();
    ...
    try {
        const { data, error } = await supabase.from('commander_secretarial_change_checklist_items').update(update).eq('id', itemId).eq('company_id', cid).select().single();
        if (error) return res.status(500).json({ error: error.message });
...
router.put('/:id/approve', async (req, res) => {
    ...
    try {
        const { data: checklist } = await supabase.from('commander_secretarial_change_checklist_items').select('required, completed').eq('company_id', cid).eq('change_case_id', id);
...
router.get('/:id/events', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_secretarial_change_events').select('*').eq('company_id', cid).eq('change_case_id', id).order('created_at', { ascending: false });
        if (error) return res.status(500).json({ error: error.message });
