...
// inserting into commander_notifications directly — this keeps routing,
...
async function _verifyNotification(id, cid) {
    const { data } = await supabase.from('commander_notifications').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
    return data || null;
}

async function _writeEvent(cid, notificationId, eventType, oldStatus, newStatus, actorUserId, notes, meta) {
    await supabase.from('commander_notification_events').insert({
        company_id: cid,
...
// Resolves the current HTTP caller's own commander_team_members.id (if any),
...
async function _resolveAssignment(cid, assignment) {
    ...
    if (assignment.role && CLIENT_ROLE_FIELD[assignment.role] && assignment.clientId) {
        ...
        const { data: client } = await supabase.from('commander_clients').select(field)
            .eq('id', assignment.clientId).eq('company_id', cid).maybeSingle();
        const memberId = client ? client[field] : null;
        if (memberId) {
            const { data: member } = await supabase.from('commander_team_members').select('id')
...
async function notify(params) {
    ...
    const { data: created, error } = await supabase.from('commander_notifications').insert(insertRow).select().single();
    if (error) {
...
async function _findActiveByKey(cid, notificationKey, teamMemberId) {
    let q = supabase.from('commander_notifications').select('id')
...
router.get('/summary', async (req, res) => {
    try {
        ...
        const { data, error } = await supabase.from('commander_notifications')
        ...
        console.error('GET /api/commander/notifications/summary', err);
...
router.get('/', async (req, res) => {
    try {
        ...
        let q = supabase.from('commander_notifications').select('*', { count: 'exact' })
            .eq('company_id', cid)
        ...
        console.error('GET /api/commander/notifications', err);
...
router.get('/:id', async (req, res) => {
    try {
        ...
        console.error('GET /api/commander/notifications/:id', err);
...
router.post('/', async (req, res) => {
    try {
        ...
        console.error('POST /api/commander/notifications', err);
...
router.put('/:id/read', async (req, res) => {
    try {
        ...
        const { data: updated, error } = await supabase.from('commander_notifications')
        ...
        console.error('PUT /api/commander/notifications/:id/read', err);
...
router.put('/:id/unread', async (req, res) => {
    try {
        ...
        console.error('PUT /api/commander/notifications/:id/unread', err);
...
router.put('/:id/snooze', async (req, res) => {
    try {
        ...
        console.error('PUT /api/commander/notifications/:id/snooze', err);
...
router.put('/:id/archive', async (req, res) => {
    try {
        ...
        console.error('PUT /api/commander/notifications/:id/archive', err);
...
router.put('/:id/complete', async (req, res) => {
    try {
        ...
        console.error('PUT /api/commander/notifications/:id/complete', err);
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        console.error('DELETE /api/commander/notifications/:id', err);
...
router.get('/:id/events', async (req, res) => {
    try {
        ...
        const n = await _verifyNotification(req.params.id, cid);
        if (!n) return res.status(404).json({ error: 'Notification not found.' });
        const { data, error } = await supabase.from('commander_notification_events').select('*').eq('company_id', cid).eq('notification_id', n.id).order('created_at', { ascending: false });
        if (error) throw error;
        res.json({ events: data || [] });
    } catch (err) {
        console.error('GET /api/commander/notifications/:id/events', err);
...
async function _bulkTransition(req, res, { toStatus, eventType, timestampField, blockedStatuses }) {
    try {
        ...
        for (const id of ids) {
            ...
            const { data: updated, error } = await supabase.from('commander_notifications').update(patch).eq('id', id).eq('company_id', cid).select().single();
        ...
        console.error('POST /api/commander/notifications/bulk-*', err);
