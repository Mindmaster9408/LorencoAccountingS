...
//
// NOT another Deadlines module — commander_deadlines remains the master
// task/deadline system. This module defines statutory OBLIGATIONS (what's
// due, how often) and synchronises them with commander_deadlines by linking
...
// Maps an obligation_type onto the existing commander_deadlines.deadline_type
...
async function _writeEvent(cid, clientId, sourceType, sourceId, eventType, oldStatus, newStatus, actorUserId, notes, meta) {
    await supabase.from('commander_statutory_calendar_events').insert({
...
async function _resolveAnchorDate(cid, clientId, dueRule) {
    ...
    if (anchor === 'registration_date') {
        const { data } = await supabase.from('commander_secretarial_profiles').select('registration_date').eq('company_id', cid).eq('client_id', clientId).maybeSingle();
        return data?.registration_date || null;
    }
    if (anchor === 'financial_year_end') {
        const { data } = await supabase.from('commander_taxpayer_profiles').select('financial_year_end').eq('company_id', cid).eq('client_id', clientId).maybeSingle();
...
// Resolve-or-create against commander_deadlines — the reuse target. Matches
...
async function _resolveOrCreateDeadline(cid, clientId, obligation, dueDate, periodLabel, actorUserId) {
    ...
    const { data: existing } = await supabase.from('commander_deadlines')
    ...
    const { data: created, error } = await supabase.from('commander_deadlines').insert({
...
async function generateSchedule(cid, obligationId, actorUserId, monthsAhead) {
    const { data: obligation } = await supabase.from('commander_statutory_obligations').select('*').eq('id', obligationId).eq('company_id', cid).maybeSingle();
    ...
    const { data: existingRows } = await supabase.from('commander_statutory_schedule').select('period_label').eq('obligation_id', obligationId);
    ...
    for (const occ of occurrences) {
        ...
        const { data: row, error } = await supabase.from('commander_statutory_schedule').insert({
...
async function _isDependencySatisfied(cid, dep) {
    ...
    if (dep.depends_on_type === 'schedule_item' && dep.depends_on_schedule_id) {
        const { data } = await supabase.from('commander_statutory_schedule').select('status').eq('id', dep.depends_on_schedule_id).eq('company_id', cid).maybeSingle();
    ...
    if (dep.depends_on_type === 'evidence_complete') {
        ...
        try {
            const { data: checklist } = await supabase.from('commander_secretarial_evidence_checklists').select('*').eq('id', dep.depends_on_checklist_id).eq('company_id', cid).maybeSingle();
...
async function buildStatutoryCalendar(cid, clientId) {
    let q = supabase.from('commander_statutory_schedule').select('*').eq('company_id', cid).order('due_date');
    ...
    const { data: depRows } = scheduleIds.length
        ? await supabase.from('commander_statutory_dependencies').select('*').eq('company_id', cid).in('schedule_id', scheduleIds)
        : { data: [] };
...
router.get('/obligations', async (req, res) => {
    ...
    try {
        let q = supabase.from('commander_statutory_obligations').select('*').eq('company_id', cid).order('created_at', { ascending: false });
...
router.post('/obligations', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_statutory_obligations').insert({
...
router.get('/obligations/:id', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_statutory_obligations').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
        if (error) return res.status(500).json({ error: error.message });
...
router.put('/obligations/:id', async (req, res) => {
    ...
    const existing = await supabase.from('commander_statutory_obligations').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
    ...
    try {
        const { data, error } = await supabase.from('commander_statutory_obligations').update(update).eq('id', id).eq('company_id', cid).select().single();
        if (error) return res.status(500).json({ error: error.message });
...
router.delete('/obligations/:id', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_statutory_obligations')
...
router.post('/schedule', async (req, res) => {
    ...
    try {
        ...
        const { data, error } = await supabase.from('commander_statutory_schedule').insert({
...
router.put('/schedule/:id', async (req, res) => {
    ...
    const existing = await supabase.from('commander_statutory_schedule').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
    ...
    try {
        const { data, error } = await supabase.from('commander_statutory_schedule').update(update).eq('id', id).eq('company_id', cid).select().single();
        if (error) return res.status(500).json({ error: error.message });
...
router.get('/dependencies', async (req, res) => {
    ...
    try {
        let q = supabase.from('commander_statutory_dependencies').select('*').eq('company_id', cid).order('created_at', { ascending: false });
...
router.post('/dependencies', async (req, res) => {
    ...
    const { data: schedule } = await supabase.from('commander_statutory_schedule').select('*').eq('id', scheduleId).eq('company_id', cid).maybeSingle();
    ...
    if (depends_on_type === 'schedule_item') {
        ...
        const { data: dep } = await supabase.from('commander_statutory_schedule').select('id').eq('id', depends_on_schedule_id).eq('company_id', cid).eq('client_id', schedule.client_id).maybeSingle();
    ...
    if (depends_on_type === 'evidence_complete' && depends_on_checklist_id) {
        const { data: checklist } = await supabase.from('commander_secretarial_evidence_checklists').select('id').eq('id', depends_on_checklist_id).eq('company_id', cid).eq('client_id', schedule.client_id).maybeSingle();
    ...
    try {
        const { data, error } = await supabase.from('commander_statutory_dependencies').insert({
...
router.put('/dependencies/:id/override', async (req, res) => {
    ...
    const existing = await supabase.from('commander_statutory_dependencies').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
    ...
    try {
        ...
        const { data, error } = await supabase.from('commander_statutory_dependencies').update(update).eq('id', id).eq('company_id', cid).select().single();
        if (error) return res.status(500).json({ error: error.message });
...
router.get('/events', async (req, res) => {
    ...
    try {
        let q = supabase.from('commander_statutory_calendar_events').select('*').eq('company_id', cid).order('created_at', { ascending: false }).limit(Math.min(500, parseInt(limit) || 100));
        if (client_id) q = q.eq('client_id', parseInt(client_id));
        const { data, error } = await q;
        if (error) return res.status(500).json({ error: error.message });
