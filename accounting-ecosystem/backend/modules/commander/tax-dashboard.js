...
router.get('/summary', async (req, res) => {
    ...
    try {
        const [
            ...
            supabase.from('commander_individual_tax_returns')
            ...
            supabase.from('commander_individual_tax_review_packs')
                .select('id', { count: 'exact', head: true })
                .eq('company_id', cid).eq('pack_status', 'ready_for_review'),

            supabase.from('commander_company_tax_returns')
                .select('id', { count: 'exact', head: true })
                .eq('company_id', cid).neq('status', 'cancelled'),
            supabase.from('commander_company_tax_returns')
                .select('id', { count: 'exact', head: true })
            ...
            supabase.from('commander_company_tax_review_packs')
                .select('id', { count: 'exact', head: true })
                .eq('company_id', cid).eq('pack_status', 'ready_for_review'),

            supabase.from('commander_provisional_tax_plans')
                .select('id', { count: 'exact', head: true })
                .eq('company_id', cid).neq('status', 'cancelled'),
            supabase.from('commander_provisional_tax_periods')
                .select('id', { count: 'exact', head: true })
                .eq('company_id', cid)
            ...
            supabase.from('commander_deadlines')
                ...
                .lt('due_date', t),
            supabase.from('commander_document_requests')
                .select('id', { count: 'exact', head: true })
                .eq('company_id', cid)
                .in('request_status', ['requested', 'reminder_sent', 'partially_received']),

            supabase.from('commander_individual_tax_calculations')
                .select('id', { count: 'exact', head: true })
                .eq('company_id', cid).eq('calculation_status', 'ready_for_review'),
            supabase.from('commander_company_tax_calculations')
                .select('id', { count: 'exact', head: true })
                .eq('company_id', cid).eq('calculation_status', 'ready_for_review'),
        ]);
...
router.get('/risk', async (req, res) => {
    ...
    try {
        const [
            ...
            supabase.from('commander_deadlines')
                .select('id, title, due_date, status, deadline_type, client_id, clients:commander_clients!client_id(name)')
                .eq('company_id', cid)
                .not('status', 'in', '(completed,submitted,missed,cancelled)')
...
router.get('/activity', async (req, res) => {
    ...
    try {
        const results = await Promise.all([
            supabase.from('commander_individual_tax_events')
                .select('id, event_type, created_at').eq('company_id', cid)
                .order('created_at', { ascending: false }).limit(8),
            supabase.from('commander_individual_tax_calculation_events')
                .select('id, event_type, created_at').eq('company_id', cid)
                .order('created_at', { ascending: false }).limit(8),
            supabase.from('commander_individual_tax_review_pack_events')
                .select('id, event_type, created_at').eq('company_id', cid)
                .order('created_at', { ascending: false }).limit(8),
            supabase.from('commander_company_tax_events')
                .select('id, event_type, created_at').eq('company_id', cid)
                .order('created_at', { ascending: false }).limit(8),
            supabase.from('commander_company_tax_calculation_events')
                .select('id, event_type, created_at').eq('company_id', cid)
                .order('created_at', { ascending: false }).limit(8),
            supabase.from('commander_company_tax_review_pack_events')
                .select('id, event_type, created_at').eq('company_id', cid)
                .order('created_at', { ascending: false }).limit(8),
            supabase.from('commander_provisional_tax_events')
                .select('id, event_type, created_at').eq('company_id', cid)
                .order('created_at', { ascending: false }).limit(8),
            supabase.from('commander_compliance_pack_events')
                .select('id, event_type, created_at').eq('company_id', cid)
                .order('created_at', { ascending: false }).limit(8),
        ]);
...
router.get('/returns', async (req, res) => {
    ...
    function buildIndQ() {
        let q = supabase.from('commander_individual_tax_returns')
            .select('id, return_name, tax_year, status, readiness_status, readiness_score, responsible_team_member_id, reviewer_team_member_id, clients:commander_clients!client_id(name)')
            .eq('company_id', cid).neq('status', 'cancelled');
    ...
    function buildCoQ() {
        let q = supabase.from('commander_company_tax_returns')
            .select('id, return_name, tax_year, status, readiness_status, readiness_score, responsible_team_member_id, reviewer_team_member_id, clients:commander_clients!client_id(name)')
            .eq('company_id', cid).neq('status', 'cancelled');
        if (tax_year)          q = q.eq('tax_year', parseInt(tax_year));
        if (status)            q = q.eq('status', status);
    ...
    function buildProvQ() {
        let q = supabase.from('commander_provisional_tax_plans')
            .select('id, plan_name, tax_year, status, responsible_team_member_id, reviewer_team_member_id, period_1_due_date, period_2_due_date, topup_due_date, clients:commander_clients!client_id(name)')
            .eq('company_id', cid).neq('status', 'cancelled');
        if (tax_year) q = q.eq('tax_year', parseInt(tax_year));
        if (status)   q = q.eq('status', status);
        if (tmId)     q = q.or(`responsible_team_member_id.eq.${tmId},reviewer_team_member_id.eq.${tmId}`);
