...
async function _writeEvent(cid, eventType, entityType, entityId, actorUserId, notes, meta) {
    await supabase.from('commander_learning_events').insert({
...
async function _verifyPlan(id, cid) {
    const { data } = await supabase.from('commander_learning_plans').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
    return data || null;
}
async function _verifyGoal(id, cid) {
    const { data } = await supabase.from('commander_learning_goals').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
    return data || null;
}
async function _verifyActivity(id, cid) {
    const { data } = await supabase.from('commander_learning_activities').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
    return data || null;
}
...
async function calculateLearningProgress(cid, planId) {
    ...
    const { data: goals } = await supabase.from('commander_learning_goals').select('*').eq('company_id', cid).eq('learning_plan_id', planId);
    const goalRows = goals || [];
    const goalIds = goalRows.map(g => g.id);

    let activityRows = [];
    if (goalIds.length) {
        const { data: activities } = await supabase.from('commander_learning_activities').select('*').eq('company_id', cid).in('goal_id', goalIds);
    ...
    const { data: cpdRows } = await supabase.from('commander_cpd_records').select('hours').eq('company_id', cid).eq('team_member_id', plan.team_member_id).eq('is_active', true);
    const cpdHoursAccumulated = (cpdRows || []).reduce((s, r) => s + (Number(r.hours) || 0), 0);

    await supabase.from('commander_learning_plans').update({ overall_progress: overallProgress }).eq('id', planId).eq('company_id', cid);
...
async function getDevelopmentBadge(cid, teamMemberId, skillId) {
    const { data: plans } = await supabase.from('commander_learning_plans').select('id, mentor_team_member_id').eq('company_id', cid).eq('team_member_id', teamMemberId).eq('status', 'active');
    if (!plans || !plans.length) return null;

    let relevant = plans;
    if (skillId) {
        const planIds = plans.map(p => p.id);
        const { data: goals } = await supabase.from('commander_learning_goals').select('learning_plan_id').eq('company_id', cid).eq('skill_id', skillId).in('learning_plan_id', planIds).not('status', 'in', '("completed","cancelled")');
...
router.get('/summary', async (req, res) => {
    try {
        ...
        const [plansRes, goalsRes, cpdRes] = await Promise.all([
            supabase.from('commander_learning_plans').select('status, target_completion_date, mentor_team_member_id').eq('company_id', cid),
            supabase.from('commander_learning_goals').select('status').eq('company_id', cid),
            supabase.from('commander_cpd_records').select('hours').eq('company_id', cid).eq('is_active', true),
        ]);
        ...
        console.error('GET /api/commander/learning-centre/summary', err);
...
router.get('/plans', async (req, res) => {
    try {
        ...
        let q = supabase.from('commander_learning_plans').select('*').eq('company_id', cid).order('created_at', { ascending: false });
        ...
        console.error('GET /api/commander/learning-centre/plans', err);
...
router.post('/plans', async (req, res) => {
    try {
        ...
        const { data, error } = await supabase.from('commander_learning_plans').insert({
        ...
        console.error('POST /api/commander/learning-centre/plans', err);
...
router.get('/plans/:id', async (req, res) => {
    try {
        ...
        const { data: goals } = await supabase.from('commander_learning_goals').select('*, commander_skills:skill_id(skill_key, display_name)').eq('company_id', cid).eq('learning_plan_id', plan.id).order('created_at');
        const goalIds = (goals || []).map(g => g.id);
        let activities = [];
        if (goalIds.length) {
            const { data } = await supabase.from('commander_learning_activities').select('*').eq('company_id', cid).in('goal_id', goalIds).order('created_at');
            activities = data || [];
        }

        res.json({ plan, goals: goals || [], activities });
    } catch (err) {
        console.error('GET /api/commander/learning-centre/plans/:id', err);
...
router.put('/plans/:id', async (req, res) => {
    try {
        ...
        const { data, error } = await supabase.from('commander_learning_plans').update(patch).eq('id', req.params.id).eq('company_id', cid).select().single();
        if (error) throw error;
        ...
        console.error('PUT /api/commander/learning-centre/plans/:id', err);
...
router.delete('/plans/:id', async (req, res) => {
    try {
        ...
        const manager = await _requireManager(req, res);
        if (!manager) return;
        const { data, error } = await supabase.from('commander_learning_plans').update({ status: 'cancelled' }).eq('id', req.params.id).eq('company_id', cid).select().single();
        if (error) throw error;
        if (!data) return res.status(404).json({ error: 'Learning plan not found.' });
        await _writeEvent(cid, 'plan_archived', 'plan', data.id, req.user?.userId);
        res.json({ plan: data, archived: true });
    } catch (err) {
        console.error('DELETE /api/commander/learning-centre/plans/:id', err);
...
router.get('/goals', async (req, res) => {
    try {
        ...
        const { data, error } = await supabase.from('commander_learning_goals').select('*, commander_skills:skill_id(skill_key, display_name)').eq('company_id', cid).eq('learning_plan_id', plan.id).order('created_at');
        if (error) throw error;
        res.json({ goals: data || [] });
    } catch (err) {
        console.error('GET /api/commander/learning-centre/goals', err);
...
router.post('/goals', async (req, res) => {
    try {
        ...
        const { data, error } = await supabase.from('commander_learning_goals').insert({
        ...
        console.error('POST /api/commander/learning-centre/goals', err);
...
router.put('/goals/:id', async (req, res) => {
    try {
        ...
        const { data, error } = await supabase.from('commander_learning_goals').update(patch).eq('id', goal.id).eq('company_id', cid).select().single();
        if (error) throw error;
        ...
        console.error('PUT /api/commander/learning-centre/goals/:id', err);
...
router.delete('/goals/:id', async (req, res) => {
    try {
        ...
        const { data, error } = await supabase.from('commander_learning_goals').update({ status: 'cancelled' }).eq('id', goal.id).eq('company_id', cid).select().single();
        if (error) throw error;
        await _writeEvent(cid, 'goal_archived', 'goal', data.id, req.user?.userId);
        await _recalc(cid, goal.learning_plan_id);
        res.json({ goal: data, archived: true });
    } catch (err) {
        console.error('DELETE /api/commander/learning-centre/goals/:id', err);
...
router.get('/activities', async (req, res) => {
    try {
        ...
        const { data, error } = await supabase.from('commander_learning_activities').select('*').eq('company_id', cid).eq('goal_id', goal.id).order('created_at');
        if (error) throw error;
        res.json({ activities: data || [] });
    } catch (err) {
        console.error('GET /api/commander/learning-centre/activities', err);
...
router.post('/activities', async (req, res) => {
    try {
        ...
        const { data, error } = await supabase.from('commander_learning_activities').insert({
        ...
        console.error('POST /api/commander/learning-centre/activities', err);
...
router.put('/activities/:id', async (req, res) => {
    try {
        ...
        const { data, error } = await supabase.from('commander_learning_activities').update(patch).eq('id', activity.id).eq('company_id', cid).select().single();
        if (error) throw error;
        ...
        console.error('PUT /api/commander/learning-centre/activities/:id', err);
...
router.delete('/activities/:id', async (req, res) => {
    try {
        ...
        const { data, error } = await supabase.from('commander_learning_activities').update({ status: 'cancelled' }).eq('id', activity.id).eq('company_id', cid).select().single();
        if (error) throw error;
        ...
        console.error('DELETE /api/commander/learning-centre/activities/:id', err);
...
router.get('/progress/:plan_id', async (req, res) => {
    try {
        ...
        console.error('GET /api/commander/learning-centre/progress/:plan_id', err);
...
router.post('/progress/:plan_id/snapshot', async (req, res) => {
    try {
        ...
        const progress = await calculateLearningProgress(cid, plan.id);
        const { data, error } = await supabase.from('commander_learning_progress').insert({
        ...
        console.error('POST /api/commander/learning-centre/progress/:plan_id/snapshot', err);
...
router.get('/progress/:plan_id/history', async (req, res) => {
    try {
        ...
        const { data, error } = await supabase.from('commander_learning_progress').select('*').eq('company_id', cid).eq('learning_plan_id', plan.id).order('snapshot_date', { ascending: false });
        if (error) throw error;
        res.json({ snapshots: data || [] });
    } catch (err) {
        console.error('GET /api/commander/learning-centre/progress/:plan_id/history', err);
...
router.get('/cpd', async (req, res) => {
    try {
        const cid = req.companyId;
        const me = await _myTeamMember(cid, req.user);
        let q = supabase.from('commander_cpd_records').select('*').eq('company_id', cid).eq('is_active', true).order('issue_date', { ascending: false });
        ...
        console.error('GET /api/commander/learning-centre/cpd', err);
...
router.post('/cpd', async (req, res) => {
    try {
        ...
        const { data, error } = await supabase.from('commander_cpd_records').insert({
        ...
        console.error('POST /api/commander/learning-centre/cpd', err);
...
router.put('/cpd/:id', async (req, res) => {
    try {
        ...
        const { data, error } = await supabase.from('commander_cpd_records').update(patch).eq('id', req.params.id).eq('company_id', cid).select().single();
        if (error) throw error;
        ...
        console.error('PUT /api/commander/learning-centre/cpd/:id', err);
...
router.delete('/cpd/:id', async (req, res) => {
    try {
        ...
        const manager = await _requireManager(req, res);
        if (!manager) return;
        const { data, error } = await supabase.from('commander_cpd_records').update({ is_active: false }).eq('id', req.params.id).eq('company_id', cid).select().single();
        if (error) throw error;
        if (!data) return res.status(404).json({ error: 'CPD record not found.' });
        await _writeEvent(cid, 'cpd_archived', 'cpd_record', data.id, req.user?.userId);
        res.json({ cpd_record: data, archived: true });
    } catch (err) {
        console.error('DELETE /api/commander/learning-centre/cpd/:id', err);
        res.status(500).json({ error: 'Failed to archive CPD record.' });
    }
});

// ── Skills Matrix integration — suggested development goals ─────────────────
// Direct read of commander_team_skills (owned by Codebox 59) — no scoring
...
router.get('/suggested-goals/:team_member_id', async (req, res) => {
    try {
        ...
        const { data, error } = await supabase.from('commander_team_skills').select('*, commander_skills:skill_id(skill_key, display_name)').eq('company_id', cid).eq('team_member_id', requestedId);
        if (error) throw error;

        const suggestions = (data || [])
            .filter(r => r.target_level != null && r.target_level > r.current_level)
            .map(r => ({
                skill_id: r.skill_id, skill_name: r.commander_skills?.display_name || null,
        ...
        console.error('GET /api/commander/learning-centre/suggested-goals/:team_member_id', err);
...
router.get('/events', async (req, res) => {
    try {
        const cid = req.companyId;
        let q = supabase.from('commander_learning_events').select('*').eq('company_id', cid).order('created_at', { ascending: false }).limit(200);
        ...
        console.error('GET /api/commander/learning-centre/events', err);
