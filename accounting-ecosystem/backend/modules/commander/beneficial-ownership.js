...
//
// This module does NOT duplicate commander_company_shareholders (Codebox 62)
...
async function _writeEvent(cid, clientId, sourceType, sourceId, eventType, oldStatus, newStatus, actorUserId, notes, meta) {
    await supabase.from('commander_beneficial_ownership_events').insert({
...
async function _generateReadinessItems(cid, clientId, actorUserId) {
    const [ownersRes, chainsRes, existingRes] = await Promise.all([
        supabase.from('commander_beneficial_owners').select('*').eq('company_id', cid).eq('client_id', clientId).neq('status', 'archived'),
        supabase.from('commander_ownership_chains').select('*').eq('company_id', cid).eq('client_id', clientId).neq('chain_status', 'archived'),
        supabase.from('commander_bo_readiness_items').select('beneficial_owner_id, ownership_chain_id, item_type').eq('company_id', cid).eq('client_id', clientId),
    ]);
    ...
    const { data, error } = await supabase.from('commander_bo_readiness_items').insert(toInsert).select();
...
// Pure aggregation. Reuses commander_company_shareholders live — never copies
// or duplicates it.
...
async function getBeneficialOwnershipProfile(cid, clientId) {
    ...
    const [shareholdersRes, ownersRes, chainsRes, readinessRes] = await Promise.all([
        supabase.from('commander_company_shareholders').select('*').eq('company_id', cid).eq('client_id', clientId).eq('status', 'active'),
        supabase.from('commander_beneficial_owners').select('*').eq('company_id', cid).eq('client_id', clientId).neq('status', 'archived'),
        supabase.from('commander_ownership_chains').select('*').eq('company_id', cid).eq('client_id', clientId).neq('chain_status', 'archived'),
        supabase.from('commander_bo_readiness_items').select('*').eq('company_id', cid).eq('client_id', clientId),
    ]);
...
router.get('/summary', async (req, res) => {
    ...
    try {
        const [owners, chains, readinessItems] = await Promise.all([
            supabase.from('commander_beneficial_owners').select('status, is_reportable, is_natural_person, client_id').eq('company_id', cid),
            supabase.from('commander_ownership_chains').select('chain_status, confidence, client_id').eq('company_id', cid),
            supabase.from('commander_bo_readiness_items').select('client_id, status, required').eq('company_id', cid),
        ]);
...
router.get('/owners', async (req, res) => {
    ...
    try {
        let q = supabase.from('commander_beneficial_owners').select('*', { count: 'exact' }).eq('company_id', cid).order('created_at', { ascending: false });
...
router.post('/owners', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_beneficial_owners').insert({
...
router.get('/owners/:id', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_beneficial_owners').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
        if (error) return res.status(500).json({ error: error.message });
...
router.put('/owners/:id', async (req, res) => {
    ...
    const existing = await supabase.from('commander_beneficial_owners').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
    ...
    try {
        const { data, error } = await supabase.from('commander_beneficial_owners').update(update).eq('id', id).eq('company_id', cid).select().single();
        if (error) return res.status(500).json({ error: error.message });
...
router.delete('/owners/:id', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_beneficial_owners')
...
router.get('/chains', async (req, res) => {
    ...
    try {
        let q = supabase.from('commander_ownership_chains').select('*', { count: 'exact' }).eq('company_id', cid).order('created_at', { ascending: false });
        if (client_id) q = q.eq('client_id', parseInt(client_id));
...
router.post('/chains', async (req, res) => {
    ...
    if (root_holder_reference_id) {
        const { data: sh } = await supabase.from('commander_company_shareholders').select('id').eq('id', root_holder_reference_id).eq('company_id', cid).eq('client_id', clientId).maybeSingle();
    ...
    if (ultimate_owner_id) {
        const { data: bo } = await supabase.from('commander_beneficial_owners').select('id').eq('id', ultimate_owner_id).eq('company_id', cid).eq('client_id', clientId).maybeSingle();
    ...
    try {
        const { data, error } = await supabase.from('commander_ownership_chains').insert({
...
router.get('/chains/:id', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_ownership_chains').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
        if (error) return res.status(500).json({ error: error.message });
...
router.put('/chains/:id', async (req, res) => {
    ...
    const existing = await supabase.from('commander_ownership_chains').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
    ...
    if (update.ultimate_owner_id) {
        const { data: bo } = await supabase.from('commander_beneficial_owners').select('id').eq('id', update.ultimate_owner_id).eq('company_id', cid).eq('client_id', c.client_id).maybeSingle();
        if (!bo) return res.status(400).json({ error: 'ultimate_owner_id does not match an existing beneficial owner for this client.' });
    }
    ...
    try {
        const { data, error } = await supabase.from('commander_ownership_chains').update(update).eq('id', id).eq('company_id', cid).select().single();
        if (error) return res.status(500).json({ error: error.message });
...
router.delete('/chains/:id', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_ownership_chains')
...
router.get('/client/:clientId/readiness', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_bo_readiness_items').select('*').eq('company_id', cid).eq('client_id', clientId).order('item_type');
        if (error) return res.status(500).json({ error: error.message });
...
router.put('/readiness/:itemId', async (req, res) => {
    ...
    const existing = await supabase.from('commander_bo_readiness_items').select('*').eq('id', itemId).eq('company_id', cid).maybeSingle();
    ...
    try {
        const { data, error } = await supabase.from('commander_bo_readiness_items').update(update).eq('id', itemId).eq('company_id', cid).select().single();
        if (error) return res.status(500).json({ error: error.message });
...
router.post('/client/:clientId/recalculate-readiness', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_bo_readiness_items').select('*').eq('company_id', cid).eq('client_id', clientId);
        if (error) return res.status(500).json({ error: error.message });
...
router.get('/events', async (req, res) => {
    ...
    try {
        let q = supabase.from('commander_beneficial_ownership_events').select('*').eq('company_id', cid).order('created_at', { ascending: false }).limit(Math.min(500, parseInt(limit) || 100));
        if (client_id) q = q.eq('client_id', parseInt(client_id));
...
router.get('/:sourceType/:sourceId/events', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_beneficial_ownership_events').select('*')
