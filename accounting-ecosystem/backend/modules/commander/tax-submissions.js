...
async function _verifyEvidence(submissionId, evidenceId, cid) {
    const { data } = await supabase
        .from('commander_tax_submission_evidence')
        .select('*')
        .eq('id', evidenceId)
...
async function _extractClientId(sourceType, sourceId, cid) {
    const table = sourceType === 'individual_tax_return' ? 'commander_individual_tax_returns'
                : sourceType === 'company_tax_return'    ? 'commander_company_tax_returns'
                : 'commander_provisional_tax_plans';
    const { data } = await supabase
        .from(table)
...
async function _writeEvent(cid, submissionId, eventType, oldStatus, newStatus, userId, notes, metadata) {
    await supabase.from('commander_tax_submission_events').insert({
        company_id:    cid,
...
router.get('/:id/events', async (req, res) => {
    ...
    try {
        ...
        const { data, error } = await supabase
            .from('commander_tax_submission_events')
