/* =============================================================
   ...
   Mounted ADDITIONALLY at /api/commander/individual-tax (alongside individual-tax.js).
...
async function verifyReturnOwnership(cid, returnId) {
    const { data } = await supabase
        .from('commander_individual_tax_returns')
        .select('*')
        .eq('id', returnId)
        .eq('company_id', cid)
        .single();
...
async function verifyCalcOwnership(cid, calcId) {
    const { data } = await supabase
        .from('commander_individual_tax_calculations')
        .select('*')
        .eq('id', calcId)
        .eq('company_id', cid)
        .single();
...
async function logCalcEvent(cid, calcId, returnId, eventType, extras = {}) {
    await supabase.from('commander_individual_tax_calculation_events').insert({
        company_id:     cid,
...
async function runDraftCalculation(cid, taxReturn) {
    ...
    const { data: incomeEntries } = await supabase
        .from('commander_individual_tax_income_entries')
    ...
    const { data: deductionEntries } = await supabase
        .from('commander_individual_tax_deduction_entries')
...
router.get('/calculations/:id/events', async (req, res) => {
    ...
    const { data, error } = await supabase
        .from('commander_individual_tax_calculation_events')
        .select('*')
...
router.post('/calculations/:id/submit-review', async (req, res) => {
    ...
    try {
        const { data: calc, error } = await supabase
            .from('commander_individual_tax_calculations')
            .update({ calculation_status: 'ready_for_review', updated_at: new Date().toISOString(), updated_by: req.user?.userId || null })
            .eq('id', calcId)
            .eq('company_id', cid)
            .select()
            .single();
        ...
        await auditFromReq(req, 'individual_tax_calculation_submitted_review', 'commander_individual_tax_calculations', calcId);
...
router.post('/calculations/:id/approve', async (req, res) => {
    ...
    try {
        ...
        await auditFromReq(req, 'individual_tax_calculation_approved', 'commander_individual_tax_calculations', calcId);
...
router.post('/calculations/:id/reject', async (req, res) => {
    ...
    try {
        ...
        await auditFromReq(req, 'individual_tax_calculation_rejected', 'commander_individual_tax_calculations', calcId, { metadata: { reason } });
...
router.post('/:returnId/calculations/run-draft', async (req, res) => {
    ...
    try {
        ...
        await auditFromReq(req, 'individual_tax_calculation_run', 'commander_individual_tax_calculations', calc.id, {
