...
async function _writeEvent(cid, refs, eventType, oldStatus, newStatus, actorUserId, notes, meta) {
    await supabase.from('commander_strategic_events').insert({
        company_id: cid,
...
async function _fetchPlan(cid, id) {
    const { data } = await supabase.from('commander_strategic_plans').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
    return data || null;
}
async function _fetchObjective(cid, id) {
    const { data } = await supabase.from('commander_strategic_objectives').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
    return data || null;
}
async function _fetchInitiative(cid, id) {
    const { data } = await supabase.from('commander_strategic_initiatives').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
    return data || null;
}
async function _fetchKpiLink(cid, id) {
    const { data } = await supabase.from('commander_strategic_kpi_links').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
    return data || null;
}
async function _fetchReview(cid, id) {
    const { data } = await supabase.from('commander_strategic_reviews').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
    return data || null;
}
...
async function _sharedSourceData(cid, sourcesNeeded) {
    ...
    if (sourcesNeeded.has('kpi_history')) {
        jobs.push(supabase.from('commander_kpi_snapshots').select('*').eq('company_id', cid).eq('status', 'active')
    ...
    if (sourcesNeeded.has('partner_scorecard')) {
        jobs.push(supabase.from('commander_partner_scorecards').select('*').eq('company_id', cid).eq('scorecard_type', 'commander')
            .order('created_at', { ascending: false }).limit(1).maybeSingle()
            .then(r => { data.partner_scorecard = r.data || null; }));
    }
    if (sourcesNeeded.has('profitability')) {
        jobs.push(supabase.from('commander_profitability_snapshots').select('profitability_status, warnings').eq('company_id', cid)
            .order('created_at', { ascending: false }).limit(500)
            .then(r => { data.profitability = r.data || []; }));
    }
    if (sourcesNeeded.has('client_success')) {
        jobs.push(supabase.from('commander_client_success').select('relationship_status').eq('company_id', cid)
            .then(r => { data.client_success = r.data || []; }));
    }
    if (sourcesNeeded.has('risk')) {
        jobs.push(Promise.all([
            getRules(cid, ['risk_high_min', 'risk_critical_min']),
            supabase.from('commander_risks').select('inherent_risk').eq('company_id', cid).not('status', 'in', '("closed","cancelled")'),
        ]).then(([rules, rows]) => { data.risk = { rules, rows: rows.data || [] }; }));
    }
    if (sourcesNeeded.has('qms')) {
        jobs.push(Promise.all([
            supabase.from('commander_quality_reviews').select('status').eq('company_id', cid),
            supabase.from('commander_quality_findings').select('severity').eq('company_id', cid).in('status', ['open', 'in_progress']),
    ...
    if (sourcesNeeded.has('tax')) {
        jobs.push(Promise.all([
            supabase.from('commander_individual_tax_returns').select('id', { count: 'exact', head: true }).eq('company_id', cid).not('status', 'in', '("completed","cancelled")'),
            supabase.from('commander_company_tax_returns').select('id', { count: 'exact', head: true }).eq('company_id', cid).not('status', 'in', '("completed","cancelled")'),
            supabase.from('commander_tax_payments').select('id', { count: 'exact', head: true }).eq('company_id', cid).eq('direction', 'payable').in('status', ['outstanding', 'partially_paid']),
    ...
    if (sourcesNeeded.has('secretarial')) {
        jobs.push(Promise.all([
            ...
            supabase.from('commander_secretarial_integrity_runs').select('critical_count, high_count').eq('company_id', cid).order('scan_started_at', { ascending: false }).limit(1).maybeSingle(),
...
async function _refreshKpiLinks(cid, links) {
    ...
    for (const link of links) {
        ...
        if (resolved && resolved.value != null) {
            const { data } = await supabase.from('commander_strategic_kpi_links')
...
async function getStrategicPlanHealth(cid, planId) {
    ...
    const { data: objectiveRows } = await supabase.from('commander_strategic_objectives').select('*').eq('company_id', cid).eq('plan_id', planId);
    const objectives = objectiveRows || [];
    const objectiveIds = objectives.map(o => o.id);

    const [initiativeRows, kpiLinkRows] = await Promise.all([
        objectiveIds.length ? supabase.from('commander_strategic_initiatives').select('*').eq('company_id', cid).in('objective_id', objectiveIds) : Promise.resolve({ data: [] }),
        objectiveIds.length ? supabase.from('commander_strategic_kpi_links').select('*').eq('company_id', cid).in('objective_id', objectiveIds) : Promise.resolve({ data: [] }),
    ]);
...
router.get('/summary', async (req, res) => {
    ...
    try {
        ...
        const { data: objRows } = activePlanIds.length
            ? await supabase.from('commander_strategic_objectives').select('objective_status').eq('company_id', cid).in('plan_id', activePlanIds)
            : { data: [] };
        const objectives = objRows || [];
        const atRiskObjectives = objectives.filter(o => ['at_risk', 'off_track'].includes(o.objective_status)).length;

        const { data: reviewRows } = await supabase.from('commander_strategic_reviews').select('review_status, next_review_date').eq('company_id', cid);
...
router.get('/plans', async (req, res) => {
    ...
    try {
        let q = supabase.from('commander_strategic_plans').select('*').eq('company_id', cid).order('plan_year', { ascending: false });
...
router.post('/plans', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_strategic_plans').insert({
...
router.put('/plans/:id', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_strategic_plans').update(update).eq('id', id).eq('company_id', cid).select().single();
        if (error) return res.status(500).json({ error: error.message });
...
router.delete('/plans/:id', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_strategic_plans')
...
// module needing to know about commander_strategic_plans at all.
router.get('/objectives', async (req, res) => {
    ...
    try {
        let q = supabase.from('commander_strategic_objectives').select('id, plan_id, objective_title, objective_area, objective_status, priority, progress_percentage')
...
router.get('/plans/:id/objectives', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_strategic_objectives').select('*').eq('company_id', cid).eq('plan_id', planId).order('created_at');
        if (error) return res.status(500).json({ error: error.message });
        res.json({ objectives: data || [] });
    } catch (err) {
        console.error('Strategic-planning GET objectives error:', err.message);
...
router.post('/plans/:id/objectives', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_strategic_objectives').insert({
...
router.get('/objectives/:objectiveId', async (req, res) => {
    ...
    try {
        const [{ data: initiatives }, { data: kpiLinks }] = await Promise.all([
            supabase.from('commander_strategic_initiatives').select('*').eq('company_id', cid).eq('objective_id', objective.id),
            supabase.from('commander_strategic_kpi_links').select('*').eq('company_id', cid).eq('objective_id', objective.id),
        ]);
...
router.put('/objectives/:objectiveId', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_strategic_objectives').update(update).eq('id', id).eq('company_id', cid).select().single();
        if (error) return res.status(500).json({ error: error.message });
...
router.delete('/objectives/:objectiveId', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_strategic_objectives')
...
router.get('/objectives/:objectiveId/initiatives', async (req, res) => {
    const cid = req.companyId;
    try {
        const { data, error } = await supabase.from('commander_strategic_initiatives').select('*').eq('company_id', cid).eq('objective_id', req.params.objectiveId).order('created_at');
        if (error) return res.status(500).json({ error: error.message });
...
router.post('/objectives/:objectiveId/initiatives', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_strategic_initiatives').insert({
...
router.put('/initiatives/:initiativeId', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_strategic_initiatives').update(update).eq('id', id).eq('company_id', cid).select().single();
        if (error) return res.status(500).json({ error: error.message });
...
router.delete('/initiatives/:initiativeId', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_strategic_initiatives')
...
router.get('/objectives/:objectiveId/kpis', async (req, res) => {
    const cid = req.companyId;
    try {
        const { data, error } = await supabase.from('commander_strategic_kpi_links').select('*').eq('company_id', cid).eq('objective_id', req.params.objectiveId).order('created_at');
        if (error) return res.status(500).json({ error: error.message });
...
router.post('/objectives/:objectiveId/kpis', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_strategic_kpi_links').insert({
...
router.put('/kpis/:kpiId', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_strategic_kpi_links').update(update).eq('id', id).eq('company_id', cid).select().single();
        if (error) return res.status(500).json({ error: error.message });
...
router.delete('/kpis/:kpiId', async (req, res) => {
    ...
    try {
        const { error } = await supabase.from('commander_strategic_kpi_links').delete().eq('id', id).eq('company_id', cid);
        if (error) return res.status(500).json({ error: error.message });
...
router.get('/plans/:id/reviews', async (req, res) => {
    const cid = req.companyId;
    try {
        const { data, error } = await supabase.from('commander_strategic_reviews').select('*').eq('company_id', cid).eq('plan_id', req.params.id).order('created_at', { ascending: false });
        if (error) return res.status(500).json({ error: error.message });
...
router.post('/plans/:id/reviews', async (req, res) => {
    ...
    try {
        ...
        const { data, error } = await supabase.from('commander_strategic_reviews').insert({
        ...
        if (plan.plan_status === 'active') {
            await supabase.from('commander_strategic_plans').update({ plan_status: 'under_review', updated_at: new Date().toISOString() }).eq('id', planId).eq('company_id', cid);
        }
...
router.put('/reviews/:reviewId', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_strategic_reviews').update(update).eq('id', id).eq('company_id', cid).select().single();
        if (error) return res.status(500).json({ error: error.message });
...
router.put('/reviews/:reviewId/complete', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_strategic_reviews')
        ...
        if (plan && plan.plan_status === 'under_review') {
            await supabase.from('commander_strategic_plans').update({ plan_status: 'active', updated_at: new Date().toISOString() }).eq('id', review.plan_id).eq('company_id', cid);
        }
...
router.get('/events', async (req, res) => {
    const cid = req.companyId;
    try {
        const { data, error } = await supabase.from('commander_strategic_events').select('*').eq('company_id', cid).order('created_at', { ascending: false }).limit(500);
        if (error) return res.status(500).json({ error: error.message });
...
router.get('/:sourceType/:sourceId/events', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_strategic_events').select('*').eq('company_id', cid).eq(column, req.params.sourceId).order('created_at', { ascending: false });
        if (error) return res.status(500).json({ error: error.message });
        res.json({ events: data || [] });
    } catch (err) {
