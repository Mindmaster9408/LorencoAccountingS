...
router.get('/summary', async (req, res) => {
  ...
  try {
    const [
      ...
      // Active clients
      supabase.from('commander_clients')
        .select('id', { count: 'exact', head: true })
        .eq('company_id', cid).eq('is_active', true),

      // Active engagements
      supabase.from('commander_client_engagements')
        .select('id', { count: 'exact', head: true })
      ...
      supabase.from('commander_workflow_runs')
        .select('id', { count: 'exact', head: true })
        .eq('company_id', cid)
        .in('status', ['pending', 'in_progress']),

      // Billing packs awaiting review or approval
      supabase.from('commander_billing_packs')
        .select('id', { count: 'exact', head: true })
        .eq('company_id', cid)
        .in('status', ['draft', 'reviewed']),

      // Queued or ready service periods (no workflow generated yet)
      supabase.from('commander_engagement_periods')
        .select('id', { count: 'exact', head: true })
        .eq('company_id', cid)
...
router.get('/workload', async (req, res) => {
  ...
  try {
    const [members, tasks, deadlines, engagements] = await Promise.all([
      ...
      supabase.from('commander_team_members')
...
router.get('/risk', async (req, res) => {
  ...
  try {
    const [overdueDeadlines, dueThisWeek, reviewBacklog, approvalBacklog, billingPending, periodsPending] = await Promise.all([
      ...
      supabase.from('commander_deadlines')
        .select('id, title, due_date, status, priority, compliance_area, deadline_type, responsible_team_member_id, commander_clients:client_id(name), commander_team_members!responsible_team_member_id(display_name)')
        .eq('company_id', cid)
        .not('status', 'in', '(completed,submitted,missed,cancelled)')
        .lt('due_date', t)
      ...
      supabase.from('commander_tasks')
        .select('id, title, status, review_status, due_date, preparer_team_member_id, reviewer_team_member_id, commander_clients:client_id(name), commander_team_members!reviewer_team_member_id(display_name)')
        .eq('company_id', cid)
        .in('review_status', ['pending', 'in_review'])
        .not('status', 'in', '(completed,cancelled)')
      ...
      supabase.from('commander_tasks')
        .select('id, title, status, approval_status, due_date, approver_team_member_id, commander_clients:client_id(name), commander_team_members!approver_team_member_id(display_name)')
        .eq('company_id', cid)
        .eq('approval_status', 'pending')
        .not('status', 'in', '(completed,cancelled)')
        .order('created_at', { ascending: true })
      ...
      supabase.from('commander_billing_packs')
        .select('id, pack_number, pack_name, status, proposed_invoice_value, created_at, commander_clients:client_id(name)')
        .eq('company_id', cid)
        .in('status', ['draft', 'reviewed'])
        .order('created_at', { ascending: true })
        .limit(10),
      ...
      supabase.from('commander_engagement_periods')
        .select('id, period_label, status, period_end, due_date, commander_client_engagements:engagement_id(engagement_name), commander_clients:client_id(name)')
        .eq('company_id', cid)
        .in('status', ['queued', 'ready'])
    ...
    res.json({
      overdue_deadlines: fmt(overdueDeadlines, r => ({
        ...
        client_name:    r.commander_clients?.name || null,
        responsible:    r.commander_team_members?.display_name || null,
      ...
      review_backlog: fmt(reviewBacklog, r => ({
        ...
        due_date:     r.due_date,
        client_name:  r.commander_clients?.name || null,
        reviewer:     r.commander_team_members?.display_name || null,
      })),
      ...
      approval_backlog: fmt(approvalBacklog, r => ({
        ...
        due_date:        r.due_date,
        client_name:     r.commander_clients?.name || null,
        approver:        r.commander_team_members?.display_name || null,
      })),
      ...
      periods_pending: fmt(periodsPending, r => ({
        ...
        due_date:        r.due_date,
        engagement_name: r.commander_client_engagements?.engagement_name || null,
        client_name:     r.commander_clients?.name || null,
      })),
    });
  } catch (err) {
...
//
// Note: commander_client_engagement_events.engagement_id has no FK constraint,
...
router.get('/activity', async (req, res) => {
  ...
  try {
    const [engEvents, deadlineEvents, billingEvents] = await Promise.all([
      ...
      supabase.from('commander_client_engagement_events')
      ...
      supabase.from('commander_deadline_events')
        .select('id, event_type, old_status, new_status, created_at, actor_user_id, commander_deadlines:deadline_id(title, commander_clients:client_id(name))')
        .eq('company_id', cid)
        .order('created_at', { ascending: false })
        .limit(15),

      // Billing pack events — FK exists so embedded join works
      supabase.from('commander_billing_pack_events')
        .select('id, event_type, old_status, new_status, created_at, actor_user_id, commander_billing_packs:billing_pack_id(pack_number, pack_name, commander_clients:client_id(name))')
        .eq('company_id', cid)
        .order('created_at', { ascending: false })
        .limit(15),
    ]);
    ...
    if (engIds.length) {
      const { data: engs } = await supabase
        .from('commander_client_engagements')
        .select('id, engagement_name, commander_clients:client_id(name)')
        .eq('company_id', cid)
        .in('id', engIds);
      for (const eng of (engs || [])) {
        engMap[eng.id] = { name: eng.engagement_name, client: eng.commander_clients?.name || null };
      }
    }
    ...
    for (const e of (deadlineEvents.data || [])) {
      const dl = e.commander_deadlines;
      events.push({
        source:      'deadline',
        event_type:  e.event_type,
        ...
        created_by:  e.actor_user_id ? String(e.actor_user_id) : null,
        label:       dl?.title || null,
        client_name: dl?.commander_clients?.name || null,
      });
    }

    for (const e of (billingEvents.data || [])) {
      const pk = e.commander_billing_packs;
      events.push({
        source:      'billing',
        event_type:  e.event_type,
        old_status:  e.old_status,
        ...
        client_name: pk?.commander_clients?.name || null,
      });
    }
