...
// commander_secretarial_profiles.company_status (Codebox 62/63) — never
...
async function _writeEvent(cid, clientId, sourceType, sourceId, eventType, oldStatus, newStatus, actorUserId, notes, meta) {
    await supabase.from('commander_entity_lifecycle_events').insert({
...
async function _getOrInitProfile(cid, clientId, actorUserId) {
    const { data: existing } = await supabase.from('commander_entity_lifecycle_profiles').select('*').eq('company_id', cid).eq('client_id', clientId).maybeSingle();
...
async function getEntityLifecycleProfile(cid, clientId) {
    ...
    const [transitionsRes, checklistRes] = await Promise.all([
        supabase.from('commander_entity_lifecycle_transitions').select('*').eq('company_id', cid).eq('client_id', clientId).order('created_at', { ascending: false }),
        supabase.from('commander_entity_lifecycle_checklist_items').select('*').eq('company_id', cid),
    ]);
    ...
    const evidenceReadinessSummary = await _safe(async () => {
        const { data: checklists } = await supabase.from('commander_secretarial_evidence_checklists').select('*').eq('company_id', cid).eq('client_id', clientId);
...
router.get('/summary', async (req, res) => {
    ...
    try {
        const [profilesRes, transitionsRes] = await Promise.all([
            supabase.from('commander_entity_lifecycle_profiles').select('current_lifecycle_status, risk_status').eq('company_id', cid),
            supabase.from('commander_entity_lifecycle_transitions').select('transition_status').eq('company_id', cid),
        ]);
...
router.post('/client/:clientId/profile', async (req, res) => {
    ...
    try {
        ...
        if (entity_category && entity_category !== profile.entity_category) {
            const { data, error } = await supabase.from('commander_entity_lifecycle_profiles').update({ entity_category, updated_by: req.user.userId, updated_at: new Date().toISOString() }).eq('id', profile.id).select().single();
...
router.put('/client/:clientId/profile', async (req, res) => {
    ...
    try {
        ...
        const { data, error } = await supabase.from('commander_entity_lifecycle_profiles').update(update).eq('id', profile.id).eq('company_id', cid).select().single();
        if (error) return res.status(500).json({ error: error.message });
...
router.get('/transitions', async (req, res) => {
    ...
    try {
        let q = supabase.from('commander_entity_lifecycle_transitions').select('*').eq('company_id', cid).order('created_at', { ascending: false });
...
router.post('/transitions', async (req, res) => {
    ...
    try {
        ...
        const { data, error } = await supabase.from('commander_entity_lifecycle_transitions').insert({
...
router.get('/transitions/:id', async (req, res) => {
    ...
    try {
        const [transitionRes, checklistRes] = await Promise.all([
            supabase.from('commander_entity_lifecycle_transitions').select('*').eq('id', id).eq('company_id', cid).maybeSingle(),
            supabase.from('commander_entity_lifecycle_checklist_items').select('*').eq('company_id', cid).eq('transition_id', id).order('sort_order'),
        ]);
...
router.put('/transitions/:id', async (req, res) => {
    ...
    const existing = await supabase.from('commander_entity_lifecycle_transitions').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
    ...
    try {
        const { data, error } = await supabase.from('commander_entity_lifecycle_transitions').update(update).eq('id', id).eq('company_id', cid).select().single();
        if (error) return res.status(500).json({ error: error.message });
...
router.delete('/transitions/:id', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_entity_lifecycle_transitions')
...
router.post('/transitions/:id/generate-checklist', async (req, res) => {
    ...
    const { data: transition } = await supabase.from('commander_entity_lifecycle_transitions').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
    if (!transition) return res.status(404).json({ error: 'Transition not found' });

    try {
        const { count } = await supabase.from('commander_entity_lifecycle_checklist_items').select('id', { count: 'exact', head: true }).eq('company_id', cid).eq('transition_id', id);
        ...
        if (count > 0 && req.query.force === 'true') {
            await supabase.from('commander_entity_lifecycle_checklist_items').delete().eq('company_id', cid).eq('transition_id', id);
        }
        ...
        const { data, error } = await supabase.from('commander_entity_lifecycle_checklist_items').insert(rows).select();
        if (error) return res.status(500).json({ error: error.message });
...
router.get('/transitions/:id/checklist', async (req, res) => {
    ...
    if (!id) return res.status(400).json({ error: 'Invalid transition ID' });
    try {
        const { data, error } = await supabase.from('commander_entity_lifecycle_checklist_items').select('*').eq('company_id', cid).eq('transition_id', id).order('sort_order');
        if (error) return res.status(500).json({ error: error.message });
...
router.put('/transitions/:id/checklist/:itemId', async (req, res) => {
    ...
    const existing = await supabase.from('commander_entity_lifecycle_checklist_items').select('*').eq('id', itemId).eq('transition_id', id).eq('company_id', cid).maybeSingle();
    ...
    try {
        const { data, error } = await supabase.from('commander_entity_lifecycle_checklist_items').update(update).eq('id', itemId).eq('company_id', cid).select().single();
        if (error) return res.status(500).json({ error: error.message });
...
router.put('/transitions/:id/approve', async (req, res) => {
    ...
    try {
        const { data: checklist } = await supabase.from('commander_entity_lifecycle_checklist_items').select('required, completed').eq('company_id', cid).eq('transition_id', id);
...
router.put('/transitions/:id/implement', async (req, res) => {
    ...
    try {
        const { data: profile } = await supabase.from('commander_entity_lifecycle_profiles').select('*').eq('id', transition.lifecycle_profile_id).eq('company_id', cid).maybeSingle();
        ...
        const { data: updatedProfile, error: profileError } = await supabase.from('commander_entity_lifecycle_profiles')
...
router.get('/events', async (req, res) => {
    ...
    try {
        let q = supabase.from('commander_entity_lifecycle_events').select('*').eq('company_id', cid).order('created_at', { ascending: false }).limit(Math.min(500, parseInt(limit) || 100));
        if (client_id) q = q.eq('client_id', parseInt(client_id));
        const { data, error } = await q;
        if (error) return res.status(500).json({ error: error.message });
...
router.get('/:sourceType/:sourceId/events', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_entity_lifecycle_events').select('*')
