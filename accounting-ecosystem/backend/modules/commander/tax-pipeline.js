...
function _cfg(sourceType) {
    const configs = {
        individual_tax_return: {
            table:     'commander_individual_tax_returns',
            nameField: 'return_name',
            calcTable: 'commander_individual_tax_calculations',
            calcFK:    'tax_return_id',
            packTable: 'commander_individual_tax_review_packs',
            packFK:    'tax_return_id',
            hasCalc:   true,
            hasPack:   true,
        },
        company_tax_return: {
            table:     'commander_company_tax_returns',
            nameField: 'return_name',
            calcTable: 'commander_company_tax_calculations',
            calcFK:    'company_tax_return_id',
            packTable: 'commander_company_tax_review_packs',
            packFK:    'company_tax_return_id',
            hasCalc:   true,
            hasPack:   true,
        },
        provisional_tax_plan: {
            table:     'commander_provisional_tax_plans',
            nameField: 'plan_name',
            calcTable: null,
            calcFK:    null,
            packTable: null,
...
async function _runAutoChecks(sourceType, sourceId, cid, newStage) {
    ...
    if (newStage === 'submitted') {
        ...
        if (cfg2.table !== 'commander_provisional_tax_plans') {
...
async function _fetchRecord(sourceType, sourceId, cid) {
    const cfg = _cfg(sourceType);
    // commander_provisional_tax_plans has no readiness_status column (unlike
    ...
    const cols = cfg.table === 'commander_provisional_tax_plans' ? baseCols : baseCols + ', readiness_status';
...
async function _fetchHistory(sourceType, sourceId, cid) {
    const { data } = await supabase
        .from('commander_tax_pipeline_events')
...
router.get('/summary', async (req, res) => {
    ...
    try {
        ...
        const [indRes, compRes, provRes] = await Promise.all([
            supabase.from('commander_individual_tax_returns').select('filing_stage').eq('company_id', cid),
            supabase.from('commander_company_tax_returns').select('filing_stage').eq('company_id', cid),
            supabase.from('commander_provisional_tax_plans').select('filing_stage').eq('company_id', cid),
        ]);
