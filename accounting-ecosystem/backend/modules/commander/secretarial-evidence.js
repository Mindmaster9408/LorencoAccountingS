...
// commander_document_requests (migration 073, document-requests.js) remains
...
// items — it delegates entirely to commander_bo_readiness_items (Codebox 65)
...
async function _writeEvent(cid, clientId, sourceType, sourceId, eventType, oldStatus, newStatus, actorUserId, notes, meta) {
    await supabase.from('commander_secretarial_evidence_events').insert({
...
async function _ensureDefaultTemplates(cid, actorUserId) {
    const { data: existing } = await supabase.from('commander_secretarial_evidence_templates').select('template_type').eq('company_id', cid);
    ...
    const { data } = await supabase.from('commander_secretarial_evidence_templates').insert(toInsert).select();
    for (const t of (data || [])) {
...
async function _resolveTemplateTypeFromSource(cid, sourceType, sourceId) {
    if (sourceType === 'change_case' && sourceId) {
        const { data } = await supabase.from('commander_secretarial_change_cases').select('change_type').eq('id', sourceId).eq('company_id', cid).maybeSingle();
...
// Reuses commander_document_requests (migration 073) directly — never a
...
async function _createLinkedDocumentRequest(cid, clientId, item, actorUserId) {
    const { data, error } = await supabase.from('commander_document_requests').insert({
...
async function _syncItemFromDocumentRequest(cid, item) {
    ...
    const { data: docReq } = await supabase.from('commander_document_requests').select('request_status').eq('id', item.linked_document_request_id).eq('company_id', cid).maybeSingle();
    ...
    if (mapped !== item.status && item.status !== 'verified') {
        const { data: updated } = await supabase.from('commander_secretarial_evidence_items')
...
async function getChecklistReadiness(cid, checklist) {
    ...
    const { data: items } = await supabase.from('commander_secretarial_evidence_items').select('*').eq('company_id', cid).eq('checklist_id', checklist.id);
...
async function getEvidenceSummary(cid) {
    const [checklistsRes, itemsRes] = await Promise.all([
        supabase.from('commander_secretarial_evidence_checklists').select('id, client_id, source_type').eq('company_id', cid),
        supabase.from('commander_secretarial_evidence_items').select('checklist_id, status, required').eq('company_id', cid),
    ]);
...
router.get('/templates', async (req, res) => {
    ...
    try {
        ...
        const { data, error } = await supabase.from('commander_secretarial_evidence_templates').select('*').eq('company_id', cid).order('template_type');
...
router.post('/templates', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_secretarial_evidence_templates').insert({
...
router.get('/templates/:id', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_secretarial_evidence_templates').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
        if (error) return res.status(500).json({ error: error.message });
...
router.put('/templates/:id', async (req, res) => {
    ...
    const existing = await supabase.from('commander_secretarial_evidence_templates').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
    ...
    try {
        const { data, error } = await supabase.from('commander_secretarial_evidence_templates').update(update).eq('id', id).eq('company_id', cid).select().single();
        if (error) return res.status(500).json({ error: error.message });
...
router.delete('/templates/:id', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_secretarial_evidence_templates')
...
router.get('/checklists', async (req, res) => {
    ...
    try {
        let q = supabase.from('commander_secretarial_evidence_checklists').select('*').eq('company_id', cid).order('created_at', { ascending: false });
...
router.post('/checklists/generate', async (req, res) => {
    ...
    try {
        if (source_id) {
            const { data: dup } = await supabase.from('commander_secretarial_evidence_checklists').select('id').eq('company_id', cid).eq('source_type', source_type).eq('source_id', source_id).maybeSingle();
        ...
        let template;
        if (template_id) {
            const { data } = await supabase.from('commander_secretarial_evidence_templates').select('*').eq('id', template_id).eq('company_id', cid).maybeSingle();
            template = data;
        } else {
            const { data } = await supabase.from('commander_secretarial_evidence_templates').select('*').eq('company_id', cid).eq('template_type', templateType).eq('is_active', true).maybeSingle();
            template = data;
        }
        ...
        const { data: checklist, error } = await supabase.from('commander_secretarial_evidence_checklists').insert({
        ...
        if (source_type === 'bo_verification' || template.template_type === 'beneficial_ownership') {
            ...
            if (rows.length) {
                const { data: createdItems, error: itemsError } = await supabase.from('commander_secretarial_evidence_items').insert(rows).select();
...
router.get('/checklists/:id', async (req, res) => {
    ...
    try {
        const { data: checklist, error } = await supabase.from('commander_secretarial_evidence_checklists').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
        if (error) return res.status(500).json({ error: error.message });
...
router.put('/checklists/:id', async (req, res) => {
    ...
    const existing = await supabase.from('commander_secretarial_evidence_checklists').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
    ...
    try {
        const { data, error } = await supabase.from('commander_secretarial_evidence_checklists').update(update).eq('id', id).eq('company_id', cid).select().single();
        if (error) return res.status(500).json({ error: error.message });
...
router.post('/checklists/:id/regenerate', async (req, res) => {
    ...
    const { data: checklist } = await supabase.from('commander_secretarial_evidence_checklists').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
    if (!checklist) return res.status(404).json({ error: 'Checklist not found' });
    ...
    try {
        ...
        const { data: template } = await supabase.from('commander_secretarial_evidence_templates').select('*').eq('id', checklist.template_id).eq('company_id', cid).maybeSingle();
        if (!template) return res.status(404).json({ error: 'Template not found' });

        const { data: existingItems } = await supabase.from('commander_secretarial_evidence_items').select('evidence_type').eq('company_id', cid).eq('checklist_id', id);
        ...
        let created = [];
        if (rows.length) {
            const { data, error } = await supabase.from('commander_secretarial_evidence_items').insert(rows).select();
            if (error) return res.status(500).json({ error: error.message });
            created = data;
...
router.get('/checklists/:id/items', async (req, res) => {
    ...
    if (!id) return res.status(400).json({ error: 'Invalid checklist ID' });
    try {
        const { data: items, error } = await supabase.from('commander_secretarial_evidence_items').select('*').eq('company_id', cid).eq('checklist_id', id).order('created_at');
        if (error) return res.status(500).json({ error: error.message });
        const synced = await Promise.all((items || []).map(i => _syncItemFromDocumentRequest(cid, i)));
...
router.put('/items/:id', async (req, res) => {
    ...
    const existing = await supabase.from('commander_secretarial_evidence_items').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
    ...
    try {
        const { data, error } = await supabase.from('commander_secretarial_evidence_items').update(update).eq('id', id).eq('company_id', cid).select().single();
        if (error) return res.status(500).json({ error: error.message });

        const { data: checklist } = await supabase.from('commander_secretarial_evidence_checklists').select('client_id').eq('id', existing.data.checklist_id).maybeSingle();
...
router.post('/items/:id/link-document-request', async (req, res) => {
    ...
    const { data: docReq } = await supabase.from('commander_document_requests').select('id, client_id').eq('id', document_request_id).eq('company_id', cid).maybeSingle();
    ...
    try {
        const { data, error } = await supabase.from('commander_secretarial_evidence_items')
...
router.get('/events', async (req, res) => {
    ...
    try {
        let q = supabase.from('commander_secretarial_evidence_events').select('*').eq('company_id', cid).order('created_at', { ascending: false }).limit(Math.min(500, parseInt(limit) || 100));
        if (client_id) q = q.eq('client_id', parseInt(client_id));
        const { data, error } = await q;
        if (error) return res.status(500).json({ error: error.message });
...
router.get('/:sourceType/:sourceId/events', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_secretarial_evidence_events').select('*')
