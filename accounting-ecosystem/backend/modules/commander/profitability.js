...
// Billing/WIP (commander_billing_packs, Codebox billing.js) remains the
...
//
// CRITICAL, READ BEFORE CHANGING ANY FORMULA: commander_team_members has NO
...
const SNAPSHOT_TYPES = ['client', 'engagement', 'service', 'commander', 'manual'];
...
// Statuses in commander_time_entries.billing_status (migration 061) that
...
async function _requireManager(req, res) {
    return teamAccess.requireManager(req, res, supabase, 'Only owners, partners, admins, and practice managers can manage profitability reviews.');
}
...
async function _writeEvent(cid, sourceType, sourceId, eventType, oldStatus, newStatus, actorUserId, notes, meta) {
    await supabase.from('commander_profitability_events').insert({
...
async function calculateProfitability({ companyId, periodStart, periodEnd, clientId, engagementId, serviceId }) {
    ...
    let engagement = null;
    if (engagementId) {
        const { data } = await supabase.from('commander_client_engagements').select('*').eq('id', engagementId).eq('company_id', companyId).maybeSingle();
    ...
    // task_id scoping — commander_time_entries has no direct engagement_id/
    // service_id column; the only link is via commander_tasks.engagement_id /
    // .service_id (migration 066, plain integers, no FK).
    let scopedTaskIds = null;
    if (engagementId || serviceId) {
        let taskQuery = supabase.from('commander_tasks').select('id').eq('company_id', companyId);
    ...
    let teQuery = supabase.from('commander_time_entries').select('*')
    ...
    if (scopedTaskIds === null) {
        ...
        if (entriesWithTask.length) {
            ...
            const { data: taskRows } = await supabase.from('commander_tasks').select('id, engagement_id').in('id', taskIds).eq('company_id', companyId);
    ...
    let packQuery = supabase.from('commander_billing_packs').select('*')
    ...
    // column exists on commander_team_members; never guessed, always flagged.
    const estimatedCost = 0;
    warnings.push('TEAM_COST_RATE_MISSING');
    assumptions.push('estimated_cost = 0 — commander_team_members has no cost-rate/internal-rate column in this schema. margin_percentage is therefore always 100 (or null) until a real cost-rate data source is added; realization_percentage is the reliable signal in this pass.');
    ...
    if (resolvedClientId) {
        try {
            const { data: outOfScopeRows } = await supabase.from('commander_work_authorizations').select('id')
...
router.get('/summary', async (req, res) => {
    ...
    try {
        const { data: rows } = await supabase.from('commander_profitability_snapshots')
        ...
        const { data: reviewRows } = await supabase.from('commander_profitability_reviews').select('review_status').eq('company_id', cid).neq('review_status', 'cancelled');
...
router.post('/snapshots', async (req, res) => {
    ...
    try {
        ...
        const { data, error } = await supabase.from('commander_profitability_snapshots').insert({
...
router.get('/snapshots', async (req, res) => {
    ...
    try {
        let q = supabase.from('commander_profitability_snapshots').select('*').eq('company_id', cid).order('created_at', { ascending: false }).limit(Math.min(300, parseInt(limit) || 100));
...
router.get('/snapshots/:id', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_profitability_snapshots').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
        if (error) return res.status(500).json({ error: error.message });
...
router.get('/reviews', async (req, res) => {
    ...
    try {
        let q = supabase.from('commander_profitability_reviews').select('*').eq('company_id', cid).order('created_at', { ascending: false }).limit(Math.min(500, parseInt(limit) || 200));
        if (client_id) q = q.eq('client_id', parseInt(client_id));
        if (engagement_id) q = q.eq('engagement_id', parseInt(engagement_id));
...
router.post('/reviews', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_profitability_reviews').insert({
...
async function _fetchReview(cid, id) {
    const { data } = await supabase.from('commander_profitability_reviews').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
    return data || null;
}
...
router.put('/reviews/:id', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_profitability_reviews').update(update).eq('id', id).eq('company_id', cid).select().single();
        if (error) return res.status(500).json({ error: error.message });
...
router.delete('/reviews/:id', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_profitability_reviews')
...
router.get('/:sourceType/:sourceId/events', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_profitability_events').select('*')
