...
async function _fetchAuthorization(cid, id) {
    const { data } = await supabase.from('commander_work_authorizations').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
    return data || null;
}

async function _writeEvent(cid, authorizationId, eventType, oldStatus, newStatus, actorUserId, notes, meta) {
    await supabase.from('commander_work_authorization_events').insert({
...
async function checkWorkAuthorization({ companyId, clientId, workType, sourceModule, sourceType, sourceId, riskLevel, metadata, actorUserId }) {
    ...
    let existingQuery = supabase.from('commander_work_authorizations').select('*')
    ...
    if (existing) {
        const { data, error } = await supabase.from('commander_work_authorizations')
...
router.get('/summary', async (req, res) => {
    ...
    try {
        const { data: rows } = await supabase.from('commander_work_authorizations')
...
router.get('/', async (req, res) => {
    ...
    try {
        let q = supabase.from('commander_work_authorizations').select('*').eq('company_id', cid).order('created_at', { ascending: false }).limit(Math.min(500, parseInt(limit) || 200));
...
router.get('/:id/events', async (req, res) => {
    ...
    if (!id) return res.status(400).json({ error: 'Invalid authorization ID' });
    try {
        const { data, error } = await supabase.from('commander_work_authorization_events').select('*').eq('company_id', cid).eq('authorization_id', id).order('created_at', { ascending: false });
        if (error) return res.status(500).json({ error: error.message });
