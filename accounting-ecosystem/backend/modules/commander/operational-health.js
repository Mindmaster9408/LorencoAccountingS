...
// any table outside its own two (commander_health_check_runs/_events) — it
...
const ANCHOR_TABLES = [
    { table: 'commander_team_members', label: 'Team' },
    { table: 'commander_clients', label: 'Clients' },
    { table: 'commander_tasks', label: 'Tasks' },
    { table: 'commander_notifications', label: 'Notifications' },
    { table: 'commander_reminders', label: 'Reminders' },
    { table: 'commander_risks', label: 'Risk Register' },
    { table: 'commander_quality_reviews', label: 'QMS' },
    { table: 'commander_client_success', label: 'Client Success' },
    { table: 'commander_secretarial_profiles', label: 'Secretarial' },
    { table: 'commander_onboarding_profiles', label: 'Client Onboarding' },
    { table: 'commander_client_engagements', label: 'Engagements' },
    { table: 'commander_profitability_snapshots', label: 'Profitability' },
    { table: 'commander_pricing_reviews', label: 'Pricing Reviews' },
    { table: 'commander_partner_scorecards', label: 'Partner Scorecards' },
    { table: 'commander_strategic_plans', label: 'Strategic Planning' },
    { table: 'commander_executive_reports', label: 'Executive Reporting' },
    { table: 'commander_automation_rules', label: 'Automation' },
];
...
const MIGRATION_PROBES = [
    { table: 'commander_team_members', migration: '~055 (foundation)' },
    { table: 'commander_kpi_snapshots', migration: '110ish (KPI History, Codebox 51)' },
    { table: 'commander_partner_scorecards', migration: '132 (Codebox 75)' },
    { table: 'commander_strategic_plans', migration: '133 (Codebox 76)' },
    { table: 'commander_executive_reports', migration: '134 (Codebox 77)' },
    { table: 'commander_automation_rules', migration: '135 (Codebox 78)' },
    { table: 'commander_health_check_runs', migration: '136 (this codebox)' },
];
...
const INTEGRATION_LINKS = [
    { table: 'commander_notifications', column: 'assigned_team_member_id', label: 'Notifications → Team Member' },
    { table: 'commander_reminders', column: 'assigned_team_member_id', label: 'Reminders → Team Member' },
    { table: 'commander_executive_action_register', column: 'owner_team_member_id', label: 'Executive Actions → Team Member' },
];
...
async function _checkConfiguration(cid) {
    ...
    const { data: profileRows } = await supabase.from('commander_profiles').select('id').eq('company_id', cid).limit(1);
    if (profileRows && profileRows.length) passed++;
    else findings.push(_finding('configuration', 'medium', 'commander_profile_missing', 'No commander_profiles row exists for this company yet.', null));

    const { count: ownerCount } = await supabase.from('commander_team_members').select('id', { count: 'exact', head: true })
...
async function _checkAutomation(cid) {
    ...
    const [{ count: activeRules }, { data: failedRuns }, { data: warningRuns }, { count: awaitingApproval }] = await Promise.all([
        ...
        supabase.from('commander_automation_runs').select('id').eq('company_id', cid).eq('run_status', 'failed').eq('dry_run', false).gte('created_at', _daysAgo(30)),
        supabase.from('commander_automation_runs').select('id').eq('company_id', cid).eq('run_status', 'completed_with_warnings').gte('created_at', _daysAgo(30)),
...
async function _checkRoleLinks(cid) {
    ...
    const { data: unlinked } = await supabase.from('commander_team_members')
    ...
    // was never updated).
    const { data: linked } = await supabase.from('commander_team_members')
...
async function _checkStaleData(cid) {
    ...
    const { data: latestSnapshot } = await supabase.from('commander_kpi_snapshots').select('generated_at').eq('company_id', cid).eq('status', 'active').order('generated_at', { ascending: false }).limit(1).maybeSingle();
    ...
    const { count: staleNotifications } = await supabase.from('commander_notifications').select('id', { count: 'exact', head: true })
    ...
    const { count: staleReports } = await supabase.from('commander_executive_reports').select('id', { count: 'exact', head: true })
    ...
    const { count: staleRules } = await supabase.from('commander_automation_rules').select('id', { count: 'exact', head: true })
...
async function _checkIntegrations(cid) {
    ...
    const { data: activeMembers } = await supabase.from('commander_team_members').select('id').eq('company_id', cid).eq('is_active', true);
...
router.get('/summary', async (req, res) => {
    ...
    try {
        const { data } = await supabase.from('commander_health_check_runs')
        ...
        console.error('GET /api/commander/operational-health/summary', err);
...
router.post('/run', async (req, res) => {
    ...
    try {
        ...
        const { data: run, error: insertError } = await supabase.from('commander_health_check_runs').insert({
            company_id: cid, run_status: 'running', created_by: req.user?.userId || null,
        }).select().single();
        if (insertError) throw insertError;

        await supabase.from('commander_health_check_events').insert({ company_id: cid, run_id: run.id, event_type: 'health_check_started', actor_user_id: req.user?.userId || null, metadata: {} });

        let result;
        try {
            result = await computeOperationalHealth(cid);
        } catch (e) {
            await supabase.from('commander_health_check_runs').update({ run_status: 'failed', completed_at: new Date().toISOString() }).eq('id', run.id).eq('company_id', cid);
            await supabase.from('commander_health_check_events').insert({ company_id: cid, run_id: run.id, event_type: 'health_check_failed', actor_user_id: req.user?.userId || null, notes: e.message, metadata: {} });
            throw e;
        }

        const now = new Date().toISOString();
        const { data: updated, error: updateError } = await supabase.from('commander_health_check_runs').update({
        ...
        await supabase.from('commander_health_check_events').insert({
        ...
        await auditFromReq(req, 'operational_health_check_run', 'commander_health_check_run', run.id, { overall_status: result.overallStatus });

        return res.status(201).json({ run: updated });
    } catch (err) {
        console.error('POST /api/commander/operational-health/run', err);
...
router.get('/runs', async (req, res) => {
    ...
    try {
        ...
        const { data, count, error } = await supabase.from('commander_health_check_runs')
        ...
        console.error('GET /api/commander/operational-health/runs', err);
...
router.get('/runs/:id', async (req, res) => {
    const cid = req.companyId;
    try {
        const { data, error } = await supabase.from('commander_health_check_runs').select('*').eq('id', req.params.id).eq('company_id', cid).maybeSingle();
        if (error) throw error;
        if (!data) return res.status(404).json({ error: 'Health check run not found.' });
        return res.json({ run: data });
    } catch (err) {
        console.error('GET /api/commander/operational-health/runs/:id', err);
...
router.get('/runs/:id/events', async (req, res) => {
    const cid = req.companyId;
    try {
        const { data: run } = await supabase.from('commander_health_check_runs').select('id').eq('id', req.params.id).eq('company_id', cid).maybeSingle();
        if (!run) return res.status(404).json({ error: 'Health check run not found.' });
        const { data, error } = await supabase.from('commander_health_check_events').select('*').eq('run_id', run.id).eq('company_id', cid).order('created_at', { ascending: false });
        if (error) throw error;
        return res.json({ events: data || [] });
    } catch (err) {
        console.error('GET /api/commander/operational-health/runs/:id/events', err);
