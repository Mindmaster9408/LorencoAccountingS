...
async function createRunAndGenerateTasks(req, {
  ...
  if (runSteps.length) {
    const { error: rsErr } = await supabase
      ...
      .insert(runSteps);
    if (rsErr) {
      await supabase.from('commander_workflow_runs').delete().eq('id', run.id).eq('company_id', req.companyId);
      throw rsErr;
    }
  }
  ...
  if (tasksToInsert.length) {
    const { data: tData, error: tiErr } = await supabase
      ...
      .select();
    if (tiErr) {
      await supabase.from('commander_workflow_run_steps').delete().eq('run_id', run.id);
      await supabase.from('commander_workflow_runs').delete().eq('id', run.id).eq('company_id', req.companyId);
      throw tiErr;
    }
    insertedTasks = tData || [];
