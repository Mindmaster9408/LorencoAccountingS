...
async function logEngagementEvent(companyId, engagementId, eventType, opts = {}) {
  try {
    await supabase.from('commander_client_engagement_events').insert({
...
router.get('/services', async (req, res) => {
  ...
  let q = supabase
    .from('commander_service_catalog')
    .select('*')
...
router.post('/services', async (req, res) => {
  ...
  const { data, error } = await supabase
    ...
    .single();
  if (error) return res.status(500).json({ error: error.message });
  await auditFromReq(req, 'CREATE', 'commander_service_catalog', data.id, { module: 'commander' });
...
router.put('/services/:id', async (req, res) => {
  ...
  const { data, error } = await supabase
    ...
    .single();
  if (error) return res.status(500).json({ error: error.message });
  await auditFromReq(req, 'UPDATE', 'commander_service_catalog', data.id, { module: 'commander' });
  res.json({ service: data });
});
...
router.delete('/services/:id', async (req, res) => {
  ...
  const { error } = await supabase
    ...
    .eq('company_id', req.companyId);
  if (error) return res.status(500).json({ error: error.message });
  await auditFromReq(req, 'DEACTIVATE', 'commander_service_catalog', parseInt(req.params.id), { module: 'commander' });
...
router.post('/clients/:clientId/engagements', async (req, res) => {
  ...
  await auditFromReq(req, 'CREATE', 'commander_client_engagement', data.id, { module: 'commander', client_id: clientId });
...
router.put('/engagements/:id', async (req, res) => {
  ...
  await auditFromReq(req, 'UPDATE', 'commander_client_engagement', eng.id, { module: 'commander' });
...
router.put('/engagements/:id/pause', async (req, res) => {
  ...
  await auditFromReq(req, 'PAUSE', 'commander_client_engagement', eng.id, { module: 'commander' });
  res.json({ engagement: data });
});
...
router.put('/engagements/:id/reactivate', async (req, res) => {
  ...
  await auditFromReq(req, 'REACTIVATE', 'commander_client_engagement', eng.id, { module: 'commander' });
  res.json({ engagement: data });
});
...
router.put('/engagements/:id/end', async (req, res) => {
  ...
  await auditFromReq(req, 'END', 'commander_client_engagement', eng.id, { module: 'commander' });
  res.json({ engagement: data });
});
...
router.delete('/engagements/:id', async (req, res) => {
  ...
  await auditFromReq(req, 'CANCEL', 'commander_client_engagement', eng.id, { module: 'commander' });
  res.json({ success: true });
});
...
router.get('/engagements/:id/history', async (req, res) => {
  ...
  const { data, error } = await supabase
    .from('commander_client_engagement_events')
    .select('*')
...
router.get('/engagements/:id/generation-preview', async (req, res) => {
  ...
  if (eng.workflow_template_id) {
    const { data: tpl } = await supabase
      .from('commander_workflow_templates')
      .select(
    ...
    if (template) {
      const { count } = await supabase
        .from('commander_workflow_template_steps')
...
router.post('/engagements/:id/generate-workflow', async (req, res) => {
  ...
  try {
    ...
    await auditFromReq(req, 'GENERATE_WORKFLOW_FROM_ENGAGEMENT', 'commander_client_engagement', eng.id, {
      module: 'commander', workflow_run_id: result.run.id
    });
