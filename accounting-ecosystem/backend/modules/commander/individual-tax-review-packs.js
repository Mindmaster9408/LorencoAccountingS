/* =============================================================
   ...
   Routes mounted at /api/commander/individual-tax via index.js.
...
async function verifyPackOwnership(cid, packId) {
    const { data } = await supabase
        .from('commander_individual_tax_review_packs')
        .select('*')
        .eq('id', packId)
        .eq('company_id', cid)
        .single();
...
async function logPackEvent(cid, packId, returnId, eventType, extras = {}) {
    try {
        await supabase.from('commander_individual_tax_review_pack_events').insert({
            company_id:     cid,
...
async function buildSnapshot(cid, taxReturn, calcId) {
    ...
    const { data: items } = await supabase
        .from('commander_individual_tax_items')
...
router.post('/:returnId/review-packs/generate', async (req, res) => {
    ...
    try {
        ...
        await auditFromReq(req, 'individual_tax_review_pack_generated', 'commander_individual_tax_review_packs', pack.id, {
...
router.put('/review-packs/:id/submit-review', async (req, res) => {
    ...
    await auditFromReq(req, 'individual_tax_review_pack_submitted_review', 'commander_individual_tax_review_packs', packId);
...
router.put('/review-packs/:id/approve', async (req, res) => {
    ...
    await auditFromReq(req, 'individual_tax_review_pack_approved', 'commander_individual_tax_review_packs', packId);
...
router.put('/review-packs/:id/reject', async (req, res) => {
    ...
    await auditFromReq(req, 'individual_tax_review_pack_rejected', 'commander_individual_tax_review_packs', packId);
...
router.get('/review-packs/:id/events', async (req, res) => {
    ...
    const { data, error } = await supabase
        .from('commander_individual_tax_review_pack_events')
        .select('*')
