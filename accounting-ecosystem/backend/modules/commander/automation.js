...
async function _actionCreateReminder(a, ctx, opts) {
    ...
    const { data, error } = await supabase.from('commander_reminders').insert({
        company_id: opts.cid,
...
async function _actionCreateExecutiveAction(a, ctx, opts) {
    ...
    const { data: report } = await supabase.from('commander_executive_reports').select('id').eq('id', reportId).eq('company_id', opts.cid).maybeSingle();
...
async function _findCompletedRunByKey(cid, key) {
    if (!key) return null;
    const { data } = await supabase.from('commander_automation_runs')
...
async function _insertStep(cid, runId, order, type, name, status, input, output, errorMessage) {
    await supabase.from('commander_automation_run_steps').insert({
...
async function _writeRunEvent(cid, ruleId, runId, eventType, oldStatus, newStatus, userId, notes, meta) {
    await supabase.from('commander_automation_events').insert({
...
async function evaluateAutomationRule(cid, rule, triggerContext, opts) {
    ...
    const { data: run, error: runInsertError } = await supabase.from('commander_automation_runs').insert({
    ...
    try {
        ...
        if (!dryRun) {
            ...
            if (existing) {
                ...
                finalStatus = 'skipped';
                await supabase.from('commander_automation_runs').update({
        ...
        if (!dryRun && ['completed', 'completed_with_warnings'].includes(finalStatus)) {
            await supabase.from('commander_automation_rules').update({ last_run_at: now }).eq('id', rule.id).eq('company_id', cid);
        }
...
async function _verifyRule(id, cid) {
    const { data } = await supabase.from('commander_automation_rules').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
    return data || null;
}
...
router.get('/summary', async (req, res) => {
    ...
    try {
        const [{ count: activeRules }, { count: awaitingApproval }, { data: failedRuns }, { data: warningRuns }] = await Promise.all([
            supabase.from('commander_automation_rules').select('id', { count: 'exact', head: true }).eq('company_id', cid).eq('rule_status', 'active'),
            supabase.from('commander_automation_rules').select('id', { count: 'exact', head: true }).eq('company_id', cid).eq('requires_approval', true).is('approved_at', null).not('rule_status', 'in', '("archived","cancelled")'),
            supabase.from('commander_automation_runs').select('id').eq('company_id', cid).eq('run_status', 'failed').eq('dry_run', false).order('created_at', { ascending: false }).limit(20),
            supabase.from('commander_automation_runs').select('id').eq('company_id', cid).eq('run_status', 'completed_with_warnings').order('created_at', { ascending: false }).limit(20),
        ]);
        return res.json({
        ...
        console.error('GET /api/commander/automation/summary', err);
...
router.get('/rules', async (req, res) => {
    ...
    try {
        ...
        let q = supabase.from('commander_automation_rules').select('*', { count: 'exact' }).eq('company_id', cid);
        ...
        console.error('GET /api/commander/automation/rules', err);
...
router.post('/rules', async (req, res) => {
    ...
    try {
        ...
        const { data: rule, error } = await supabase.from('commander_automation_rules').insert({
        ...
        await auditFromReq(req, 'automation_rule_created', 'commander_automation_rule', rule.id, {});

        return res.status(201).json({ rule });
    } catch (err) {
        console.error('POST /api/commander/automation/rules', err);
...
router.get('/rules/:id', async (req, res) => {
    ...
    try {
        ...
        console.error('GET /api/commander/automation/rules/:id', err);
...
router.put('/rules/:id', async (req, res) => {
    ...
    try {
        ...
        const { data: updated, error } = await supabase.from('commander_automation_rules').update({ ...patch, updated_by: req.user?.userId }).eq('id', rule.id).eq('company_id', cid).select().single();
        if (error) throw error;

        await _writeRuleEvent(cid, rule.id, 'rule_updated', null, null, req.user?.userId, null, { fields: Object.keys(patch) });
        await auditFromReq(req, 'automation_rule_updated', 'commander_automation_rule', rule.id, { fields: Object.keys(patch) });

        return res.json({ rule: updated });
    } catch (err) {
        console.error('PUT /api/commander/automation/rules/:id', err);
...
router.delete('/rules/:id', async (req, res) => {
    ...
    try {
        ...
        const { data: updated, error } = await supabase.from('commander_automation_rules').update({ rule_status: 'cancelled', cancellation_reason: String(reason).trim(), updated_by: req.user?.userId }).eq('id', rule.id).eq('company_id', cid).select().single();
        if (error) throw error;

        await _writeRuleEvent(cid, rule.id, 'rule_cancelled', rule.rule_status, 'cancelled', req.user?.userId, String(reason).trim(), {});
        await auditFromReq(req, 'automation_rule_cancelled', 'commander_automation_rule', rule.id, {});

        return res.json({ rule: updated });
    } catch (err) {
        console.error('DELETE /api/commander/automation/rules/:id', err);
...
router.put('/rules/:id/approve', async (req, res) => {
    ...
    try {
        ...
        const now = new Date().toISOString();
        const { data: updated, error } = await supabase.from('commander_automation_rules').update({
        ...
        await auditFromReq(req, 'automation_rule_approved', 'commander_automation_rule', rule.id, {});

        return res.json({ rule: updated });
    } catch (err) {
        console.error('PUT /api/commander/automation/rules/:id/approve', err);
...
router.put('/rules/:id/activate', async (req, res) => {
    ...
    try {
        ...
        const { data: updated, error } = await supabase.from('commander_automation_rules').update({ rule_status: 'active', updated_by: req.user?.userId }).eq('id', rule.id).eq('company_id', cid).select().single();
        if (error) throw error;

        await _writeRuleEvent(cid, rule.id, 'rule_activated', rule.rule_status, 'active', req.user?.userId, null, {});
        await auditFromReq(req, 'automation_rule_activated', 'commander_automation_rule', rule.id, {});

        return res.json({ rule: updated });
    } catch (err) {
        console.error('PUT /api/commander/automation/rules/:id/activate', err);
...
router.put('/rules/:id/pause', async (req, res) => {
    ...
    try {
        ...
        const { data: updated, error } = await supabase.from('commander_automation_rules').update({ rule_status: 'paused', updated_by: req.user?.userId }).eq('id', rule.id).eq('company_id', cid).select().single();
        if (error) throw error;

        await _writeRuleEvent(cid, rule.id, 'rule_paused', 'active', 'paused', req.user?.userId, null, {});
        await auditFromReq(req, 'automation_rule_paused', 'commander_automation_rule', rule.id, {});

        return res.json({ rule: updated });
    } catch (err) {
        console.error('PUT /api/commander/automation/rules/:id/pause', err);
...
router.put('/rules/:id/disable', async (req, res) => {
    ...
    try {
        ...
        const { data: updated, error } = await supabase.from('commander_automation_rules').update({ rule_status: 'disabled', updated_by: req.user?.userId }).eq('id', rule.id).eq('company_id', cid).select().single();
        if (error) throw error;

        await _writeRuleEvent(cid, rule.id, 'rule_disabled', rule.rule_status, 'disabled', req.user?.userId, null, {});
        await auditFromReq(req, 'automation_rule_disabled', 'commander_automation_rule', rule.id, {});

        return res.json({ rule: updated });
    } catch (err) {
        console.error('PUT /api/commander/automation/rules/:id/disable', err);
...
router.put('/rules/:id/archive', async (req, res) => {
    ...
    try {
        ...
        const { data: updated, error } = await supabase.from('commander_automation_rules').update({ rule_status: 'archived', updated_by: req.user?.userId }).eq('id', rule.id).eq('company_id', cid).select().single();
        if (error) throw error;

        await _writeRuleEvent(cid, rule.id, 'rule_archived', rule.rule_status, 'archived', req.user?.userId, null, {});
        await auditFromReq(req, 'automation_rule_archived', 'commander_automation_rule', rule.id, {});

        return res.json({ rule: updated });
    } catch (err) {
        console.error('PUT /api/commander/automation/rules/:id/archive', err);
...
router.post('/rules/:id/test', async (req, res) => {
    ...
    try {
        ...
        console.error('POST /api/commander/automation/rules/:id/test', err);
...
router.post('/rules/:id/run', async (req, res) => {
    ...
    try {
        ...
        console.error('POST /api/commander/automation/rules/:id/run', err);
...
router.get('/runs', async (req, res) => {
    ...
    try {
        ...
        let q = supabase.from('commander_automation_runs').select('*', { count: 'exact' }).eq('company_id', cid);
        ...
        console.error('GET /api/commander/automation/runs', err);
...
router.get('/runs/:id', async (req, res) => {
    const cid = req.companyId;
    try {
        const { data, error } = await supabase.from('commander_automation_runs').select('*').eq('id', req.params.id).eq('company_id', cid).maybeSingle();
        if (error) throw error;
        if (!data) return res.status(404).json({ error: 'Automation run not found.' });
        return res.json({ run: data });
    } catch (err) {
        console.error('GET /api/commander/automation/runs/:id', err);
...
router.get('/runs/:id/steps', async (req, res) => {
    const cid = req.companyId;
    try {
        const { data: run } = await supabase.from('commander_automation_runs').select('id').eq('id', req.params.id).eq('company_id', cid).maybeSingle();
        if (!run) return res.status(404).json({ error: 'Automation run not found.' });
        const { data, error } = await supabase.from('commander_automation_run_steps').select('*').eq('run_id', run.id).eq('company_id', cid).order('step_order');
        if (error) throw error;
        return res.json({ steps: data || [] });
    } catch (err) {
        console.error('GET /api/commander/automation/runs/:id/steps', err);
...
router.get('/events', async (req, res) => {
    ...
    try {
        let q = supabase.from('commander_automation_events').select('*', { count: 'exact' }).eq('company_id', cid);
        if (rule_id) q = q.eq('rule_id', parseInt(rule_id, 10));
        ...
        console.error('GET /api/commander/automation/events', err);
...
router.get('/rules/:id/events', async (req, res) => {
    ...
    try {
        const rule = await _verifyRule(req.params.id, cid);
        if (!rule) return res.status(404).json({ error: 'Automation rule not found.' });
        const { data, error } = await supabase.from('commander_automation_events').select('*').eq('rule_id', rule.id).eq('company_id', cid).order('created_at', { ascending: false });
        if (error) throw error;
        return res.json({ events: data || [] });
    } catch (err) {
        console.error('GET /api/commander/automation/rules/:id/events', err);
...
const SEED_RULES = [
    {
        ...
        actions: [{ type: 'create_notification', category: 'system', severity: 'medium', title: 'Executive report published: {{source.report_title}}', message: 'A new executive report is ready for review.', sourceType: 'commander_executive_report', assignment: { role: 'partner' } }],
        safety_level: 'low',
    },
    {
    ...
    },
    {
        ...
        actions: [{ type: 'create_notification', category: 'compliance', severity: 'critical', title: 'Critical secretarial integrity finding', message: '{{source.finding_title}}', sourceType: 'commander_secretarial_integrity_finding', assignment: { role: 'partner' } }],
    ...
    },
    {
        ...
        actions: [{ type: 'create_notification', category: 'client', severity: 'info', title: 'Client onboarding completed', message: 'Onboarding for {{source.client_name}} is complete.', sourceType: 'commander_onboarding_profile', assignment: { role: 'partner' } }],
        safety_level: 'low',
    },
];
...
router.post('/seed-defaults', async (req, res) => {
    ...
    try {
        ...
        const { data: existing } = await supabase.from('commander_automation_rules').select('rule_key').eq('company_id', cid);
        ...
        const { data: inserted, error } = await supabase.from('commander_automation_rules').insert(toInsert).select('id, rule_key, rule_name');
        if (error) throw error;

        for (const r of inserted || []) {
            await _writeRuleEvent(cid, r.id, 'rule_created', null, 'draft', req.user?.userId, 'Seeded default rule.', { seeded: true });
        }
        await auditFromReq(req, 'automation_rules_seeded', 'commander_automation_rule', null, { inserted_count: (inserted || []).length });

        return res.status(201).json({ inserted: (inserted || []).length, rules: inserted || [] });
    } catch (err) {
        console.error('POST /api/commander/automation/seed-defaults', err);
