...
async function fetchHealthData(cid, clientId = null) {
  ...
  const [clients, deadlines, tasks, periods, engagements, recurringEngagements, wipPacks] = await Promise.all([
    ...
    (() => {
      let q = supabase.from('commander_clients').select('*').eq('company_id', cid).eq('is_active', true);
...
// ─── Write score back to commander_clients + insert snapshot ──────────────────
...
async function persistScore(cid, result, calculatedBy) {
  const [clientUpdate, snapshotInsert] = await Promise.all([
    ...
    supabase.from('commander_client_health_snapshots')
      .insert({
        company_id:    cid,
...
// ─── GET /summary ─────────────────────────────────────────────────────────────
// Reads cached health_status from commander_clients. Fast single query.
...
router.get('/:clientId', async (req, res) => {
  ...
  try {
    ...
    const { data: latestSnap } = await supabase
      .from('commander_client_health_snapshots')
...
// Writes score back to commander_clients and inserts a snapshot.

router.post('/recalculate', async (req, res) => {
  const cid      = req.companyId;
  const clientId = req.body.client_id ? parseInt(req.body.client_id) : null;

  // Resolve actor: prefer commander_team_members ID if available, else null
  const actorId = null; // Future: cross-ref req.user.userId → commander_team_members.user_id
  ...
  try {
    ...
    // Audit log
    await auditFromReq(req, 'RECALCULATE', 'commander_client_health', clientId || 0, {
      module:         'commander',
...
// =============================================================================
//
// Route map (all under /api/commander/client-health):
...
async function verifyActionBelongsToCompany(cid, actionId) {
  const { data } = await supabase
    .from('commander_client_health_actions')
    .select('*')
    .eq('id', actionId)
    .eq('company_id', cid)
    .single();
...
// linked commander_task with the same title and due date.
...
router.post('/:clientId/actions/from-risk', async (req, res) => {
  ...
  try {
    if (actionType === 'create_task') {
      ...
      await auditFromReq(req, 'CREATE', 'commander_task', linkedTaskId, {
        module: 'commander', action: 'health_action_task_created',
    ...
    await auditFromReq(req, 'CREATE', 'commander_client_health_action', actionData.id, {
      module: 'commander', action: 'health_action_created',
...
router.post('/:clientId/actions', async (req, res) => {
  ...
  try {
    ...
    await auditFromReq(req, 'CREATE', 'commander_client_health_action', data.id, {
      module: 'commander', action: 'health_action_created',
...
router.put('/actions/:id/complete', async (req, res) => {
  ...
  try {
    ...
    await auditFromReq(req, 'UPDATE', 'commander_client_health_action', actionId, {
      module: 'commander', action: 'health_action_completed',
...
router.put('/actions/:id/dismiss', async (req, res) => {
  ...
  try {
    ...
    await auditFromReq(req, 'UPDATE', 'commander_client_health_action', actionId, {
      module: 'commander', action: 'health_action_dismissed',
      client_id: existing.client_id,
    });
    res.json({ action: data });
  } catch (err) {
...
router.put('/actions/:id', async (req, res) => {
  ...
  try {
    ...
    await auditFromReq(req, 'UPDATE', 'commander_client_health_action', actionId, {
      module: 'commander', action: 'health_action_updated',
      client_id: existing.client_id,
    });
    res.json({ action: data });
  } catch (err) {
