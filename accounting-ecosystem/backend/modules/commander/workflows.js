...
router.post('/templates', async (req, res) => {
  try {
    ...
    await auditFromReq(req, 'CREATE', 'commander_workflow_template', tpl.id, { module: 'commander' });
...
router.put('/templates/:id', async (req, res) => {
  try {
    ...
    await auditFromReq(req, 'UPDATE', 'commander_workflow_template', tpl.id, { module: 'commander' });
    res.json({ template: tpl });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
router.delete('/templates/:id', async (req, res) => {
  try {
    ...
    await auditFromReq(req, 'DEACTIVATE', 'commander_workflow_template', tpl.id, { module: 'commander' });
...
router.post('/templates/:id/steps', async (req, res) => {
  try {
    ...
    await auditFromReq(req, 'CREATE', 'commander_workflow_template_step', step.id, { module: 'commander' });
...
router.put('/templates/:templateId/steps/:stepId(\\d+)', async (req, res) => {
  try {
    ...
    await auditFromReq(req, 'UPDATE', 'commander_workflow_template_step', step.id, { module: 'commander' });
    res.json({ step });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
router.delete('/templates/:templateId/steps/:stepId', async (req, res) => {
  try {
    ...
    await auditFromReq(req, 'DELETE', 'commander_workflow_template_step', parseInt(req.params.stepId), { module: 'commander' });
    res.json({ success: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
router.put('/templates/:id/steps/reorder', async (req, res) => {
  try {
    ...
    await auditFromReq(req, 'UPDATE', 'workflow_template_steps_reordered', templateId, { module: 'commander', metadata: { stepIds } });
    res.json({ success: true });
  } catch (err) {
...
router.post('/generate', async (req, res) => {
  try {
    ...
    await auditFromReq(req, 'CREATE', 'commander_workflow_run', result.run.id, {
      module: 'commander',
      deadline_id: result.deadline ? result.deadline.id : null
    });

    if (result.deadline) {
      await auditFromReq(req, 'CREATE', 'commander_deadline', result.deadline.id, {
        module: 'commander',
...
router.get('/runs/:id', async (req, res) => {
  try {
    ...
    const { data: steps } = await supabase
      .from('commander_workflow_run_steps')
      .select('*')
...
router.put('/runs/:id/link-deadline', async (req, res) => {
  try {
    ...
    await auditFromReq(req, 'UPDATE', 'commander_workflow_run', runId, { module: 'commander', linked_deadline_id: deadline_id });
...
router.put('/runs/:id/unlink-deadline', async (req, res) => {
  try {
    ...
    await auditFromReq(req, 'UPDATE', 'commander_workflow_run', runId, { module: 'commander', unlinked_deadline_id: prevDeadlineId });
