...
async function logBulkEvent(cid, opId, eventType, userId, notes, meta) {
    await supabase.from('commander_tax_bulk_operation_events').insert({
...
async function buildPreviewData(cid, { operation_type, tax_year, filters, options }) {
    ...
    // Base client query
    let q = supabase.from('commander_clients')
    ...
    if (filters.provisional_taxpayer === true || filters.provisional_taxpayer === 'true') {
        const { data: profiles } = await supabase.from('commander_taxpayer_profiles')
    ...
    if (filters.has_active_engagement === true || filters.has_active_engagement === 'true') {
        const { data: engs } = await supabase.from('commander_client_engagements')
    ...
    if (filters.missing_compliance_pack === true || filters.missing_compliance_pack === 'true') {
        ...
        let packQ = supabase.from('commander_compliance_packs')
    ...
    if (['apply_tax_checklist', 'create_document_requests', 'mixed_tax_season_setup'].includes(operation_type)) {
        if (options.checklist_template_id) {
            const { count: itemCount } = await supabase.from('commander_tax_checklist_template_items')
...
async function _executeCreatePack(cid, client, options, taxYear, userId, result) {
    ...
    let checkQ = supabase.from('commander_compliance_packs')
    ...
    const { data: pack, error } = await supabase.from('commander_compliance_packs').insert({
        company_id:              cid,
        client_id:               client.id,
        pack_type:               packType,
        pack_name:               packName,
...
async function _executeApplyChecklist(cid, client, options, templateItems, taxYear, userId, result, warnings) {
    ...
    const { data: existing } = await supabase.from('commander_document_requests')
    ...
    for (const item of docItems) {
        ...
        const { error } = await supabase.from('commander_document_requests').insert({
            company_id:          cid,
            client_id:           client.id,
...
async function _executeAssignOwner(cid, client, options, result) {
    ...
    const { error } = await supabase.from('commander_clients')
...
async function _executeAssignReviewer(cid, client, options, taxYear, result) {
    ...
    for (const r of (indReturns || [])) {
        ...
        await supabase.from('commander_individual_tax_returns')
    ...
    for (const r of (coReturns || [])) {
        if (r.reviewer_team_member_id && !options.override_existing) continue;
        await supabase.from('commander_company_tax_returns')
            .update({ reviewer_team_member_id: newReviewerId, updated_at: now() })
            .eq('id', r.id);
        updated++;
    }
...
async function _executeCreateAction(cid, client, options, taxYear, userId, result, opId) {
    ...
    const { data: existing } = await supabase.from('commander_tax_work_actions')
        .select('id').eq('company_id', cid).eq('client_id', client.id)
    ...
    const { data: action, error } = await supabase.from('commander_tax_work_actions').insert({
        company_id:    cid,
        client_id:     client.id,
...
async function executeOperation(cid, op, userId) {
    ...
    const { data: clients } = await supabase.from('commander_clients')
        .select('id, name, client_type, responsible_team_member_id')
    ...
    if (['apply_tax_checklist', 'create_document_requests', 'mixed_tax_season_setup'].includes(operation_type)
        ...
        const { data: items } = await supabase.from('commander_tax_checklist_template_items')
    ...
    if (itemRows.length > 0)
        await supabase.from('commander_tax_bulk_operation_items').insert(itemRows);
...
router.get('/', async (req, res) => {
    ...
    let q = supabase.from('commander_tax_bulk_operations')
...
router.post('/', async (req, res) => {
    ...
    const [ownerOk, reviewerOk] = await Promise.all([
        verifyOwn(cid, 'commander_team_members', options && options.assign_responsible_team_member_id),
        verifyOwn(cid, 'commander_team_members', options && options.assign_reviewer_team_member_id),
    ]);
    ...
    const { data, error } = await supabase.from('commander_tax_bulk_operations').insert({
        company_id:       cid,
    ...
    await auditFromReq(req, 'CREATE', 'commander_tax_bulk_operation', data.id, { module: 'commander', operation_type });
...
router.get('/:id/items', async (req, res) => {
    ...
    const opCheck = await supabase.from('commander_tax_bulk_operations')
        .select('id').eq('company_id', cid).eq('id', opId).maybeSingle();
    if (!opCheck.data) return res.status(404).json({ error: 'Operation not found' });

    const { data, error } = await supabase.from('commander_tax_bulk_operation_items')
        .select('id, client_id, item_status, message, created_records, error_detail, created_at, clients:commander_clients!client_id(name)')
...
router.get('/:id/events', async (req, res) => {
    ...
    const { data, error } = await supabase.from('commander_tax_bulk_operation_events')
...
router.post('/:id/execute', async (req, res) => {
    ...
    const { data: op, error: opErr } = await supabase.from('commander_tax_bulk_operations')
    ...
    // Mark as running
    await supabase.from('commander_tax_bulk_operations')
    ...
    await auditFromReq(req, 'UPDATE', 'commander_tax_bulk_operation', opId,
        { module: 'commander', final_status: finalStatus, summary });

    // Fetch items to return inline for the results panel
    const { data: items } = await supabase.from('commander_tax_bulk_operation_items')
        .select('id, client_id, item_status, message, error_detail, clients:commander_clients!client_id(name)')
        .eq('company_id', cid).eq('operation_id', opId)
        .order('created_at', { ascending: true });
...
router.put('/:id/cancel', async (req, res) => {
    ...
    const { data: op } = await supabase.from('commander_tax_bulk_operations')
    ...
    const { error } = await supabase.from('commander_tax_bulk_operations')
...
router.get('/:id', async (req, res) => {
    const cid  = req.companyId;
    const opId = parseInt(req.params.id);
    const { data, error } = await supabase.from('commander_tax_bulk_operations')
        .select('*').eq('company_id', cid).eq('id', opId).maybeSingle();
    if (error) return res.status(500).json({ error: error.message });
