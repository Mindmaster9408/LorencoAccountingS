...
async function _requireManager(req, res) {
    return teamAccess.requireManager(req, res, supabase, 'The Planning Board is only available to owners, partners, admins, and practice managers.');
}

async function _writeEvent(cid, noteId, eventType, actorUserId, notes, meta) {
    await supabase.from('commander_planning_events').insert({
...
async function _verifyNote(id, cid) {
    const { data } = await supabase.from('commander_planning_notes').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
    return data || null;
}
...
async function _buildTeamItemPool(cid) {
    ...
    const { data: members, error } = await supabase.from('commander_team_members')
    ...
    const { data: atRiskRows } = await supabase.from('commander_client_success')
    ...
    const { data: returnRows } = await supabase.from('commander_annual_returns')
    ...
    const { data: pendingChangeRows } = await supabase.from('commander_secretarial_change_cases')
    ...
    const { data: blockedBoRows } = await supabase.from('commander_bo_readiness_items')
    ...
    // visit to /commander/secretarial-calendar.html for the authoritative view.
    const statutoryWindow = new Date(Date.now() + 30 * 86400000).toISOString().slice(0, 10);
    const { data: upcomingStatutoryRows } = await supabase.from('commander_statutory_schedule')
    ...
    if (upcomingStatutoryRows && upcomingStatutoryRows.length) {
        const { data: depRows } = await supabase.from('commander_statutory_dependencies')
    ...
    const { data: blockedEvidenceRows } = await supabase.from('commander_secretarial_evidence_items')
    ...
    if (blockedEvidenceRows && blockedEvidenceRows.length) {
        ...
        const { data: checklistRows } = await supabase.from('commander_secretarial_evidence_checklists').select('id, client_id').in('id', checklistIds).eq('company_id', cid);
    ...
    // still lives on /commander/entity-lifecycle.html.
    const { data: pendingLifecycleRows } = await supabase.from('commander_entity_lifecycle_transitions')
    ...
    // from a board load).
    const { data: criticalIntegrityRows } = await supabase.from('commander_secretarial_integrity_findings')
    ...
    const { data: unacceptedRiskRows } = await supabase.from('commander_client_engagements')
    ...
    // every board load.
    const { data: outOfScopeRows } = await supabase.from('commander_work_authorizations')
...
router.get('/summary', async (req, res) => {
    try {
        ...
        const [pool, teamCapacity, notesCount, unreadNotifCount] = await Promise.all([
            ...
            supabase.from('commander_planning_notes').select('id', { count: 'exact', head: true }).eq('company_id', cid).eq('week_start', weekStart).neq('status', 'archived'),
            supabase.from('commander_notifications').select('id', { count: 'exact', head: true }).eq('company_id', cid).eq('notification_status', 'new'),
        ]);
        ...
        console.error('GET /api/commander/planning-board/summary', err);
...
router.get('/week', async (req, res) => {
    try {
        ...
        const [pool, notesRes] = await Promise.all([
            _buildTeamItemPool(cid),
            supabase.from('commander_planning_notes').select('*').eq('company_id', cid).eq('week_start', weekStart).neq('status', 'archived').order('created_at', { ascending: true }),
        ]);
        const items = pool.items;
        ...
        console.error('GET /api/commander/planning-board/week', err);
...
router.get('/team', async (req, res) => {
    try {
        ...
        const [pool, teamCapacity, notesRes, skillRows, onboardingRows] = await Promise.all([
            _buildTeamItemPool(cid),
            capacity.buildTeamCapacity(cid),
            supabase.from('commander_planning_notes').select('team_member_id').eq('company_id', cid).eq('week_start', weekStart).neq('status', 'archived').not('team_member_id', 'is', null),
            ...
            supabase.from('commander_team_skills').select('team_member_id, current_level, target_level').eq('company_id', cid),
            ...
            supabase.from('commander_onboarding_profiles').select('assigned_team_member_id, onboarding_status').eq('company_id', cid).not('assigned_team_member_id', 'is', null).not('onboarding_status', 'in', '("completed","cancelled")'),
        ]);
        ...
        const { data: strategicInitRows } = await supabase.from('commander_strategic_initiatives')
        ...
        const board = pool.members.map(m => {
            ...
            return {
                ...
                work_queue_link: '/commander/work-queue.html?team_member_id=' + m.id,
                capacity_link: '/commander/capacity.html?member_id=' + m.id,
            };
        }).sort((a, b) => {
        ...
        console.error('GET /api/commander/planning-board/team', err);
...
// queries commander_deadlines directly.
...
router.get('/deadlines', async (req, res) => {
    try {
        ...
        const { data, error } = await supabase.from('commander_deadlines')
            .select('id, title, due_date, status, priority, compliance_area, deadline_type, responsible_team_member_id, client_id, commander_clients:client_id(name), commander_team_members!responsible_team_member_id(display_name)')
            .eq('company_id', cid)
        ...
        const deadlines = (data || []).map(d => ({
            ...
            responsible_team_member_name: d.commander_team_members?.display_name || null,
        ...
        console.error('GET /api/commander/planning-board/deadlines', err);
...
router.get('/capacity', async (req, res) => {
    try {
        ...
        console.error('GET /api/commander/planning-board/capacity', err);
...
router.get('/planning-notes', async (req, res) => {
    try {
        ...
        const weekStart = _mondayOf(req.query.week_start);
        let q = supabase.from('commander_planning_notes').select('*').eq('company_id', cid).eq('week_start', weekStart);
        ...
        console.error('GET /api/commander/planning-board/planning-notes', err);
...
router.post('/planning-notes', async (req, res) => {
    try {
        ...
        const { data, error } = await supabase.from('commander_planning_notes').insert(insertRow).select().single();
        if (error) throw error;

        await _writeEvent(cid, data.id, 'note_created', req.user?.userId);
        res.status(201).json({ note: data });
    } catch (err) {
        console.error('POST /api/commander/planning-board/planning-notes', err);
...
router.put('/planning-notes/:id', async (req, res) => {
    try {
        ...
        const { data, error } = await supabase.from('commander_planning_notes').update(patch).eq('id', note.id).eq('company_id', cid).select().single();
        if (error) throw error;

        await _writeEvent(cid, note.id, 'note_updated', req.user?.userId);
        res.json({ note: data });
    } catch (err) {
        console.error('PUT /api/commander/planning-board/planning-notes/:id', err);
...
router.delete('/planning-notes/:id', async (req, res) => {
    try {
        ...
        const { data, error } = await supabase.from('commander_planning_notes').update({ status: 'archived' }).eq('id', note.id).eq('company_id', cid).select().single();
        if (error) throw error;

        await _writeEvent(cid, note.id, 'note_archived', req.user?.userId);
        res.json({ note: data, archived: true });
    } catch (err) {
        console.error('DELETE /api/commander/planning-board/planning-notes/:id', err);
...
router.post('/events', async (req, res) => {
    try {
        ...
        console.error('POST /api/commander/planning-board/events', err);
...
router.get('/events', async (req, res) => {
    try {
        ...
        let q = supabase.from('commander_planning_events').select('*').eq('company_id', cid).order('created_at', { ascending: false }).limit(100);
        ...
        console.error('GET /api/commander/planning-board/events', err);
