...
async function _writeEvent(cid, eventType, entityType, entityId, actorUserId, notes, meta) {
    await supabase.from('commander_skill_events').insert({
...
router.get('/summary', async (req, res) => {
    try {
        ...
        const [categories, skills, teamSkills, certifications, teamCerts] = await Promise.all([
            supabase.from('commander_skill_categories').select('id', { count: 'exact', head: true }).eq('company_id', cid).eq('is_active', true),
            supabase.from('commander_skills').select('id', { count: 'exact', head: true }).eq('company_id', cid).eq('is_active', true),
            supabase.from('commander_team_skills').select('current_level, target_level').eq('company_id', cid),
            supabase.from('commander_certifications').select('id', { count: 'exact', head: true }).eq('company_id', cid).eq('is_active', true),
            supabase.from('commander_team_certifications').select('status, expiry_date').eq('company_id', cid).eq('is_active', true),
        ]);
        ...
        console.error('GET /api/commander/skills-matrix/summary', err);
...
router.post('/seed-defaults', async (req, res) => {
    try {
        ...
        const { data: existingCats } = await supabase.from('commander_skill_categories').select('id, category_key').eq('company_id', cid);
        const catIdByKey = {};
        ...
        if (catsToInsert.length) {
            const { data, error } = await supabase.from('commander_skill_categories').insert(catsToInsert).select();
            if (error) throw error;
            insertedCats = data || [];
            insertedCats.forEach(c => { catIdByKey[c.category_key] = c.id; });
        }

        const { data: existingSkills } = await supabase.from('commander_skills').select('skill_key').eq('company_id', cid);
        ...
        if (skillsToInsert.length) {
            const { data, error } = await supabase.from('commander_skills').insert(skillsToInsert).select();
            if (error) throw error;
            insertedSkills = data || [];
        }
        ...
        console.error('POST /api/commander/skills-matrix/seed-defaults', err);
...
router.get('/categories', async (req, res) => {
    try {
        const cid = req.companyId;
        let q = supabase.from('commander_skill_categories').select('*').eq('company_id', cid).order('sort_order');
        if (req.query.include_archived !== 'true') q = q.eq('is_active', true);
        const { data, error } = await q;
        if (error) throw error;
        res.json({ categories: data || [] });
    } catch (err) {
        console.error('GET /api/commander/skills-matrix/categories', err);
...
router.post('/categories', async (req, res) => {
    try {
        ...
        const { data, error } = await supabase.from('commander_skill_categories').insert({
        ...
        console.error('POST /api/commander/skills-matrix/categories', err);
...
router.put('/categories/:id', async (req, res) => {
    try {
        ...
        const { data, error } = await supabase.from('commander_skill_categories').update(patch).eq('id', req.params.id).eq('company_id', cid).select().single();
        if (error) throw error;
        if (!data) return res.status(404).json({ error: 'Category not found.' });
        await _writeEvent(cid, 'category_updated', 'category', data.id, req.user?.userId);
        res.json({ category: data });
    } catch (err) {
        console.error('PUT /api/commander/skills-matrix/categories/:id', err);
...
router.delete('/categories/:id', async (req, res) => {
    try {
        ...
        const manager = await _requireManager(req, res);
        if (!manager) return;
        const { data, error } = await supabase.from('commander_skill_categories').update({ is_active: false }).eq('id', req.params.id).eq('company_id', cid).select().single();
        if (error) throw error;
        if (!data) return res.status(404).json({ error: 'Category not found.' });
        await _writeEvent(cid, 'category_archived', 'category', data.id, req.user?.userId);
        res.json({ category: data, archived: true });
    } catch (err) {
        console.error('DELETE /api/commander/skills-matrix/categories/:id', err);
...
router.get('/skills', async (req, res) => {
    try {
        const cid = req.companyId;
        let q = supabase.from('commander_skills').select('*, commander_skill_categories:category_id(category_key, display_name)').eq('company_id', cid).order('sort_order');
        if (req.query.include_archived !== 'true') q = q.eq('is_active', true);
        ...
        console.error('GET /api/commander/skills-matrix/skills', err);
...
router.post('/skills', async (req, res) => {
    try {
        ...
        const { data, error } = await supabase.from('commander_skills').insert({
        ...
        console.error('POST /api/commander/skills-matrix/skills', err);
...
router.put('/skills/:id', async (req, res) => {
    try {
        ...
        const { data, error } = await supabase.from('commander_skills').update(patch).eq('id', req.params.id).eq('company_id', cid).select().single();
        if (error) throw error;
        ...
        console.error('PUT /api/commander/skills-matrix/skills/:id', err);
...
router.delete('/skills/:id', async (req, res) => {
    try {
        ...
        const manager = await _requireManager(req, res);
        if (!manager) return;
        const { data, error } = await supabase.from('commander_skills').update({ is_active: false }).eq('id', req.params.id).eq('company_id', cid).select().single();
        if (error) throw error;
        if (!data) return res.status(404).json({ error: 'Skill not found.' });
        await _writeEvent(cid, 'skill_archived', 'skill', data.id, req.user?.userId);
        res.json({ skill: data, archived: true });
    } catch (err) {
        console.error('DELETE /api/commander/skills-matrix/skills/:id', err);
...
router.get('/team-skills', async (req, res) => {
    try {
        ...
        let q = supabase.from('commander_team_skills').select('*, commander_skills:skill_id(skill_key, display_name, category_id)').eq('company_id', cid);
        ...
        let membersById = {};
        if (memberIds.length) {
            const { data: members } = await supabase.from('commander_team_members').select('id, display_name').in('id', memberIds);
        ...
        if (req.query.category_id) rows = rows.filter(r => r.commander_skills?.category_id === parseInt(req.query.category_id, 10));

        res.json({ team_skills: rows });
    } catch (err) {
        console.error('GET /api/commander/skills-matrix/team-skills', err);
...
router.post('/team-skills', async (req, res) => {
    try {
        ...
        const { data, error } = await supabase.from('commander_team_skills').upsert(row, { onConflict: 'company_id,team_member_id,skill_id' }).select().single();
        if (error) throw error;
        ...
        console.error('POST /api/commander/skills-matrix/team-skills', err);
...
router.put('/team-skills/:id', async (req, res) => {
    try {
        ...
        const { data, error } = await supabase.from('commander_team_skills').update(patch).eq('id', req.params.id).eq('company_id', cid).select().single();
        if (error) throw error;
        ...
        console.error('PUT /api/commander/skills-matrix/team-skills/:id', err);
...
router.delete('/team-skills/:id', async (req, res) => {
    try {
        ...
        const manager = await _requireManager(req, res);
        if (!manager) return;
        const { data, error } = await supabase.from('commander_team_skills').update({ current_level: 0, target_level: null, is_preferred: false, is_restricted: false, updated_by: req.user?.userId || null }).eq('id', req.params.id).eq('company_id', cid).select().single();
        if (error) throw error;
        if (!data) return res.status(404).json({ error: 'Team skill record not found.' });
        await _writeEvent(cid, 'team_skill_archived', 'team_skill', data.id, req.user?.userId);
        res.json({ team_skill: data, reset: true });
    } catch (err) {
        console.error('DELETE /api/commander/skills-matrix/team-skills/:id', err);
...
router.get('/certifications', async (req, res) => {
    try {
        const cid = req.companyId;
        let q = supabase.from('commander_certifications').select('*').eq('company_id', cid).order('certification_name');
        if (req.query.include_archived !== 'true') q = q.eq('is_active', true);
        const { data, error } = await q;
        if (error) throw error;
        res.json({ certifications: data || [] });
    } catch (err) {
        console.error('GET /api/commander/skills-matrix/certifications', err);
...
router.post('/certifications', async (req, res) => {
    try {
        ...
        const { data, error } = await supabase.from('commander_certifications').insert({
        ...
        console.error('POST /api/commander/skills-matrix/certifications', err);
...
router.put('/certifications/:id', async (req, res) => {
    try {
        ...
        if (patch.category_id !== undefined) patch.category_id = patch.category_id ? parseInt(patch.category_id, 10) : null;
        const { data, error } = await supabase.from('commander_certifications').update(patch).eq('id', req.params.id).eq('company_id', cid).select().single();
        if (error) throw error;
        ...
        console.error('PUT /api/commander/skills-matrix/certifications/:id', err);
...
router.delete('/certifications/:id', async (req, res) => {
    try {
        ...
        const manager = await _requireManager(req, res);
        if (!manager) return;
        const { data, error } = await supabase.from('commander_certifications').update({ is_active: false }).eq('id', req.params.id).eq('company_id', cid).select().single();
        if (error) throw error;
        if (!data) return res.status(404).json({ error: 'Certification not found.' });
        ...
        console.error('DELETE /api/commander/skills-matrix/certifications/:id', err);
...
router.get('/team-certifications', async (req, res) => {
    try {
        const cid = req.companyId;
        const me = await _myTeamMember(cid, req.user);
        let q = supabase.from('commander_team_certifications').select('*, commander_certifications:certification_id(certification_name, issuer)').eq('company_id', cid).eq('is_active', true);
        ...
        console.error('GET /api/commander/skills-matrix/team-certifications', err);
...
router.post('/team-certifications', async (req, res) => {
    try {
        ...
        const { data, error } = await supabase.from('commander_team_certifications').insert({
        ...
        console.error('POST /api/commander/skills-matrix/team-certifications', err);
...
router.put('/team-certifications/:id', async (req, res) => {
    try {
        ...
        const { data, error } = await supabase.from('commander_team_certifications').update(patch).eq('id', req.params.id).eq('company_id', cid).select().single();
        if (error) throw error;
        ...
        console.error('PUT /api/commander/skills-matrix/team-certifications/:id', err);
...
router.delete('/team-certifications/:id', async (req, res) => {
    try {
        ...
        const manager = await _requireManager(req, res);
        if (!manager) return;
        const { data, error } = await supabase.from('commander_team_certifications').update({ is_active: false }).eq('id', req.params.id).eq('company_id', cid).select().single();
        if (error) throw error;
        if (!data) return res.status(404).json({ error: 'Team certification not found.' });
        ...
        console.error('DELETE /api/commander/skills-matrix/team-certifications/:id', err);
...
async function getCompetency(cid, teamMemberId, sourceModule) {
    const [memberRes, skillsRes, certsRes] = await Promise.all([
        supabase.from('commander_team_members').select('id, display_name').eq('id', teamMemberId).eq('company_id', cid).maybeSingle(),
        supabase.from('commander_team_skills').select('*, commander_skills:skill_id(skill_key, display_name, category_id)').eq('company_id', cid).eq('team_member_id', teamMemberId),
        supabase.from('commander_team_certifications').select('*, commander_certifications:certification_id(certification_name)').eq('company_id', cid).eq('team_member_id', teamMemberId).eq('is_active', true),
    ]);
    ...
    const certifications = (certsRes.data || []).map(c => ({
        certification_name: c.commander_certifications?.certification_name || 'Unknown certification',
    ...
    const relevantSkills = relevantSkillKeys.length
        ? allSkills.filter(s => relevantSkillKeys.includes(s.commander_skills?.skill_key))
        : allSkills;
    ...
    const restrictions = allSkills.filter(s => s.is_restricted).map(s => s.commander_skills?.display_name).filter(Boolean);
    const missingCompetencies = allSkills.filter(s => s.target_level != null && s.target_level > s.current_level)
        .map(s => ({ skill: s.commander_skills?.display_name, current_level: s.current_level, target_level: s.target_level }));
    ...
    return {
        ...
        relevant_skills: relevantSkills.map(s => ({
            skill_key: s.commander_skills?.skill_key, display_name: s.commander_skills?.display_name,
...
router.get('/competency/:team_member_id', async (req, res) => {
    try {
        ...
        console.error('GET /api/commander/skills-matrix/competency/:team_member_id', err);
...
router.get('/events', async (req, res) => {
    try {
        const cid = req.companyId;
        let q = supabase.from('commander_skill_events').select('*').eq('company_id', cid).order('created_at', { ascending: false }).limit(200);
        ...
        console.error('GET /api/commander/skills-matrix/events', err);
...
// commander_team_skills/commander_team_certifications themselves.
