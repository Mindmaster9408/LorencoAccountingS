...
const SCORECARD_TYPES = ['partner', 'manager', 'team', 'commander'];
...
async function _writeEvent(cid, scorecardId, reviewId, eventType, oldStatus, newStatus, actorUserId, notes, meta) {
    await supabase.from('commander_partner_scorecard_events').insert({
...
async function _resolveScopeMemberIds(cid, scorecardType, teamMemberId, teamKey) {
    if (scorecardType === 'commander') return null;
    if (scorecardType === 'team') {
        if (!teamKey) throw new Error('team_key is required for a team scorecard.');
        const { data } = await supabase.from('commander_team_members').select('id')
...
async function _profitabilityComponent(cid, memberIds, periodStart, periodEnd) {
    let clientQ = supabase.from('commander_clients').select('id').eq('company_id', cid).eq('is_active', true);
    ...
    const base = { source: 'commander_profitability_snapshots (Codebox 73)', formula: '100 − unprofitable×20 − low_margin×10 − high_writeoffs×5 − low_realization×5 (latest snapshot per owned client)', weight: WEIGHTS.profitability };
    if (!clientIds.length) return { ...base, score: null, confidence: 'none', warning: 'NO_OWNED_CLIENTS' };

    const { data: snapRows } = await supabase.from('commander_profitability_snapshots')
...
async function _qualityComponent(cid, memberIds) {
    let reviewQ = supabase.from('commander_quality_reviews').select('status').eq('company_id', cid);
    let findingQ = supabase.from('commander_quality_findings').select('severity').eq('company_id', cid).in('status', ['open', 'in_progress']);
    ...
    return {
        source: 'commander_quality_reviews + commander_quality_findings (Codebox 48)',
...
async function _clientComponent(cid, memberIds) {
    ...
    const { data: clientRows } = await clientQ;
    const clientIds = (clientRows || []).map(c => c.id);
    const base = { source: 'commander_client_success (Codebox 61)', formula: '100 − watch×3 − at_risk×10 − critical×20 (relationship_status of owned clients)', weight: WEIGHTS.client };
    if (memberIds && !clientIds.length) return { ...base, score: null, confidence: 'none', warning: 'NO_OWNED_CLIENTS' };

    let succQ = supabase.from('commander_client_success').select('relationship_status').eq('company_id', cid);
...
async function _riskComponent(cid, memberIds, rules) {
    ...
    let q = supabase.from('commander_risks').select('inherent_risk').eq('company_id', cid).not('status', 'in', '("closed","cancelled")');
    ...
    return {
        source: 'commander_risks (Codebox 49)', formula: '100 − critical_risk×15 − high_risk×8 − other_open_risk×2',
...
async function _engagementComponent(cid, memberIds) {
    let q = supabase.from('commander_client_engagements')
    ...
    const { data } = await q;
    const rows = data || [];
    const base = { source: 'commander_client_engagements (Codebox 71)', formula: '100 − missing_letter×5 − high_risk_no_acceptance×15 − due_for_review×3 (active-like engagements)', weight: WEIGHTS.engagement };
...
async function _learningComponent(cid, memberIds) {
    const base = { source: 'commander_learning_plans (Codebox 60) + skills-matrix.getCompetency() (Codebox 59)', formula: '100 − missing_competencies×5 − overdue_learning_plans×10, averaged per member', weight: WEIGHTS.learning };
    if (!memberIds) {
        // Practice-wide: average across all active members.
        const { data: allMembers } = await supabase.from('commander_team_members').select('id').eq('company_id', cid).eq('is_active', true);
    ...
    const today = new Date().toISOString().slice(0, 10);
    const { data: planRows } = await supabase.from('commander_learning_plans').select('team_member_id, status, target_completion_date').eq('company_id', cid).in('team_member_id', memberIds);
...
async function _notificationComponent(cid, memberIds) {
    let q = supabase.from('commander_notifications').select('notification_status, severity, due_date').eq('company_id', cid);
    if (memberIds) q = q.in('assigned_team_member_id', memberIds);
    const { data } = await q;
    const rows = data || [];
    const base = { source: 'commander_notifications (Codebox 54)', formula: '100 − overdue×10 − critical_unread×5', weight: WEIGHTS.notification };
...
router.get('/summary', async (req, res) => {
    ...
    try {
        const { data: rows } = await supabase.from('commander_partner_scorecards')
        ...
        const { data: reviewRows } = await supabase.from('commander_partner_scorecard_reviews').select('review_status').eq('company_id', cid);
...
router.get('/practice', async (req, res) => {
    ...
    try {
        res.json({ scorecard: await buildScorecard(req.companyId, { scorecardType: 'commander', periodStart: start, periodEnd: end }) });
    } catch (err) {
...
router.get('/team-keys', async (req, res) => {
    try {
        const { data } = await supabase.from('commander_team_members').select('department').eq('company_id', req.companyId).eq('is_active', true).not('department', 'is', null);
...
router.post('/snapshots', async (req, res) => {
    ...
    try {
        ...
        const { data, error } = await supabase.from('commander_partner_scorecards').insert({
...
router.get('/snapshots', async (req, res) => {
    ...
    try {
        let q = supabase.from('commander_partner_scorecards').select('*').eq('company_id', cid).order('created_at', { ascending: false }).limit(Math.min(500, parseInt(limit) || 200));
        ...
        let nameById = {};
        if (memberIds.length) {
            const { data: members } = await supabase.from('commander_team_members').select('id, display_name').in('id', memberIds).eq('company_id', cid);
...
router.get('/snapshots/:id', async (req, res) => {
    const cid = req.companyId;
    try {
        const { data, error } = await supabase.from('commander_partner_scorecards').select('*').eq('id', req.params.id).eq('company_id', cid).maybeSingle();
        if (error) return res.status(500).json({ error: error.message });
...
router.get('/trends', async (req, res) => {
    ...
    try {
        let q = supabase.from('commander_partner_scorecards').select('id, period_key, generated_at:created_at, overall_score, profitability_score, quality_score, capacity_score, client_score, risk_score, engagement_score, learning_score, planning_score, notification_score')
...
async function _fetchReview(cid, id) {
    const { data } = await supabase.from('commander_partner_scorecard_reviews').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
    return data || null;
}
...
router.get('/reviews', async (req, res) => {
    ...
    try {
        let q = supabase.from('commander_partner_scorecard_reviews').select('*').eq('company_id', cid).order('created_at', { ascending: false });
...
router.post('/reviews', async (req, res) => {
    ...
    const { data: scorecard } = await supabase.from('commander_partner_scorecards').select('id').eq('id', scorecardId).eq('company_id', cid).maybeSingle();
    if (!scorecard) return res.status(404).json({ error: 'Scorecard not found' });

    try {
        const { data, error } = await supabase.from('commander_partner_scorecard_reviews').insert({
...
router.put('/reviews/:id', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_partner_scorecard_reviews').update(update).eq('id', id).eq('company_id', cid).select().single();
        if (error) return res.status(500).json({ error: error.message });
...
router.get('/events', async (req, res) => {
    ...
    try {
        let q = supabase.from('commander_partner_scorecard_events').select('*').eq('company_id', cid).order('created_at', { ascending: false });
        if (scorecard_id) q = q.eq('scorecard_id', parseInt(scorecard_id));
