...
// Mounted at /api/commander/tax-actions
'use strict';
...
async function fetchAction(cid, actionId) {
    const { data } = await supabase
        .from('commander_tax_work_actions')
        .select('*')
        .eq('id', actionId)
        .eq('company_id', cid)
        .single();
...
async function logEvent(cid, actionId, eventType, payload) {
    await supabase.from('commander_tax_work_action_events').insert({
...
const SOURCE_MAP = {
    individual_return:    { table: 'commander_individual_tax_returns',    statusField: 'status',             reviewerField: 'reviewer_team_member_id' },
    company_return:       { table: 'commander_company_tax_returns',       statusField: 'status',             reviewerField: 'reviewer_team_member_id' },
    provisional_plan:     { table: 'commander_provisional_tax_plans',     statusField: 'status',             reviewerField: 'reviewer_team_member_id' },
    individual_calculation: { table: 'commander_individual_tax_calculations', statusField: 'calculation_status', reviewerField: null },
    company_calculation:  { table: 'commander_company_tax_calculations',  statusField: 'calculation_status', reviewerField: null },
    individual_review_pack: { table: 'commander_individual_tax_review_packs', statusField: 'pack_status',    reviewerField: null },
    company_review_pack:  { table: 'commander_company_tax_review_packs',  statusField: 'pack_status',        reviewerField: null },
};
...
router.get('/review-queue', async (req, res) => {
    ...
    try {
        let indQ = supabase.from('commander_individual_tax_returns')
            .select('id, return_name, tax_year, status, readiness_status, reviewer_team_member_id, clients:commander_clients!client_id(name)')
            .eq('company_id', cid).eq('status', 'ready_for_review').neq('status', 'cancelled');
        if (rvId) indQ = indQ.eq('reviewer_team_member_id', rvId);

        let coQ = supabase.from('commander_company_tax_returns')
            .select('id, return_name, tax_year, status, readiness_status, reviewer_team_member_id, clients:commander_clients!client_id(name)')
            .eq('company_id', cid).eq('status', 'ready_for_review');
        if (rvId) coQ = coQ.eq('reviewer_team_member_id', rvId);

        let provQ = supabase.from('commander_provisional_tax_plans')
            .select('id, plan_name, tax_year, status, reviewer_team_member_id, clients:commander_clients!client_id(name)')
            .eq('company_id', cid).eq('status', 'ready_for_review');
        if (rvId) provQ = provQ.eq('reviewer_team_member_id', rvId);

        let indCalcQ = supabase.from('commander_individual_tax_calculations')
        ...
        let coCalcQ = supabase.from('commander_company_tax_calculations')
            .select('id, calculation_name, tax_year, calculation_status, warning_flags')
            .eq('company_id', cid).eq('calculation_status', 'ready_for_review');

        let indPackQ = supabase.from('commander_individual_tax_review_packs')
        ...
        let coPackQ = supabase.from('commander_company_tax_review_packs')
        ...
        const [indRes, coRes, provRes, indCalcRes, coCalcRes, indPackRes, coPackRes, membersRes] =
            await Promise.all([
                ...
                supabase.from('commander_team_members').select('id, display_name').eq('company_id', cid),
            ]);
...
router.post('/from-dashboard-risk', async (req, res) => {
    ...
    const [memberOk, clientOk] = await Promise.all([
        verifyOwn(cid, 'commander_team_members', assigned_team_member_id),
        verifyOwn(cid, 'commander_clients', client_id),
    ]);
    ...
    try {
        const { data, error } = await supabase.from('commander_tax_work_actions').insert({
            company_id:               cid,
...
// ─── POST /:id/create-task ────────────────────────────────────────────────────
// Creates a commander_task linked to this action's client.
...
router.post('/:id/create-task', async (req, res) => {
    ...
    const memberOk = await verifyOwn(cid, 'commander_team_members', assigned_team_member_id);
    if (!memberOk) return res.status(400).json({ error: 'assigned_team_member_id not found in this company' });

    try {
        const { data: task, error: tErr } = await supabase.from('commander_tasks').insert({
            company_id:                 cid,
        ...
        const { error: uErr } = await supabase.from('commander_tax_work_actions')
...
// Creates a commander_document_request linked to this action.
...
router.post('/:id/create-document-request', async (req, res) => {
    ...
    try {
        const { data: docReq, error: dErr } = await supabase.from('commander_document_requests').insert({
            company_id:              cid,
            client_id:               action.client_id,
            request_title:           title,
...
router.post('/:id/assign-reviewer', async (req, res) => {
    ...
    const memberOk = await verifyOwn(cid, 'commander_team_members', reviewer_team_member_id);
...
router.put('/:id/complete', async (req, res) => {
    ...
    try {
        const { error } = await supabase.from('commander_tax_work_actions')
...
router.get('/', async (req, res) => {
    ...
    try {
        let q = supabase.from('commander_tax_work_actions')
            .select('*, assignee:commander_team_members!assigned_team_member_id(display_name)', { count: 'exact' })
            .eq('company_id', cid)
...
router.put('/:id', async (req, res) => {
    ...
    if (updates.assigned_team_member_id) {
        const ok = await verifyOwn(cid, 'commander_team_members', updates.assigned_team_member_id);
    ...
    try {
        const { data, error } = await supabase.from('commander_tax_work_actions')
