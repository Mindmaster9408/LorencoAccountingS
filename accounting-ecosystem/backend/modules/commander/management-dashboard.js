...
async function computeSummary(cid) {
    ...
    const [
            ...
            _count('commander_clients', q => q.eq('company_id', cid).eq('is_active', true)),
            _count('commander_team_members', q => q.eq('company_id', cid).eq('is_active', true)),
            _count('commander_tasks', q => q.eq('company_id', cid).in('status', ['open', 'in_progress'])),
            _count('commander_tasks', q => q.eq('company_id', cid).in('status', ['open', 'in_progress']).lt('due_date', today)),

            supabase.from('commander_team_members').select('id, weekly_capacity_hours').eq('company_id', cid).eq('is_active', true).eq('capacity_is_active', true),
            supabase.from('commander_tasks').select('assigned_to, estimated_hours').eq('company_id', cid).in('status', ['open', 'in_progress']),

            _count('commander_individual_tax_returns', q => q.eq('company_id', cid).not('status', 'in', '("completed","cancelled")')),
            _count('commander_company_tax_returns',    q => q.eq('company_id', cid).not('status', 'in', '("completed","cancelled")')),
            _count('commander_individual_tax_returns', q => q.eq('company_id', cid).eq('status', 'ready_for_review')),
            _count('commander_company_tax_returns',    q => q.eq('company_id', cid).eq('status', 'ready_for_review')),
            _count('commander_individual_tax_returns', q => q.eq('company_id', cid).eq('filing_stage', 'ready_to_submit')),
            _count('commander_company_tax_returns',    q => q.eq('company_id', cid).eq('filing_stage', 'ready_to_submit')),
            _count('commander_individual_tax_returns', q => q.eq('company_id', cid).not('filing_stage', 'in', '("completed","cancelled")')),
            _count('commander_company_tax_returns',    q => q.eq('company_id', cid).not('filing_stage', 'in', '("completed","cancelled")')),
            _count('commander_tax_payments',           q => q.eq('company_id', cid).eq('direction', 'payable').in('status', ['outstanding', 'partially_paid'])),
            _count('commander_sars_statement_lines',   q => q.eq('company_id', cid).in('reconciliation_status', ['unmatched', 'disputed'])),
            _count('commander_tax_dispute_cases',      q => q.eq('company_id', cid).not('case_status', 'in', '("completed","cancelled")')),
            _count('commander_tax_completion_packs',   q => q.eq('company_id', cid).not('pack_status', 'in', '("completed","cancelled")')),

            _count('commander_quality_reviews',  q => q.eq('company_id', cid).not('status', 'in', '("completed","cancelled")')),
            _count('commander_quality_reviews',  q => q.eq('company_id', cid).eq('status', 'failed')),
            _count('commander_quality_reviews',  q => q.eq('company_id', cid).eq('status', 'needs_correction')),
            _count('commander_quality_findings', q => q.eq('company_id', cid).in('status', ['open', 'in_progress'])),
            _count('commander_quality_findings', q => q.eq('company_id', cid).in('status', ['open', 'in_progress']).eq('severity', 'critical')),
            _count('commander_quality_findings', q => q.eq('company_id', cid).in('status', ['open', 'in_progress']).eq('severity', 'high')),

            _count('commander_risks', q => q.eq('company_id', cid).not('status', 'in', '("closed","cancelled")')),
            supabase.from('commander_risks').select('inherent_risk, status').eq('company_id', cid).not('status', 'in', '("closed","cancelled")'),

            _count('commander_clients', q => q.eq('company_id', cid).eq('is_active', true).eq('health_status', 'good')),
            _count('commander_clients', q => q.eq('company_id', cid).eq('is_active', true).eq('health_status', 'watch')),
            _count('commander_clients', q => q.eq('company_id', cid).eq('is_active', true).eq('health_status', 'at_risk')),
            _count('commander_clients', q => q.eq('company_id', cid).eq('is_active', true).eq('health_status', 'critical')),
            _count('commander_clients', q => q.eq('company_id', cid).eq('is_active', true).or('health_status.is.null,health_status.eq.unknown')),

            _count('commander_knowledge_articles', q => q.eq('company_id', cid).eq('status', 'draft')),
            _count('commander_knowledge_articles', q => q.eq('company_id', cid).eq('status', 'under_review')),
            _count('commander_knowledge_articles', q => q.eq('company_id', cid).eq('status', 'approved')),

            _count('commander_sop_templates', q => q.eq('company_id', cid).eq('status', 'draft')),
            _count('commander_sop_templates', q => q.eq('company_id', cid).eq('status', 'under_review')),
            _count('commander_sop_templates', q => q.eq('company_id', cid).eq('status', 'approved')),

            _count('commander_billing_packs', q => q.eq('company_id', cid).eq('status', 'draft')),
            _count('commander_billing_packs', q => q.eq('company_id', cid).eq('status', 'locked')),
            supabase.from('commander_billing_packs').select('recoverable_value, billable_value, status').eq('company_id', cid).not('status', 'in', '("cancelled")'),

            _count('commander_reminders', q => q.eq('company_id', cid).in('status', ['open', 'snoozed']).lt('due_date', reminderOverdueCutoff)),
            _count('commander_reminders', q => q.eq('company_id', cid).in('status', ['open', 'snoozed']).gte('due_date', today).lte('due_date', weekOut)),

            _count('commander_document_requests', q => q.eq('company_id', cid).in('request_status', ['requested', 'reminder_sent', 'partially_received'])),
            _count('commander_document_requests', q => q.eq('company_id', cid).in('request_status', ['requested', 'reminder_sent', 'partially_received']).lt('required_by_date', documentOverdueCutoff)),

            _count('commander_client_communications', q => q.eq('company_id', cid).eq('response_required', true).in('response_status', ['waiting', 'overdue']).is('cancelled_at', null)),

            _count('commander_deadlines', q => q.eq('company_id', cid).in('status', ['open', 'pending', 'in_progress', 'waiting_client', 'waiting_review'])),
            _count('commander_compliance_packs', q => q.eq('company_id', cid).eq('readiness_status', 'blocked')),
        ]);
        ...
        // commander_client_success. Kept out of the main Promise.all above
        // only for readability, matching the same precedent used for
        // client-health snapshot recency in computeExecutiveFeed().
        const relationshipRows = await supabase.from('commander_client_success').select('relationship_status').eq('company_id', cid);
        ...
        const [boOwnerRows, boBlockedRows] = await Promise.all([
            supabase.from('commander_beneficial_owners').select('status, is_reportable').eq('company_id', cid),
            supabase.from('commander_bo_readiness_items').select('client_id').eq('company_id', cid).eq('status', 'blocked').eq('required', true),
        ]);
        ...
        const [lifecycleProfileRows, lifecyclePendingRows] = await Promise.all([
            supabase.from('commander_entity_lifecycle_profiles').select('risk_status, compliance_status').eq('company_id', cid),
            supabase.from('commander_entity_lifecycle_transitions').select('client_id').eq('company_id', cid).in('transition_status', ['ready_for_review', 'approved']),
        ]);
        ...
        const [latestIntegrityRunRes, openIntegrityFindingsRes] = await Promise.all([
            supabase.from('commander_secretarial_integrity_runs').select('overall_score, critical_count, high_count, scan_started_at, passed').eq('company_id', cid).order('scan_started_at', { ascending: false }).limit(1).maybeSingle(),
            supabase.from('commander_secretarial_integrity_findings').select('id', { count: 'exact', head: true }).eq('company_id', cid).eq('status', 'open'),
        ]);
        ...
        const { data: onboardingRows } = await supabase.from('commander_onboarding_profiles')
        ...
        const [engRows, taxClientRows, secClientRows, timeClientRows] = await Promise.all([
            supabase.from('commander_client_engagements').select('client_id, engagement_status, status, risk_level, risk_accepted_by, engagement_letter_status, next_review_date').eq('company_id', cid),
            supabase.from('commander_taxpayer_profiles').select('client_id').eq('company_id', cid),
            supabase.from('commander_secretarial_profiles').select('client_id').eq('company_id', cid),
            supabase.from('commander_time_entries').select('client_id').eq('company_id', cid),
        ]);
        ...
        // operation) for a read-only dashboard load.
        const { data: authRows } = await supabase.from('commander_work_authorizations')
        ...
        const { data: profitRows } = await supabase.from('commander_profitability_snapshots')
        ...
        const { data: pricingRows } = await supabase.from('commander_pricing_reviews')
        ...
        const { data: scorecardRows } = await supabase.from('commander_partner_scorecards')
        ...
        const latestPractice = scorecards.find(s => s.scorecard_type === 'commander') || null;
        ...
        const { data: planRows } = await supabase.from('commander_strategic_plans').select('id, plan_status').eq('company_id', cid);
        ...
        const { data: strategicObjRows } = activePlanIds.length
            ? await supabase.from('commander_strategic_objectives').select('id, objective_status').eq('company_id', cid).in('plan_id', activePlanIds)
            : { data: [] };
        const strategicObjectiveIds = (strategicObjRows || []).map(o => o.id);
        const { data: strategicInitRows } = strategicObjectiveIds.length
            ? await supabase.from('commander_strategic_initiatives').select('initiative_status').eq('company_id', cid).in('objective_id', strategicObjectiveIds)
            : { data: [] };
        const { data: strategicReviewRows } = await supabase.from('commander_strategic_reviews').select('review_status, next_review_date').eq('company_id', cid);
        ...
        const { data: latestReportRows } = await supabase.from('commander_executive_reports')
        ...
        const { count: reportsAwaitingApproval } = await supabase.from('commander_executive_reports')
            .select('id', { count: 'exact', head: true }).eq('company_id', cid).eq('report_status', 'under_review');
        const { count: outstandingExecutiveActions } = await supabase.from('commander_executive_action_register')
        ...
        const { count: activeAutomationRules } = await supabase.from('commander_automation_rules')
            .select('id', { count: 'exact', head: true }).eq('company_id', cid).eq('rule_status', 'active');
        const { data: failedAutomationRunRows } = await supabase.from('commander_automation_runs')
            .select('id').eq('company_id', cid).eq('run_status', 'failed').eq('dry_run', false).order('created_at', { ascending: false }).limit(20);
        const { data: automationWarningRunRows } = await supabase.from('commander_automation_runs')
        ...
        const { data: latestHealthRunRows } = await supabase.from('commander_health_check_runs')
        ...
        // dashboard load.
        const { data: latestReadinessRunRows } = await supabase.from('commander_pilot_readiness_runs')
            .select('id, run_name, readiness_status, overall_score, decision, created_at').eq('company_id', cid)
            .order('created_at', { ascending: false }).limit(1);
        const { count: openCriticalPilotIssues } = await supabase.from('commander_pilot_known_issues')
        ...
        return {
            ...
            partner_scorecards: {
                commander_score: latestPractice ? latestPractice.overall_score : null,
                commander_score_generated_at: latestPractice ? latestPractice.created_at : null,
...
router.get('/summary', async (req, res) => {
    try {
        ...
        console.error('GET /api/commander/management-dashboard/summary', err);
...
async function computeExecutiveFeed(cid, limit) {
    ...
    const [qmsEv, riskEv, disputeEv, completionEv, kbEv, sopEv, comms, reminders, billing] = await Promise.all([
            supabase.from('commander_quality_events').select('id, event_type, old_status, new_status, notes, created_at').eq('company_id', cid).order('created_at', { ascending: false }).limit(limit),
            supabase.from('commander_risk_events').select('id, event_type, old_status, new_status, notes, created_at').eq('company_id', cid).order('created_at', { ascending: false }).limit(limit),
            supabase.from('commander_tax_dispute_events').select('id, event_type, old_status, new_status, notes, created_at').eq('company_id', cid).order('created_at', { ascending: false }).limit(limit),
            supabase.from('commander_tax_completion_events').select('id, event_type, old_status, new_status, notes, created_at').eq('company_id', cid).order('created_at', { ascending: false }).limit(limit),
            supabase.from('commander_knowledge_events').select('id, event_type, old_status, new_status, notes, created_at').eq('company_id', cid).order('created_at', { ascending: false }).limit(limit),
            supabase.from('commander_sop_events').select('id, event_type, old_status, new_status, notes, created_at').eq('company_id', cid).order('created_at', { ascending: false }).limit(limit),
            supabase.from('commander_client_communications').select('id, communication_type, response_status, created_at').eq('company_id', cid).order('created_at', { ascending: false }).limit(limit),
            supabase.from('commander_reminders').select('id, reminder_type, severity, status, created_at').eq('company_id', cid).order('created_at', { ascending: false }).limit(limit),
            supabase.from('commander_billing_packs').select('id, status, period_start, period_end, updated_at').eq('company_id', cid).order('updated_at', { ascending: false }).limit(limit),
        ]);
        ...
        const clientHealth = await supabase.from('commander_client_health_snapshots').select('id, client_id, health_status, calculated_at').eq('company_id', cid).order('calculated_at', { ascending: false }).limit(limit);
...
router.get('/executive-feed', async (req, res) => {
    try {
        ...
        console.error('GET /api/commander/management-dashboard/executive-feed', err);
...
async function computeAlerts(cid) {
    ...
    const [
            ...
            supabase.from('commander_risks').select('id, title, inherent_risk').eq('company_id', cid).not('status', 'in', '("closed","cancelled")').gte('inherent_risk', riskCriticalMin),
            supabase.from('commander_quality_findings').select('id, finding_title, review_id').eq('company_id', cid).in('status', ['open', 'in_progress']).eq('severity', 'critical'),
            supabase.from('commander_quality_reviews').select('id, review_title').eq('company_id', cid).eq('status', 'failed'),

            supabase.from('commander_risks').select('id, title, inherent_risk').eq('company_id', cid).not('status', 'in', '("closed","cancelled")').gte('inherent_risk', riskHighMin).lt('inherent_risk', riskCriticalMin),
            supabase.from('commander_quality_findings').select('id, finding_title, review_id').eq('company_id', cid).in('status', ['open', 'in_progress']).eq('severity', 'high'),

            supabase.from('commander_tasks').select('id, title').eq('company_id', cid).in('status', ['open', 'in_progress']).lt('due_date', today).limit(20),
            supabase.from('commander_reminders').select('id, reminder_type, due_date').eq('company_id', cid).in('status', ['open', 'snoozed']).lt('due_date', reminderOverdueCutoff).limit(20),
            supabase.from('commander_document_requests').select('id, document_category, required_by_date').eq('company_id', cid).in('request_status', ['requested', 'reminder_sent', 'partially_received']).lt('required_by_date', documentOverdueCutoff).limit(20),
            supabase.from('commander_deadlines').select('id, title, due_date').eq('company_id', cid).in('status', ['open', 'pending', 'in_progress', 'waiting_client', 'waiting_review']).lt('due_date', deadlineOverdueCutoff).limit(20),

            supabase.from('commander_compliance_packs').select('id, pack_type').eq('company_id', cid).eq('readiness_status', 'blocked'),
            supabase.from('commander_individual_tax_returns').select('id').eq('company_id', cid).eq('readiness_status', 'blocked'),
            supabase.from('commander_company_tax_returns').select('id').eq('company_id', cid).eq('readiness_status', 'blocked'),

            supabase.from('commander_knowledge_articles').select('id, title').eq('company_id', cid).eq('status', 'under_review'),
            supabase.from('commander_sop_templates').select('id, title').eq('company_id', cid).eq('status', 'under_review'),
            supabase.from('commander_risks').select('id, title, inherent_risk').eq('company_id', cid).eq('status', 'open').gte('inherent_risk', riskPartnerAcceptanceMin),

            supabase.from('commander_billing_packs').select('id, period_start, period_end').eq('company_id', cid).eq('status', 'reviewed'),
        ]);
...
router.get('/alerts', async (req, res) => {
    try {
        ...
        console.error('GET /api/commander/management-dashboard/alerts', err);
...
async function computePartnerReview(cid) {
    ...
    const [kb, sop, completion, qmsInReview, riskAccept, billingApproval] = await Promise.all([
        supabase.from('commander_knowledge_articles').select('id, title, category, updated_at').eq('company_id', cid).eq('status', 'under_review').order('updated_at', { ascending: true }),
        supabase.from('commander_sop_templates').select('id, title, category, updated_at').eq('company_id', cid).eq('status', 'under_review').order('updated_at', { ascending: true }),
        supabase.from('commander_tax_completion_packs').select('id, client_id, source_type, updated_at').eq('company_id', cid).eq('pack_status', 'review_pending').order('updated_at', { ascending: true }),
        supabase.from('commander_quality_reviews').select('id, review_title, review_type, updated_at').eq('company_id', cid).eq('status', 'in_review').order('updated_at', { ascending: true }),
        supabase.from('commander_risks').select('id, title, category, inherent_risk, updated_at').eq('company_id', cid).eq('status', 'open').gte('inherent_risk', riskPartnerAcceptanceMin).order('inherent_risk', { ascending: false }),
        supabase.from('commander_billing_packs').select('id, period_start, period_end, updated_at').eq('company_id', cid).eq('status', 'reviewed').order('updated_at', { ascending: true }),
    ]);
...
router.get('/partner-review', async (req, res) => {
    try {
        ...
        console.error('GET /api/commander/management-dashboard/partner-review', err);
...
async function computePracticeScore(cid) {
    ...
    const [
            ...
            _count('commander_quality_findings', q => q.eq('company_id', cid).in('status', ['open', 'in_progress']).eq('severity', 'critical')),
            _count('commander_quality_findings', q => q.eq('company_id', cid).in('status', ['open', 'in_progress']).eq('severity', 'high')),
            _count('commander_quality_findings', q => q.eq('company_id', cid).in('status', ['open', 'in_progress']).eq('severity', 'medium')),
            _count('commander_quality_findings', q => q.eq('company_id', cid).in('status', ['open', 'in_progress']).eq('severity', 'low')),
            _count('commander_deadlines', q => q.eq('company_id', cid).in('status', ['open', 'pending', 'in_progress', 'waiting_client', 'waiting_review']).lt('due_date', deadlineOverdueCutoff)),
            _count('commander_compliance_packs', q => q.eq('company_id', cid).eq('readiness_status', 'blocked')),
            _count('commander_deadlines', q => q.eq('company_id', cid).eq('status', 'missed')),
            _count('commander_risks', q => q.eq('company_id', cid).not('status', 'in', '("closed","cancelled")').gte('inherent_risk', riskCriticalMin)),
            _count('commander_risks', q => q.eq('company_id', cid).not('status', 'in', '("closed","cancelled")').gte('inherent_risk', riskHighMin).lt('inherent_risk', riskCriticalMin)),
            _count('commander_risks', q => q.eq('company_id', cid).not('status', 'in', '("closed","cancelled")').lt('inherent_risk', riskHighMin)),
            supabase.from('commander_team_members').select('id, weekly_capacity_hours').eq('company_id', cid).eq('is_active', true).eq('capacity_is_active', true),
            supabase.from('commander_tasks').select('assigned_to, estimated_hours').eq('company_id', cid).in('status', ['open', 'in_progress']),
            _count('commander_tax_payments', q => q.eq('company_id', cid).eq('direction', 'payable').in('status', ['outstanding', 'partially_paid'])),
            _count('commander_sars_statement_lines', q => q.eq('company_id', cid).in('reconciliation_status', ['unmatched', 'disputed'])),
...
router.get('/practice-score', async (req, res) => {
    try {
        ...
        console.error('GET /api/commander/management-dashboard/practice-score', err);
