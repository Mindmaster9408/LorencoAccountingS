...
async function _writeEvent(cid, snapshotId, eventType, actorUserId, notes, meta) {
    await supabase.from('commander_resource_forecast_events').insert({
...
// are exact."). Only commander_tasks carries a genuine per-item estimate
...
router.get('/summary', async (req, res) => {
    try {
        ...
        console.error('GET /api/commander/resource-forecasting/summary', err);
...
router.get('/forecast', async (req, res) => {
    try {
        ...
        console.error('GET /api/commander/resource-forecasting/forecast', err);
    ...
    try {
        ...
        const team = teamCapacity.map(cap => {
            ...
            return {
                ...
                work_queue_link: '/commander/work-queue.html?team_member_id=' + cap.member_id,
            };
        }).sort((a, b) => {
        ...
        console.error('GET /api/commander/resource-forecasting/team', err);
...
router.get('/clients', async (req, res) => {
    try {
        ...
        console.error('GET /api/commander/resource-forecasting/clients', err);
...
router.get('/deadlines', async (req, res) => {
    try {
        ...
        const { data, error } = await supabase.from('commander_deadlines')
            .select('id, title, due_date, status, priority, responsible_team_member_id, client_id, commander_clients:client_id(name), commander_team_members!responsible_team_member_id(display_name)')
            .eq('company_id', cid)
        ...
        const deadlines = (data || []).map(d => {
            ...
            return {
                id: d.id, title: d.title, client_name: d.commander_clients?.name || null,
                due_date: d.due_date, is_overdue: isOverdue,
                owner_team_member_id: d.responsible_team_member_id,
                owner_name: d.commander_team_members?.display_name || 'Unassigned',
        ...
        console.error('GET /api/commander/resource-forecasting/deadlines', err);
...
async function _computeDeadlineForecast(cid, weekStart, weeks) {
    ...
    return (data || []).map(d => {
        ...
        const weekBucket = d.due_date < weekStart ? 0 : (d.due_date <= windowEnd ? Math.floor(_daysBetween(weekStart, d.due_date) / 7) : null);
        return {
            id: d.id, title: d.title, client_name: d.commander_clients?.name || null, due_date: d.due_date, is_overdue: isOverdue,
            owner_name: d.commander_team_members?.display_name || 'Unassigned',
            estimated_hours: 0.5, confidence: 'default_placeholder',
...
router.post('/snapshots', async (req, res) => {
    try {
        ...
        const { data, error } = await supabase.from('commander_resource_forecast_snapshots').insert({
        ...
        console.error('POST /api/commander/resource-forecasting/snapshots', err);
...
router.get('/snapshots', async (req, res) => {
    try {
        ...
        let q = supabase.from('commander_resource_forecast_snapshots')
        ...
        console.error('GET /api/commander/resource-forecasting/snapshots', err);
...
router.get('/snapshots/:id', async (req, res) => {
    try {
        ...
        const { data, error } = await supabase.from('commander_resource_forecast_snapshots').select('*').eq('id', req.params.id).eq('company_id', cid).maybeSingle();
        if (error) throw error;
        ...
        console.error('GET /api/commander/resource-forecasting/snapshots/:id', err);
...
router.delete('/snapshots/:id', async (req, res) => {
    try {
        ...
        const { data: existing, error: fetchErr } = await supabase.from('commander_resource_forecast_snapshots').select('id, status').eq('id', req.params.id).eq('company_id', cid).maybeSingle();
        ...
        const { data, error } = await supabase.from('commander_resource_forecast_snapshots').update({ status: 'archived' }).eq('id', existing.id).eq('company_id', cid).select().single();
        if (error) throw error;

        await _writeEvent(cid, existing.id, 'forecast_snapshot_archived', req.user?.userId);
        res.json({ snapshot: data, archived: true });
    } catch (err) {
        console.error('DELETE /api/commander/resource-forecasting/snapshots/:id', err);
...
router.get('/snapshots/:id/events', async (req, res) => {
    try {
        ...
        const { data: existing } = await supabase.from('commander_resource_forecast_snapshots').select('id').eq('id', req.params.id).eq('company_id', cid).maybeSingle();
        if (!existing) return res.status(404).json({ error: 'Forecast snapshot not found.' });

        const { data, error } = await supabase.from('commander_resource_forecast_events').select('*').eq('company_id', cid).eq('snapshot_id', existing.id).order('created_at', { ascending: false });
        if (error) throw error;

        res.json({ events: data || [] });
    } catch (err) {
        console.error('GET /api/commander/resource-forecasting/snapshots/:id/events', err);
