/* =============================================================
   ...
   Mounted at /api/commander/tax-configs
...
async function verifyConfigOwnership(configId, companyId) {
    ...
    let q = supabase
        .from('commander_tax_year_configs')
        .select('*')
        .eq('id', configId);
...
async function logConfigEvent(configId, companyId, eventType, extras = {}) {
    try {
        await supabase.from('commander_tax_config_events').insert({
            config_id:     configId,
    ...
    for (const [yearStr, consts] of Object.entries(TAX_YEAR_CONSTANTS)) {
        ...
        const { data: newConfig, error: cfgErr } = await supabase
            ...
            .insert({
                ...
                // main_and_first_dependant is a single SARS-published rate covering both
                // the main member and the first dependent (same monthly credit for each).
                medical_credit_main_member:    consts.medical_credits_monthly?.main_and_first_dependant || null,
                medical_credit_first_dependent:consts.medical_credits_monthly?.main_and_first_dependant || null,
                medical_credit_additional_dep: consts.medical_credits_monthly?.additional_dependant     || null,
                tax_threshold_under_65:        consts.thresholds?.below_65   || null,
                tax_threshold_65_to_74:        consts.thresholds?.age_65_74  || null,
                tax_threshold_75_plus:         consts.thresholds?.age_75_plus || null,
...
router.post('/:id/brackets', async (req, res) => {
    ...
    await auditFromReq(req, 'CREATE', 'commander_tax_bracket', data.id, { module: 'commander' });
...
router.get('/:id/events', async (req, res) => {
    ...
    const { data, error } = await supabase
        .from('commander_tax_config_events')
        .select('*')
        .eq('config_id', cfg.id)
...
router.put('/:id/activate', async (req, res) => {
    ...
    await auditFromReq(req, 'UPDATE', 'commander_tax_year_config', cfg.id, { module: 'commander', action: 'activate' });
...
router.put('/:id/archive', async (req, res) => {
    ...
    await auditFromReq(req, 'UPDATE', 'commander_tax_year_config', cfg.id, { module: 'commander', action: 'archive' });
...
router.put('/:id/lock', async (req, res) => {
    ...
    await auditFromReq(req, 'UPDATE', 'commander_tax_year_config', cfg.id, { module: 'commander', action: 'lock' });
...
router.put('/:id', async (req, res) => {
    ...
    await auditFromReq(req, 'UPDATE', 'commander_tax_year_config', cfg.id, { module: 'commander' });
...
router.post('/', async (req, res) => {
    ...
    await auditFromReq(req, 'CREATE', 'commander_tax_year_config', data.id, { module: 'commander' });
