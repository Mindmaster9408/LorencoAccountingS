...
async function buildTeamCapacity(cid) {
  ...
  const [members, tasks, deadlines, engagements, periods] = await Promise.all([
    ...
    supabase.from('commander_engagement_periods')
      .select('id, engagement_id, status, commander_client_engagements:engagement_id(responsible_team_member_id)')
      .eq('company_id', cid)
  ...
  for (const p of (periods.data || [])) {
    const eng = p.commander_client_engagements;
...
router.get('/risks', async (req, res) => {
  ...
  try {
    const [
      ...
      supabase.from('commander_tasks')
        .select('id, title, due_date, commander_clients:client_id(name)')
        .eq('company_id', cid)
      ...
      supabase.from('commander_engagement_periods')
        .select('id, period_label, commander_clients:client_id(name), commander_client_engagements:engagement_id(engagement_name, workflow_template_id)')
        .eq('company_id', cid)
        .in('status', ['queued', 'ready'])
      ...
      supabase.from('commander_tasks')
        .select('id, title, due_date, commander_clients:client_id(name), preparer:preparer_team_member_id(display_name)')
        .eq('company_id', cid)
    ...
    const periodsWithNoTemplate = (periodsNoTemplate.data || []).filter(p => {
      return !p.commander_client_engagements?.workflow_template_id;
    });
    ...
    res.json({
      ...
      queued_periods_without_template: periodsWithNoTemplate.map(p => ({
        ...
        client_name:     p.commander_clients?.name || null,
        engagement_name: p.commander_client_engagements?.engagement_name || null,
      })),
