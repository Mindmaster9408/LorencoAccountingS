...
// (Codebox 15/16: commander_service_catalog, commander_client_engagements,
// commander_client_engagement_events, commander_engagement_periods — router
// modules/commander/engagements.js, 638 lines, mounted at the practice
...
async function _fetchEngagement(cid, id) {
    const { data } = await supabase.from('commander_client_engagements').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
    return data || null;
}

async function _writeEvent(cid, clientId, engagementId, letterId, eventType, oldStatus, newStatus, actorUserId, notes, meta) {
    await supabase.from('commander_engagement_management_events').insert({
...
async function getClientEngagementProfile(cid, clientId) {
    ...
    const { data: engagements } = await supabase.from('commander_client_engagements').select('*').eq('company_id', cid).eq('client_id', clientId).order('created_at', { ascending: false });
...
async function _detectPossibleGaps(cid, clientId, activeEngagements) {
    ...
    try {
        const { count: taxCount } = await supabase.from('commander_taxpayer_profiles').select('id', { count: 'exact', head: true }).eq('company_id', cid).eq('client_id', clientId);
    ...
    try {
        const { count: paye } = await supabase.from('commander_taxpayer_profiles').select('id', { count: 'exact', head: true }).eq('company_id', cid).eq('client_id', clientId).eq('paye_registered', true);
    ...
    try {
        const [secProfile, changeCases] = await Promise.all([
            supabase.from('commander_secretarial_profiles').select('id', { count: 'exact', head: true }).eq('company_id', cid).eq('client_id', clientId),
            supabase.from('commander_secretarial_change_cases').select('id', { count: 'exact', head: true }).eq('company_id', cid).eq('client_id', clientId),
        ]);
    ...
    try {
        const { count: timeEntries } = await supabase.from('commander_time_entries').select('id', { count: 'exact', head: true }).eq('company_id', cid).eq('client_id', clientId);
...
router.get('/summary', async (req, res) => {
    ...
    try {
        const { data: rows } = await supabase.from('commander_client_engagements')
...
router.get('/', async (req, res) => {
    ...
    try {
        let q = supabase.from('commander_client_engagements').select('*').eq('company_id', cid).order('created_at', { ascending: false }).limit(Math.min(500, parseInt(limit) || 200));
...
router.post('/', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_client_engagements').insert(insertRow).select().single();
        if (error) return res.status(500).json({ error: error.message });
...
router.get('/:id', async (req, res) => {
    ...
    try {
        ...
        const { data: letters } = await supabase.from('commander_engagement_letters').select('*').eq('company_id', cid).eq('engagement_id', id).order('created_at', { ascending: false });
...
router.put('/:id', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_client_engagements').update(update).eq('id', id).eq('company_id', cid).select().single();
        if (error) return res.status(500).json({ error: error.message });
...
router.delete('/:id', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_client_engagements')
...
Object.keys(TRANSITIONS).forEach(action => {
    router.put(`/:id/${action}`, async (req, res) => {
        ...
        if (['activate', 'resume'].includes(action) && ['high', 'critical'].includes(eng.risk_level) && !eng.risk_accepted_by) {
            ...
            await supabase.from('commander_client_engagements').update({
...
router.get('/:id/letters', async (req, res) => {
    ...
    if (!id) return res.status(400).json({ error: 'Invalid engagement ID' });
    try {
        const { data, error } = await supabase.from('commander_engagement_letters').select('*').eq('company_id', cid).eq('engagement_id', id).order('created_at', { ascending: false });
        if (error) return res.status(500).json({ error: error.message });
...
router.post('/:id/letters', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_engagement_letters').insert({
        ...
        if (['not_required', 'required'].includes(eng.engagement_letter_status)) {
            await supabase.from('commander_client_engagements').update({ engagement_letter_status: 'drafted', updated_at: new Date().toISOString() }).eq('id', id).eq('company_id', cid);
        }
...
async function _fetchLetter(cid, letterId) {
    const { data } = await supabase.from('commander_engagement_letters').select('*').eq('id', letterId).eq('company_id', cid).maybeSingle();
    return data || null;
}
...
router.put('/letters/:letterId', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_engagement_letters').update(update).eq('id', letterId).eq('company_id', cid).select().single();
        if (error) return res.status(500).json({ error: error.message });
...
Object.keys(LETTER_ACTIONS).forEach(action => {
    ...
    router.put(`/letters/:letterId/${action}`, async (req, res) => {
        ...
        try {
            ...
            await supabase.from('commander_client_engagements').update(engUpdate).eq('id', existing.engagement_id).eq('company_id', cid);
...
router.delete('/letters/:letterId', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_engagement_letters').update({ letter_status: 'cancelled', updated_by: req.user.userId, updated_at: new Date().toISOString() }).eq('id', letterId).eq('company_id', cid).select().single();
        if (error) return res.status(500).json({ error: error.message });
        res.json({ letter: data });
    } catch (err) {
...
router.get('/:id/events', async (req, res) => {
    ...
    if (!id) return res.status(400).json({ error: 'Invalid engagement ID' });
    try {
        const { data, error } = await supabase.from('commander_engagement_management_events').select('*').eq('company_id', cid).eq('engagement_id', id).order('created_at', { ascending: false });
        if (error) return res.status(500).json({ error: error.message });
