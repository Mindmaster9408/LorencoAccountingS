...
async function _verifyPaymentEvent(eventId, cid) {
    const { data } = await supabase
        .from('commander_tax_payment_events')
...
async function _writeReconEvent(cid, lineId, eventType, opts) {
    await supabase.from('commander_sars_statement_reconciliation_events').insert({
        company_id:       cid,
...
router.get('/lines/:id/events', async (req, res) => {
    ...
    try {
        ...
        const { data, error } = await supabase
            .from('commander_sars_statement_reconciliation_events')
