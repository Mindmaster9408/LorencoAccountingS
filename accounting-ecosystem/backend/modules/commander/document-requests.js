/* =============================================================
   ...
   Mounted at /api/commander/document-requests
   ============================================================= */
'use strict';
...
async function verifyRequestOwnership(cid, reqId) {
    const { data } = await supabase
        .from('commander_document_requests')
        .select('*')
        .eq('id', reqId)
        .eq('company_id', cid)
...
router.get('/checklists', async (req, res) => {
    ...
    let q = supabase
        .from('commander_document_checklists')
        .select('*')
...
router.post('/checklists', async (req, res) => {
    ...
    const { data, error } = await supabase
        ...
        .select().single();
    if (error) return res.status(500).json({ error: error.message });
    await auditFromReq(req, 'CREATE', 'commander_document_checklist', data.id, { module: 'commander' });
...
router.get('/checklists/:id/items', async (req, res) => {
    const { data: cl } = await supabase
        .from('commander_document_checklists').select('id').eq('id', req.params.id).eq('company_id', req.companyId).single();
    if (!cl) return res.status(404).json({ error: 'Checklist not found' });

    const { data, error } = await supabase
        .from('commander_document_checklist_items')
        .select('*')
...
router.post('/checklists/:id/items', async (req, res) => {
    ...
    const { data: existing } = await supabase
        .from('commander_document_checklist_items').select('sort_order').eq('checklist_id', req.params.id).order('sort_order', { ascending: false }).limit(1).single();
...
router.post('/checklists/:id/apply', async (req, res) => {
    ...
    const [clOk, clientOk] = await Promise.all([
        supabase.from('commander_document_checklists').select('id, checklist_name').eq('id', checklistId).eq('company_id', cid).eq('is_active', true).single(),
        verifyBelongsToCompany(cid, 'commander_clients', parseInt(client_id)),
    ]);
    ...
    await auditFromReq(req, 'CREATE', 'commander_document_checklist_apply', checklistId, {
        module: 'commander',
...
router.put('/checklists/:id', async (req, res) => {
    ...
    const { data, error } = await supabase
        ...
        .select().single();
    if (error) return res.status(500).json({ error: error.message });
    await auditFromReq(req, 'UPDATE', 'commander_document_checklist', data.id, { module: 'commander' });
...
router.delete('/checklists/:id', async (req, res) => {
    ...
    if (error) return res.status(500).json({ error: error.message });
    await auditFromReq(req, 'DEACTIVATE', 'commander_document_checklist', parseInt(req.params.id), { module: 'commander' });
    res.json({ success: true });
});
...
router.get('/', async (req, res) => {
    ...
    if (search) {
        ...
        results = results.filter(r =>
            ...
            (r.commander_clients?.name && r.commander_clients.name.toLowerCase().includes(s))
        );
    }
...
router.post('/', async (req, res) => {
    ...
    const [clientOk, memberOk, taskOk, deadlineOk, engOk, commOk] = await Promise.all([
        verifyBelongsToCompany(cid, 'commander_clients',               parseInt(client_id)),
        verifyBelongsToCompany(cid, 'commander_team_members',          assigned_team_member_id),
        verifyBelongsToCompany(cid, 'commander_tasks',                 related_task_id),
        verifyBelongsToCompany(cid, 'commander_deadlines',             related_deadline_id),
        verifyBelongsToCompany(cid, 'commander_client_engagements',    related_engagement_id),
        verifyBelongsToCompany(cid, 'commander_client_communications', related_communication_id),
    ]);
    if (!clientOk)   return res.status(400).json({ error: 'client_id not found in this company' });
    ...
    await auditFromReq(req, 'CREATE', 'commander_document_request', data.id, {
        module: 'commander', action: 'document_requested', client_id: data.client_id,
    });
...
router.get('/:id', async (req, res) => {
    const { data, error } = await supabase
        .from('commander_document_requests')
        .select(`*, commander_clients:client_id(name), assignee:assigned_team_member_id(id, display_name)`)
        .eq('id', req.params.id)
        .eq('company_id', req.companyId)
        .single();
...
router.put('/:id/received', async (req, res) => {
    ...
    await auditFromReq(req, 'UPDATE', 'commander_document_request', data.id, {
        module: 'commander', action: 'document_received',
    });
    res.json({ document_request: enrichRequest(data) });
});
...
router.put('/:id/reminder-sent', async (req, res) => {
    ...
    await auditFromReq(req, 'UPDATE', 'commander_document_request', data.id, {
        module: 'commander', action: 'document_reminder_sent',
...
router.put('/:id/waive', async (req, res) => {
    ...
    await auditFromReq(req, 'UPDATE', 'commander_document_request', data.id, {
        module: 'commander', action: 'document_waived',
    });
    res.json({ document_request: enrichRequest(data) });
});
...
router.put('/:id', async (req, res) => {
    ...
    await auditFromReq(req, 'UPDATE', 'commander_document_request', data.id, { module: 'commander' });
    res.json({ document_request: enrichRequest(data) });
});
...
router.delete('/:id', async (req, res) => {
    ...
    await auditFromReq(req, 'UPDATE', 'commander_document_request', parseInt(req.params.id), {
        module: 'commander', action: 'document_request_cancelled',
    });
    res.json({ success: true });
});
