...
async function _writeEvent(cid, clientId, sourceType, sourceId, eventType, oldStatus, newStatus, actorUserId, notes, meta) {
    await supabase.from('commander_secretarial_integrity_events').insert({
...
async function runIntegrityAudit(cid, actorUserId, runType) {
    ...
    const { data: run, error: runError } = await supabase.from('commander_secretarial_integrity_runs')
    ...
    const [
        ...
        _fetchSafe('clients', supabase.from('commander_clients').select('id, name, registration_number').eq('company_id', cid).eq('is_active', true)),
        _fetchSafe('secretarial_profiles', supabase.from('commander_secretarial_profiles').select('id, client_id, company_type, registered_address, company_status').eq('company_id', cid)),
        _fetchSafe('directors', supabase.from('commander_company_directors').select('id, client_id, director_name, status').eq('company_id', cid)),
        _fetchSafe('shareholders', supabase.from('commander_company_shareholders').select('id, client_id, shareholder_name, percentage, status').eq('company_id', cid)),
        _fetchSafe('annual_returns', supabase.from('commander_annual_returns').select('id, client_id, return_year, due_date, status').eq('company_id', cid)),
        _fetchSafe('taxpayer_profiles', supabase.from('commander_taxpayer_profiles').select('id, client_id, financial_year_end').eq('company_id', cid)),
        _fetchSafe('lifecycle_profiles', supabase.from('commander_entity_lifecycle_profiles').select('id, client_id, current_lifecycle_status, trading_status').eq('company_id', cid)),
        _fetchSafe('lifecycle_transitions', supabase.from('commander_entity_lifecycle_transitions').select('id, client_id, lifecycle_profile_id, transition_type, transition_status, approved_at, implemented_at').eq('company_id', cid)),
        _fetchSafe('change_cases', supabase.from('commander_secretarial_change_cases').select('id, client_id, change_type, case_status').eq('company_id', cid)),
        _fetchSafe('resolutions', supabase.from('commander_secretarial_resolutions').select('id, client_id, change_case_id').eq('company_id', cid)),
        _fetchSafe('meetings', supabase.from('commander_secretarial_meetings').select('id, client_id, change_case_id').eq('company_id', cid)),
        _fetchSafe('decisions', supabase.from('commander_secretarial_decisions').select('id, client_id, meeting_id, resolution_id, change_case_id').eq('company_id', cid)),
        _fetchSafe('bo_owners', supabase.from('commander_beneficial_owners').select('id, client_id, status').eq('company_id', cid)),
        _fetchSafe('bo_chains', supabase.from('commander_ownership_chains').select('id, client_id, chain_name, ultimate_owner_id').eq('company_id', cid)),
        _fetchSafe('bo_readiness_items', supabase.from('commander_bo_readiness_items').select('id, client_id, item_name, beneficial_owner_id, ownership_chain_id, status, required').eq('company_id', cid)),
        _fetchSafe('evidence_checklists', supabase.from('commander_secretarial_evidence_checklists').select('id, client_id, title, source_type, source_id, template_id').eq('company_id', cid)),
        _fetchSafe('evidence_templates', supabase.from('commander_secretarial_evidence_templates').select('id').eq('company_id', cid)),
        _fetchSafe('statutory_obligations', supabase.from('commander_statutory_obligations').select('id').eq('company_id', cid)),
        _fetchSafe('statutory_dependencies', supabase.from('commander_statutory_dependencies').select('id, client_id, schedule_id, depends_on_schedule_id, depends_on_checklist_id').eq('company_id', cid)),
    ]);
    ...
    if (findings.length) {
        ...
        const { data, error } = await supabase.from('commander_secretarial_integrity_findings').insert(rows).select();
        ...
        await supabase.from('commander_secretarial_integrity_events').insert(eventRows);
    }

    const { data: updatedRun, error: updateError } = await supabase.from('commander_secretarial_integrity_runs')
        .update({
...
router.get('/summary', async (req, res) => {
    ...
    try {
        const [latestRunRes, openFindingsRes] = await Promise.all([
            supabase.from('commander_secretarial_integrity_runs').select('*').eq('company_id', cid).order('scan_started_at', { ascending: false }).limit(1).maybeSingle(),
            supabase.from('commander_secretarial_integrity_findings').select('severity').eq('company_id', cid).eq('status', 'open'),
        ]);
...
router.get('/runs', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_secretarial_integrity_runs').select('*')
...
router.get('/runs/:id', async (req, res) => {
    ...
    try {
        const [runRes, findingsRes] = await Promise.all([
            supabase.from('commander_secretarial_integrity_runs').select('*').eq('id', id).eq('company_id', cid).maybeSingle(),
            supabase.from('commander_secretarial_integrity_findings').select('*').eq('run_id', id).eq('company_id', cid),
        ]);
...
router.get('/findings', async (req, res) => {
    ...
    try {
        let q = supabase.from('commander_secretarial_integrity_findings').select('*').eq('company_id', cid).order('created_at', { ascending: false }).limit(Math.min(500, parseInt(limit) || 200));
        ...
        let nameById = {};
        if (clientIds.length) {
            const { data: clientRows } = await supabase.from('commander_clients').select('id, name').in('id', clientIds).eq('company_id', cid);
...
router.get('/findings/:id', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_secretarial_integrity_findings').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
        if (error) return res.status(500).json({ error: error.message });
...
router.put('/findings/:id', async (req, res) => {
    ...
    const member = await _requireManager(req, res);
    if (!member) return;
    const existing = await supabase.from('commander_secretarial_integrity_findings').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
    if (!existing.data) return res.status(404).json({ error: 'Finding not found' });
    try {
        const { data, error } = await supabase.from('commander_secretarial_integrity_findings')
...
Object.keys(FINDING_ACTIONS).forEach(path => {
    ...
    router.put(`/findings/:id/${path}`, async (req, res) => {
        ...
        try {
            ...
            const { data, error } = await supabase.from('commander_secretarial_integrity_findings').update(update).eq('id', id).eq('company_id', cid).select().single();
            if (error) return res.status(500).json({ error: error.message });
...
router.get('/events', async (req, res) => {
    ...
    try {
        let q = supabase.from('commander_secretarial_integrity_events').select('*').eq('company_id', cid).order('created_at', { ascending: false }).limit(Math.min(500, parseInt(limit) || 100));
        if (client_id) q = q.eq('client_id', parseInt(client_id));
        const { data, error } = await q;
        if (error) return res.status(500).json({ error: error.message });
