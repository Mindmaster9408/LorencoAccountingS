/* =============================================================
   ...
   Mounted at /api/commander/communications
   ============================================================= */
'use strict';
...
const VISIBILITIES     = ['commander', 'manager_only', 'partner_only'];
...
async function verifyCommOwnership(cid, commId) {
    const { data } = await supabase
        .from('commander_client_communications')
        .select('*')
        .eq('id', commId)
        .eq('company_id', cid)
...
router.get('/', async (req, res) => {
    ...
    try {
        ...
        const [clientsRes, membersRes] = await Promise.all([
            clientIds.length
                ? supabase.from('commander_clients').select('id, name').in('id', clientIds).eq('company_id', cid)
                : { data: [] },
            memberIds.length
                ? supabase.from('commander_team_members').select('id, display_name').in('id', memberIds).eq('company_id', cid)
                : { data: [] },
        ]);
...
router.post('/', async (req, res) => {
    ...
    const {
        ...
        is_internal = false, visibility = 'commander',
    ...
    try {
        ...
        const [clientOk, taskOk, deadlineOk, engOk, reminderOk, healthOk, memberOk] = await Promise.all([
            ...
            verifyBelongsToCompany(cid, 'commander_reminders',             related_reminder_id),
            verifyBelongsToCompany(cid, 'commander_client_health_actions', related_health_action_id),
        ...
        const { data, error } = await supabase
            ...
            .insert({
                ...
                visibility:              visibility || 'commander',
        ...
        await auditFromReq(req, 'CREATE', 'commander_client_communication', data.id, {
            module: 'commander', action: 'communication_created',
    ...
    try {
        ...
        const { data: reminder, error: remErr } = await supabase
            ...
            .insert({
                ...
                action_url:              '/commander/communications.html',
                created_at:              now,
                updated_at:              now,
                created_by:              req.user.userId,
            })
        ...
        await auditFromReq(req, 'CREATE', 'commander_reminder', reminder.id, {
            module: 'commander', action: 'communication_reminder_created',
...
router.post('/:id/create-task', async (req, res) => {
    ...
    try {
        ...
        await auditFromReq(req, 'CREATE', 'commander_task', task.id, {
            module: 'commander', action: 'communication_followup_task_created',
            communication_id: commId, client_id: comm.client_id,
        });
...
router.put('/:id/mark-responded', async (req, res) => {
    ...
    try {
        ...
        await auditFromReq(req, 'UPDATE', 'commander_client_communication', commId, {
            module: 'commander', action: 'communication_response_received',
        });
...
router.put('/:id/cancel-response', async (req, res) => {
    ...
    try {
        ...
        await auditFromReq(req, 'UPDATE', 'commander_client_communication', commId, {
            module: 'commander', action: 'communication_response_cancelled',
        });
...
router.put('/:id', async (req, res) => {
    ...
    try {
        ...
        await auditFromReq(req, 'UPDATE', 'commander_client_communication', commId, {
            module: 'commander', action: 'communication_updated', fields: Object.keys(updates),
        });
...
router.delete('/:id', async (req, res) => {
    ...
    try {
        ...
        await auditFromReq(req, 'DELETE', 'commander_client_communication', commId, {
            module: 'commander', action: 'communication_cancelled',
        });
