...
async function _verifyChangeCase(cid, clientId, changeCaseId) {
    ...
    const { data } = await supabase.from('commander_secretarial_change_cases').select('id').eq('id', changeCaseId).eq('company_id', cid).eq('client_id', clientId).maybeSingle();
    return !!data;
}
async function _verifyMeeting(cid, clientId, meetingId) {
    if (!meetingId) return true;
    const { data } = await supabase.from('commander_secretarial_meetings').select('id').eq('id', meetingId).eq('company_id', cid).eq('client_id', clientId).maybeSingle();
    return !!data;
}
async function _verifyResolution(cid, clientId, resolutionId) {
    if (!resolutionId) return true;
    const { data } = await supabase.from('commander_secretarial_resolutions').select('id').eq('id', resolutionId).eq('company_id', cid).eq('client_id', clientId).maybeSingle();
    return !!data;
}

async function _writeGovEvent(cid, sourceType, sourceId, clientId, eventType, oldStatus, newStatus, actorUserId, notes, meta) {
    await supabase.from('commander_secretarial_governance_events').insert({
...
// (commander_secretarial_events, Codebox 62) rather than building a second
...
router.get('/summary', async (req, res) => {
    ...
    try {
        const [resolutions, meetings, decisions] = await Promise.all([
            supabase.from('commander_secretarial_resolutions').select('resolution_status').eq('company_id', cid),
            supabase.from('commander_secretarial_meetings').select('meeting_status').eq('company_id', cid),
            supabase.from('commander_secretarial_decisions').select('decision_status, follow_up_required, follow_up_due_date').eq('company_id', cid),
        ]);
...
router.get('/resolutions', async (req, res) => {
    ...
    try {
        let q = supabase.from('commander_secretarial_resolutions').select('*', { count: 'exact' }).eq('company_id', cid).order('created_at', { ascending: false });
...
router.post('/resolutions', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_secretarial_resolutions').insert({
...
router.get('/resolutions/:id', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_secretarial_resolutions').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
        if (error) return res.status(500).json({ error: error.message });
...
router.put('/resolutions/:id', async (req, res) => {
    ...
    const existing = await supabase.from('commander_secretarial_resolutions').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
    ...
    try {
        const { data, error } = await supabase.from('commander_secretarial_resolutions').update(update).eq('id', id).eq('company_id', cid).select().single();
        if (error) return res.status(500).json({ error: error.message });
...
router.delete('/resolutions/:id', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_secretarial_resolutions')
...
router.get('/meetings', async (req, res) => {
    ...
    try {
        let q = supabase.from('commander_secretarial_meetings').select('*', { count: 'exact' }).eq('company_id', cid).order('meeting_date', { ascending: false, nullsFirst: false });
        if (client_id) q = q.eq('client_id', parseInt(client_id));
        if (change_case_id) q = q.eq('change_case_id', parseInt(change_case_id));
...
router.post('/meetings', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_secretarial_meetings').insert({
            company_id: cid, client_id: clientId, change_case_id: change_case_id ? parseInt(change_case_id) : null,
...
router.get('/meetings/:id', async (req, res) => {
    ...
    try {
        const [meeting, attendees] = await Promise.all([
            supabase.from('commander_secretarial_meetings').select('*').eq('id', id).eq('company_id', cid).maybeSingle(),
            supabase.from('commander_secretarial_meeting_attendees').select('*').eq('meeting_id', id).eq('company_id', cid).order('attendee_name'),
        ]);
...
router.put('/meetings/:id', async (req, res) => {
    ...
    const existing = await supabase.from('commander_secretarial_meetings').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
    ...
    try {
        const { data, error } = await supabase.from('commander_secretarial_meetings').update(update).eq('id', id).eq('company_id', cid).select().single();
        if (error) return res.status(500).json({ error: error.message });
...
router.delete('/meetings/:id', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_secretarial_meetings')
...
router.get('/meetings/:id/attendees', async (req, res) => {
    ...
    const id = parseInt(req.params.id);
    if (!id) return res.status(400).json({ error: 'Invalid meeting ID' });
    const meeting = await supabase.from('commander_secretarial_meetings').select('id').eq('id', id).eq('company_id', cid).maybeSingle();
    if (!meeting.data) return res.status(404).json({ error: 'Meeting not found' });

    try {
        const { data, error } = await supabase.from('commander_secretarial_meeting_attendees').select('*').eq('meeting_id', id).eq('company_id', cid).order('attendee_name');
        if (error) return res.status(500).json({ error: error.message });
...
router.post('/meetings/:id/attendees', async (req, res) => {
    ...
    const meeting = await supabase.from('commander_secretarial_meetings').select('id, client_id').eq('id', id).eq('company_id', cid).maybeSingle();
    if (!meeting.data) return res.status(404).json({ error: 'Meeting not found' });
    ...
    try {
        const { data, error } = await supabase.from('commander_secretarial_meeting_attendees').insert({
...
router.put('/attendees/:attendeeId', async (req, res) => {
    ...
    const existing = await supabase.from('commander_secretarial_meeting_attendees').select('*').eq('id', attendeeId).eq('company_id', cid).maybeSingle();
    ...
    try {
        const meeting = await supabase.from('commander_secretarial_meetings').select('client_id').eq('id', existing.data.meeting_id).eq('company_id', cid).maybeSingle();
        const { data, error } = await supabase.from('commander_secretarial_meeting_attendees').update(update).eq('id', attendeeId).eq('company_id', cid).select().single();
        if (error) return res.status(500).json({ error: error.message });
...
router.delete('/attendees/:attendeeId', async (req, res) => {
    ...
    try {
        const { error } = await supabase.from('commander_secretarial_meeting_attendees').delete().eq('id', attendeeId).eq('company_id', cid);
        if (error) return res.status(500).json({ error: error.message });
...
router.get('/decisions', async (req, res) => {
    ...
    try {
        let q = supabase.from('commander_secretarial_decisions').select('*', { count: 'exact' }).eq('company_id', cid).order('created_at', { ascending: false });
        if (client_id) q = q.eq('client_id', parseInt(client_id));
...
router.post('/decisions', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_secretarial_decisions').insert({
...
router.get('/decisions/:id', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_secretarial_decisions').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
        if (error) return res.status(500).json({ error: error.message });
...
router.put('/decisions/:id', async (req, res) => {
    ...
    const existing = await supabase.from('commander_secretarial_decisions').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
    ...
    try {
        const { data, error } = await supabase.from('commander_secretarial_decisions').update(update).eq('id', id).eq('company_id', cid).select().single();
        if (error) return res.status(500).json({ error: error.message });
...
router.delete('/decisions/:id', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_secretarial_decisions')
...
router.get('/events', async (req, res) => {
    ...
    try {
        let q = supabase.from('commander_secretarial_governance_events').select('*').eq('company_id', cid).order('created_at', { ascending: false }).limit(Math.min(500, parseInt(limit) || 100));
        if (client_id) q = q.eq('client_id', parseInt(client_id));
...
router.get('/:sourceType/:sourceId/events', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_secretarial_governance_events').select('*')
