...
async function resolveClientIds(cid, clientType) {
    ...
    const { data } = await supabase.from('commander_clients')
...
async function _dataProgress(cid, q) {
    ...
    const [indRes, coRes, provRes, indPackRes, coPackRes, compPackRes] = await Promise.all([
        applyReturnFilters(supabase.from('commander_individual_tax_returns').select('status, readiness_status').eq('company_id', cid), f),
        applyReturnFilters(supabase.from('commander_company_tax_returns').select('status, readiness_status').eq('company_id', cid), f),
        applyReturnFilters(supabase.from('commander_provisional_tax_plans').select('status').eq('company_id', cid), f),
        (() => {
            let qt = supabase.from('commander_individual_tax_review_packs').select('pack_status').eq('company_id', cid);
        ...
        (() => {
            let qt = supabase.from('commander_company_tax_review_packs').select('pack_status').eq('company_id', cid);
            if (q.tax_year) qt = qt.eq('tax_year', parseInt(q.tax_year));
            if (clientIds && clientIds.length) qt = qt.in('client_id', clientIds);
            if (q.reviewer_team_member_id) qt = qt.eq('reviewer_team_member_id', parseInt(q.reviewer_team_member_id));
            return qt;
        ...
        (() => {
            let qt = supabase.from('commander_compliance_packs').select('status').eq('company_id', cid).in('pack_type', TAX_PACK_TYPES);
            if (q.tax_year) qt = qt.eq('tax_year', parseInt(q.tax_year));
            if (clientIds && clientIds.length) qt = qt.in('client_id', clientIds);
...
async function _dataStatusBreakdown(cid, q) {
    ...
    const [indRes, coRes, provRes, compPackRes, indPackRes, coPackRes] = await Promise.all([
        ...
        applyReturnFilters(supabase.from('commander_provisional_tax_plans').select('status').eq('company_id', cid), f),
        (() => {
            let qt = supabase.from('commander_compliance_packs').select('status, pack_type').eq('company_id', cid);
            if (q.tax_year) qt = qt.eq('tax_year', parseInt(q.tax_year));
            if (clientIds && clientIds.length) qt = qt.in('client_id', clientIds);
            if (q.responsible_team_member_id) qt = qt.eq('owner_team_member_id', parseInt(q.responsible_team_member_id));
            return qt;
...
async function _dataDocOutstanding(cid, q) {
    ...
    let docQ = supabase.from('commander_document_requests')
    ...
    let clientQ = supabase.from('commander_clients')
    ...
    const [docRes, clientRes, teamRes] = await Promise.all([docQ, clientQ,
        supabase.from('commander_team_members').select('id, display_name').eq('company_id', cid).eq('is_active', true)]);
...
async function _dataReviewBottlenecks(cid, q) {
    const [indReturnRes, coReturnRes, indPackRes, coPackRes, teamRes] = await Promise.all([
        (() => {
            let qt = supabase.from('commander_individual_tax_returns')
                .select('id, return_name, tax_year, client_id, reviewer_team_member_id, updated_at, clients:commander_clients!client_id(name)')
        ...
        (() => {
            let qt = supabase.from('commander_company_tax_returns')
                .select('id, return_name, tax_year, client_id, reviewer_team_member_id, updated_at, clients:commander_clients!client_id(name)')
                .eq('company_id', cid).eq('status', 'ready_for_review');
            if (q.tax_year) qt = qt.eq('tax_year', parseInt(q.tax_year));
            if (q.reviewer_team_member_id) qt = qt.eq('reviewer_team_member_id', parseInt(q.reviewer_team_member_id));
        ...
        (() => {
            let qt = supabase.from('commander_individual_tax_review_packs')
                .select('id, pack_name, tax_year, client_id, reviewer_team_member_id, pack_status, updated_at, clients:commander_clients!client_id(name)')
        ...
        (() => {
            let qt = supabase.from('commander_company_tax_review_packs')
                .select('id, pack_name, tax_year, client_id, reviewer_team_member_id, pack_status, updated_at, clients:commander_clients!client_id(name)')
                .eq('company_id', cid).in('pack_status', ['ready_for_review', 'rejected']);
            if (q.tax_year) qt = qt.eq('tax_year', parseInt(q.tax_year));
            if (q.reviewer_team_member_id) qt = qt.eq('reviewer_team_member_id', parseInt(q.reviewer_team_member_id));
...
async function _dataPartnerSummary(cid, q) {
    ...
    const [teamRes, indRes, coRes, provRes, compPackRes, docRes, actionRes, deadlineRes, allClientsRes] = await Promise.all([
        supabase.from('commander_team_members').select('id, display_name').eq('company_id', cid).eq('is_active', true),
        (() => {
            let qt = supabase.from('commander_individual_tax_returns').select('client_id, status, responsible_team_member_id').eq('company_id', cid);
            if (q.tax_year) qt = qt.eq('tax_year', parseInt(q.tax_year));
            if (clientIds && clientIds.length) qt = qt.in('client_id', clientIds);
            return qt;
        })(),
        (() => {
            let qt = supabase.from('commander_company_tax_returns').select('client_id, status, responsible_team_member_id').eq('company_id', cid);
            if (q.tax_year) qt = qt.eq('tax_year', parseInt(q.tax_year));
            if (clientIds && clientIds.length) qt = qt.in('client_id', clientIds);
            return qt;
        })(),
        (() => {
            let qt = supabase.from('commander_provisional_tax_plans').select('client_id, status, responsible_team_member_id').eq('company_id', cid);
            if (q.tax_year) qt = qt.eq('tax_year', parseInt(q.tax_year));
            if (clientIds && clientIds.length) qt = qt.in('client_id', clientIds);
            return qt;
        })(),
        (() => {
            let qt = supabase.from('commander_compliance_packs').select('client_id, status, owner_team_member_id').eq('company_id', cid).in('pack_type', TAX_PACK_TYPES);
            if (q.tax_year) qt = qt.eq('tax_year', parseInt(q.tax_year));
            if (clientIds && clientIds.length) qt = qt.in('client_id', clientIds);
            return qt;
        })(),
        (() => {
            let qt = supabase.from('commander_document_requests').select('client_id, request_status, required_by_date').eq('company_id', cid).in('request_status', OUTSTANDING_DOC_STATUSES);
            if (clientIds && clientIds.length) qt = qt.in('client_id', clientIds);
            return qt;
        })(),
        (() => {
            let qt = supabase.from('commander_tax_work_actions').select('client_id').eq('company_id', cid).eq('action_status', 'open');
            if (clientIds && clientIds.length) qt = qt.in('client_id', clientIds);
            return qt;
        })(),
        (() => {
            let qt = supabase.from('commander_deadlines').select('client_id').eq('company_id', cid)
                .not('status', 'in', DEADLINE_EXCLUDE_STATUSES).lt('due_date', today());
            if (clientIds && clientIds.length) qt = qt.in('client_id', clientIds);
            return qt;
        })(),
        (() => {
            let qt = supabase.from('commander_clients').select('id, responsible_team_member_id').eq('company_id', cid).eq('is_active', true);
...
async function _dataBulkSummary(cid) {
    const [opsRes, itemsRes] = await Promise.all([
        supabase.from('commander_tax_bulk_operations')
        ...
        supabase.from('commander_tax_bulk_operation_items').select('operation_id, item_status').eq('company_id', cid),
    ]);
...
async function _dataRiskSummary(cid, q) {
    ...
    const [overdueRes, indBlockedRes, coBlockedRes, indReadyRes, coReadyRes, indPacksExistRes, coPacksExistRes, docOutRes, actionsRes] = await Promise.all([
        supabase.from('commander_deadlines')
            .select('id, title, due_date, client_id, clients:commander_clients!client_id(name)')
            .eq('company_id', cid).not('status', 'in', DEADLINE_EXCLUDE_STATUSES).lt('due_date', t).order('due_date').limit(30),
        (() => {
            let qt = supabase.from('commander_individual_tax_returns')
                .select('id, return_name, tax_year, client_id, clients:commander_clients!client_id(name)')
        ...
        (() => {
            let qt = supabase.from('commander_individual_tax_returns').select('id, return_name, tax_year, client_id').eq('company_id', cid).eq('status', 'ready_for_review');
            if (q.tax_year) qt = qt.eq('tax_year', parseInt(q.tax_year));
            return qt.limit(100);
        })(),
        (() => {
            let qt = supabase.from('commander_company_tax_returns').select('id, return_name, tax_year, client_id').eq('company_id', cid).eq('status', 'ready_for_review');
            if (q.tax_year) qt = qt.eq('tax_year', parseInt(q.tax_year));
            return qt.limit(100);
        })(),
        (() => {
            let qt = supabase.from('commander_individual_tax_review_packs').select('tax_return_id, pack_status').eq('company_id', cid).in('pack_status', ['draft','generated','ready_for_review','reviewed','approved']);
            if (q.tax_year) qt = qt.eq('tax_year', parseInt(q.tax_year));
            return qt;
        })(),
        (() => {
            let qt = supabase.from('commander_company_tax_review_packs').select('company_tax_return_id, pack_status').eq('company_id', cid).in('pack_status', ['draft','generated','ready_for_review','reviewed','approved']);
            if (q.tax_year) qt = qt.eq('tax_year', parseInt(q.tax_year));
            return qt;
        })(),
        supabase.from('commander_document_requests').select('id, client_id').eq('company_id', cid).in('request_status', OUTSTANDING_DOC_STATUSES).limit(500),
        supabase.from('commander_tax_work_actions').select('id, client_id, action_type, action_title, action_status').eq('company_id', cid).eq('action_status', 'open').order('created_at').limit(30),
    ]);
    ...
    if (highRiskClientIds.length > 0) {
        const { data: hrc } = await supabase.from('commander_clients')
...
router.get('/progress/report-data', async (req, res) => {
    try {
        ...
        await auditFromReq(req, 'VIEW', 'tax_report', null, { module: 'commander', report: 'progress' });
...
router.get('/progress/report-html', async (req, res) => {
    try {
        const [d, ctx] = await Promise.all([_dataProgress(req.companyId, req.query), _exportCtx(req.companyId, req.query)]);
        await auditFromReq(req, 'VIEW', 'tax_report', null, { module: 'commander', report: 'progress', format: 'html' });
...
router.get('/progress/report-pdf', async (req, res) => {
    ...
    try {
        const [d, ctx] = await Promise.all([_dataProgress(req.companyId, req.query), _exportCtx(req.companyId, req.query)]);
        await auditFromReq(req, 'DOWNLOAD', 'tax_report', null, { module: 'commander', report: 'progress', format: 'pdf' });
...
router.get('/partner-summary/report-data', async (req, res) => {
    try {
        ...
        await auditFromReq(req, 'VIEW', 'tax_report', null, { module: 'commander', report: 'partner_summary' });
        res.json({ ...d, _meta: ctx });
    } catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.get('/partner-summary/report-html', async (req, res) => {
    try {
        const [d, ctx] = await Promise.all([_dataPartnerSummary(req.companyId, req.query), _exportCtx(req.companyId, req.query)]);
        await auditFromReq(req, 'VIEW', 'tax_report', null, { module: 'commander', report: 'partner_summary', format: 'html' });
        res.setHeader('Content-Type', 'text/html; charset=utf-8');
...
router.get('/partner-summary/report-pdf', async (req, res) => {
    ...
    try {
        const [d, ctx] = await Promise.all([_dataPartnerSummary(req.companyId, req.query), _exportCtx(req.companyId, req.query)]);
        await auditFromReq(req, 'DOWNLOAD', 'tax_report', null, { module: 'commander', report: 'partner_summary', format: 'pdf' });
        const doc = new PDFDocument({ margin: 50, size: 'A4', bufferPages: true });
        res.setHeader('Content-Type', 'application/pdf');
...
router.get('/document-outstanding/report-data', async (req, res) => {
    try {
        ...
        await auditFromReq(req, 'VIEW', 'tax_report', null, { module: 'commander', report: 'document_outstanding' });
        res.json({ ...d, _meta: ctx });
    } catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.get('/document-outstanding/report-html', async (req, res) => {
    try {
        const [d, ctx] = await Promise.all([_dataDocOutstanding(req.companyId, req.query), _exportCtx(req.companyId, req.query)]);
        await auditFromReq(req, 'VIEW', 'tax_report', null, { module: 'commander', report: 'document_outstanding', format: 'html' });
        res.setHeader('Content-Type', 'text/html; charset=utf-8');
...
router.get('/document-outstanding/report-pdf', async (req, res) => {
    ...
    try {
        const [d, ctx] = await Promise.all([_dataDocOutstanding(req.companyId, req.query), _exportCtx(req.companyId, req.query)]);
        await auditFromReq(req, 'DOWNLOAD', 'tax_report', null, { module: 'commander', report: 'document_outstanding', format: 'pdf' });
        const doc = new PDFDocument({ margin: 50, size: 'A4', bufferPages: true });
        res.setHeader('Content-Type', 'application/pdf');
...
router.get('/review-bottlenecks/report-data', async (req, res) => {
    try {
        ...
        await auditFromReq(req, 'VIEW', 'tax_report', null, { module: 'commander', report: 'review_bottlenecks' });
        res.json({ ...d, _meta: ctx });
    } catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.get('/review-bottlenecks/report-html', async (req, res) => {
    try {
        const [d, ctx] = await Promise.all([_dataReviewBottlenecks(req.companyId, req.query), _exportCtx(req.companyId, req.query)]);
        await auditFromReq(req, 'VIEW', 'tax_report', null, { module: 'commander', report: 'review_bottlenecks', format: 'html' });
        res.setHeader('Content-Type', 'text/html; charset=utf-8');
...
router.get('/review-bottlenecks/report-pdf', async (req, res) => {
    ...
    try {
        const [d, ctx] = await Promise.all([_dataReviewBottlenecks(req.companyId, req.query), _exportCtx(req.companyId, req.query)]);
        await auditFromReq(req, 'DOWNLOAD', 'tax_report', null, { module: 'commander', report: 'review_bottlenecks', format: 'pdf' });
        const doc = new PDFDocument({ margin: 50, size: 'A4', bufferPages: true });
        res.setHeader('Content-Type', 'application/pdf');
...
router.get('/risk-summary/report-data', async (req, res) => {
    try {
        ...
        await auditFromReq(req, 'VIEW', 'tax_report', null, { module: 'commander', report: 'risk_summary' });
        res.json({ ...d, _meta: ctx });
    } catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.get('/risk-summary/report-html', async (req, res) => {
    try {
        const [d, ctx] = await Promise.all([_dataRiskSummary(req.companyId, req.query), _exportCtx(req.companyId, req.query)]);
        await auditFromReq(req, 'VIEW', 'tax_report', null, { module: 'commander', report: 'risk_summary', format: 'html' });
        res.setHeader('Content-Type', 'text/html; charset=utf-8');
...
router.get('/risk-summary/report-pdf', async (req, res) => {
    ...
    try {
        const [d, ctx] = await Promise.all([_dataRiskSummary(req.companyId, req.query), _exportCtx(req.companyId, req.query)]);
        await auditFromReq(req, 'DOWNLOAD', 'tax_report', null, { module: 'commander', report: 'risk_summary', format: 'pdf' });
        const doc = new PDFDocument({ margin: 50, size: 'A4', bufferPages: true });
        res.setHeader('Content-Type', 'application/pdf');
...
router.get('/tax-season-pack/report-data', async (req, res) => {
    try {
        ...
        await auditFromReq(req, 'VIEW', 'tax_report', null, { module: 'commander', report: 'tax_season_pack' });
...
router.get('/tax-season-pack/report-html', async (req, res) => {
    try {
        ...
        await auditFromReq(req, 'VIEW', 'tax_report', null, { module: 'commander', report: 'tax_season_pack', format: 'html' });
...
router.get('/tax-season-pack/report-pdf', async (req, res) => {
    ...
    try {
        ...
        await auditFromReq(req, 'DOWNLOAD', 'tax_report', null, { module: 'commander', report: 'tax_season_pack', format: 'pdf' });
...
router.post('/snapshots', async (req, res) => {
    ...
    const { data, error } = await supabase.from('commander_tax_reporting_snapshots').insert({
        company_id:   cid,
    ...
    await auditFromReq(req, 'CREATE', 'commander_tax_reporting_snapshot', data.id,
        { module: 'commander', report_type, tax_year });
