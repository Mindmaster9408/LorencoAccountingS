 ...
 * ============================================================================
 * Mounted at /api/commander/billing by practice/index.js
...
async function recalculatePack(companyId, packId) {
  const { data: lines } = await supabase
    .from('commander_billing_pack_lines')
...
async function logPackEvent(companyId, packId, eventType, opts = {}) {
  try {
    await supabase.from('commander_billing_pack_events').insert({
...
router.get('/wip', async (req, res) => {
  ...
  let q = supabase
    ...
    .select(`
      ...
      commander_clients:client_id(id, name)
    `)
  ...
  rows.forEach(r => {
    const clientId   = r.commander_clients?.id   || null;
    const clientName = r.commander_clients?.name || 'No Client';
...
router.post('/packs', async (req, res) => {
  ...
  if (lineErr) {
    ...
    await supabase.from('commander_billing_packs').delete().eq('id', pack.id);
  ...
  await auditFromReq(req, 'billing_pack_created', 'commander_billing_pack', pack.id, {
    module:      'commander',
    client_id:   parsedClientId,
...
router.get('/packs', async (req, res) => {
  ...
  let q = supabase
    .from('commander_billing_packs')
    .select('*, commander_clients:client_id(name)', { count: 'exact' })
    .eq('company_id', req.companyId)
...
router.get('/packs/:id', async (req, res) => {
  ...
  const { data: lines, error: lineErr } = await supabase
    ...
    .select(`
      *,
      commander_time_entries:time_entry_id(id, date, description, user_id, time_type, effective_rate, billing_status, billing_notes),
      commander_tasks:task_id(id, title),
      commander_workflow_runs:workflow_run_id(id, name)
    `)
...
router.put('/packs/:id', async (req, res) => {
  ...
  await auditFromReq(req, 'billing_pack_updated', 'commander_billing_pack', pack.id, { module: 'commander' });
  res.json({ pack: data });
});
...
router.put('/packs/:id/lines/:lineId/writeoff', async (req, res) => {
  ...
  await auditFromReq(req, 'billing_line_written_off', 'commander_billing_pack_line', parseInt(req.params.lineId), {
    module:         'commander',
    pack_id:        pack.id,
    reason:         reason.trim(),
    writeoff_value: writeoffValue
  ...
  const { data: updatedPack } = await supabase
    .from('commander_billing_packs').select('*').eq('id', pack.id).single();
...
router.put('/packs/:id/lines/:lineId/exclude', async (req, res) => {
  ...
  await auditFromReq(req, 'billing_line_excluded', 'commander_billing_pack_line', parseInt(req.params.lineId), {
    module:  'commander',
    pack_id: pack.id
  });
...
router.put('/packs/:id/recalculate', async (req, res) => {
  ...
  await auditFromReq(req, 'billing_pack_recalculated', 'commander_billing_pack', pack.id, { module: 'commander' });
...
router.put('/packs/:id/approve', async (req, res) => {
  ...
  await auditFromReq(req, 'billing_pack_approved', 'commander_billing_pack', pack.id, { module: 'commander' });
  res.json({ pack: data });
});
...
router.put('/packs/:id/lock', async (req, res) => {
  ...
  await auditFromReq(req, 'billing_pack_locked', 'commander_billing_pack', pack.id, {
    module:         'commander',
...
router.delete('/packs/:id', async (req, res) => {
  ...
  await auditFromReq(req, 'billing_pack_cancelled', 'commander_billing_pack', pack.id, {
    module:           'commander',
...
router.get('/packs/:id/history', async (req, res) => {
  ...
  const { data, error } = await supabase
    .from('commander_billing_pack_events')
    .select('*')
    .eq('billing_pack_id', pack.id)
    .eq('company_id', req.companyId)
    .order('created_at', { ascending: false });
...
async function buildReportData(companyId, packId) {
  ...
  const [entriesRes, tasksRes, runsRes, clientRes, profileRes, companyRes] = await Promise.all([
    entryIds.length > 0
      ? supabase.from('commander_time_entries')
    ...
    supabase.from('commander_clients')
      ...
      .eq('company_id', companyId)
      .single(),
    supabase.from('commander_profiles')
      .select('commander_email, commander_phone, address_line1, address_city, address_province, address_postal_code, tax_practitioner_number, vat_registration_number')
      .eq('company_id', companyId)
      .single(),
  ...
  return {
    practice: {
      name:                    practiceName,
      email:                   profile?.commander_email                || null,
      phone:                   profile?.commander_phone                || null,
...
router.get('/packs/:id/report-data', async (req, res) => {
  ...
  await auditFromReq(req, 'billing_report_viewed', 'commander_billing_pack', parseInt(req.params.id), {
    module:  'commander',
    format:  'json'
  });
...
<div class="footer">
  ...
  await stampReportGenerated(req.companyId, req.params.id, req.user?.userId);
  await auditFromReq(req, 'billing_report_generated', 'commander_billing_pack', parseInt(req.params.id), {
    module: 'commander',
    format: 'html'
  });
