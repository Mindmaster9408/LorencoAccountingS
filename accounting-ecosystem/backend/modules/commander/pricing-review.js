...
async function _requireManager(req, res) {
    return teamAccess.requireManager(req, res, supabase, 'Only owners, partners, admins, and practice managers can manage pricing reviews.');
}
...
async function _fetchReview(cid, id) {
    const { data } = await supabase.from('commander_pricing_reviews').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
    return data || null;
}

async function _writeEvent(cid, reviewId, eventType, oldStatus, newStatus, actorUserId, notes, meta) {
    await supabase.from('commander_pricing_events').insert({
...
async function buildPricingReview(cid, { clientId, engagementId, profitabilityReviewId }) {
    ...
    let engagement = null;
    if (engagementId) {
        const { data } = await supabase.from('commander_client_engagements').select('*').eq('id', engagementId).eq('company_id', cid).eq('client_id', clientId).maybeSingle();
    ...
    if (profitabilityReviewId) {
        const { data } = await supabase.from('commander_profitability_reviews').select('*').eq('id', profitabilityReviewId).eq('company_id', cid).maybeSingle();
        if (!data) throw new Error('Profitability review not found');
        profitabilityReview = data;
        if (data.snapshot_id) {
            const { data: snap } = await supabase.from('commander_profitability_snapshots').select('*').eq('id', data.snapshot_id).eq('company_id', cid).maybeSingle();
            snapshot = snap || null;
        }
    } else {
        ...
        try {
            let q = supabase.from('commander_profitability_snapshots').select('*').eq('company_id', cid).eq('client_id', clientId).order('created_at', { ascending: false }).limit(1);
...
router.get('/summary', async (req, res) => {
    ...
    try {
        const { data: rows } = await supabase.from('commander_pricing_reviews').select('pricing_status').eq('company_id', cid);
        const reviews = rows || [];
...
router.get('/', async (req, res) => {
    ...
    try {
        let q = supabase.from('commander_pricing_reviews').select('*').eq('company_id', cid).order('created_at', { ascending: false }).limit(Math.min(500, parseInt(limit) || 200));
...
router.post('/', async (req, res) => {
    ...
    if (engagement_id) {
        const { data: eng } = await supabase.from('commander_client_engagements').select('id, fee_basis, fee_amount').eq('id', parseInt(engagement_id)).eq('company_id', cid).eq('client_id', clientId).maybeSingle();
    ...
    try {
        const { data, error } = await supabase.from('commander_pricing_reviews').insert({
        ...
        if (Array.isArray(review_items) && review_items.length) {
            ...
            if (rows.length) await supabase.from('commander_pricing_review_items').insert(rows);
        }
...
router.get('/:id', async (req, res) => {
    ...
    try {
        ...
        const { data: items } = await supabase.from('commander_pricing_review_items').select('*').eq('company_id', cid).eq('pricing_review_id', id).order('sort_order');
...
router.put('/:id', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_pricing_reviews').update(update).eq('id', id).eq('company_id', cid).select().single();
        if (error) return res.status(500).json({ error: error.message });
...
router.delete('/:id', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('commander_pricing_reviews')
...
router.put('/:id/implement', async (req, res) => {
    ...
    try {
        ...
        // this NEVER writes to commander_client_engagements. See migration
...
router.get('/:id/items', async (req, res) => {
    ...
    if (!id) return res.status(400).json({ error: 'Invalid review ID' });
    try {
        const { data, error } = await supabase.from('commander_pricing_review_items').select('*').eq('company_id', cid).eq('pricing_review_id', id).order('sort_order');
        if (error) return res.status(500).json({ error: error.message });
        res.json({ items: data || [] });
    } catch (err) {
...
router.post('/:id/items', async (req, res) => {
    ...
    try {
        const { count } = await supabase.from('commander_pricing_review_items').select('id', { count: 'exact', head: true }).eq('company_id', cid).eq('pricing_review_id', id);
        const { data, error } = await supabase.from('commander_pricing_review_items').insert({
...
router.put('/items/:itemId', async (req, res) => {
    ...
    const { data: existing } = await supabase.from('commander_pricing_review_items').select('*').eq('id', itemId).eq('company_id', cid).maybeSingle();
    ...
    try {
        const { data, error } = await supabase.from('commander_pricing_review_items').update(update).eq('id', itemId).eq('company_id', cid).select().single();
        if (error) return res.status(500).json({ error: error.message });
        res.json({ item: data });
    } catch (err) {
...
router.delete('/items/:itemId', async (req, res) => {
    ...
    try {
        const { error } = await supabase.from('commander_pricing_review_items').delete().eq('id', itemId).eq('company_id', cid);
        if (error) return res.status(500).json({ error: error.message });
...
router.get('/:id/events', async (req, res) => {
    ...
    if (!id) return res.status(400).json({ error: 'Invalid review ID' });
    try {
        const { data, error } = await supabase.from('commander_pricing_events').select('*').eq('company_id', cid).eq('pricing_review_id', id).order('created_at', { ascending: false });
        if (error) return res.status(500).json({ error: error.message });
