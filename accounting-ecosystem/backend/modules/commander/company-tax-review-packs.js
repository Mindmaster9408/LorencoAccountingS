/* =============================================================
   ...
   Routes mounted at /api/commander/company-tax via index.js.
...
async function verifyPackOwnership(cid, packId) {
    const { data } = await supabase
        .from('commander_company_tax_review_packs')
        .select('*')
        .eq('id', packId)
        .eq('company_id', cid)
        .single();
...
async function logPackEvent(cid, packId, returnId, eventType, extras = {}) {
    try {
        await supabase.from('commander_company_tax_review_pack_events').insert({
            company_id:           cid,
...
async function buildSnapshot(cid, taxReturn, calcId) {
    ...
    if (calcId) {
        const { data: c } = await supabase
            .from('commander_company_tax_calculations')
            .select('*')
            .eq('id', calcId)
            .eq('company_id', cid)
            .single();
...
router.post('/:returnId/review-packs/generate', async (req, res) => {
    ...
    try {
        ...
        await auditFromReq(req, 'company_tax_review_pack_generated', 'commander_company_tax_review_packs', pack.id, {
            metadata: { company_tax_return_id: returnId },
        });
...
router.put('/review-packs/:id/submit-review', async (req, res) => {
    ...
    await auditFromReq(req, 'company_tax_review_pack_submitted_review', 'commander_company_tax_review_packs', packId);
...
router.put('/review-packs/:id/approve', async (req, res) => {
    ...
    await auditFromReq(req, 'company_tax_review_pack_approved', 'commander_company_tax_review_packs', packId);
...
router.put('/review-packs/:id/reject', async (req, res) => {
    ...
    await auditFromReq(req, 'company_tax_review_pack_rejected', 'commander_company_tax_review_packs', packId);
...
router.get('/review-packs/:id/events', async (req, res) => {
    ...
    const { data, error } = await supabase
        .from('commander_company_tax_review_pack_events')
        .select('*')
