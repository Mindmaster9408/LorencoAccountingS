/* =============================================================
   ...
   Mounted at /api/commander/company-tax
   ============================================================= */
'use strict';
...
async function verifyReturnBelongsToCompany(cid, returnId) {
    ...
    const { data } = await supabase
        .from('commander_company_tax_returns')
...
async function verifyProfileBelongsToCompany(cid, profileId) {
    ...
    const { data } = await supabase
        .from('commander_taxpayer_profiles')
        .select('id')
...
async function logCtEvent(cid, returnId, eventType, opts = {}) {
    try {
        await supabase.from('commander_company_tax_events').insert({
            company_id:            cid,
...
async function recalculateReadiness(cid, returnId) {
    const { data: items } = await supabase
        .from('commander_company_tax_readiness_items')
...
// GET /api/commander/company-tax/:id
...
// POST /api/commander/company-tax
router.post('/', async (req, res) => {
    ...
    await auditFromReq(req, 'CREATE', 'commander_company_tax_return', data.id, { module: 'commander' });
    res.status(201).json({ company_tax_return: data });
});

// PUT /api/commander/company-tax/:id
router.put('/:id', async (req, res) => {
    ...
    await auditFromReq(req, 'UPDATE', 'commander_company_tax_return', data.id, { module: 'commander' });
    res.json({ company_tax_return: data });
});

// DELETE /api/commander/company-tax/:id  (soft cancel only)
router.delete('/:id', async (req, res) => {
    ...
    await auditFromReq(req, 'CANCEL', 'commander_company_tax_return', parseInt(req.params.id), { module: 'commander' });
    res.json({ success: true });
});

// POST /api/commander/company-tax/:id/generate-default-items
router.post('/:id/generate-default-items', async (req, res) => {
    ...
    await auditFromReq(req, 'CREATE', 'commander_company_tax_readiness_items', returnId, { module: 'commander', count: inserted.length });

    res.json({ items_created: inserted.length, items_skipped: existingNames.size, readiness });
});

// POST /api/commander/company-tax/:id/recalculate-readiness
...
// GET /api/commander/company-tax/:id/adjustments
router.get('/:id/adjustments', async (req, res) => {
    ...
    const { data, error } = await supabase
        .from('commander_company_tax_adjustments')
        .select('*')
        .eq('company_id', cid)
...
// POST /api/commander/company-tax/:id/adjustments
router.post('/:id/adjustments', async (req, res) => {
    ...
    await auditFromReq(req, 'CREATE', 'commander_company_tax_adjustment', data.id, { module: 'commander' });
    res.status(201).json({ adjustment: data });
});

// PUT /api/commander/company-tax/:id/adjustments/:adjustmentId
router.put('/:id/adjustments/:adjustmentId', async (req, res) => {
    ...
    await auditFromReq(req, 'UPDATE', 'commander_company_tax_adjustment', data.id, { module: 'commander' });
    res.json({ adjustment: data });
});

// DELETE /api/commander/company-tax/:id/adjustments/:adjustmentId
router.delete('/:id/adjustments/:adjustmentId', async (req, res) => {
    ...
    await auditFromReq(req, 'DELETE', 'commander_company_tax_adjustment', parseInt(req.params.adjustmentId), { module: 'commander' });
    res.json({ success: true });
});

// ─── Routes: Readiness Items ──────────────────────────────────────────────────

// GET /api/commander/company-tax/:id/items
...
// POST /api/commander/company-tax/:id/items
router.post('/:id/items', async (req, res) => {
    ...
    await auditFromReq(req, 'CREATE', 'commander_company_tax_readiness_item', data.id, { module: 'commander' });
    res.status(201).json({ item: data });
});

// PUT /api/commander/company-tax/:id/items/:itemId
router.put('/:id/items/:itemId', async (req, res) => {
    ...
    await auditFromReq(req, 'UPDATE', 'commander_company_tax_readiness_item', data.id, { module: 'commander' });
    res.json({ item: data });
});

// DELETE /api/commander/company-tax/:id/items/:itemId
router.delete('/:id/items/:itemId', async (req, res) => {
    ...
    await auditFromReq(req, 'DELETE', 'commander_company_tax_readiness_item', parseInt(req.params.itemId), { module: 'commander' });
    res.json({ success: true });
});

// ─── Routes: Events ───────────────────────────────────────────────────────────

// GET /api/commander/company-tax/:id/events
router.get('/:id/events', async (req, res) => {
    ...
    const { data, error } = await supabase
        .from('commander_company_tax_events')
        .select('*')
        .eq('company_id', cid)
        .eq('company_tax_return_id', parseInt(req.params.id))
        .order('created_at', { ascending: true });
