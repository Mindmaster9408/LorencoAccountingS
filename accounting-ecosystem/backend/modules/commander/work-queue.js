...
async function _writeEvent(cid, teamMemberId, eventType, sourceModule, sourceType, sourceId, actorUserId, notes, meta) {
    await supabase.from('commander_work_queue_events').insert({
        company_id: cid,
...
async function _fetchTasks(cid, tmId) {
    const { data, error } = await supabase.from('commander_tasks')
        .select('id, title, client_id, due_date, status, review_status, approval_status, priority, estimated_hours, assigned_to, preparer_team_member_id, reviewer_team_member_id, approver_team_member_id, commander_clients:client_id(name, risk_rating)')
        .eq('company_id', cid)
    ...
    return (data || []).map(t => {
        ...
        return {
            source_module: 'tasks', source_type: 'commander_task', source_id: t.id, client_id: t.client_id,
            role, title: t.title, client_name: t.commander_clients?.name || null,
            client_risk_rating: t.commander_clients?.risk_rating || null,
            ...
            deep_link: '/commander/tasks.html?open=' + t.id,
        };
    });
}

async function _fetchDeadlines(cid, tmId) {
    const { data, error } = await supabase.from('commander_deadlines')
        .select('id, title, due_date, status, priority, compliance_area, deadline_type, responsible_team_member_id, client_id, commander_clients:client_id(name, risk_rating)')
        .eq('company_id', cid)
    ...
    return (data || []).map(d => ({
        source_module: 'deadlines', source_type: 'commander_deadline', source_id: d.id, client_id: d.client_id,
        role: 'responsible', title: d.title, client_name: d.commander_clients?.name || null,
        client_risk_rating: d.commander_clients?.risk_rating || null,
        due_date: d.due_date, status: d.status, manual_priority: d.priority,
        severity_band: null, blocked: false, waiting_on: null,
        deep_link: '/commander/deadlines.html?open=' + d.id,
    }));
}

async function _fetchReminders(cid, tmId) {
    const { data, error } = await supabase.from('commander_reminders')
        .select('id, title, reminder_type, due_date, status, severity, assigned_team_member_id, client_id, commander_clients:client_id(name, risk_rating)')
        .eq('company_id', cid)
    ...
    return (data || []).map(r => ({
        source_module: 'reminders', source_type: 'commander_reminder', source_id: r.id, client_id: r.client_id,
        role: 'assignee', title: r.title || r.reminder_type, client_name: r.commander_clients?.name || null,
        client_risk_rating: r.commander_clients?.risk_rating || null,
        ...
        deep_link: '/commander/reminders.html?open=' + r.id,
    }));
}

async function _fetchRisks(cid, tmId) {
    const { data, error } = await supabase.from('commander_risks')
        .select('id, title, category, inherent_risk, status, next_review_date, owner_team_member_id, linked_client_id, commander_clients:linked_client_id(name, risk_rating)')
        .eq('company_id', cid)
    ...
    return (data || []).map(r => ({
        source_module: 'risk-register', source_type: 'commander_risk', source_id: r.id, client_id: r.linked_client_id,
        role: 'owner', title: r.title, client_name: r.commander_clients?.name || null,
        client_risk_rating: r.commander_clients?.risk_rating || null,
        ...
        deep_link: '/commander/risk-register.html?open=' + r.id,
    }));
}

async function _fetchQmsReviews(cid, tmId) {
    const { data, error } = await supabase.from('commander_quality_reviews')
        .select('id, review_title, status, client_id, assigned_reviewer_team_member_id, commander_clients:client_id(name, risk_rating)')
        .eq('company_id', cid)
    ...
    return (data || []).map(r => ({
        source_module: 'qms', source_type: 'commander_quality_review', source_id: r.id, client_id: r.client_id,
        role: 'reviewer', title: r.review_title, client_name: r.commander_clients?.name || null,
        client_risk_rating: r.commander_clients?.risk_rating || null,
        ...
        deep_link: '/commander/quality-management.html?open=' + r.id,
    }));
}

async function _fetchQmsFindings(cid, tmId) {
    const { data, error } = await supabase.from('commander_quality_findings')
    ...
    return (data || []).map(f => ({
        source_module: 'qms', source_type: 'commander_quality_finding', source_id: f.id, client_id: null,
        ...
        deep_link: '/commander/quality-management.html?finding=' + f.id,
    }));
}

async function _fetchCompliancePacks(cid, tmId) {
    const { data, error } = await supabase.from('commander_compliance_packs')
        .select('id, pack_name, pack_type, status, readiness_status, client_id, owner_team_member_id, reviewer_team_member_id, commander_clients:client_id(name, risk_rating)')
        .eq('company_id', cid)
        .not('status', 'in', '("completed","cancelled")')
    ...
    return (data || []).map(p => {
        ...
        return {
            source_module: 'compliance-packs', source_type: 'commander_compliance_pack', source_id: p.id, client_id: p.client_id,
            role, title: p.pack_name || p.pack_type, client_name: p.commander_clients?.name || null,
            client_risk_rating: p.commander_clients?.risk_rating || null,
            ...
            deep_link: '/commander/compliance-packs.html?open=' + p.id,
        };
    });
}

async function _fetchDocumentRequests(cid, tmId) {
    const { data, error } = await supabase.from('commander_document_requests')
        .select('id, request_title, document_category, request_status, required_by_date, assigned_team_member_id, client_id, commander_clients:client_id(name, risk_rating)')
        .eq('company_id', cid)
    ...
    return (data || []).map(d => ({
        source_module: 'document-requests', source_type: 'commander_document_request', source_id: d.id, client_id: d.client_id,
        role: 'assignee', title: d.request_title || d.document_category, client_name: d.commander_clients?.name || null,
        client_risk_rating: d.commander_clients?.risk_rating || null,
        ...
        deep_link: '/commander/document-requests.html?open=' + d.id,
    }));
}

async function _fetchCommunications(cid, tmId) {
    const { data, error } = await supabase.from('commander_client_communications')
        .select('id, subject, communication_type, response_status, response_due_date, response_required, assigned_team_member_id, client_id, commander_clients:client_id(name, risk_rating)')
        .eq('company_id', cid)
    ...
    return (data || []).map(c => ({
        source_module: 'communications', source_type: 'commander_client_communication', source_id: c.id, client_id: c.client_id,
        role: 'assignee', title: c.subject || c.communication_type, client_name: c.commander_clients?.name || null,
        client_risk_rating: c.commander_clients?.risk_rating || null,
        ...
        deep_link: '/commander/communications.html?open=' + c.id,
    }));
}

async function _fetchTaxReturns(cid, tmId, table, category, urlBase) {
    const { data, error } = await supabase.from(table)
        .select('id, tax_year, status, client_id, responsible_team_member_id, reviewer_team_member_id, commander_clients:client_id(name, risk_rating)')
        .eq('company_id', cid)
        .not('status', 'in', '("completed","cancelled")')
    ...
    return (data || []).map(r => {
        ...
        const clientName = r.commander_clients?.name || 'Unknown client';
        return {
...
async function _buildActiveQueue(cid, tmId) {
    ...
    const results = await Promise.all([
        ...
        _fetchTaxReturns(cid, tmId, 'commander_individual_tax_returns', 'tax-individual', '/commander/individual-tax.html'),
        _fetchTaxReturns(cid, tmId, 'commander_company_tax_returns', 'tax-company', '/commander/company-tax.html'),
    ]);
...
async function _fetchCompletedRecent(cid, tmId, days) {
    ...
    const [tasks, compliance, taxIndiv, taxCo] = await Promise.all([
        supabase.from('commander_tasks').select('id, title, status, updated_at, client_id, commander_clients:client_id(name)')
            .eq('company_id', cid).eq('status', 'completed').gte('updated_at', since)
            .or(`assigned_to.eq.${tmId},preparer_team_member_id.eq.${tmId},reviewer_team_member_id.eq.${tmId},approver_team_member_id.eq.${tmId}`),
        supabase.from('commander_compliance_packs').select('id, pack_name, pack_type, status, updated_at, client_id, commander_clients:client_id(name)')
            .eq('company_id', cid).eq('status', 'completed').gte('updated_at', since)
            .or(`owner_team_member_id.eq.${tmId},reviewer_team_member_id.eq.${tmId}`),
        supabase.from('commander_individual_tax_returns').select('id, tax_year, status, updated_at, client_id, commander_clients:client_id(name)')
            .eq('company_id', cid).eq('status', 'completed').gte('updated_at', since)
            .or(`responsible_team_member_id.eq.${tmId},reviewer_team_member_id.eq.${tmId}`),
        supabase.from('commander_company_tax_returns').select('id, tax_year, status, updated_at, client_id, commander_clients:client_id(name)')
            .eq('company_id', cid).eq('status', 'completed').gte('updated_at', since)
            .or(`responsible_team_member_id.eq.${tmId},reviewer_team_member_id.eq.${tmId}`),
    ]);

    const items = [];
    (tasks.data || []).forEach(t => items.push({ source_module: 'tasks', source_type: 'commander_task', source_id: t.id, title: t.title, client_name: t.commander_clients?.name || null, completed_at: t.updated_at, deep_link: '/commander/tasks.html?open=' + t.id }));
    (compliance.data || []).forEach(p => items.push({ source_module: 'compliance-packs', source_type: 'commander_compliance_pack', source_id: p.id, title: p.pack_name || p.pack_type, client_name: p.commander_clients?.name || null, completed_at: p.updated_at, deep_link: '/commander/compliance-packs.html?open=' + p.id }));
    (taxIndiv.data || []).forEach(r => items.push({ source_module: 'tax-individual', source_type: 'commander_individual_tax_returns', source_id: r.id, title: `${r.commander_clients?.name || 'Unknown client'} — ${r.tax_year || 'tax return'}`, client_name: r.commander_clients?.name || null, completed_at: r.updated_at, deep_link: '/commander/individual-tax.html?open=' + r.id }));
    (taxCo.data || []).forEach(r => items.push({ source_module: 'tax-company', source_type: 'commander_company_tax_returns', source_id: r.id, title: `${r.commander_clients?.name || 'Unknown client'} — ${r.tax_year || 'tax return'}`, client_name: r.commander_clients?.name || null, completed_at: r.updated_at, deep_link: '/commander/company-tax.html?open=' + r.id }));
...
async function _requireTeamMember(req, res) {
    ...
    if (requestedId && me && teamAccess.isManagerRole(me.role)) {
        const { data } = await supabase.from('commander_team_members').select('id, display_name, role').eq('id', requestedId).eq('company_id', cid).eq('is_active', true).maybeSingle();
        if (data) return data;
    }
...
router.get('/summary', async (req, res) => {
    try {
        ...
        const { count: unreadNotifications } = await supabase.from('commander_notifications').select('id', { count: 'exact', head: true })
        ...
        console.error('GET /api/commander/work-queue/summary', err);
...
router.get('/my-work', async (req, res) => {
    try {
        ...
        console.error('GET /api/commander/work-queue/my-work', err);
...
router.get('/today', async (req, res) => {
    try {
        ...
        console.error('GET /api/commander/work-queue/today', err);
...
router.get('/overdue', async (req, res) => {
    try {
        ...
        console.error('GET /api/commander/work-queue/overdue', err);
...
router.get('/upcoming', async (req, res) => {
    try {
        ...
        console.error('GET /api/commander/work-queue/upcoming', err);
...
router.get('/waiting-on-me', async (req, res) => {
    try {
        ...
        console.error('GET /api/commander/work-queue/waiting-on-me', err);
...
router.get('/waiting-on-others', async (req, res) => {
    try {
        ...
        console.error('GET /api/commander/work-queue/waiting-on-others', err);
...
router.get('/completed', async (req, res) => {
    try {
        ...
        console.error('GET /api/commander/work-queue/completed', err);
...
router.get('/notifications', async (req, res) => {
    try {
        ...
        const { data, error } = await supabase.from('commander_notifications').select('*')
        ...
        console.error('GET /api/commander/work-queue/notifications', err);
...
router.get('/preferences', async (req, res) => {
    try {
        ...
        const member = await _requireTeamMember(req, res);
        if (!member) return;
        const { data } = await supabase.from('commander_work_queue_preferences').select('*').eq('company_id', cid).eq('team_member_id', member.id).maybeSingle();
        res.json({
        ...
        console.error('GET /api/commander/work-queue/preferences', err);
...
router.put('/preferences', async (req, res) => {
    try {
        ...
        const { data, error } = await supabase.from('commander_work_queue_preferences').upsert(patch, { onConflict: 'company_id,team_member_id' }).select().single();
        if (error) throw error;
        res.json({ preferences: data });
    } catch (err) {
        console.error('PUT /api/commander/work-queue/preferences', err);
...
router.post('/events', async (req, res) => {
    try {
        ...
        console.error('POST /api/commander/work-queue/events', err);
...
router.get('/events', async (req, res) => {
    try {
        ...
        const member = await _requireTeamMember(req, res);
        if (!member) return;
        const { data, error } = await supabase.from('commander_work_queue_events').select('*')
            .eq('company_id', cid).eq('team_member_id', member.id)
            .order('created_at', { ascending: false }).limit(100);
        if (error) throw error;
        res.json({ events: data || [] });
    } catch (err) {
        console.error('GET /api/commander/work-queue/events', err);
