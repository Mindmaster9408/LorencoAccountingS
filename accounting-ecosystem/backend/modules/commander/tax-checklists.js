...
async function logTemplateEvent(cid, templateId, eventType, opts = {}) {
    try {
        await supabase.from('commander_tax_checklist_template_events').insert({
            company_id:    cid,
...
router.get('/templates', async (req, res) => {
    ...
    let q = supabase
        .from('commander_tax_checklist_templates')
        .select('*')
        .eq('company_id', cid)
        .order('template_name');
    ...
    if (data && data.length > 0) {
        ...
        const { data: itemCounts } = await supabase
            .from('commander_tax_checklist_template_items')
            .select('template_id')
            .eq('company_id', cid)
            .in('template_id', ids);
...
router.post('/templates', async (req, res) => {
    ...
    await auditFromReq(req, 'CREATE', 'commander_tax_checklist_template', data.id, { module: 'commander' });
...
router.put('/templates/:id', async (req, res) => {
    ...
    const { data: existing } = await supabase
        .from('commander_tax_checklist_templates').select('id').eq('id', req.params.id).eq('company_id', cid).single();
    ...
    await auditFromReq(req, 'UPDATE', 'commander_tax_checklist_template', data.id, { module: 'commander' });
    res.json({ template: data });
});
...
router.delete('/templates/:id', async (req, res) => {
    ...
    await auditFromReq(req, 'DEACTIVATE', 'commander_tax_checklist_template', parseInt(req.params.id), { module: 'commander' });
...
router.get('/templates/:id/items', async (req, res) => {
    ...
    const { data: tpl } = await supabase
        .from('commander_tax_checklist_templates').select('id').eq('id', templateId).eq('company_id', cid).single();
...
router.post('/seed-defaults', async (req, res) => {
    ...
    for (const tpl of SEED_TEMPLATES) {
        ...
        if (itemRows.length > 0) {
            await supabase.from('commander_tax_checklist_template_items').insert(itemRows);
        }
    ...
    await auditFromReq(req, 'CREATE', 'commander_tax_checklist_defaults', 0, {
        module: 'commander',
        created,
        skipped,
    });
...
router.post('/templates/:id/apply', async (req, res) => {
    ...
    const checks = [
        supabase.from('commander_tax_checklist_templates').select('id, is_active').eq('id', templateId).eq('company_id', cid).single(),
        verifyBelongsToCompany(cid, 'commander_clients', client_id),
        compliance_pack_id        ? verifyBelongsToCompany(cid, 'commander_compliance_packs', compliance_pack_id)      : Promise.resolve(true),
        individual_tax_return_id  ? verifyBelongsToCompany(cid, 'commander_individual_tax_returns', individual_tax_return_id) : Promise.resolve(true),
        company_tax_return_id     ? verifyBelongsToCompany(cid, 'commander_company_tax_returns', company_tax_return_id)     : Promise.resolve(true),
    ];
    ...
    const { data: existingDocRequests } = create_document_requests
        ? await supabase.from('commander_document_requests')
    ...
    const { data: existingIndItems } = (create_tax_items && individual_tax_return_id)
        ? await supabase.from('commander_individual_tax_items')
            .select('item_label')
    ...
    const { data: existingCoItems } = (create_tax_items && company_tax_return_id)
        ? await supabase.from('commander_company_tax_readiness_items')
            .select('item_name')
    ...
    for (const item of items) {
        ...
        if (tType === 'document_request' && create_document_requests) {
            ...
            const { error: drErr } = await supabase.from('commander_document_requests').insert({
                company_id:          cid,
        ...
        // ── Compliance pack items ──────────────────────────────────────────────
            ...
            const { error: piErr } = await supabase.from('commander_compliance_pack_items').insert({
                company_id:   cid,
        ...
        // ── Individual tax items ───────────────────────────────────────────────
            ...
            const { error: itErr } = await supabase.from('commander_individual_tax_items').insert({
                company_id:    cid,
        ...
        // ── Company tax readiness items ────────────────────────────────────────
            ...
            const { error: ctErr } = await supabase.from('commander_company_tax_readiness_items').insert({
                company_id:            cid,
    ...
    await auditFromReq(req, 'APPLY', 'commander_tax_checklist_template', templateId, {
        module: 'commander', client_id: clientIdInt, doc_requests_created: docRequestsCreated, pack_items_created: packItemsCreated, tax_items_created: taxItemsCreated,
    });
