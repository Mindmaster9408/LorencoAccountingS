...
async function _verifyRun(id, cid) {
    const { data } = await supabase.from('commander_pilot_readiness_runs').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
    return data || null;
}
async function _verifyChecklistItem(id, cid) {
    const { data } = await supabase.from('commander_pilot_checklist_items').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
    return data || null;
}
async function _verifyIssue(id, cid) {
    const { data } = await supabase.from('commander_pilot_known_issues').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
    return data || null;
}

async function _writeEvent(cid, { runId, checklistItemId, issueId }, eventType, oldStatus, newStatus, userId, notes, meta) {
    await supabase.from('commander_pilot_events').insert({
...
async function computePilotReadiness(cid) {
    ...
    const { data: latestHealthRun } = await supabase.from('commander_health_check_runs')
    ...
    const { data: failedAutomationRuns } = await supabase.from('commander_automation_runs')
    ...
    const { data: issueRows } = await supabase.from('commander_pilot_known_issues').select('*').eq('company_id', cid).in('issue_status', OPEN_ISSUE_STATUSES);
    ...
    // 5. Checklist items.
    const { data: checklistRows } = await supabase.from('commander_pilot_checklist_items').select('*').eq('company_id', cid);
...
const DEFAULT_CHECKLIST = [
    ...
    { check_category: 'access_control', check_title: 'Team user links valid', severity: 'high', sort_order: 3, linked_url: '/commander/team.html' },
    { check_category: 'clients', check_title: 'Client create/view works', severity: 'high', sort_order: 4, linked_url: '/commander/clients.html' },
    { check_category: 'planning', check_title: 'Planning Board loads', severity: 'medium', sort_order: 5, linked_url: '/commander/planning-board.html' },
    { check_category: 'core_operations', check_title: 'My Work loads', severity: 'medium', sort_order: 6, linked_url: '/commander/work-queue.html' },
    { check_category: 'core_operations', check_title: 'Notifications load', severity: 'medium', sort_order: 7, linked_url: '/commander/notifications.html' },
    { check_category: 'secretarial', check_title: 'Secretarial profile loads', severity: 'medium', sort_order: 8, linked_url: '/commander/secretarial.html' },
    { check_category: 'engagement', check_title: 'Engagement management loads', severity: 'medium', sort_order: 9, linked_url: '/commander/engagement-management.html' },
    { check_category: 'reporting', check_title: 'Executive Reporting generate works', severity: 'high', sort_order: 10, linked_url: '/commander/executive-reporting.html' },
    { check_category: 'automation', check_title: 'Automation dry-run works', severity: 'medium', sort_order: 11, linked_url: '/commander/automation.html' },
    { check_category: 'health', check_title: 'Operational Health run works', severity: 'high', sort_order: 12, linked_url: '/commander/operational-health.html' },
    { check_category: 'security', check_title: 'No localStorage violations in new files', severity: 'critical', sort_order: 13 },
    { check_category: 'core_operations', check_title: 'Management Dashboard loads', severity: 'high', sort_order: 14, linked_url: '/commander/management-dashboard.html' },
...
async function _seedDefaults(cid, userId) {
    const { data: existing } = await supabase.from('commander_pilot_checklist_items').select('check_title').eq('company_id', cid).eq('is_default', true);
    ...
    const { data: inserted, error } = await supabase.from('commander_pilot_checklist_items').insert(toInsert).select('id, check_title');
...
router.get('/summary', async (req, res) => {
    ...
    try {
        const [{ data: latestRun }, { count: openCriticalIssues }] = await Promise.all([
            supabase.from('commander_pilot_readiness_runs').select('id, overall_score, readiness_status, decision, created_at').eq('company_id', cid).order('created_at', { ascending: false }).limit(1).maybeSingle(),
            supabase.from('commander_pilot_known_issues').select('id', { count: 'exact', head: true }).eq('company_id', cid).eq('severity', 'critical').in('issue_status', OPEN_ISSUE_STATUSES),
        ]);
        return res.json({ latest_run: latestRun || null, open_critical_issues: openCriticalIssues || 0 });
    } catch (err) {
        console.error('GET /api/commander/pilot-readiness/summary', err);
...
router.post('/run', async (req, res) => {
    ...
    try {
        ...
        const { data: run, error } = await supabase.from('commander_pilot_readiness_runs').insert({
        ...
        await auditFromReq(req, 'pilot_readiness_run', 'commander_pilot_readiness_run', run.id, { readiness_status: result.readinessStatus });
        ...
        console.error('POST /api/commander/pilot-readiness/run', err);
...
router.get('/runs', async (req, res) => {
    ...
    try {
        ...
        const { data, count, error } = await supabase.from('commander_pilot_readiness_runs')
        ...
        console.error('GET /api/commander/pilot-readiness/runs', err);
...
router.get('/runs/:id', async (req, res) => {
    ...
    try {
        ...
        console.error('GET /api/commander/pilot-readiness/runs/:id', err);
...
router.put('/runs/:id/decision', async (req, res) => {
    ...
    try {
        ...
        const { data: updated, error } = await supabase.from('commander_pilot_readiness_runs').update({
        ...
        await auditFromReq(req, 'pilot_readiness_decision_recorded', 'commander_pilot_readiness_run', run.id, { decision });

        return res.json({ run: updated });
    } catch (err) {
        console.error('PUT /api/commander/pilot-readiness/runs/:id/decision', err);
...
router.get('/checklist', async (req, res) => {
    ...
    try {
        ...
        let q = supabase.from('commander_pilot_checklist_items').select('*').eq('company_id', cid);
        ...
        console.error('GET /api/commander/pilot-readiness/checklist', err);
...
router.post('/checklist/seed-defaults', async (req, res) => {
    ...
    try {
        ...
        await auditFromReq(req, 'pilot_checklist_seeded', 'commander_pilot_checklist_item', null, { inserted_count: result.inserted });
        return res.status(201).json(result);
    } catch (err) {
        console.error('POST /api/commander/pilot-readiness/checklist/seed-defaults', err);
...
router.put('/checklist/:id', async (req, res) => {
    ...
    try {
        ...
        const { data: updated, error } = await supabase.from('commander_pilot_checklist_items').update({ ...patch, updated_by: req.user?.userId }).eq('id', item.id).eq('company_id', cid).select().single();
        if (error) throw error;
        ...
        await auditFromReq(req, 'pilot_checklist_item_updated', 'commander_pilot_checklist_item', item.id, { fields: Object.keys(patch) });

        return res.json({ item: updated });
    } catch (err) {
        console.error('PUT /api/commander/pilot-readiness/checklist/:id', err);
...
router.get('/issues', async (req, res) => {
    ...
    try {
        ...
        let q = supabase.from('commander_pilot_known_issues').select('*', { count: 'exact' }).eq('company_id', cid);
        ...
        console.error('GET /api/commander/pilot-readiness/issues', err);
...
router.post('/issues', async (req, res) => {
    ...
    try {
        ...
        const { data, error } = await supabase.from('commander_pilot_known_issues').insert({
        ...
        await auditFromReq(req, 'pilot_known_issue_created', 'commander_pilot_known_issue', data.id, {});

        return res.status(201).json({ issue: data });
    } catch (err) {
        console.error('POST /api/commander/pilot-readiness/issues', err);
...
router.get('/issues/:id', async (req, res) => {
    ...
    try {
        ...
        console.error('GET /api/commander/pilot-readiness/issues/:id', err);
...
router.put('/issues/:id', async (req, res) => {
    ...
    try {
        ...
        const { data: updated, error } = await supabase.from('commander_pilot_known_issues').update(patch).eq('id', issue.id).eq('company_id', cid).select().single();
        if (error) throw error;
        ...
        await auditFromReq(req, 'pilot_known_issue_updated', 'commander_pilot_known_issue', issue.id, { fields: Object.keys(patch) });

        return res.json({ issue: updated });
    } catch (err) {
        console.error('PUT /api/commander/pilot-readiness/issues/:id', err);
...
router.put('/issues/:id/resolve', async (req, res) => {
    ...
    try {
        ...
        const now = new Date().toISOString();
        const { data: updated, error } = await supabase.from('commander_pilot_known_issues').update({
        ...
        await auditFromReq(req, 'pilot_known_issue_resolved', 'commander_pilot_known_issue', issue.id, {});

        return res.json({ issue: updated });
    } catch (err) {
        console.error('PUT /api/commander/pilot-readiness/issues/:id/resolve', err);
...
router.put('/issues/:id/accept-risk', async (req, res) => {
    ...
    try {
        ...
        await auditFromReq(req, 'pilot_known_issue_risk_accepted', 'commander_pilot_known_issue', issue.id, {});

        return res.json({ issue: updated });
    } catch (err) {
        console.error('PUT /api/commander/pilot-readiness/issues/:id/accept-risk', err);
...
router.get('/events', async (req, res) => {
    ...
    try {
        let q = supabase.from('commander_pilot_events').select('*', { count: 'exact' }).eq('company_id', cid);
        ...
        console.error('GET /api/commander/pilot-readiness/events', err);
