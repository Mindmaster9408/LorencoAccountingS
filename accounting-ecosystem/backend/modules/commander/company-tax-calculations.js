/* =============================================================
   ...
   Mounted ADDITIONALLY at /api/commander/company-tax (fallthrough after company-tax.js).
...
async function logCalcEvent(cid, calcId, returnId, eventType, extras = {}) {
    try {
        await supabase.from('commander_company_tax_calculation_events').insert({
            company_id:             cid,
...
router.get('/calculations/:id/events', async (req, res) => {
    ...
    const { data, error } = await supabase
        .from('commander_company_tax_calculation_events')
        .select('*')
...
router.post('/calculations/:id/submit-review', async (req, res) => {
    ...
    try {
        ...
        await auditFromReq(req, 'company_tax_calculation_submitted_review', 'commander_company_tax_calculations', calcId);
...
router.post('/calculations/:id/approve', async (req, res) => {
    ...
    try {
        ...
        await auditFromReq(req, 'company_tax_calculation_approved', 'commander_company_tax_calculations', calcId);
...
router.post('/calculations/:id/reject', async (req, res) => {
    ...
    try {
        ...
        await auditFromReq(req, 'company_tax_calculation_rejected', 'commander_company_tax_calculations', calcId, { metadata: { reason } });
...
router.post('/:returnId/calculations/run-draft', async (req, res) => {
    ...
    try {
        ...
        await auditFromReq(req, 'company_tax_calculation_run', 'commander_company_tax_calculations', calc.id, { metadata: {
            return_id: returnId,
            version: nextVersion,
        } });
