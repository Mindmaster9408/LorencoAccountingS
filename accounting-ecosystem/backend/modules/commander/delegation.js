...
const SOURCE_REGISTRY = {
    tasks: {
        table: 'commander_tasks', sourceType: 'commander_task',
        ...
        deepLink: id => '/commander/tasks.html?open=' + id,
    },
    deadlines: {
        table: 'commander_deadlines', sourceType: 'commander_deadline',
        roles: { responsible: 'responsible_team_member_id' },
        defaultRole: 'responsible', titleField: 'title',
        deepLink: id => '/commander/deadlines.html?open=' + id,
    },
    'risk-register': {
        table: 'commander_risks', sourceType: 'commander_risk',
        roles: { owner: 'owner_team_member_id' },
        defaultRole: 'owner', titleField: 'title',
        deepLink: id => '/commander/risk-register.html?open=' + id,
    },
    'qms-review': {
        table: 'commander_quality_reviews', sourceType: 'commander_quality_review',
        roles: { reviewer: 'assigned_reviewer_team_member_id' },
        defaultRole: 'reviewer', titleField: 'review_title',
        deepLink: id => '/commander/quality-management.html?open=' + id,
    },
    'qms-finding': {
        table: 'commander_quality_findings', sourceType: 'commander_quality_finding',
        roles: { responsible: 'responsible_team_member_id' },
        defaultRole: 'responsible', titleField: 'finding_title',
        deepLink: id => '/commander/quality-management.html?finding=' + id,
    },
    'tax-individual': {
        table: 'commander_individual_tax_returns', sourceType: 'commander_individual_tax_returns',
        ...
        deepLink: id => '/commander/individual-tax.html?open=' + id,
    },
    'tax-company': {
        table: 'commander_company_tax_returns', sourceType: 'commander_company_tax_returns',
        roles: { preparer: 'responsible_team_member_id', reviewer: 'reviewer_team_member_id' },
        defaultRole: 'preparer', titleField: null,
        deepLink: id => '/commander/company-tax.html?open=' + id,
    },
    'compliance-packs': {
        table: 'commander_compliance_packs', sourceType: 'commander_compliance_pack',
        roles: { owner: 'owner_team_member_id', reviewer: 'reviewer_team_member_id' },
        defaultRole: 'owner', titleField: ['pack_name', 'pack_type'],
        deepLink: id => '/commander/compliance-packs.html?open=' + id,
    },
    'document-requests': {
        table: 'commander_document_requests', sourceType: 'commander_document_request',
        roles: { assignee: 'assigned_team_member_id' },
        defaultRole: 'assignee', titleField: ['request_title', 'document_category'],
        deepLink: id => '/commander/document-requests.html?open=' + id,
    },
    reminders: {
        table: 'commander_reminders', sourceType: 'commander_reminder',
        roles: { assignee: 'assigned_team_member_id' },
        defaultRole: 'assignee', titleField: ['title', 'reminder_type'],
        deepLink: id => '/commander/reminders.html?open=' + id,
    },
};
...
async function _writeEvent(cid, delegationId, eventType, oldOwnerId, newOwnerId, actorUserId, notes, meta) {
    await supabase.from('commander_work_delegation_events').insert({
...
async function _validateNewOwner(cid, newOwnerId) {
    const { data } = await supabase.from('commander_team_members').select('id').eq('id', newOwnerId).eq('company_id', cid).eq('is_active', true).maybeSingle();
...
async function _getSourceTitle(registry, sourceId) {
    ...
    const { data } = await supabase.from(registry.table).select('id, tax_year, commander_clients:client_id(name)').eq('id', sourceId).maybeSingle();
    if (data) return `${data.commander_clients?.name || 'Unknown client'} — ${data.tax_year || 'tax return'}`;
    return `${registry.table} #${sourceId}`;
}
...
async function changeOwnership({ cid, sourceModule, sourceType, sourceId, role, newOwnerId, delegatedBy, reason, notes, effectiveDate, actorUserId }) {
    ...
    const { data: delegation, error } = await supabase.from('commander_work_delegations').insert({
...
async function _enrichDelegations(delegations, includeAdvisory) {
    ...
    let membersById = {};
    if (memberIds.size) {
        const { data } = await supabase.from('commander_team_members').select('id, display_name').in('id', [...memberIds]);
...
async function _verifyDelegation(id, cid) {
    const { data } = await supabase.from('commander_work_delegations').select('*').eq('id', id).eq('company_id', cid).maybeSingle();
    return data || null;
}
...
router.get('/summary', async (req, res) => {
    try {
        ...
        let q = supabase.from('commander_work_delegations').select('delegation_status, previous_owner_id, new_owner_id, delegated_by').eq('company_id', cid);
        ...
        console.error('GET /api/commander/delegation/summary', err);
...
router.get('/', async (req, res) => {
    try {
        ...
        let q = supabase.from('commander_work_delegations').select('*').eq('company_id', cid).order('created_at', { ascending: false });
        ...
        console.error('GET /api/commander/delegation', err);
...
router.get('/competency-preview', async (req, res) => {
    try {
        ...
        console.error('GET /api/commander/delegation/competency-preview', err);
...
router.get('/:id', async (req, res) => {
    try {
        ...
        console.error('GET /api/commander/delegation/:id', err);
...
router.post('/', async (req, res) => {
    try {
        ...
        console.error('POST /api/commander/delegation', err);
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const { data, error } = await supabase.from('commander_work_delegations').update(patch).eq('id', delegation.id).eq('company_id', cid).select().single();
        if (error) throw error;

        const [enriched] = await _enrichDelegations([data]);
        res.json({ delegation: enriched });
    } catch (err) {
        console.error('PUT /api/commander/delegation/:id', err);
...
router.put('/:id/accept', async (req, res) => {
    try {
        ...
        const { data, error } = await supabase.from('commander_work_delegations').update({ delegation_status: 'accepted' }).eq('id', delegation.id).eq('company_id', cid).select().single();
        if (error) throw error;
        ...
        console.error('PUT /api/commander/delegation/:id/accept', err);
...
router.put('/:id/decline', async (req, res) => {
    try {
        ...
        const { data, error } = await supabase.from('commander_work_delegations').update({ delegation_status: 'declined' }).eq('id', delegation.id).eq('company_id', cid).select().single();
        if (error) throw error;
        ...
        console.error('PUT /api/commander/delegation/:id/decline', err);
...
router.put('/:id/cancel', async (req, res) => {
    try {
        ...
        const { data, error } = await supabase.from('commander_work_delegations').update({ delegation_status: 'cancelled' }).eq('id', delegation.id).eq('company_id', cid).select().single();
        if (error) throw error;
        ...
        console.error('PUT /api/commander/delegation/:id/cancel', err);
...
router.put('/:id/complete', async (req, res) => {
    try {
        ...
        const { data, error } = await supabase.from('commander_work_delegations').update({ delegation_status: 'completed', completed_at: new Date().toISOString() }).eq('id', delegation.id).eq('company_id', cid).select().single();
        if (error) throw error;
        ...
        console.error('PUT /api/commander/delegation/:id/complete', err);
...
router.get('/:id/events', async (req, res) => {
    try {
        ...
        const { data, error } = await supabase.from('commander_work_delegation_events').select('*').eq('company_id', cid).eq('delegation_id', delegation.id).order('created_at', { ascending: false });
        if (error) throw error;

        res.json({ events: data || [] });
    } catch (err) {
        console.error('GET /api/commander/delegation/:id/events', err);
