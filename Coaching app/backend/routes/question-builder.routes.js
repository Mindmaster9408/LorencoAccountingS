...
//   GET    /contexts                                List distinct categories/contexts + standard recommended list
//
...
// Runs once on server startup. See also: backend/database/006_add_question_builder.sql
...
ensureQuestionBuilderTables().catch(err =>
    console.error('[question-builder.routes] Failed to ensure tables:', err)
);
...
// ─── GET /questions ──────────────────────────────────────────────────────────
// List global questions. Optional query params: category, context_key, active.
router.get('/questions', async (req, res) => {
    ...
    try {
        ...
        console.error('[question-builder] GET /questions', err);
    ...
    try {
        const result = await query(
            ...
            [
                ...
                sanitiseInt(scaleMin),
                sanitiseInt(scaleMax),
        ...
        console.error('[question-builder] POST /questions', err);
...
router.put('/questions/:id', async (req, res) => {
    ...
    try {
        ...
        console.error('[question-builder] PUT /questions/:id', err);
        res.status(500).json({ error: 'Failed to update question.' });
    }
});

// ─── DELETE /questions/:id ───────────────────────────────────────────────────
// Soft delete only — sets is_active = false. Data is never destroyed.
router.delete('/questions/:id', async (req, res) => {
    ...
    try {
        ...
        console.error('[question-builder] DELETE /questions/:id', err);
        res.status(500).json({ error: 'Failed to deactivate question.' });
    }
});

// ─── GET /contexts ───────────────────────────────────────────────────────────
// Returns the standard recommended context keys plus any in-use distinct values.
router.get('/contexts', async (req, res) => {
    try {
        const result = await query(
            `SELECT DISTINCT category, context_key
             FROM coaching_questions
             WHERE is_active = true
             ORDER BY category, context_key`
        );
        res.json({ standard: STANDARD_CONTEXTS, inUse: result.rows });
    } catch (err) {
        console.error('[question-builder] GET /contexts', err);
...
router.get('/client/:clientId/context/:contextKey', async (req, res) => {
    ...
    try {
        ...
        console.error('[question-builder] GET /client/:clientId/context/:contextKey', err);
        res.status(500).json({ error: 'Failed to get client context questions.' });
    }
});

// ─── POST /client/:clientId/context/:contextKey/assign ───────────────────────
// Assign one or more questions to a client/context. Skips already-assigned questions.
router.post('/client/:clientId/context/:contextKey/assign', async (req, res) => {
    ...
    try {
        ...
        console.error('[question-builder] POST .../assign', err);
        res.status(500).json({ error: 'Failed to assign questions.' });
    }
});

// ─── PUT /client/:clientId/context/:contextKey/answers ───────────────────────
// Upsert answers for assigned questions. Each answer must include questionId.
