 ...
 * Lorenco Accounting Ecosystem — Coaching App
 *
 * Provides:
 * - Cloud-backed storage layer (all business data → Supabase via /api/kv)
 * - Modern JS method polyfills
...
(function () {
    ...
    var LOCAL_KEYS = {
        auth_token:1, token:1, paytime_token:1, sean_token:1, eco_token:1,
        user:1, sean_user:1, current_user:1,
    ...
    function _tok() {
        try {
            return _nGet('auth_token') || _nGet('token') ||
                   _nGet('paytime_token') || _nGet('sean_token') || '';
        } catch(e) { return ''; }
    }
    ...
    (function _preload() {
        try {
            ...
            if (xhr.status === 200) {
                ...
                _online = true;
                console.log(
                    '%c\u2601\ufe0f  Paytime COACHING \u2014 all data in Supabase (zero local storage)',
                    'color:#667eea;font-weight:bold;'
                );
            } else if (xhr.status === 401) {
                // Expired token — clear it (use native refs to avoid recursion)
                try { _nDel('auth_token'); } catch(e) {}
                try { _nDel('token');      } catch(e) {}
                try { _nDel('user');       } catch(e) {}
            }
        } catch(e) {
            console.warn('\u26a0\ufe0f  Cloud storage preload failed (Paytime COACHING):', e.message);
        }
    }());
    ...
    window.safeLocalStorage = {
        ...
        // Reload all user data from cloud (use after login)
        reload: function () {
