 ...
 *
 * Exports: renderSpilClientPanel(client, containerId)
 */

import { apiRequest } from './api.js';
import { escapeHtml } from './config.js';

// Dimension colour map — mirrors spil.config.js
const DIM_COLORS = {
    STRUKTUUR:  '#0ea5e9',
    PRESTASIE:  '#f59e0b',
    INSIG:      '#8b5cf6',
...
export async function renderSpilClientPanel(client, containerId) {
    ...
    document.getElementById('spil-client-open-btn')
        .addEventListener('click', () => window.open('spil.html', '_blank'));

    const listEl = document.getElementById('spil-client-list');

    // Only attempt API fetch if client has a server-assigned id
    if (!client.id) {
        listEl.innerHTML = `<div style="color:#64748b;font-size:13px;padding:12px 0">
            Save this client first before creating SPIL profiles.
        </div>`;
        return;
    }
    ...
    // Filter to profiles linked to this client
    ...
    // Also match by email as a fallback (for profiles created before linking existed)
    ...
    const rows = all.map(p => {
        ...
        const badge = hasResults
            ? `<span style="
                ...
                font-weight:600;background:#dcfce7;color:#166534">Voltooi</span>`
            : `<span style="
                display:inline-block;padding:2px 8px;border-radius:12px;font-size:11px;
                font-weight:600;background:#fef9c3;color:#713f12">Konsep</span>`;

        const code = p.spil_code
            ? `<span style="
                font-family:monospace;font-size:12px;font-weight:700;color:#0f172a;
                background:#f8fafc;padding:2px 8px;border-radius:4px;border:1px solid #e2e8f0;
                margin-left:8px">${escapeHtml(p.spil_code)}</span>`
            : '';

        const ranking = p.spil_code
            ? buildMiniRanking(p.spil_code)
            : '';
        ...
        return `
            <div style="
                ...
                margin-bottom:8px;background:#fafcff;gap:8px;flex-wrap:wrap
            ">
                <div style="flex:1;min-width:120px">
                    <div style="font-size:13px;font-weight:600;color:#1e293b">
                        ${escapeHtml(p.respondent_name)}
                    </div>
                ...
                <div style="display:flex;align-items:center;gap:8px">
                    ...
                    <a href="spil.html" target="_blank" style="
                        padding:5px 10px;background:#0ea5e9;color:#fff;text-decoration:none;
...
function buildMiniRanking(spilCode) {
    if (!spilCode) return '';
    // spilCode is like "SPIL-PRESTASIE/INSIG/LIEFDE"
    const parts = spilCode.replace(/^SPIL-/, '').split('/');
    ...
    const dots = parts.slice(0, 3).map(key => {
        ...
        return `<span title="${name}" style="
            ...
            background:${color};margin-right:3px
        "></span>`;
    }).join('');
